...
# v0.1.52 小修批：⑧a 公网源 Files API 同名上传静默写「-1」副本隐患改
# scp 直传（顺带移除硬编码 Bearer token，源码零凭据）、⑤a web 路径修正
# 为 crates/os-api/web、⑧b 远端服务名 os-api 探测/可 env 钉死）
#
...
# 远端 systemd 服务名（v0.1.49 部署代理修正：aliyun 实机为 os-api.service，
# 旧脚本 restart nexos-os-api 报 Unit not found）。env NEXOS_DEPLOY_SERVICE 可
# 钉死；缺省部署时探测远端在位者（os-api 优先，兜底 nexos-os-api）。
ALIYUN_SERVICE="${NEXOS_DEPLOY_SERVICE:-}"
...
#     web 真身在 crates/os-api/web（v0.1.49 部署代理修正：旧 $EXPORT/web
#     路径已过时，npm ci 直接报 ENOENT）
run "(cd \"$EXPORT/crates/os-api/web\" && npm ci)"
run "(cd \"$EXPORT/crates/os-api/web\" && npm run build)"
...
# ⑧a aliyun 公网源同步（install.sh 的源节点）
#     发版只刷 106 分发目录的话，公网源会 404（08-30 DGX Spark 实测踩过）。
#     scp 直传（v0.1.52：v0.1.48 部署代理发现 Files API 同名上传会静默写
#     「xxx-1」副本——公网源停旧版；scp 同名覆盖无此患，且免 token：源码
#     不再含 Bearer 凭据，直传走已配免密 key。传 .new 临时名 + 远端 mv 原子
#     换名，避免传输中断留下截断半件被公网源捞走）
ALIYUN_UP=1
if [[ -z "$DRY_RUN" ]]; then
...
if [[ "$ALIYUN_UP" == 1 ]]; then
  ...
  # ⑧b 服务名解析：env 钉死优先；否则探测远端在位者（os-api 优先，兜底
  # nexos-os-api——对照 UPDATE_APP 自重启「os-api → fallback」同款次序）
  if [[ -z "$ALIYUN_SERVICE" ]]; then
    if [[ -n "$DRY_RUN" ]]; then
      plan "探测远端服务名（os-api 优先，兜底 nexos-os-api；env NEXOS_DEPLOY_SERVICE 可钉死）"
      ALIYUN_SERVICE="os-api"
    else
      ALIYUN_SERVICE=$($ALIYUN_SSH 'for s in os-api nexos-os-api; do systemctl list-unit-files "$s.service" 2>/dev/null | grep -q "^$s\.service" && { echo "$s"; break; }; done' 2>/dev/null | tr -d "\r\n ")
      [[ -n "$ALIYUN_SERVICE" ]] || ALIYUN_SERVICE="os-api"
      echo "  远端服务名: $ALIYUN_SERVICE"
    fi
  fi
  if [[ -n "$DRY_RUN" ]]; then
    plan "curl 探活 $ALIYUN_BASE_URL/ 可达后：scp 直传三分发件（os-api-x86_64/aarch64、p2p-node-aarch64，本地 /tank/os-data/dist → aliyun:/tank/os-data/dist，.new+mv 原子换名）"
  else
    $ALIYUN_SSH 'mkdir -p /tank/os-data/dist'
    for name in os-api-x86_64-latest os-api-aarch64-latest p2p-node-aarch64-latest; do
      if scp -q -P "$ALIYUN_SSH_PORT" -o BatchMode=yes -o StrictHostKeyChecking=no \
             "/tank/os-data/dist/$name" "root@${ALIYUN_HOST}:/tank/os-data/dist/$name.new" \
         && $ALIYUN_SSH "mv '/tank/os-data/dist/$name.new' '/tank/os-data/dist/$name'"; then
        echo "  aliyun 同步: $name"
      else
        echo "  aliyun 同步失败 $name"
      fi
    done
  fi
  ...
  if [[ -n "$DRY_RUN" ]]; then
    ...
    if [[ "$HZ_OK" == 1 ]]; then
      ...
      echo "  回滚：$ALIYUN_SSH 'cp -a /opt/nexos/os-api.bak.<时间戳> /opt/nexos/os-api && systemctl restart $ALIYUN_SERVICE'"
      exit 1
    fi
  fi
else
...
if [[ -n "$GITHUB" ]]; then
  ...
  if [[ -n "$DRY_RUN" ]]; then
    plan "GH_TOKEN=\$(ssh -p 179 root@198.51.100.114 'cat /root/.gh_token_changeos') && git push https://x-access-token:***@github.com/changeos/NexOS.git main --tags"
  else
...
echo "回滚（aliyun）：ssh -p 221 root@203.0.113.2 'cp -a /opt/nexos/os-api.bak.<时间戳> /opt/nexos/os-api && mv /opt/nexos/crates/os-api/static-dist.old /opt/nexos/crates/os-api/static-dist && systemctl restart ${ALIYUN_SERVICE:-os-api}'"
