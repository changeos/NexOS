...
import registerStreaming from '../src/entry'
...
function showFatal(err: unknown): void {
  if (typeof document === 'undefined') {
    console.error('[nexos-app-streaming standalone]', err)
    return
  }
  ...
  if (app) {
    app.innerHTML = `
      <div style="max-width:560px;margin:15vh auto 0;padding:0 20px;font:14px/1.7 system-ui,sans-serif;color:#e6e4e9">
        <h2 style="font-size:18px;margin:0 0 10px;color:#E95420">流媒体中心（独立模式）装载失败</h2>
...
async function main(): Promise<void> {
  ...
  await registerStreaming(ctx as Parameters<typeof registerStreaming>[0])
