...
// vue + vue-i18n 完整打进本产物（应用包从此完全自包含，内网离线可跑，
// 不引 CDN），然后：
...
//      token 输入条（记住后自动重试一次）。
// （照 apps/film/standalone/standalone-host.ts 同款，改应用名。）
// =============================================================================
...
import registerQrTransfer from '../src/entry'
...
//    sdk（@nexos/app-sdk v0.1.28+）：就绪实例（独立模式 SDK 源码经 vite
//    resolve.alias 指向主前端 crates/os-api/web/src/sdk/ 唯一事实源打进
...
/** 基础样式底：桌面 tokens.css 的暗色近似（组件全部 var(…, fallback) 消费）。 */
...
function showFatal(err: unknown): void {
  if (typeof document === 'undefined') {
    console.error('[nexos-app-qrtransfer standalone]', err)
    return
  }
  ...
  if (app) {
    app.innerHTML = `
      <div style="max-width:560px;margin:15vh auto 0;padding:0 20px;font:14px/1.7 system-ui,sans-serif;color:#e6e4e9">
        <h2 style="font-size:18px;margin:0 0 10px;color:#E95420">二维码传输（独立模式）装载失败</h2>
...
async function main(): Promise<void> {
  ...
  await registerQrTransfer(ctx as Parameters<typeof registerQrTransfer>[0])
