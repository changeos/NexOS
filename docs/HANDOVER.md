...
- 跳板机：`203.0.113.114:179` root / `<跳板机密码占位>`（**敏感，已暴露在历史会话，建议任务结束后换密码**）
- 配置：`git config --global http."https://github.com/".proxy socks5h://127.0.0.1:1080`
- **SSH SOCKS5 隧道不持久**（会话/机器重启后断），需拉 GitHub 时重启：
  ```bash
  sshpass -p '<跳板机密码占位>' ssh -f -N -o ServerAliveInterval=30 -o ExitOnForwardFailure=yes \
    -D 127.0.0.1:1080 -p 179 root@203.0.113.114
  ```
