...
> ℹ️ **live 组件保留在系统**（v0.1.51 三引擎剥离批核定）：live.rs 是 P2P 联邦
> 基础能力（联邦直播大厅），不随 streaming 引擎剥离——`/api/v1/live/*` 与
> `/ws/live/*` 照常常开不门控。streaming 引擎本体已删除出 os-api（独立产品线
> StreamingStudio 承接），本文名义宿主「流媒体中心」现为独立应用。
