...
- 云锚点（203.0.113.114:179）：`scp p2p-node → systemctl restart nexos-p2p`
...
| 113 node-113 | 192.0.2.113 | oem/<密码占位>，/opt/nexos |
| aliyun | 203.0.113.2:8558 | SSH 221 挂了；API token change-me-admin-token |
| 云锚点 | 203.0.113.114:179 | root <跳板机密码占位>，独立 p2p-node |
