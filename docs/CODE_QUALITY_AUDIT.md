...
根因排查：本环境 git 全局配置了 `http.https://github.com/.proxy socks5h://127.0.0.1:1080`，但跳板机 SSH 隧道不可达（203.0.113.114:179 Connection reset），导致所有 git/cargo 的 github 操作走死代理超时。清除该 git config 后直连 github.com（curl 200）成功 clone advisory-db。
