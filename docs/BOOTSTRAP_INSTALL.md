...
- `203.0.113.2` 是 aliyun 公网入口；**任一公网入口都可以换用**，例如云锚点
  `http://203.0.113.114:8558/api/v1/provisioning/install.sh` 同样成立——脚本由
...
```
 ...
 写 systemd unit nexos-os-api.service
   NEXOS_P2P_BOOTSTRAP=<公网入口>:7070[,203.0.113.2:7070,203.0.113.114:7070]
...
| method | path | 鉴权 | 说明 |
|--------|------|------|------|
| GET  | `/api/v1/provisioning/install.sh`            | 公开 | 动态生成安装脚本文本（`text/x-shellscript` 原文直传，见 `http.rs::direct_passthrough_bytes`）。bootstrap 缺省列表 = 源节点通告地址（env `NEXOS_GIT_ADVERTISE_HOST`/`NEXOS_P2P_ADVERTISE`，回环/0.0.0.0 剔除）+ `203.0.113.2:7070` + `203.0.113.114:7070` |
