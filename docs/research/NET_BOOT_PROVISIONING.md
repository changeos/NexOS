...
| **jumpbox 下载先例** | MEMORY.md（apt 镜像 403 → 跳板机 203.0.113.114 拉官方 tar；GitHub 走跳板） | P2"缺 ISO 经 NexOS 跳板下载"的直接先例：国内镜像直连失败 → 跳板隧道拉取 |
...
【P2 跳板下载】：POST iso-repo/sync → aliyun/tuna 直连（实测可达）→ 失败转 jumpbox
 （203.0.113.114 隧道先例）→ zsync 增量刷新点版本 → sha256 校验入仓。
```
