...
> **本文目标**：把 28 crate 的 Rust workspace 从源码组装成一台可运行的 OS 系统——覆盖构建、`osd` 启动编排、配置、systemd 集成、ISO 安装、HA 集群、升级/回滚、监控接入的全流程。
>
> **关联文档**：
> - `OS_系统_Rust技术路线规划.md`（技术路线与架构决策存档，v2 重写）
> - [ARCHITECTURE.md](ARCHITECTURE.md) —— crate 分层与当前规模
> - `docs/HANDOVER.md` —— 实现状态档案（历史快照）
> - `docs/SANDBOX.md` —— root/内核操作的沙箱测试方案
> - `docs/NET_BOOT.md` —— iPXE 网络装机（装机实操线）
>
> **重要现状**（截至 2026-08-05；2026-08-20 增补 §9 实机部署现状；2026-09-25 数字复核）：crate 已从 22 扩到 **28**（新增 os-p2p / os-identity / os-mcp / os-nexhub 等），测试 os-api cargo test 基线 **1,715** / 全 workspace 约 5,000+；`osd` 的 cgroup v2 配额已接通真实 `cgroups-rs`，但**进程拉起（systemd 调用）/NTP** 仍是骨架（待 root 沙箱集成）；`os-iso` 安装器的真实裸机执行（分区/建池/写盘）留 TODO（iPXE 网络装机已另线落地，见 NET_BOOT.md）；`os-update` bootloader 激活留 TODO。本指南既描述**目标终态**也标注**当前阻塞点**，便于分阶段落地。
...
`osd` 是立项期规划（历史版技术路线 §3.13，git 历史可查）定义的 **PID1 之后的 OS 编排器**，统管 20+ Rust 组件 + Samba + QEMU + youki 的进程生命周期。本节描述它的启动流程、依赖图、cgroup 配额、健康检查机制。
...
osd 与 systemd 有两种集成模式（立项期规划，历史版 §3.13）。两种模式下 osd 的编排逻辑（拓扑/cgroup/健康）完全相同，区别在"谁当 PID1"与"组件进程怎么拉起"。
...
`os-services` 的 monitor 子模块（`crates/os-services/src/monitor.rs`）提供 metric 采集 / 日志收集 / 告警引擎，并暴露 Prometheus 兼容的 `/metrics` 端点。
...
| `NEXOS_P2P_BOOTSTRAP` | 空（孤网等入站） | P2P 引导节点 `host:port,...`（如 cloud 锚点 `203.0.113.114:7070`） |
