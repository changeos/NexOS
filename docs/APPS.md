...
> ⚠️ **引擎已剥离（v0.1.51 三引擎剥离批）**：film / streaming / surveillance
> 三引擎已彻底删除出 os-api（独立产品线 FilmStudio / StreamingStudio 于各自
> NexHub 仓承接）。本文以 film 为例的应用包开发指南与 SDK 契约仍有效，但
> §7 引擎门控中的 film / streaming 两行已成为历史——安装声明这两个引擎的
> 应用会被应用中心 400 拦截（见 §7 末尾「已剥离引擎」）。
...
于 v0.1.26 批次，qrtransfer / streaming 于 v0.1.30 批次；~~film / streaming
于 v0.1.51 三引擎剥离批删除出 os-api~~，见下「已剥离引擎」）：

| engine 键 | 应用 | handler | 门控范围（未装全 404） |
|-----------|------|---------|------------------------|
| `qrtransfer` | 二维码传输 | `handlers/qr_transfer.rs` | `/api/v1/qr/*` 全部 9 条 |
| ~~`film`~~ | ~~film（影片制作）~~ | ~~`handlers/film.rs`~~ | ~~`/api/v1/film/*` 全部 21 条~~ **已剥离** |
| ~~`streaming`~~ | ~~流媒体中心~~ | ~~`handlers/streaming.rs`~~ | ~~`/api/v1/streaming/*` 全部 18 条~~ **已剥离** |

**已剥离引擎（v0.1.51 三引擎剥离批）**：film / streaming（含并入流媒体中心
的 surveillance）引擎已删除出 os-api，由独立产品线 **FilmStudio /
StreamingStudio**（各自 NexHub 仓）承接。历史 `nexos-app-film` /
`nexos-app-streaming` 仓仍保留在 NexHub（历史发布线），但安装时按 manifest
`engine` 字段（或 `id`）命中即 **400 拦截**：`STRIPPED_ENGINES` +
`is_stripped_engine_manifest`（apps_handler.rs），提示改用独立版——装了
也没有内置后端，直接拦截比 404 门控更诚实。
