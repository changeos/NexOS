...
> ⚠️ **已剥离（v0.1.51 三引擎剥离批）**：surveillance 组件（后端 surveillance.rs
> 16 路由 + 前端 Surveillance.vue 视图 + client.ts 端点组）已删除出 NexOS——
> 摄像头监控能力由独立产品线 **StreamingStudio**（NexHub 仓，surveillance 并入
> 流媒体中心独立化决策）承接。本文为历史契约存档。
