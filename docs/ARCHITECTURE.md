...
> ⚠️ **三引擎已剥离（v0.1.51）**：film / streaming / surveillance 引擎与组件已
> 删除出 os-api（独立产品线 FilmStudio / StreamingStudio 承接；live 联邦直播
> 保留）。下文组件清单/路由计数中的 streaming / surveillance / film 字样为
> 剥离前快照，见 docs/APPS.md §7「已剥离引擎」。
...
>
> **更新**：2026-09-25。workspace 现为 **28 crate**（26 业务 + `os-integration` + `nettest`，
> 相比下文 08-21 口径新增 `os-p2p`（P2P 组网层）与 `os-identity`（指纹账本））；
> 三引擎剥离（v0.1.51）后 os-api 为 **46 个常驻 RouteHandler（+1 条件 extra）**、
> **377 条静态路由**（grep `spec(` 剔除测试模块口径）。当前规模/测试/应用数以
> [README.md](../README.md) 与 MEMORY.md（仓库根）为准；下文 §2/§2.1/§8 的 27 crate /
> 30 handler / 397 路由为 2026-08-21 剥离前快照，保留作历史对照。
...
本项目（NexOS）是**独立个体的操作系统（连接 OS）的 Rust 系统核心**——不是 NAS 系统，
存储是附带能力；核心方向是 P2P 组网（全分布式 Kademlia，见
[NEXOS_P2P_NETWORK_DESIGN.md](./NEXOS_P2P_NETWORK_DESIGN.md)）。它覆盖**存储、网络、
计算、安全、共识、协议、P2P 联邦、AI 协作、客户端**的完整系统软件层——从 ZFS 池管理、
KVM/OCI 计算、防火墙/DHCP/PXE 网络，到 SMB/NFS/WebDAV/FTP/SFTP 协议、联邦大厅/IM/直播、
链上身份与支付验真，再到 A/B OTA 升级、HA 集群（Raft）、CLIP 语义搜索与多 Agent AI
协作，全部用 Rust 编排。最终产物形态包括一个 systemd 编排的守护进程（`osd`）、
网关进程（`os-api`，生产主形态）、一个可安装 ISO（`os-iso`），以及手机/桌面/CLI/MCP
四端客户端；媒体应用以独立产品线（FilmStudio/StreamingStudio）经应用生态分发。
...
+ Error，不含实现。契约规范统一（见 workspace 根 `Cargo.toml` 注释；历史出处：立项版技术路线 §15.1）：
...
1. **先读契约**：找到你关心的 crate，读其 `src/lib.rs` 顶部注释（模块自述）+ trait 定义。
   契约规范见 workspace 根 `Cargo.toml` 顶部注释（历史出处：立项版技术路线 §15.1）。
...
> 2026-09-25 复核刷新（三引擎剥离后）；历史 08-21 口径见括号。

| 维度 | 数值 | 口径/证据 |
|------|------|----------|
| workspace crate | **28**（26 业务 + os-integration + nettest；08-21 为 27） | 根 `Cargo.toml` members；另有 `crates/os-web/` 旧前端存档非 crate |
| Rust 代码量 | **约 31.6 万行**（`crates/**/*.rs`，含测试；08-21 约 21.1 万） | `find + wc -l`，2026-09-25 |
| 前端代码量 | **约 6.7 万行**（Vue/TS） | `crates/os-api/web/src`，2026-09-25 |
| RouteHandler 组件 | **46 常驻 + 1 条件 extra**（剥离前 32） | `main.rs` register_component 全量枚举；os-nexhub 2 个经桥接注册 |
| 静态 API 路由 | **377 条**（os-api 325 + os-nexhub 52；剥离前 397——film/streaming/surveillance 删除、新增 capabilities/terminal/transfer/netboot 等抵消后回落） | grep `spec(` 剔除 `#[cfg(test)]` 模块，2026-09-25 |
| 内置桌面应用 | **28 个**（appRegistry builtinApps 29 项含设置；+ 应用中心运行时可装应用包） | `appRegistry.ts` |
| 测试 | os-api **1,715**（cargo test 基线）；workspace 测试函数约 **5,000+**（grep 口径 5,060） | 2026-09-25 |
...
| 运行时存储 | SQLite 分库自治（gateway/im/media/monitor/apps/tips/ci/hub_lobby/chain-nodes 等）+ 钱包/节点 JSON | 各组件 env 指定路径；「DB 收敛」在路线图 |
| 生产形态 | os-api systemd 服务，单端口四流量（Web/API/git/metrics；缺省 8080，生产经 `NEXOS_HTTP_PORT` 配 8558） | DEPLOYMENT.md §9 |
