...
| P2 发现融合与实网 | mDNS LAN 种子接入 + **观测端点 gossip + TCP 打洞（连接阶梯落地）** + 真实部署：cloud 仅作冷启动锚点与交换所 + 两台真机跨 LAN 实测（优先打洞、中继兜底）+ 节点列表 API/UI（网络页显示拓扑/桶/每连接的阶梯等级） | 两节点跨 LAN 直连或打洞成功；打洞失败自动落中继 ✅ P2a 引擎（2026-08-20）；真机 cloud 锚点 203.0.113.114:7070 + 本机组网已通；**P2b os-api/UI 接入** ✅（2026-08-21，见 §7） |
...
| `NEXOS_P2P_BOOTSTRAP` | 空（孤网等入站） | 引导节点 `host:port,...`（逗号分隔，非法项跳过）——如 `203.0.113.114:7070` | os-p2p bootstrap |
...
- [x] 本机冒烟：`NEXOS_P2P_ENABLE=1 NEXOS_P2P_BOOTSTRAP=203.0.113.114:7070` 启动
