...
/// （含 `inputSchema`——按 tool 参数表渲染 properties/required；无参 tool
/// 为空对象 schema）。
...
/// `tools/call` 处理：从 params.name 定位 tool → client.call_tool（带
/// params.arguments 参数对象）→ MCP result。
///
/// MCP `tools/call` 响应结构：`{content: [{type:"text", text: "..."}], isError: bool}`。
/// 调用失败（os-api 不可达 / tool 不存在 / 参数缺失）时 `isError: true`（tool
/// 执行失败）或 JSON-RPC error（协议层：未知 tool / 缺参数）。
async fn handle_tool_call<B: crate::api::HttpBackend>(
    ...
    // 例外：参数缺失/类型错误是**协议层**错误（invalid_params），上抛为 JSON-RPC
    // error——客户端能立即修正参数重试，不该算 tool 执行失败。
    match client.call_tool(tool, &args).await {
        Ok(v) => Ok(json!({
            "content": [{ "type": "text", "text": v.to_string() }],
            "isError": false
        })),
        Err(e @ OsMcpError::JsonRpc { .. }) => Err(e),
        Err(e) => {
            // 把方法与路径也塞进文本，便于 AI 助手 / 用户排障（「os-api 是否在跑？」）。
            let url = build_url(client.base(), tool);
            Ok(json!({
                "content": [{ "type": "text", "text": format!("调用 {name} 失败（{} {url}）: {e}", tool.method) }],
                "isError": true
            }))
        }
    }
...
mod tests {
    ...
    /// 测试用 mock backend：所有请求返回预设 JSON 字符串。
    struct OkBackend {
        body: String,
        /// 记录收到的请求（断言参数流转发用）。
        seen: std::sync::Mutex<Vec<crate::api::BackendRequest>>,
    }
    impl crate::api::HttpBackend for OkBackend {
        async fn request(
            &self,
            req: &crate::api::BackendRequest,
        ) -> Result<String, OsMcpError> {
    ...
    async fn tools_call_backend_failure_returns_is_error_true() {
        ...
        impl crate::api::HttpBackend for Fail {
            async fn request(
                ...
                Err(OsMcpError::Api(format!("os-api 连接拒绝: {}", req.url)))
            }
        }
    ...
    /// NexHub 工具面（v0.1.50）：tools/list 的 inputSchema 按参数表渲染。
    #[tokio::test]
    async fn tools_list_renders_param_schema_for_nexhub_tools() {
        let client = mock_client("{}");
        let req = parse_req(r#"{"jsonrpc":"2.0","method":"tools/list","id":10}"#);
        let resp = handle_request(&req, &client).await.unwrap();
        let v = serde_json::to_value(&resp).unwrap();
        let tools = v["result"]["tools"].as_array().unwrap();
        let read_file = tools
            .iter()
            .find(|t| t["name"] == "nexhub_read_file")
            .expect("nexhub_read_file 应在 tools/list");
        let schema = &read_file["inputSchema"];
        assert_eq!(schema["type"], "object");
        let props = schema["properties"].as_object().unwrap();
        assert!(props.contains_key("repo") && props.contains_key("path"));
        assert_eq!(props["path"]["type"], "string");
        let required = schema["required"].as_array().unwrap();
        assert!(required.iter().any(|r| r == "repo"));
        assert!(required.iter().any(|r| r == "path"));
        // 无参 tool（nexhub_list_repos）：空 properties + 空 required
        let list_repos = tools
            .iter()
            .find(|t| t["name"] == "nexhub_list_repos")
            .unwrap();
        assert!(list_repos["inputSchema"]["properties"]
            .as_object()
            .unwrap()
            .is_empty());
        assert_eq!(list_repos["inputSchema"]["required"].as_array().unwrap().len(), 0);
        // 8 个 nexhub 工具全部出现
        let names: Vec<&str> = tools.iter().map(|t| t["name"].as_str().unwrap()).collect();
        for n in [
            "nexhub_list_repos",
            ...
            assert!(names.contains(&n), "tools/list 缺 {n}");
        }
    }

    /// NexHub 工具面：tools/call 的 arguments 流转到后端请求（路径模板替换 + POST body）。
    #[tokio::test]
    async fn tools_call_forwards_arguments_to_backend_request() {
        let client = OsApiClient::new(
            "http://127.0.0.1:8080",
            OkBackend {
                body: r#"{"content":"fn main() {}"}"#.to_string(),
                seen: std::sync::Mutex::new(Vec::new()),
            },
        );
        let req = parse_req(
            r#"{"jsonrpc":"2.0","method":"tools/call","params":{"name":"nexhub_read_file","arguments":{"repo":"demo","path":"src/main.rs"}},"id":11}"#,
        );
        let resp = handle_request(&req, &client).await.unwrap();
        let v = serde_json::to_value(&resp).unwrap();
        assert_eq!(v["result"]["isError"], false);
    ...
    /// NexHub 工具面：缺必填参数 → JSON-RPC invalid_params error。
    #[tokio::test]
    async fn tools_call_missing_argument_returns_invalid_params() {
        let client = mock_client("{}");
        let req = parse_req(
            r#"{"jsonrpc":"2.0","method":"tools/call","params":{"name":"nexhub_read_file","arguments":{"repo":"demo"}},"id":12}"#,
        );
        let resp = handle_request(&req, &client).await.unwrap();
        let v = serde_json::to_value(&resp).unwrap();
        assert_eq!(v["error"]["code"], error_code::INVALID_PARAMS);
        assert!(v["error"]["message"].as_str().unwrap().contains("path"));
    }
