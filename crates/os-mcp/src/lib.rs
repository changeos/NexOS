...
//! - `tools/list`：返回全部 tool（name + description + inputSchema——带参工具
//!   渲染 properties/required）。
//! - `tools/call`：调一个 tool（GET 拼 query / POST 组 JSON body → os-api → JSON 返回；
//!   参数经 `params.arguments` 传入）。
//! - `ping`：MCP 心跳，返回空 result。
//!
//! # 模块
//!
//! - [`tools`]：MCP tools 注册表（表驱动：10 个无参只读 OS tool + 8 个 NexHub
//!   仓库操作 tool（v0.1.50：带参读 + 写操作），见 [`tools::ALL_TOOLS`]）。
//! - [`api`]：os-api HTTP 客户端（reqwest + 可注入的 HttpBackend trait + 路径
//!   模板替换 + admin token 注入 + 写操作 mcp 来源标记）。
...
pub use api::{BackendRequest, HttpBackend, OsApiClient, ReqwestBackend};
...
pub use tools::{all_tools, build_url, find_tool, OsTool, ToolParam, ALL_TOOLS};
