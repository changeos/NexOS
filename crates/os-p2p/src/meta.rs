...
//! - **节点自述信息**（2026-09-14，v0.1.45 批——联邦节点信息协议）：node-meta
//!   只有地址簿，集群侧无法确证节点版本/硬件（Spark 版本测不出 / 新 3060
//!   节点 0x03f13c02 身份未知两次踩坑）。心跳 gossip 摘要捎带**节点自述**：
//!   `agent_version`（os-api 版本）/ `hostname` / `platform`（发行版或内核首段）
//!   / `gpu`（nvidia-smi 名字列表或 "none"）——os-p2p 保持 crate 纯（不做硬件
//!   探测），os-api 启动时探测一次经 [`Handle::set_self_desc`] 注入，digest
//!   自广播首条携带（60s 常驻心跳搭车，零新流量）；merge 侧随新鲜报告落库、
//!   转述 digest 继续透传。旧节点（无字段）双向兼容：收不到 = null，旧节点
//!   忽略未知字段。**全集群可见需逐节点升级**（旧版本中继会剥掉 desc 转述）。
//!
...
pub struct NodeMetaEntry {
    ...
    /// 节点自述：os-api/agent 版本（如 "0.1.44"；`None` = 旧版节点未自述）。
    /// 来源仅对端 digest 自广播/转述（版本不可从外部确证——Spark 踩坑），
    /// 直连观测不产生该知识。旧持久化格式无此字段 → 加载为 None。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub agent_version: Option<String>,
    /// 节点自述：机器名（`None` = 旧版节点未自述——前端展示 NodeID 缩写 +
    /// 待升级提示）。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub hostname: Option<String>,
    /// 节点自述：os 信息（发行版串如 "Ubuntu 24.04.1 LTS"，或 "linux 6.8"
    /// 内核首段）。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub platform: Option<String>,
    /// 节点自述：GPU 摘要（nvidia-smi 卡名列表，同型多卡带 ×N；无卡
    /// "none"——与 capabilities 探测链同款 nvidia-smi 调用，os-api 侧探测）。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub gpu: Option<String>,
}

/// 本节点自述信息（os-api 启动时探测一次、经 [`crate::Handle::set_self_desc`]
/// 注入；digest 自广播首条携带）。os-p2p 不做硬件探测（crate 保持纯）——
/// 版本取 os-api 的 `CARGO_PKG_VERSION`、hostname/platform/gpu 由 os-api 侧
/// 探测链产出（gpu 复用 capabilities 同款 nvidia-smi 调用）。
///
/// 全字段可选：未注入（CLI p2p-node 独立跑 / 探测失败）= 全 None，自广播
/// 不携带 desc 字段（`skip_serializing_if`——旧节点线格式零变化）。
#[derive(Debug, Clone, Default, Serialize, Deserialize, PartialEq, Eq)]
pub struct NodeSelfDesc {
    /// os-api（agent）版本，如 "0.1.44"。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub agent_version: Option<String>,
    /// 机器名。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub hostname: Option<String>,
    /// os 信息（发行版串或 linux + 内核首段）。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub platform: Option<String>,
    /// GPU 摘要（nvidia-smi 名字列表或 "none"）。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub gpu: Option<String>,
}
...
pub struct MetaDigestEntry {
    ...
    #[serde(default)]
    pub exit_offered: bool,
    /// 节点自述四字段（2026-09-14，v0.1.45 批）：自广播首条携带注入的自述、
    /// 注册表转述条目透传既存自述。旧版本 digest 无此字段 → 解析为 None
    /// （不作自述知识）；本版本对 None 不序列化（`skip_serializing_if`）——
    /// 旧节点收到的线格式零变化（且 serde 默认忽略未知字段，双保险）。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub agent_version: Option<String>,
    /// 机器名（见 [`NodeMetaEntry::hostname`]）。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub hostname: Option<String>,
    /// os 信息（见 [`NodeMetaEntry::platform`]）。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub platform: Option<String>,
    /// GPU 摘要（见 [`NodeMetaEntry::gpu`]）。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub gpu: Option<String>,
}

impl NodeMetaEntry {
    /// 采纳报告携带的自述四字段（**仅 Some 覆盖，None 保持**）：报告方可能
    /// 是旧版本节点中继转述（线格式无 desc 字段 → None），以其 None 抹掉
    /// 已存自述会让知识经旧中继丢失；新值整包覆盖（同节点重启换版本/换卡
    /// 场景，报告新鲜即采纳新自述）。
    fn adopt_desc(&mut self, r: &MetaDigestEntry) {
        if r.agent_version.is_some() {
            self.agent_version = r.agent_version.clone();
        }
        if r.hostname.is_some() {
            self.hostname = r.hostname.clone();
        }
        if r.platform.is_some() {
            self.platform = r.platform.clone();
        }
        if r.gpu.is_some() {
            self.gpu = r.gpu.clone();
        }
    }
}
...
pub struct NodeMetaStore {
    ...
    /// 负责随 gossip 广播。
    self_exit: bool,
    /// 本节点自述信息（agent 版本/hostname/platform/gpu）：os-api 启动时探测
    /// 一次经 [`crate::Handle::set_self_desc`] 注入，digest 自广播首条携带。
    /// **不持久化**——进程内事实（版本/主机名/硬件随每次启动重探），缺省
    /// 全 None（CLI p2p-node 独立跑 = 不自述）。
    self_desc: NodeSelfDesc,
}
...
impl NodeMetaStore {
    ...
    pub fn with_exit_offer(path: Option<PathBuf>, exit_offered: bool) -> Self {
        let mut store = Self {
            ...
            self_desc: NodeSelfDesc::default(),
        };
    ...
    /// 注入本节点自述信息（os-api 启动装配的落点，2026-09-14）：探测一次
    /// （版本/hostname/platform/gpu）整包覆盖。下一轮 gossip（≤6 tick）自广播
    /// 首条即携带，直连对端 1 轮内学到；经转述全网 1-2 轮（旧版本中继会剥掉
    /// desc——全集群可见需逐节点升级）。
    pub fn set_self_desc(&mut self, desc: NodeSelfDesc) {
        self.self_desc = desc;
    }

    /// 本节点当前自述信息（观察面；未注入 = 全 None）。
    #[must_use]
    pub fn self_desc(&self) -> &NodeSelfDesc {
        &self.self_desc
    }
    ...
    pub fn record_conn(&mut self, id: &NodeId, addr: SocketAddr, now: u64) {
        ...
        let entry = self
            ...
            .or_insert_with(|| NodeMetaEntry {
                ...
                verified: true,
                exit_offered: false,
                // 直连观测不产生自述知识（版本/hostname/硬件由对端自广播决定）
                agent_version: None,
                hostname: None,
                platform: None,
                gpu: None,
        ...
        // 既有条目的 exit_offered 位保持；自述四字段同理（见 merge_digest）。
            ...
            match self.entries.get_mut(&r.id) {
                ...
                Some(entry) => match entry.state {
                    ...
                    MetaState::Active { .. } => {
                        ...
                        // 自述四字段同款声明转发语义（adopt_desc 内部仅 Some
                        // 覆盖——旧版本中继的 None 不抹知识）。
    ...
    /// 只能由这里补上）。空注册表也广播——只有自广播一条。自广播首条携带
    /// [`NodeMetaStore::self_desc`]（os-api 启动注入的自述；未注入 = 全 None，
    /// 线格式不带 desc 字段）。
    fn digest_with_self(
        &self,
        self_id: &NodeId,
...
/// 哪、活着、我是什么（自述）"。重装系统换新 NodeID 后无需先被对端直连观测
/// 到，经一次 gossip 即被全网感知（旧身份条目则由指纹探测天然判不可信，无需
/// 额外修复）。`advertise=None` 时跳过 addr——id/alive/last_seen 仍带（活性
/// 信号独立于地址信号）。verified=true：本机对自身活性的第一手结论。
/// `exit_offered` = 本机出口声明（network-exit：`NEXOS_P2P_EXIT_OFFER=1` 或
/// 运行期 offer 端点）——全网凭这一位发现可用出口。`desc` = 本机自述四字段
/// （agent 版本/hostname/platform/gpu，2026-09-14 联邦节点信息协议——os-api
/// 启动经 [`crate::Handle::set_self_desc`] 注入一次；未注入 = 全 None，线格式
/// 不带 desc 字段——旧节点收到的线格式零变化）。
...
mod tests {
    ...
    // 24. 节点自述广播与合并（联邦节点信息协议，2026-09-14 v0.1.45 批）：
    //     set_self_desc → digest 自广播首条携带四字段；未注入 = 全 None（CLI
    //     p2p-node 独立跑）；merge 新建/新鲜更新/复活三路径采自述；旧版本中继
    //     的 None 报告不抹既有知识；陈旧报告不回灌；转述 digest 继续透传
    #[test]
    fn self_desc_broadcast_merge_and_relay() {
        let mut store = NodeMetaStore::new(None);
        let self_id = nid(140);
        let now = 8_000u64;
        let desc = NodeSelfDesc {
            agent_version: Some("0.1.45".into()),
            hostname: Some("nexos-a".into()),
            platform: Some("Ubuntu 24.04.1 LTS".into()),
            gpu: Some("NVIDIA GeForce RTX 3060".into()),
        };
        // 未注入（默认）：自广播不带自述（全 None——旧节点线格式零变化）
        let d0 = store.digest_with_self(&self_id, None, now).remove(0);
        assert_eq!(
            (d0.agent_version, d0.hostname, d0.platform, d0.gpu),
            (None, None, None, None),
            "未注入 = 全 None"
        );
        // 注入：自广播首条携带四字段（下一轮 gossip 生效）
        store.set_self_desc(desc.clone());
        assert_eq!(store.self_desc(), &desc, "观察面回读注入值");
        let d1 = store.digest_with_self(&self_id, Some(addr(70)), now).remove(0);
        assert_eq!(d1.agent_version.as_deref(), Some("0.1.45"));
        assert_eq!(d1.hostname.as_deref(), Some("nexos-a"));
        assert_eq!(d1.platform.as_deref(), Some("Ubuntu 24.04.1 LTS"));
        assert_eq!(d1.gpu.as_deref(), Some("NVIDIA GeForce RTX 3060"));
        assert_eq!(d1.addr, Some(addr(70)), "自广播其余字段不受影响");

        // 对端视角合并：未知节点新建即采自述（None 字段 = 报告方不知道）
        let mut peer = NodeMetaStore::new(None);
        let remote = nid(141);
        peer.merge_digest(
            &nid(142),
            &[MetaDigestEntry {
                ...
                exit_offered: false,
                agent_version: Some("0.1.45".into()),
                hostname: Some("nexos-b".into()),
                platform: Some("linux 6.8.0-30-generic".into()),
                gpu: Some("none".into()),
            }],
            now,
            Duration::from_secs(12),
        );
        let e = entry_of(&peer, &remote);
        assert_eq!(e.agent_version.as_deref(), Some("0.1.45"), "新建条目学到版本");
        assert_eq!(e.hostname.as_deref(), Some("nexos-b"));
        assert_eq!(e.platform.as_deref(), Some("linux 6.8.0-30-generic"));
        assert_eq!(e.gpu.as_deref(), Some("none"), "无卡节点的诚实自述");

        // 转述：peer 的 digest 继续透传自述（全网 1-2 轮感知）
        let relayed = peer.digest(now + 1);
        let r = relayed.iter().find(|d| d.id == remote).unwrap();
        assert_eq!(r.agent_version.as_deref(), Some("0.1.45"), "转述透传版本");

        // 新鲜报告整包覆盖（同节点重启换版本/换卡场景）
        peer.merge_digest(
            &nid(142),
            &[MetaDigestEntry {
                id: remote.clone(),
                ...
                verified: true,
                exit_offered: false,
                agent_version: Some("0.1.46".into()),
                hostname: Some("nexos-b2".into()),
                platform: Some("Ubuntu 24.04.2 LTS".into()),
                gpu: Some("NVIDIA GeForce RTX 4090".into()),
            }],
            now + 5,
            Duration::from_secs(12),
        );
        let e = entry_of(&peer, &remote);
        assert_eq!(e.agent_version.as_deref(), Some("0.1.46"), "新鲜报告采纳新自述");
        assert_eq!(e.gpu.as_deref(), Some("NVIDIA GeForce RTX 4090"), "换卡同步感知");

        // 旧版本中继的 None 报告不抹既有知识（last_seen 新鲜但 desc 全缺）
        peer.merge_digest(
            &nid(142),
            &[MetaDigestEntry {
                id: remote.clone(),
        ...
        let e = entry_of(&peer, &remote);
        assert_eq!(
            e.agent_version.as_deref(),
            Some("0.1.46"),
            "None 报告（旧版本中继剥掉 desc）不抹既有自述"
        );
        assert_eq!(e.gpu.as_deref(), Some("NVIDIA GeForce RTX 4090"));
        ...
        assert_eq!(
            entry_of(&peer, &remote).agent_version.as_deref(),
            Some("0.1.46"),
            "陈旧报告不回灌旧自述"
        );

        // 复活路径也采自述：出局条目经带 desc 的新鲜 alive 报告复活
        peer.record_conn(&remote, addr(4), now);
        strike_out(&mut peer, &remote, now + 20);
        peer.merge_digest(
            &nid(142),
            &[MetaDigestEntry {
                id: remote.clone(),
                ...
                verified: true,
                exit_offered: false,
                agent_version: Some("0.1.47".into()),
                hostname: Some("nexos-b3".into()),
                platform: None,
                gpu: None,
            }],
            now + 30,
        ...
        // 直连观测不产生/不覆盖自述知识（版本由对端自广播决定）
        let mut fresh = NodeMetaStore::new(None);
        fresh.record_conn(&nid(143), addr(5), 100);
        let e = entry_of(&fresh, &nid(143));
        assert_eq!(
            (e.agent_version, e.hostname, e.platform, e.gpu),
            (None, None, None, None),
            "直连建档不自述"
        );
    }

    // 25. 自述线格式 + 持久化兼容：None 字段序列化省略（旧节点收到的线格式
    //     零变化）；旧 JSON（无 desc 字段）解析 → None（双向兼容）；带 desc
    //     落盘重载保真（重启不丢已学到的对端自述）
    #[test]
    fn self_desc_wire_and_persist_compat() {
        // 线格式：新节点 digest 带 desc 四字段
        let entry = MetaDigestEntry {
            id: nid(144),
            addr: Some(addr(1)),
            last_seen: 42,
        ...
        let json = serde_json::to_value(&entry).unwrap();
        assert_eq!(json["agent_version"], serde_json::json!("0.1.45"));
        assert_eq!(json["hostname"], serde_json::json!("nexos-a"));
        let back: MetaDigestEntry = serde_json::from_value(json).unwrap();
        assert_eq!(back.gpu.as_deref(), Some("none"));
        // None 字段省略（skip_serializing_if——旧节点线格式零变化）
        let bare = MetaDigestEntry {
            agent_version: None,
            hostname: None,
            platform: None,
            gpu: None,
        ...
        for f in ["agent_version", "hostname", "platform", "gpu"] {
            assert!(
                json.get(f).is_none(),
                "None 字段 {f} 不序列化（旧节点线格式零变化）"
            );
        }
        // 旧版本 digest JSON（无 desc 字段）→ 解析为 None（向后兼容）
        let legacy = serde_json::json!({
            "id": nid(144).to_hex(),
            "addr": "203.0.113.1:41000",
            "last_seen": 42,
        ...
        let old: MetaDigestEntry = serde_json::from_value(legacy).unwrap();
        assert_eq!(
            (old.agent_version, old.hostname, old.platform, old.gpu),
            (None, None, None, None),
            "旧 digest 无 desc 字段 → None"
        );

        // 持久化：带 desc 条目落盘重载保真
        let dir = std::env::temp_dir().join(format!("p2p-meta-desc-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let file = dir.join("node-meta-desc.json");
        let _ = std::fs::remove_file(&file);
        let a = nid(145);
        let mut store = NodeMetaStore::new(Some(file.clone()));
        store.merge_digest(
            ...
            &[MetaDigestEntry {
                ...
                hostname: Some("nexos-a".into()),
                platform: Some("Ubuntu 24.04.1 LTS".into()),
                gpu: Some("NVIDIA GeForce RTX 3060 ×2".into()),
            }],
            1000,
            Duration::from_secs(12),
        );
        let json = store.flush_final().expect("合并即脏");
        write_meta_atomic(&file, &json).unwrap();
        let reloaded = NodeMetaStore::new(Some(file.clone()));
        let e = entry_of(&reloaded, &a);
        assert_eq!(e.agent_version.as_deref(), Some("0.1.45"), "自述持久化保真");
        assert_eq!(e.hostname.as_deref(), Some("nexos-a"));
        assert_eq!(e.platform.as_deref(), Some("Ubuntu 24.04.1 LTS"));
        assert_eq!(
            e.gpu.as_deref(),
            Some("NVIDIA GeForce RTX 3060 ×2"),
            "同型多卡 ×N 摘要保真"
        );
        // 旧持久化格式（无 desc 字段，exit_offered 时代）→ 加载为 None
        let b = nid(147);
        let legacy_persist = format!(
            "[{{\"id\":\"{}\",\"addrs\":[{{\"addr\":\"203.0.113.2:41000\",\"verified\":true}}],\
             \"first_seen\":100,\"last_seen\":200,\
             \"state\":{{\"active\":{{\"score\":60,\"consec_fail\":0}}}},\
             \"source\":\"direct\",\"verified\":true,\"exit_offered\":false}}]",
            b.to_hex(),
        );
        std::fs::write(&file, legacy_persist).unwrap();
        let migrated = NodeMetaStore::new(Some(file.clone()));
        let e = entry_of(&migrated, &b);
        assert_eq!(
            (e.agent_version, e.hostname, e.platform, e.gpu),
            (None, None, None, None),
            "旧持久化无 desc 字段 → None"
    ...
    // 26. Handle::set_self_desc 命令链路往返（注入面接线——os-api 启动装配
    //     的落点）：注入 → 应答回存快照；未注入的独立节点观察面恒全 None。
    //     （说明：自广播到达对端的端到端在 os-api p2p.rs 双实例 mesh 测试覆盖
    //     ——store 级广播/合并语义由测试 24 覆盖。）
    #[tokio::test]
    async fn self_desc_handle_wiring_roundtrip() {
        let node = crate::P2pNode::spawn(crate::P2pConfig {
            listen: "127.0.0.1:0".parse().unwrap(),
            timings: crate::Timing::testing(),
            mdns_enabled: false,
        ...
        let back = node.set_self_desc(desc.clone()).await;
        assert_eq!(back, desc, "应答回存快照（注入生效）");
        // 覆盖注入（重启探测同一落点——整包替换）
        let desc2 = NodeSelfDesc {
            gpu: Some("none".into()),
            ..desc.clone()
        };
        let back = node.set_self_desc(desc2.clone()).await;
        assert_eq!(back, desc2, "二次注入整包覆盖");
        node.shutdown().await;
    }
