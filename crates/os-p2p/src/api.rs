...
enum Cmd {
    ...
    /// 注入本节点自述信息（联邦节点信息协议，2026-09-14：agent 版本 /
    /// hostname / platform / gpu——os-api 启动时探测一次注入；digest 自广播
    /// 首条携带）。应答回存后的自述快照（观察面）。
    SetSelfDesc {
        desc: crate::meta::NodeSelfDesc,
        resp: oneshot::Sender<crate::meta::NodeSelfDesc>,
    },
    Shutdown {
        done: oneshot::Sender<()>,
    },
...
impl Handle {
    ...
    /// 注入本节点自述信息（联邦节点信息协议，2026-09-14 v0.1.45 批）：
    /// os-api 启动时探测一次（agent 版本 / hostname / platform / gpu）整包
    /// 注入——digest 自广播首条即携带（60s 常驻心跳搭车，零新流量），直连
    /// 对端 1 轮内学到。本 crate 不做硬件探测（保持纯），探测链在 os-api。
    /// 返回回存后的自述快照（停机边缘返回默认全 None——调用方经 eprintln
    /// 落启动日志可核对注入是否生效）。
    pub async fn set_self_desc(&self, desc: crate::meta::NodeSelfDesc) -> crate::meta::NodeSelfDesc {
        let (resp, rx) = oneshot::channel();
        if self.cmd.send(Cmd::SetSelfDesc { desc, resp }).is_ok() {
            rx.await.unwrap_or_default()
        } else {
            crate::meta::NodeSelfDesc::default()
        }
    }
}
...
async fn cmd_loop(shared: Arc<Shared>, mut rx: mpsc::UnboundedReceiver<Cmd>) {
    while let Some(cmd) = rx.recv().await {
        match cmd {
            ...
            Cmd::SetSelfDesc { desc, resp } => {
                let mut st = shared.state.lock().expect("state poisoned");
                st.meta.set_self_desc(desc);
                let now = st.meta.self_desc().clone();
                drop(st);
                tracing::info!(
                    agent_version = ?now.agent_version,
                    hostname = ?now.hostname,
                    "注入节点自述（下一轮元数据交互自广播首条携带）"
                );
                let _ = resp.send(now);
            }
            Cmd::Shutdown { done } => {
