...
//! | `NEXOS_LIVE_TX_BSC` | 同上 | 覆盖 BSC 主网核验交易（v0.1.47，默认见下） |
...
//! | `NEXOS_LIVE_RPC_BSC` | URL | 覆盖 BSC 端点（默认 `fallback_rpc_for(56)` 全表，v0.1.47） |
...
//! 的历史区块局限，非本测试缺陷。
//! Sepolia fixture：区块 11,728,025 的真实普通转账 0.0122 ETH——
//! hash `0xb6d79d79…907a2`，to `0x82bbedf9…b1e730`，value `12244897959183673`
//! （2026-09-18 换新：原 fixture 块 11,605,120 被 publicnode 裁剪收据——
//! 公共全节点不存全量历史收据，旧 fixture 判 Pending；fixture 须取近期
//! 交易并接受定期换血，主网 2017 年的 fixture 能活是因为公共端点普遍
//! 服务主网全量收据，Sepolia 无此待遇）。
//!
//! 结果（2026-09-18 复验，三例 × 三链全过）：
//! 主网 Verified{block:4370001} / Mismatch-to / Mismatch-value；
//! Sepolia Verified{block:11728025} / Mismatch-to / Mismatch-value；
//! BSC Verified{block:122530968} / Mismatch-to / Mismatch-value。
//!
//! ## BSC 主网真机验真（v0.1.47，2026-09-18，同上国内直连环境）
//!
//! 候选 10 家 curl eth_blockNumber×2 + eth_chainId×1 计时实测：7 家通且
//! <1.5s（publicnode 0.60s 均值最快 → 1rpc 1.12s 最慢），ankr 要 API key、
//! blockpi HTTP 521、nodereal DNS 不存在 → 剔除；表与延迟双写于
//! docs/BLOCKCHAIN_NODES.md §10。fixture：区块 122,530,968 的真实普通转账
//! 1.351034742893322440 BNB（native BNB，18 位 wei 口径与 ETH 同构）——
//! hash `0x633d81ea939f95c5b9e6130e1a12985483af551916b0aebaa0fb1dfa0f70d0e9`，
//! to `0xa1b6183ecaf30dd6e147229712b46104d286570e`，value
//! `1351034742893322440`。结果（fallback 全表 failover 首家 publicnode）：
//! Verified{block:122530968} / Mismatch-to / Mismatch-value 三例全过——
//! 证明 EVM 通用核验路径对 BSC 零适配。
//!
//! ## anvil dev 链全闭环（任务 2）
//!
...
/// Sepolia 真实普通转账（块 11,728,025，0.012244897959183673 ETH；
/// 2026-09-18 v0.1.47 批换新——原 2026-08-31 fixture（块 11,605,120）被
/// publicnode 裁剪收据（receipt null→Pending；tenderly 仍可回），公共全节点
/// 不保存全量历史收据，fixture 取「近期」交易并接受定期换血）。
const SEPOLIA_TX: RealTx = RealTx {
    chain_id: 11_155_111,
    hash: "0xb6d79d7913391b179b794bd60c2b7c7ec5741c5b9aa4a063766d54ced54907a2",
    to: "0x82bbedf9f4c086e4291810b9d8bf00cbd0b1e730",
    value: "12244897959183673",
    block: Some(11_728_025),
};

/// BSC 主网真实普通转账（块 122,530,968，1.351034742893322440 BNB；
/// v0.1.47 BSC 批，2026-09-18 从 bnbchain 官方 dataseed 取块挑的 native 转账）。
const BSC_TX: RealTx = RealTx {
    chain_id: 56,
    hash: "0x633d81ea939f95c5b9e6130e1a12985483af551916b0aebaa0fb1dfa0f70d0e9",
    to: "0xa1b6183ecaf30dd6e147229712b46104d286570e",
    value: "1351034742893322440",
    block: Some(122_530_968),
};
...
/// 探测主网/Sepolia/BSC 全部 fallback 端点，输出可达性表；断言三网各至少一家健康
/// （这是下面三条真链用例的前置条件——全挂时给出清晰的表而非逐条 RpcError）。
#[tokio::test]
...
/// BSC 主网真机验真（v0.1.47）：native BNB 转账三例断言——证明 EVM 通用
/// 核验路径对 BSC（chainId=56）零适配（fixture/实测记录见文件头）。
#[tokio::test]
#[ignore = "依赖外网公共 RPC（真实历史交易核验，见文件头验收记录）"]
async fn bsc_real_tx_verify() {
    let tx = resolve_tx("NEXOS_LIVE_TX_BSC", BSC_TX);
    let rpcs = resolve_rpcs("NEXOS_LIVE_RPC_BSC", 56);
    assert_three_way(&rpcs, &tx, PUBLIC_TIMEOUT).await;
}
