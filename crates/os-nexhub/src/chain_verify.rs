...
//!
//! # v0.1.47 增量（2026-09-18）：BNB Smart Chain 主网（chainId=56）接入
//!
//! - **零核验代码改动**：BSC 是 EVM 同构链（同一套 JSON-RPC——
//!   `eth_chainId`/`eth_getTransactionByHash`/`eth_getTransactionReceipt`、
//!   同形状 receipt status/logs），`verify_evm_tx` 的 EVM 通用路径
//!   （native + ERC-20 + AmountRule）对 BSC **原样适用**——本批只加
//!   [`fallback_rpc_for`] 的 56 号 RPC 兜底表（国内实测换血，2026-09-18）与
//!   链目录 [`chain_info`]（name/symbol/explorer 展示元数据）。
//! - **确认数语义**（与 ETH 主网/Sepolia 条目同构——验真层**没有**按块数
//!   校准的确认数阈值）：上块即查 receipt，`status==0x1` 即 Verified、
//!   无 receipt 即 Pending；不跟踪 `latest - blockNumber` 块差。BSC 侧该
//!   语义**更安全而非更弱**：BSC 2025 年 Maxwell 升级后出块 0.75s 且带
//!   快速终结（BEP-520，~2 块 ≈1.5s 终结），receipt 可回滚窗口远短于
//!   ETH 主网（PoS 两 epoch ~13 分钟才经济终结）——沿用同一条目口径，
//!   不加 confirmations 字段（既有 Sepolia 条目也没有）。
//! - 接线层零改动：BSC 支付 = currency `evm`（BNB 也是 18 位小数，
//!   `to_wei_str` 的 18 位假设成立）+ body `chain_id: 56`（或 env
//!   `NEXOS_EVM_CHAIN_ID=56`）——收款地址共用 `NEXOS_PAY_EVM_ADDR`
//!   （EVM 地址格式 0x+40hex 全 EVM 链通用，不新增 per-chain env）。
...
/// 公共 RPC 兜底列表：blockchain.rs 链预设（`chain_presets()`，7 条）+ BSC
/// 主网 56（v0.1.47，链上验真接入但暂不进节点运行预设）。
///
/// 覆盖范围：公链 ethereum(1)/bsc(56)/sepolia(11155111)/optimism(10)/
/// arbitrum(42161)/base(8453) 给无 key 公共端点；dev(1337) 给本机
/// anvil/ganache 默认端口（blockchain.rs「JSON-RPC 端口默认 8545」）；
/// custom(0) 与未知链无兜底（空）。
///
...
pub fn fallback_rpc_for(chain_id: u64) -> Vec<String> {
    match chain_id {
        ...
        // BNB Smart Chain 主网（2026-09-18 国内直连实测换血，106 上 curl
        // eth_blockNumber×2 + eth_chainId×1 逐家计时：ankr 要 API key /
        // blockpi HTTP 521 停服 / nodereal DNS 域名不存在 → 剔除；下述 7 家
        // 全通且均 <1.5s（最慢 1rpc 1.48s），按实测均值延迟升序入表——
        // publicnode 0.60s / dataseed2 0.69s / dataseed 0.71s / dataseed1 0.75s /
        // dataseed3 0.83s / dataseed4 1.00s / 1rpc 1.12s。
        // BNB 官方 dataseed 系为 geo-DNS 同一集群的 5 个入口，当一个用即可；
        // 实测记录双写于 docs/BLOCKCHAIN_NODES.md §10。）
        56 => vec![
            ...
            "https://bsc-dataseed3.bnbchain.org".into(),
            "https://bsc-dataseed4.bnbchain.org".into(),
...
// ----------------------------------------------------------------------------
// 链目录（v0.1.47 BSC 批）：验真支持链的展示元数据
// ----------------------------------------------------------------------------

/// 链目录条目：{chain_id, name, symbol, explorer}——供 UI/API 展示链身份与
/// 交易外链（`format!("{}/tx/{tx_hash}", info.explorer)`）。
///
/// 这是**展示元数据**而非核验依据：核验只看 RPC 事实（chainId 对链 / receipt
/// status），目录缺条目的链只要有 RPC 候选（env `NEXOS_CHAIN_RPC_URLS` 或
/// body `rpc_url`）照样可验——目录覆盖「有公共兜底表的链」（与
/// [`fallback_rpc_for`] 的公链条目一一对应，dev/custom 本地链无展示需求不收录）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ChainInfo {
    /// EVM 链 ID（1=以太坊主网，56=BSC 主网，11155111=Sepolia，…）
    pub chain_id: u64,
    /// 链展示名（如 "Ethereum 主网"；前端/日志用）。
    pub name: &'static str,
    /// native 币符号（ETH/BNB；金额展示与文档口径用）。
    pub symbol: &'static str,
    /// 区块浏览器根 URL（拼 `/tx/<hash>` 得交易外链）。
    pub explorer: &'static str,
}

/// 链目录静态表。ETH 主网为基准条目，BSC（56，v0.1.47）与之同构新增。
static CHAIN_INFOS: [ChainInfo; 3] = [
    ChainInfo {
        chain_id: 1,
        name: "Ethereum 主网",
        symbol: "ETH",
...
/// 按链 ID 查目录（未收录 → None；L2/dev/custom 未收录——有 RPC 仍可验）。
#[must_use]
pub fn chain_info(chain_id: u64) -> Option<&'static ChainInfo> {
    CHAIN_INFOS.iter().find(|c| c.chain_id == chain_id)
}
...
mod tests {
    ...
    // 17. fallback_rpc_for：链预设的兜底覆盖（公链给无钥端点，dev 给本机）。
    #[test]
    ...
    // ============================================================================
    // v0.1.47 BSC 批（2026-09-18）：链定义 / RPC 兜底表 / chainId=56 分流
    // ============================================================================

    // 17b. BSC(56) 兜底表结构：实测入表的 7 家按延迟序排列、全 https、
    //      实测剔除的三家（ankr 要 key / blockpi 521 / nodereal DNS 不存在）不在表。
    #[test]
    fn bsc_fallback_table_matches_field_test() {
        let bsc = fallback_rpc_for(56);
        assert_eq!(
            bsc,
            vec![
            ...
            "7 家按 2026-09-18 国内实测延迟升序"
        );
        assert!(bsc.iter().all(|u| u.starts_with("https://")), "全 https");
        assert!(
            !bsc.iter().any(|u| u.contains("ankr")),
            "ankr 国内实测要 API key，剔除"
        );
        assert!(
            !bsc.iter().any(|u| u.contains("blockpi")),
            "blockpi 国内实测 HTTP 521 停服，剔除"
        );
        assert!(
            !bsc.iter().any(|u| u.contains("nodereal")),
            "nodereal 域名 DNS 不存在，剔除"
        );
    }

    // 17c. 链目录：BSC 条目与 ETH 主网同构；L2/dev/custom 未收录（有 RPC 仍可验）。
    #[test]
    fn chain_info_covers_verify_supported_chains() {
        let eth = chain_info(1).expect("ETH 主网条目");
        assert_eq!((eth.name, eth.symbol, eth.explorer), ("Ethereum 主网", "ETH", "https://etherscan.io"));
        let bsc = chain_info(56).expect("BSC 条目");
        assert_eq!(
            (bsc.chain_id, bsc.name, bsc.symbol, bsc.explorer),
            (56, "BNB Smart Chain", "BNB", "https://bscscan.com"),
            "BSC 链定义冻结字段"
        );
        assert_eq!(chain_info(11_155_111).map(|c| c.name), Some("Sepolia 测试网"));
        assert!(chain_info(10).is_none(), "Optimism 未收录（无展示需求，非不可验）");
        assert!(chain_info(1337).is_none(), "dev 链未收录");
        assert!(chain_info(0).is_none() && chain_info(999_999).is_none());
    }

    // 17d. BSC native 全绿：chainId=0x38 分流进 EVM 通用路径零适配——
    //      同一套 verify_evm_tx 序列对 BSC 形状（0x38/BNB wei）照常出 Verified。
    #[tokio::test]
    async fn bsc_native_transfer_verified_via_generic_evm_path() {
        let (url, seen) = spawn_rpc_mock(green(
            "0x38", // BSC chainId（56）
            format!("0x{}", "cd".repeat(20)),
            one_eth_padded_hex(),
    ...
    // 17e. chainId 分流拦截：端点回 0x38（BSC）而凭证声称主网（1）→
    //      Mismatch("chain", expect=1, actual=56)，且查交易前短路。
    #[tokio::test]
    async fn bsc_endpoint_against_mainnet_proof_short_circuits() {
        let (url, seen) = spawn_rpc_mock(green(
            "0x38",
            format!("0x{}", "cd".repeat(20)),
            one_eth_padded_hex(),
