...
//!   默认 `8080`）。
//!
//! # 路由表（原生 12 + Issues/PR 协作 12（[`crate::issues`]）+ Webhooks 8
//! （[`crate::webhooks`]，v0.1.50）+ MCP 配套 2（v0.1.50）；component="code_repo"）
//!
//! | method | path                                       | 动作 |
//! |--------|--------------------------------------------|------|
...
//! | POST   | `/api/v1/coderepo/repos/:name/file`       | 写文件（admin，MCP 配套：git plumbing 落 commit，author=mcp）|
//! | GET    | `/api/v1/coderepo/repos/:name/search`     | 代码搜索（公开，MCP 配套：`git grep -F` ≤50 命中）|
...
//! | GET/POST/DELETE | `/api/v1/coderepo/repos/:name/issues`（及 `/:num`、`/comments`、`/close`、`/open`）| Issues 协作（[`crate::issues`]）|
//! | GET/POST | `/api/v1/coderepo/repos/:name/pulls`（及 `/:num`、`/comments`、`/merge`、`/close`）| Pull Requests 协作（[`crate::issues`]）|
//! | GET/POST/DELETE | `/api/v1/coderepo/repos/:name/webhooks`（及 `/:id/toggle`）| Webhooks 仓库面（admin，[`crate::webhooks`]）|
//! | GET/POST/DELETE | `/api/v1/coderepo/webhooks`（及 `/:id/toggle`）| Webhooks 全局面（admin）|
...
/// 写文件请求体（POST /repos/:name/file，v0.1.50 MCP 工具面配套）。
#[derive(Debug, Deserialize)]
struct WriteFileBody {
    ...
    /// 文件全文（UTF-8；≤2 MiB）。
    content: String,
    /// 提交信息（缺省 `mcp write: <path>`）。
    #[serde(default)]
    message: Option<String>,
    /// 目标分支（缺省 = 仓库有效默认分支；须已存在）。
    #[serde(default)]
...
// ----------------------------------------------------------------------------
// 写文件 / 代码搜索（v0.1.50 MCP 工具面配套，git plumbing——不落工作树）
// ----------------------------------------------------------------------------

/// MCP 写入的 git 提交作者名（身份标记：AI agent 经 MCP 写仓的提交可追溯）。
pub const MCP_AUTHOR_NAME: &str = "mcp";
/// MCP 写入的 git 提交作者邮箱。
pub const MCP_AUTHOR_EMAIL: &str = "mcp@nexhub.local";
/// 代码搜索命中上限（`git grep` 输出截断）。
pub const SEARCH_MATCH_LIMIT: usize = 50;

/// 同步执行带额外 env 的 git（`git --git-dir=<bare> <args>`），返回 `(success, stdout)`。
fn run_git_env_sync(git_dir: &str, envs: &[(&str, &str)], args: &[&str]) -> (bool, String) {
...
/// 写文件到裸仓分支（git plumbing，不 clone 不落工作树）：
///
/// 1. `hash-object -w --stdin` 存 blob；
/// 2. 临时 index（`GIT_INDEX_FILE`）`read-tree <branch>` → `update-index --cacheinfo`
///    替换单路径 → `write-tree` 重建 tree；
/// 3. `commit-tree`（author/committer 固定 `mcp <mcp@nexhub.local>`）；
/// 4. `update-ref refs/heads/<branch>` 快进。
///
/// 返回新 commit 完整 hash。调用方须已校验 repo/path/branch（本函数只管执行）。
fn write_file_blocking(
    ...
    let blob = {
        ...
        let mut child = cmd
            .spawn()
            .map_err(|e| format!("spawn git hash-object 失败: {e}"))?;
        use std::io::Write;
        if let Some(mut stdin) = child.stdin.take() {
            stdin.write_all(content).map_err(|e| format!("写 blob 失败: {e}"))?;
        }
        let out = child
            .wait_with_output()
            .map_err(|e| format!("等待 git hash-object 失败: {e}"))?;
    ...
    // 临时 index：read-tree → update-index → write-tree
    ...
    let env_index = [("GIT_INDEX_FILE", index_file.as_str())];
    let result = (|| -> Result<String, String> {
        let branch_ref = format!("refs/heads/{branch}");
        let (ok, out) = run_git_env_sync(bare, &env_index, &["read-tree", &branch_ref]);
        if !ok {
            return Err(format!("git read-tree 失败: {out}"));
        }
        let cacheinfo = format!("100644,{blob},{path}");
        let (ok, out) = run_git_env_sync(
            bare,
            &env_index,
            &["update-index", "--add", "--cacheinfo", &cacheinfo],
        );
        if !ok {
            return Err(format!("git update-index 失败: {out}"));
        }
        let (ok, tree) = run_git_env_sync(bare, &env_index, &["write-tree"]);
        if !ok {
            return Err(format!("git write-tree 失败: {tree}"));
        }
        // commit-tree：author/committer 固定 mcp（身份标记）
        let env_author = [
            ("GIT_AUTHOR_NAME", MCP_AUTHOR_NAME),
            ("GIT_AUTHOR_EMAIL", MCP_AUTHOR_EMAIL),
            ("GIT_COMMITTER_NAME", MCP_AUTHOR_NAME),
            ("GIT_COMMITTER_EMAIL", MCP_AUTHOR_EMAIL),
        ];
        let parent = format!("-p{branch_ref}");
        let msg = format!("-m{message}");
        let (ok, commit) = run_git_env_sync(
            bare,
            &env_author,
            &["commit-tree", &tree, &parent, &msg],
        );
        if !ok {
            return Err(format!("git commit-tree 失败: {commit}"));
        }
        let (ok, out) = run_git_env_sync(bare, &[], &["update-ref", &branch_ref, &commit]);
        if !ok {
            return Err(format!("git update-ref 失败: {out}"));
        }
        Ok(commit)
    })();
...
/// 代码搜索（`git grep -n -I -F -e <q> <branch>`，固定串子串匹配）：返回
/// `{path, line, text}` 命中列表（≤ [`SEARCH_MATCH_LIMIT`] 条，按 git 输出序）。
/// 仓库空/分支无提交 → 空列表（降级不报错）。
fn search_code_blocking(bare: &str, q: &str) -> Vec<serde_json::Value> {
    ...
    out.lines()
        ...
        .filter_map(|l| {
            // grep 指定 <ref> 时行格式 `<ref>:<path>:<line>:<text>`——先剥 ref 前缀
            let rest = l
                .strip_prefix(&format!("refs/heads/{branch}:"))
                .unwrap_or(l);
            // 剩余 `<path>:<line>:<text>`（path 含 ':' 时 splitn(3) 仍正确）
            let mut parts = rest.splitn(3, ':');
            let path = parts.next()?.to_string();
            let line = parts.next()?.to_string();
            let text = parts.next().unwrap_or_default().to_string();
            Some(serde_json::json!({ "path": path, "line": line, "text": text }))
        })
        .collect()
}
...
pub struct CodeRepoRouteHandler {
    ...
    /// Webhooks 事件回调服务（v0.1.50：hub_webhooks CRUD + 事件投递，见 webhooks.rs）。
    webhooks: std::sync::Arc<WebhookService>,
}

impl CodeRepoRouteHandler {
    /// 构造 handler（空会话列表 + 协作服务落默认 DB 路径）。
    ///
    /// 同时安装 webhook 全局投递槽（照 `nexhub_ci::install_global_core` 先例：
    /// os-api git-http push / issues / lobby 发版事件点经 [`crate::webhooks`]
    /// 的 fire 函数读槽投递；槽与 handler 字段共享同一实例，重复安装保留首个）。
    #[must_use]
    pub fn new() -> Self {
        let webhooks = std::sync::Arc::new(WebhookService::new());
        crate::webhooks::install_global_service(std::sync::Arc::clone(&webhooks));
        Self {
    ...
    /// 用空列表构造（测试注入；协作服务用内存库——零文件副作用；webhook 服务
    /// 内存库且**不装全局槽**——避免测试间投递污染）。
    #[must_use]
    pub fn with_empty() -> Self {
        Self {
            ...
            webhooks: std::sync::Arc::new(WebhookService::in_memory()),
        }
    }
...
impl RouteHandler for CodeRepoRouteHandler {
    async fn routes(&self) -> Vec<RouteSpec> {
        let mut routes = vec![
            ...
            // Webhooks 事件回调（8 条，v0.1.50——仓库/全局钩子 CRUD + 启停；
            // 全部网关 admin（管理面含 secret）；投递经全局槽 fire 函数挂钩
            // push/issue/PR/release 事件点，见 webhooks.rs）
            spec(
                HttpMethod::Get,
                "/api/v1/coderepo/repos/:name/webhooks",
                true,
                vec!["admin".into()],
            ),
            spec(
            ...
            spec(
                HttpMethod::Delete,
                "/api/v1/coderepo/webhooks/:id",
                true,
                vec!["admin".into()],
            ),
            spec(
                HttpMethod::Post,
                "/api/v1/coderepo/webhooks/:id/toggle",
                true,
                vec!["admin".into()],
            ),
            // MCP 工具面配套端点（v0.1.50 top2）：写文件（git plumbing 落 commit，
            // author=mcp）+ 代码搜索（git grep 简化面）——os-mcp 的
            // nexhub_write_file / nexhub_search_code 工具的落点。
            spec(
                HttpMethod::Post,
                "/api/v1/coderepo/repos/:name/file",
                true,
            ...
            spec(
                HttpMethod::Get,
                "/api/v1/coderepo/repos/:name/search",
                false,
                vec![],
            ),
        ];
    ...
    async fn handle(&self, req: ApiRequest) -> Result<ApiResponse, HandlerError> {
        // Webhooks 管理命名空间（repos/:name/webhooks / coderepo/webhooks）先由
        // webhook 服务认领（非该命名空间返回 None 继续分发）。
        if let Some(res) = self
            .webhooks
            .try_handle(req.method, &req.path, &req.body)
            .await
        {
            return res;
        }
        ...
        match (req.method, segs.as_slice()) {
            ...
            // —— POST /api/v1/coderepo/repos/:name/file —— 写文件（admin，v0.1.50
            //    MCP 工具面配套）：git plumbing（blob + 临时 index 重建 tree +
            //    commit-tree + update-ref）落一个 commit，author 固定标记
            //    `mcp <mcp@nexhub.local>`（AI agent 写入身份可追溯）。
            (HttpMethod::Post, ["api", "v1", "coderepo", "repos", name, "file"]) => {
                if let Err(msg) = validate_repo_name(name) {
                    return Ok(error_response(400, &msg));
                }
                let body: WriteFileBody = serde_json::from_value(req.body)
                    .map_err(|e| HandlerError::Internal(format!("解析写文件请求体失败: {e}")))?;
                let path = body.path.trim().to_string();
                if path.is_empty() {
                    return Ok(error_response(400, "path 不可为空"));
                }
                if path.starts_with('/') || path.split('/').any(|s| s == ".." || s.is_empty()) {
                    return Ok(error_response(400, "path 须为仓库相对路径且不可包含 '..'"));
                }
                if body.content.len() > 2_000_000 {
                    return Ok(error_response(400, "content 过大（≤2 MiB）"));
                }
                let bare = format!("{dir}/{name}.git");
                if !std::path::Path::new(&bare).is_dir() {
                    return Ok(error_response(404, &format!("仓库不存在: {name}")));
                ...
                {
                    Some(b) => {
                        if let Err(msg) = crate::nexhub_lobby::validate_branch_name(b) {
                            return Ok(error_response(400, &msg));
                        }
                        b.to_string()
                    }
                    None => resolve_default_branch_sync(&bare),
                };
                if !branch_exists_sync(&bare, &branch) {
                    return Ok(error_response(404, &format!("分支不存在: {branch}")));
                }
                let message = body
                    .message
                    .as_deref()
                    .map(str::trim)
                    .filter(|s| !s.is_empty())
                    .unwrap_or(&format!("mcp write: {path}"))
                    .to_string();
                let (b, br_c, p_c, c, m) =
                    (bare, branch.clone(), path.clone(), body.content, message);
                let written = tokio::task::spawn_blocking(move || {
                    write_file_blocking(&b, &br_c, &p_c, c.as_bytes(), &m)
                })
                .await
                .map_err(|e| HandlerError::Internal(format!("写文件任务 join 失败: {e}")))?;
                match written {
                    Ok(sha) => Ok(ok_json(serde_json::json!({
                        "ok": true,
                        "name": name,
                        "path": path,
                        "branch": branch,
                        "commit": sha,
                        "author": MCP_AUTHOR_NAME,
                    }))),
                    Err(e) => Ok(error_response(502, &e)),
                }
            }

            // —— GET /api/v1/coderepo/repos/:name/search?q=... —— 代码搜索（公开，
            //    v0.1.50 MCP 工具面配套）：`git grep -n -F` 固定串简化面，默认
            //    分支全树，≤50 条命中（file:line:text）。
            (HttpMethod::Get, ["api", "v1", "coderepo", "repos", name, "search"]) => {
                if let Err(msg) = validate_repo_name(name) {
                    return Ok(error_response(400, &msg));
                }
                let q = query.get("q").cloned().unwrap_or_default();
                if q.trim().is_empty() {
                    return Ok(error_response(400, "query param `q` 不可为空"));
                }
                let bare = format!("{dir}/{name}.git");
                if !std::path::Path::new(&bare).is_dir() {
                    return Ok(error_response(404, &format!("仓库不存在: {name}")));
                }
                let (b, query_str) = (bare, q.clone());
                let matches = tokio::task::spawn_blocking(move || search_code_blocking(&b, &query_str))
                    .await
                    .map_err(|e| HandlerError::Internal(format!("搜索任务 join 失败: {e}")))?;
                Ok(ok_json(serde_json::json!({
                    "name": name,
                    "q": q,
                    "matches": matches,
...
mod tests {
    ...
    // ---- 路由数量（12 原生 + issues/PR 协作 N + webhooks 8 + MCP 配套 2）----

    #[tokio::test]
    async fn routes_declares_endpoints_all_code_repo() {
        let h = CodeRepoRouteHandler::new();
        let routes = h.routes().await;
        // 原生 12 + 协作层（issues.rs route_specs，并行批次可能扩展）+ webhooks 8
        // + MCP 配套（写文件/搜索）2——总数按各源实时求和，对协作层演进稳健。
        assert_eq!(
            routes.len(),
            12 + crate::issues::route_specs().len() + crate::webhooks::route_specs().len() + 2,
            "路由数应为 12+协作+webhooks+MCP 配套: {routes:?}"
        );
        assert!(
            routes.iter().all(|r| r.handler_component == "code_repo"),
            "全部归属 code_repo 组件"
        );
        // 原生仓库中心写操作（POST / DELETE，除协作层与 webhooks——后者刻意
        // admin 含 GET（管理面含 secret））仍要求 admin
        for r in &routes {
            // 协作层命名空间（issues/pulls + release 附件，issues.rs——handler
            // 内自验链上 token / admin 回落，网关不拦）
            let is_collab = r.path.contains("/issues")
                || r.path.contains("/pulls")
                || r.path.contains("/releases");
            let is_webhook = r.path.contains("/webhooks");
            if matches!(r.method, HttpMethod::Post | HttpMethod::Delete) && !is_collab && !is_webhook
            {
                assert!(r.requires_auth, "原生写操作需 auth: {r:?}");
                assert_eq!(r.required_roles, vec!["admin".to_string()]);
            }
        }
        // GET 公开——webhooks 管理面例外（admin-only，含 secret）
        for r in &routes {
            if r.method == HttpMethod::Get && !r.path.contains("/webhooks") {
                assert!(!r.requires_auth, "GET 应公开: {r:?}");
            }
        }
        // webhook 管理面全部 admin（含 GET）
        for r in &routes {
            if r.path.contains("/webhooks") {
                assert!(
                    r.requires_auth && r.required_roles == vec!["admin".to_string()],
                    "webhook 管理面应 admin: {r:?}"
                );
            }
        }
        // 协作层（issues/pulls）全部 handler 内自验（requires_auth=false——
        // 链上 token 身份经网关直达，同 nexhub-lobby 模式）
        for r in &routes {
    ...
    // ---- MCP 配套端点：写文件（git plumbing，author=mcp）+ 代码搜索 ----

    /// 造真实裸仓（main 分支 1 提交 README.md；与 issues.rs fixture 同法）。
    async fn make_repo_with_commit(dir: &str, name: &str) {
        let bare = format!("{dir}/{name}.git");
        assert!(CodeRepoRouteHandler::create_repo_async(dir, name, "").await.is_ok());
        let work = format!("{dir}/.{name}-work");
        std::fs::create_dir_all(&work).unwrap();
        std::fs::write(format!("{work}/README.md"), "# mcp test\nhello world\n").unwrap();
        let ok = |args: &[&str]| {
            matches!(
                std::process::Command::new(args[0]).args(&args[1..]).output(),
                Ok(o) if o.status.success()
    ...
    #[tokio::test]
    async fn write_file_commits_with_mcp_author() {
        let tmp = tempdir();
        let dir = tmp.to_string();
        // handler 每请求读 env 解析仓库根——指向临时目录（ENV_LOCK 串行防并行竞态）
        let _guard = ENV_LOCK.lock().await;
        std::env::set_var("NEXOS_GIT_REPOS_DIR", &dir);
        make_repo_with_commit(&dir, "mcpw").await;
        let h = CodeRepoRouteHandler::with_empty();
        let resp = h
            .handle(post_req(
                "/api/v1/coderepo/repos/mcpw/file",
                serde_json::json!({
                    "path": "notes/agent.md",
                    "content": "# by mcp\nwritten via plumbing\n",
                    "message": "add agent notes",
                }),
            ))
            .await
            .unwrap();
        ...
        assert_eq!(resp.body["author"], "mcp");
        let commit = resp.body["commit"].as_str().unwrap().to_string();
        assert!(!commit.is_empty());
        // 文件内容可读回（HEAD 即新 commit）
        let bare = format!("{dir}/mcpw.git");
        let (ok, out) = CodeRepoRouteHandler::run_git_in_repo(
            &bare,
            &["show", "HEAD:notes/agent.md"],
        )
        .await;
        assert!(ok, "写入的文件应可读回");
        assert!(out.contains("written via plumbing"));
        // 提交作者 = mcp（身份标记）
        let (ok, author) = CodeRepoRouteHandler::run_git_in_repo(
            &bare,
            &["log", "-1", "--format=%an <%ae>"],
        )
        .await;
        assert!(ok);
        assert_eq!(author.trim(), "mcp <mcp@nexhub.local>");
        // 提交信息
        let (ok, subject) = CodeRepoRouteHandler::run_git_in_repo(
            &bare,
            &["log", "-1", "--format=%s"],
        )
        .await;
        assert!(ok);
        assert_eq!(subject.trim(), "add agent notes");
        std::env::remove_var("NEXOS_GIT_REPOS_DIR");
    }

    #[tokio::test]
    async fn write_file_validates_path_and_branch() {
        let tmp = tempdir();
        let dir = tmp.to_string();
        let _guard = ENV_LOCK.lock().await;
        std::env::set_var("NEXOS_GIT_REPOS_DIR", &dir);
        make_repo_with_commit(&dir, "mcpv").await;
        let h = CodeRepoRouteHandler::with_empty();
        // path 含 ..
        let resp = h
            .handle(post_req(
                "/api/v1/coderepo/repos/mcpv/file",
        ...
        let resp = h
            .handle(post_req(
                "/api/v1/coderepo/repos/mcpv/file",
                serde_json::json!({"path": "a.txt", "content": "x", "branch": "nope"}),
            ))
            .await
            .unwrap();
        assert_eq!(resp.status, 404);
        ...
        let resp = h
            .handle(post_req(
                "/api/v1/coderepo/repos/ghost/file",
    ...
    #[tokio::test]
    async fn search_finds_fixed_string_matches() {
        let tmp = tempdir();
        let dir = tmp.to_string();
        let _guard = ENV_LOCK.lock().await;
        std::env::set_var("NEXOS_GIT_REPOS_DIR", &dir);
        make_repo_with_commit(&dir, "mcps").await;
        // 追加一个文件（走写文件端点本体，顺带回归）
        let h = CodeRepoRouteHandler::with_empty();
        h.handle(post_req(
            "/api/v1/coderepo/repos/mcps/file",
            serde_json::json!({"path": "src/deep/nested.rs", "content": "fn unique_marker_xyz() {}\n"}),
        ))
        .await
        .unwrap();
        // 命中：默认分支全树
        let resp = h
            .handle(get_req("/api/v1/coderepo/repos/mcps/search?q=unique_marker_xyz"))
            .await
            .unwrap();
        assert_eq!(resp.status, 200, "{:?}", resp.body);
        let matches = resp.body["matches"].as_array().unwrap();
        assert_eq!(matches.len(), 1, "{matches:?}");
        assert_eq!(matches[0]["path"], "src/deep/nested.rs");
        assert_eq!(matches[0]["line"], "1");
        assert!(matches[0]["text"].as_str().unwrap().contains("unique_marker_xyz"));
        // 命中 README（多文件命中）
        let resp = h
            .handle(get_req("/api/v1/coderepo/repos/mcps/search?q=hello"))
            .await
            .unwrap();
        let matches = resp.body["matches"].as_array().unwrap();
        assert!(matches.iter().any(|m| m["path"] == "README.md"), "{matches:?}");
        // 无命中 → 空
        let resp = h
            .handle(get_req("/api/v1/coderepo/repos/mcps/search?q=zzz_nothing_zzz"))
            .await
            .unwrap();
        assert_eq!(resp.body["matches"].as_array().unwrap().len(), 0);
        // 缺 q → 400；仓库不存在 → 404
        let resp = h
            .handle(get_req("/api/v1/coderepo/repos/mcps/search"))
            .await
            .unwrap();
        assert_eq!(resp.status, 400);
        let resp = h
            .handle(get_req("/api/v1/coderepo/repos/ghost/search?q=x"))
            .await
            .unwrap();
        assert_eq!(resp.status, 404);
        std::env::remove_var("NEXOS_GIT_REPOS_DIR");
