...
//! - [`webhooks`]：`WebhookService` —— Webhooks 事件回调（v0.1.50 top1）——
//!   `hub_webhooks` CRUD（仓库/`*` 全局，admin）+ 事件投递（push/issues/pr/
//!   release → POST JSON + HMAC-SHA256 签名头 `X-NexHub-Signature`，10s 超时
//!   重试 1 次，最近 20 条投递环形日志）；事件点一行挂钩（os-api git-http
//!   push / issues 写路径 / lobby 发版）。
//!
...
/// NexHub 大厅发现层（2026-09-25 大文件拆分批）：原 ~11.8k 行单文件按域拆为
/// `lobby/` 目录（mod.rs 核心 + releases/sync/transfer/identity 域子模块 +
/// tests.rs），模块路径 `crate::nexhub_lobby::*` / `os_nexhub::nexhub_lobby::*`
/// 经此 `#[path]` 重定向与 lobby/mod.rs 重导出**零变化**（方法论同 os-api
/// handlers/im/ 拆分与 film_hub v0.1.42 域子模块先例）。
#[path = "lobby/mod.rs"]
...
pub use webhooks::{
    build_issue_payload, build_push_payload, build_pull_payload, build_release_payload,
    events_match, hmac_sha256, sign_payload, Webhook, WebhookDelivery, WebhookEvent,
    WebhookService,
};
