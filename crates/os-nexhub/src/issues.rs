...
//! | POST   | `/releases/:tag/assets` | 上传附件 `{name, content_base64}`（≤100MB；b64 信封兼容端点）| owner/admin |
//! | POST   | `/releases/:tag/assets?stream=1&name=` | 上传附件 **octet-stream 直收**（≤100MB；os-api 特挂流式路由，边收边落盘）| owner/admin |
//! | GET    | `/releases/:tag/assets/:aid` | 下载附件（os-api 特挂 octet-stream 流式直挂 + Range 206；本模块 b64 信封形态为组件直调兼容面）| 公开 |
...
//!
//! # 大载荷流式通道（v0.1.53，本批新增）
//!
//! b64 信封单请求把整文件读进内存（100MB 附件 → 峰值 ~270MB：JSON 信封
//! ~133MB + 解码副本 + sha256 副本），且撞网关 128MB 整包 body 上限——
//! 照 ISO 流式直挂先例（os-api http.rs `GET /provisioning/isos/:file`，
//! tokio unfold 256KiB + Range 206）给附件加**不经网关 JSON 信封**的通道：
//!
//! - 上传：`POST .../assets?stream=1&name=<附件名>` 请求体 `application/
//!   octet-stream` 直收——os-api 特挂路由逐块写盘 + sha256 累计，全程常量级
//!   内存；鉴权/校验/落库在 [`IssuesCore`]（begin → 流式写盘 → commit/
//!   abort 三段式，[`AssetStreamTicket`] 携带上下文）。b64 端点原样保留
//!   （无 `stream=1` 的 POST 特挂路由回落 dispatch，双端点并存）。
//! - 下载：`GET .../assets/:aid` 特挂 octet-stream 流式直挂（256KiB unfold +
//!   Range 206/416 + `x-nexos-sha256`，照 ISO 先例）；本模块的 b64 信封下载
//!   保留为组件直调（测试/旧客户端经网关 JSON 信封）兼容面。
//!
//! 两路共享同一 [`IssuesCore`]（经 [`shared_asset_stream`] 进程槽）——token
//! 验证、权限判定、SQLite 连接完全同源，产物（文件 + DB 行）逐字节一致。
//!
...
/// 单个 release 附件大小上限（100MB；b64 信封约为其 4/3，流式通道即原始
/// 字节数）。`pub`：os-api 特挂路由的运行中护栏（Content-Length 缺失/谎报时
/// 的逐块累计上限）与本常量同源。
pub const MAX_ASSET_BYTES: usize = 100 * 1024 * 1024;
/// 附件名长度上限。
...
/// 流式上传通行证（[`IssuesCore::begin_asset_stream_upload`] 签发）：begin 已
/// 过鉴权/校验/同名预检并建好目录；os-api 特挂路由把请求体流式写入 `path`
/// （256KiB 级分块、边写边算 sha256），成功 → [`IssuesCore::
/// commit_asset_stream_upload`] 落库，失败 → [`IssuesCore::
/// abort_asset_stream_upload`] 清理。begin 不落任何 DB 状态，中止无需回滚。
#[derive(Debug, Clone)]
pub struct AssetStreamTicket {
    /// 仓库名。
    pub repo: String,
    /// release tag。
    pub tag: String,
    /// 已校验的附件名。
    pub name: String,
    /// 附件 id（`ast-<纳秒 hex>`，与 b64 路径同格式）。
    pub id: String,
    /// 落盘绝对路径（`<repos_root>/.assets/<repo>/<tag>/<id>.bin`）。
    pub path: String,
}
...
/// 协作内核——[`IssuesService`] 的全部可变状态（协作 DB 连接 / 仓库根 /
/// hub_lobby 只读连接 / 凭据），2026-09-25 独立成 struct 以便 **Arc 共享给
/// 大载荷流式通道**（v0.1.53）：os-api 特挂路由经 [`shared_asset_stream`]
/// 取同一实例——流式上传/下载与 b64 信封路径验同一批 token、写同一条
/// `Mutex<Connection>`，权限判定与查-判-写的原子边界两路完全一致。
pub struct IssuesCore {
    ...
    /// Mutex 支持 `with_admin_token` 链式构造器在 Arc 包装后延迟注入）。
    admin_token: Mutex<Option<String>>,
    /// 测试注入的链上身份存储（缺省请求时走共享槽 → 惰性默认实例；Mutex 同上）。
    pinned_auth: Mutex<Option<Arc<ChainAuth>>>,
}

/// 进程级流式通道共享槽（[`IssuesService`] 各构造器注册，后注册者胜——同
/// [`register_shared_chain_auth`] 语义；生产装配只构造一次，测试并行构造的
/// 互相覆盖无害——os-nexhub 单测不读槽，只有 os-api 特挂路由消费）。
static SHARED_STREAM_CORE: Mutex<Option<Arc<IssuesCore>>> = Mutex::new(None);

/// 取当前流式通道内核（进程内从未构造过 [`IssuesService`] 时 None——os-api
/// 特挂路由对此回 503，调用方回落 b64 信封端点）。
#[must_use]
pub fn shared_asset_stream() -> Option<Arc<IssuesCore>> {
    SHARED_STREAM_CORE
        .lock()
...
/// 注册流式通道内核到进程槽（构造路径内部调用）。
fn register_shared_asset_stream(core: Arc<IssuesCore>) {
    *SHARED_STREAM_CORE.lock().expect("stream core poisoned") = Some(core);
}

/// 打开协作库（busy 3s + WAL + 建表；失败降级内存库，eprintln 不 panic）。
fn open_issues_db(path: &str) -> Arc<Mutex<Connection>> {
...
impl IssuesCore {
    ...
    fn caller(&self, headers: &Json) -> Option<Caller> {
        ...
        let pinned = self
            .pinned_auth
            .lock()
            .expect("pinned auth poisoned")
            .clone();
        let auth = pinned.unwrap_or_else(resolve_chain_auth);
        ...
        let admin = self
            .admin_token
            .lock()
            .expect("admin token poisoned")
            .clone();
        if admin.as_deref() == Some(token) {
...
///
/// 状态在 [`IssuesCore`]（Arc 内核）：b64 信封整包路径（本 struct 的方法）与
/// 大载荷流式通道（os-api 特挂路由经 [`shared_asset_stream`]）共享同一实例。
pub struct IssuesService {
    /// 协作内核（构造时注册进进程共享槽——两路通道同源）。
    core: Arc<IssuesCore>,
}
...
impl IssuesService {
    ...
    #[must_use]
    pub fn new() -> Self {
        let core = Arc::new(IssuesCore {
            db: open_issues_db(&Self::default_db_path()),
            repos_root: repos_dir(),
            lobby_db_path: lobby_db_path_default(),
            lobby_conn: Mutex::new(None),
            admin_token: Mutex::new(Self::admin_token_from_env()),
            pinned_auth: Mutex::new(None),
        });
        register_shared_asset_stream(Arc::clone(&core));
        Self { core }
    }
    ...
    pub fn with_admin_token(self, token: &str) -> Self {
        *self.core.admin_token.lock().expect("admin token poisoned") = Some(token.to_string());
        self
    }

    /// 注入链上身份存储（链式构造器，测试定格 token 域；生产走共享槽）。
    #[must_use]
    pub fn with_chain_auth(self, auth: Arc<ChainAuth>) -> Self {
        *self.core.pinned_auth.lock().expect("pinned auth poisoned") = Some(auth);
        self
    }
...
impl IssuesService {
    ...
    pub(crate) async fn try_handle(
        ...
        Some(self.core.dispatch(method, &segs, &query, headers, body).await)
    }
}
...
impl IssuesCore {
    ...
    fn upload_release_asset(
        ...
        let name = match validate_asset_name(&body.name) {
            Ok(n) => n,
    ...
    // --------------------------------------------------------------------
    // 大载荷流式通道（v0.1.53：os-api 特挂路由 POST ?stream=1 octet-stream
    // 直收 / GET :aid octet-stream 直挂的内核面——鉴权/校验/落库全在本核心，
    // 字节搬运在 os-api http.rs，两路与 b64 信封路径共享同一实例与判定逻辑）
    // --------------------------------------------------------------------

    /// 流式上传前置校验（begin）：鉴权 → release 存在 → owner/admin 权限 →
    /// 附件名校验 → Content-Length 预检（超限 413、零长 400）→ 同名 409 预检 →
    /// 生成 id + 建目录。**不落任何字节**——请求体由调用方流式写入 `ticket.path`，
    /// 写完经 [`Self::commit_asset_stream_upload`] 落库。
    ///
    /// 返回 `(状态码, 错误文案)` 形式的错误（os-api 特挂路由渲染成与 b64 路径
    /// 同形的 `{"error": ...}` JSON）。
    pub fn begin_asset_stream_upload(
        &self,
        repo: &str,
        tag: &str,
        headers: &Json,
    ...
    ) -> Result<AssetStreamTicket, (u16, String)> {
        let Some(caller) = self.caller(headers) else {
            return Err((401, AUTH_REQUIRED_MSG.to_string()));
        };
        if let Err((status, msg)) = self.require_release_err(repo, tag) {
            return Err((status, msg));
        }
        if !self.caller_can_manage_assets(&caller, repo) {
            return Err((
                403,
                "仅 admin 或仓库所有者可管理 release 附件（附件即发版物——先把仓库发布到大厅并归因你的链上身份，或联系管理员）".to_string(),
            ));
        }
        let name = validate_asset_name(raw_name).map_err(|msg| (400, msg.to_string()))?;
        if let Some(len) = content_length {
            if len == 0 {
                return Err((400, "附件内容不可为空".to_string()));
            }
            if len as usize > MAX_ASSET_BYTES {
                return Err((
                    413,
                    format!("附件过大（≤{MAX_ASSET_BYTES} 字节，Content-Length 预检）"),
                ));
            }
        }
        // 同名冲突预检（release 内附件名唯一）→ 409
        {
            let conn = self.db.lock().expect("db poisoned");
            match find_asset_by_name(&conn, repo, tag, &name) {
                Ok(Some(_)) => {
                    return Err((409, format!("附件已存在: {repo}/{tag}/{name}")));
                }
                Ok(None) => {}
        ...
        if let Some(dir) = std::path::Path::new(&path).parent() {
            if let Err(e) = std::fs::create_dir_all(dir) {
                return Err((500, format!("创建附件目录失败: {e}")));
            }
        }
        Ok(AssetStreamTicket {
            repo: repo.to_string(),
            tag: tag.to_string(),
            name,
            id: id.clone(),
    ...
    /// 流式上传收尾（commit）：字节数复核（0 → 400、超限 → 413，均清理半成品
    /// 文件）+ 插入 `hub_release_assets` 行（sha256/size 由调用方流式计算——
    /// 与 b64 路径的 [`sha256_hex`] 同一摘要语义）。约束冲突 → 409（并发同名
    /// 后到者败，清理文件）；其余落库失败 → 500（同样清理）。
    pub fn commit_asset_stream_upload(
        &self,
        ticket: &AssetStreamTicket,
        size: u64,
        sha256: &str,
    ) -> Result<ReleaseAsset, (u16, String)> {
        let cleanup = || {
        ...
        if size as usize > MAX_ASSET_BYTES {
            cleanup();
            return Err((413, format!("附件过大（≤{MAX_ASSET_BYTES} 字节）")));
        }
        let asset = ReleaseAsset {
            id: ticket.id.clone(),
            repo: ticket.repo.clone(),
            release_tag: ticket.tag.clone(),
            ...
            if let Err(e) = insert_asset(&conn, &asset) {
                ...
                if conflict {
                    return Err((
                        ...
                        format!(
                            "附件已存在: {}/{}/{}",
                            asset.repo, asset.release_tag, asset.name
                        ),
                    ));
                }
                return Err((500, format!("写入附件记录失败: {e}")));
            }
        }
        Ok(asset)
    }

    /// 流式上传中止清理（写盘/校验失败路径）：尽力删半成品文件（库行未写，
    /// 无需回滚——begin 阶段不落任何 DB 状态）。
    pub fn abort_asset_stream_upload(&self, ticket: &AssetStreamTicket) {
        let _ = std::fs::remove_file(&ticket.path);
    }

    /// 流式下载解析：release 存在性校验 + 附件行定位 → (行, 文件路径)。
    /// 文件本体由调用方（os-api 特挂路由）tokio::fs 流式直挂——本核心只做
    /// 判定，不读文件字节。404 语义与 b64 路径完全一致。
    pub fn resolve_asset_stream_download(
        &self,
        repo: &str,
        tag: &str,
        aid: &str,
    ) -> Result<(ReleaseAsset, String), (u16, String)> {
        if let Err((status, msg)) = self.require_release_err(repo, tag) {
            return Err((status, msg));
        }
        let conn = self.db.lock().expect("db poisoned");
        match find_asset(&conn, repo, tag, aid) {
            ...
            Ok(None) => Err((404, format!("附件不存在: {aid}"))),
            Err(e) => Err((500, format!("数据库错误: {e}"))),
        }
    }

    /// `require_release` 的 `(状态码, 文案)` 版（流式通道内核面统一错误形状）。
    fn require_release_err(&self, repo: &str, tag: &str) -> Result<(), (u16, String)> {
        self.require_release(repo, tag)
            .map_err(|resp| api_error_parts(&resp))
    }
}
...
/// 附件名校验（b64 信封与流式两路上传共用）：trim 后非空、≤200 字符、
/// 非路径形态（含 `/`、`\`、`.` 开头、`..`/`.`）、无控制字符。
fn validate_asset_name(raw: &str) -> Result<String, &'static str> {
    let name = raw.trim();
    if name.is_empty() {
        return Err("附件名不可为空");
    }
    if name.chars().count() > MAX_ASSET_NAME_CHARS {
        return Err("附件名过长（≤200 字符）");
    }
    if name.contains(['/', '\\']) || name == ".." || name == "." || name.starts_with('.') {
        return Err("附件名不可为路径（含 / 或以 . 开头）");
    }
    if name.chars().any(|c| c.is_control()) {
        return Err("附件名不可含控制字符");
    }
    Ok(name.to_string())
}

/// 附件 id 生成（`ast-<纳秒 hex>`；b64 与流式两路同格式）。
...
/// ApiResponse 错误 → `(状态码, error 文案)`（流式通道内核面的错误形状；
/// 与 b64 路径同一 `{"error": ...}` 信封，仅由 os-api 特挂路由负责渲染）。
fn api_error_parts(resp: &ApiResponse) -> (u16, String) {
    let msg = resp
        .body
        .get("error")
        .and_then(|v| v.as_str())
...
/// 统一 401 文案（b64 信封路径的 [`auth_required`] 与流式通道内核面共用）。
const AUTH_REQUIRED_MSG: &str = "需要 Authorization: Bearer <nexhub token>（先 POST /api/v1/nexhub/auth/challenge + /auth/verify）或系统 admin token";

/// 统一 401：写端点缺/无效身份（文案与 lobby 一致，引导三步认证）。
fn auth_required() -> ApiResponse {
    error_response(401, AUTH_REQUIRED_MSG)
}
...
mod tests {
    ...
    // ---- 大载荷流式通道内核面（v0.1.53）：begin/commit/abort/resolve + b64 对拍 ----

    /// 流式通道 admin 鉴权头（与 req_auth 同 Bearer 形态，Json 版）。
    fn admin_headers(token: &str) -> Json {
        serde_json::json!({ "authorization": format!("Bearer {token}") })
    }

    #[tokio::test]
    async fn asset_stream_core_begin_commit_and_b64_parity() {
        let dir = tempdir();
        make_repo(&dir, "demo", None);
        let svc = service(&dir);
        seed_lobby_release(&dir, "demo", "v1.0.0");
        let core = &svc.core;
        assert!(
            shared_asset_stream().is_some(),
            "构造 IssuesService 即注册进程共享槽"
        );

        // 1.5MB 内容（跨多个 256KiB 分块；核心面无分块概念，模拟 os-api 写盘）
        let content: Vec<u8> = (0..1_572_864).map(|i| (i % 253) as u8).collect();

        // begin（admin）→ ticket：name trim、id 形态、路径在 .assets 下
        let ticket = core
            .begin_asset_stream_upload(
                "demo",
                "v1.0.0",
                &admin_headers(TEST_ADMIN_TOKEN),
                "  big.bin ",
                Some(content.len() as u64),
            )
            .expect("begin 应通过");
        assert_eq!(ticket.name, "big.bin", "附件名应 trim");
        assert!(ticket.id.starts_with("ast-"));
        assert!(
            ticket.path.contains("/.assets/demo/v1.0.0/"),
            "落盘路径形态: {}",
            ticket.path
        );

        // 模拟 os-api 流式写盘（边写边算摘要由传输层负责，核心面只收结果）
        std::fs::write(&ticket.path, &content).unwrap();
        let asset = core
            .commit_asset_stream_upload(&ticket, content.len() as u64, &sha256_hex(&content))
            .expect("commit 应成功");
        assert_eq!(asset.name, "big.bin");
        assert_eq!(asset.size, content.len() as u64);
        assert_eq!(asset.sha256, sha256_hex(&content));

        // b64 路径上传同一内容（另一名）→ 产物对拍：sha256 相同 + 磁盘文件逐字节一致
        let resp = handle(
            &svc,
            req_auth(
                post_req(
                    ...
                    serde_json::json!({
                        "name": "big-b64.bin",
                        "content_base64": b64_encode(&content),
                    }),
                ),
                TEST_ADMIN_TOKEN,
            ),
        ...
        .await;
        assert_eq!(resp.status, 201, "b64 兼容端点应 201: {}", resp.body);
        assert_eq!(
            resp.body["asset"]["sha256"], asset.sha256,
            "流式与 b64 两路产物 sha256 一致"
        );
        let b64_path = format!(
            "{}/.assets/demo/v1.0.0/{}.bin",
            dir,
            resp.body["asset"]["id"].as_str().unwrap()
        );
        assert_eq!(
            std::fs::read(&b64_path).unwrap(),
            content,
            "两路落盘字节一致"
        );
        ...
        .await;
        assert_eq!(resp.status, 200);
        assert_eq!(resp.body["assets"].as_array().unwrap().len(), 2);
        let (row, path) = core
            .resolve_asset_stream_download("demo", "v1.0.0", &asset.id)
            .expect("resolve 应命中");
        assert_eq!(row.id, asset.id);
        assert_eq!(std::fs::read(&path).unwrap(), content);

        // 同名 begin → 409 预检（release 内附件名唯一）
        let err = core
            .begin_asset_stream_upload(
                "demo",
                "v1.0.0",
    ...
    #[test]
    fn asset_stream_core_validation_matrix() {
        let dir = tempdir();
        make_repo(&dir, "demo", None);
        let auth = Arc::new(ChainAuth::new());
        let svc = service(&dir).with_chain_auth(auth.clone());
        ...
        let sk = k256::ecdsa::SigningKey::random(&mut k256::elliptic_curve::rand_core::OsRng);
        let token = login(&auth, &sk);
        let begin = |headers: &Json, name: &str, len: Option<u64>| {
            core.begin_asset_stream_upload("demo", "v1.0.0", headers, name, len)
        };

        // 401（无凭据）/ 403（非 owner 链上身份）/ 200 面（admin）
        assert_eq!(begin(&serde_json::json!({}), "a.bin", Some(16)).unwrap_err().0, 401);
        assert_eq!(
            begin(&admin_headers(&token), "a.bin", Some(16))
                .unwrap_err()
                .0,
            403
        );

        // release 不存在 → 404；仓库名非法 → 400
        assert_eq!(
            core.begin_asset_stream_upload(
                "demo",
                "v9.9.9",
                &admin_headers(TEST_ADMIN_TOKEN),
                "a.bin",
        ...
        let admin = admin_headers(TEST_ADMIN_TOKEN);
        // 附件名校验（与 b64 路径同规则）
        for bad in ["", "  ", "../evil", ".hidden", "a/b", "c\\d", "n\nn"] {
            assert_eq!(
                begin(&admin, bad, Some(16)).unwrap_err().0,
                400,
                "非法名应 400: {bad:?}"
            );
        }
        // Content-Length 预检：0 → 400；超限 → 413（不落目录）
        assert_eq!(begin(&admin, "z.bin", Some(0)).unwrap_err().0, 400);
        assert_eq!(
            begin(&admin, "z.bin", Some(MAX_ASSET_BYTES as u64 + 1))
                .unwrap_err()
                .0,
            413
        );

        // commit 复核：零长 → 400 且清理半成品；超限 → 413 且清理
        let t = begin(&admin, "half.bin", None).expect("无 Content-Length 也放行（运行中护栏兜底）");
        std::fs::write(&t.path, b"abc").unwrap();
        assert_eq!(core.commit_asset_stream_upload(&t, 0, "x").unwrap_err().0, 400);
        assert!(!std::path::Path::new(&t.path).exists(), "commit 失败应清理文件");

        let t2 = begin(&admin, "over.bin", None).unwrap();
        std::fs::write(&t2.path, b"abc").unwrap();
        assert_eq!(
            core.commit_asset_stream_upload(&t2, MAX_ASSET_BYTES as u64 + 1, "x")
                .unwrap_err()
                .0,
            413
        );
        assert!(!std::path::Path::new(&t2.path).exists());

        // abort：清理半成品（库行未写，无需回滚）
        let t3 = begin(&admin, "abort.bin", Some(16)).unwrap();
        std::fs::write(&t3.path, b"partial").unwrap();
        core.abort_asset_stream_upload(&t3);
        assert!(!std::path::Path::new(&t3.path).exists(), "abort 应删半成品");

        // resolve 404 面：未知 aid / 越权仓库 / 未知 tag
        for (repo, tag) in [("demo", "v1.0.0"), ("demo", "v9.9.9")] {
            assert_eq!(
                core.resolve_asset_stream_download(repo, tag, "ast-nope")
                    .unwrap_err()
                    .0,
                404,
                "未知附件应 404: {repo}/{tag}"
    ...
    /// DELETE 请求构造（测试辅助）。
    fn del_req(path: &str) -> ApiRequest {        ApiRequest {
