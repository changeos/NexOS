...
export interface RepoIssue {
  ...
  /** 引用本 Issue 的 PR 编号（交叉引用徽章，2026-09-24）。 */
  referencing_pulls?: number[];
  created_at: string;
  updated_at: string;
}
...
export interface RepoPull {
  ...
  merged_at: string;
  comment_count: number;
  /** 本 PR 引用的 Issue 编号（fixes #n；merge 时自动关闭，2026-09-24）。 */
...
/** PR merge 响应（三策略：merge/squash/rebase + 交叉引用联动关闭清单）。 */
export interface RepoPullMergeResp {
  ok: boolean;
  repo: string;
  number: number;
  state: string;
  /** 'merge' | 'squash' | 'rebase'（回显）。 */
  merge_strategy: string;
  merged_by: string;
  merged_at: string;
  merged_sha: string;
  /** merge 联动关闭的 issue 编号（body/评论 fixes #n）。 */
  closed_issues: number[];
}

/** 大厅 release（GET /api/v1/nexhub/lobby/:repo/releases，公开）。 */
export interface LobbyRelease {
  id: string;
  repo_name: string;
  tag: string;
  title: string;
...
/** 单个 release 附件（GET .../releases/:tag/assets，2026-09-24）。 */
...
/** release 附件清单响应。 */
export interface ReleaseAssetsResp {
  repo: string;
  tag: string;
  assets: ReleaseAsset[];
}
...
export interface P2pNodeMetaEntry {
  ...
  /** 节点自述四字段（2026-09-14 联邦节点信息协议）：来源仅对端 digest
   *  自广播/转述；可选——未自述（旧版节点）时字段缺席。 */
...
export interface CombinedLanNode {
  ...
  /** 节点自述：agent 版本（如 "0.1.45"；null = 对端旧版未自述——显示「待升级
   *  自述」；来源仅对端 digest 自广播/转述，2026-09-14 联邦节点信息协议）。 */
  agent_version?: string | null;
  /** 节点自述：机器名（null = 未自述）。 */
  hostname?: string | null;
  /** 节点自述：os 信息（发行版串或 "linux 内核首段"）。 */
  platform?: string | null;
  /** 节点自述：GPU 摘要（卡名列表，同型多卡 ×N；无卡 "none"）。 */
  gpu?: string | null;
}
...
export interface CombinedP2pNode {
  ...
  im_public: boolean | null;
  /** 节点自述四字段（语义同 CombinedLanNode.agent_version 等；bucket 短 ID
   *  无从匹配 meta 条目时恒 null——显示「待升级自述」）。 */
  agent_version?: string | null;
  hostname?: string | null;
  platform?: string | null;
  gpu?: string | null;
...
export interface CombinedInactiveNode {
  ...
  since: number;
  /** 节点自述四字段（语义同 CombinedLanNode.agent_version 等——出局前学到的
   *  版本/硬件照常展示，复活/重连后经新鲜报告刷新）。 */
  agent_version?: string | null;
  hostname?: string | null;
  platform?: string | null;
  gpu?: string | null;
...
export const endpoints = {
  ...
  // 网络装机子域（/api/v1/provisioning/{isos,seed,ipxe}——P0，docs/NET_BOOT.md）
  /** ISO 仓清单（GET /api/v1/provisioning/isos）：文件/版本/架构/size/sha256/提取状态。 */
  provisioningNetbootIsos: (): Promise<unknown> => get('/api/v1/provisioning/isos'),
  /** 仓登记+校验+提取触发（POST /api/v1/provisioning/isos/scan，后台执行）。 */
  scanProvisioningNetbootIsos: (): Promise<unknown> =>
    post('/api/v1/provisioning/isos/scan', {}),
  /** 删旧版 ISO（DELETE /api/v1/provisioning/isos/:file，滚动版本策略）。 */
  deleteProvisioningNetbootIso: (file: string): Promise<unknown> =>
    del(`/api/v1/provisioning/isos/${encodeURIComponent(file)}`),
  ...
  // —— 监控摄像头（surveillance）已随 v0.1.51 三引擎剥离删除——摄像头监控
  //    能力由独立产品线 StreamingStudio 承接，主前端不再内置相关端点。 ——
  ...
  /**
   * GPU 显存（GET /api/v1/monitor/vram，需 admin；v0.1.50）：
   * vraminfo 探测式采集（缺失 → `{available:false, hint}`；命中 →
   * `{available:true, bin, gpus:[{bdf, board_vendor, memory_maker,
   * memory_type, memory_temp_c, memory_temp_modules_c?, note}], age_secs}`；
   * 后端 10s 缓存，GDDR6/GDDR5 无传感器时 memory_temp_c=null + note 说明）。
   */
  monitorVram: (): Promise<unknown> => get('/api/v1/monitor/vram'),
  ...
  // —— Webhooks 事件回调（os-nexhub webhooks.rs，v0.1.50：CRUD 全部 admin——钩子含 secret）——
  /** 列仓库 Webhook（GET /api/v1/coderepo/repos/:name/webhooks，admin）→ { repo, webhooks: [{ webhook: { id, repo, url, secret, events, enabled, last_delivery, last_status }, deliveries: [...] }] }（含全局 * 钩子 + 各自最近 20 条投递）。 */
  codeRepoWebhooks: (name: string): Promise<unknown> =>
    get(`/api/v1/coderepo/repos/${encodeURIComponent(name)}/webhooks`),
  /** 创建 Webhook（POST /api/v1/coderepo/repos/:name/webhooks，admin）body: { url, secret?, events: string[] }。 */
  createCodeRepoWebhook: (
    name: string,
    body: { url: string; secret?: string; events: string[] },
  ): Promise<unknown> =>
    post(`/api/v1/coderepo/repos/${encodeURIComponent(name)}/webhooks`, body),
  /** 删除 Webhook（DELETE /api/v1/coderepo/repos/:name/webhooks/:id，admin）。 */
  deleteCodeRepoWebhook: (name: string, id: string): Promise<unknown> =>
    del(`/api/v1/coderepo/repos/${encodeURIComponent(name)}/webhooks/${encodeURIComponent(id)}`),
  /** 启停翻转 Webhook（POST /api/v1/coderepo/repos/:name/webhooks/:id/toggle，admin）→ { ok, enabled }。 */
  toggleCodeRepoWebhook: (name: string, id: string): Promise<unknown> =>
    post(`/api/v1/coderepo/repos/${encodeURIComponent(name)}/webhooks/${encodeURIComponent(id)}/toggle`, {}),
  ...
  codeRepoPullMerge: (
    name: string,
    num: number,
    body: { merge_strategy?: string; message?: string } = {},
    opts?: NexhubOpts,
  ): Promise<RepoPullMergeResp> =>
    request(
  ...
  // —— Release / 二进制附件（2026-09-24：大厅 release 行 + 仓级 assets 四端点）——
  // release 增删在大厅（仅 admin）；附件挂在 coderepo 仓域（上传/删除=owner/admin，
  // 下载/清单公开；下载为 octet-stream 直传——浏览器直接下载）。

  /** release 列表（GET /api/v1/nexhub/lobby/:repo/releases，公开）。 */
  nexhubLobbyReleases: (repo: string): Promise<LobbyRelease[]> =>
    get(`/api/v1/nexhub/lobby/${encodeURIComponent(repo)}/releases`),
  /** 创建 release（POST .../releases {tag,title?,notes?}；仅 admin；git tag 到默认分支头）。 */
  nexhubLobbyReleaseCreate: (
    repo: string,
    body: { tag: string; title?: string; notes?: string },
    opts?: NexhubOpts,
  ): Promise<LobbyRelease> =>
    request(`/api/v1/nexhub/lobby/${encodeURIComponent(repo)}/releases`, {
      method: 'POST',
      body,
      headers: nexhubAuthHeaders(opts),
    }),
  /** 删除 release（DELETE .../releases/:tag；仅 admin；库行 + git tag 一并删除）。 */
  nexhubLobbyReleaseDelete: (repo: string, tag: string, opts?: NexhubOpts): Promise<unknown> =>
    request(
      `/api/v1/nexhub/lobby/${encodeURIComponent(repo)}/releases/${encodeURIComponent(tag)}`,
      { method: 'DELETE', headers: nexhubAuthHeaders(opts) },
    ),
  /** release 附件清单（GET .../repos/:name/releases/:tag/assets，公开）。 */
  codeRepoReleaseAssets: (name: string, tag: string): Promise<ReleaseAssetsResp> =>
    get(
      `/api/v1/coderepo/repos/${encodeURIComponent(name)}/releases/${encodeURIComponent(tag)}/assets`,
    ),
  /** 上传附件（POST .../assets {name, content_base64}，解码后 ≤100MB；owner/admin）。 */
  codeRepoReleaseAssetUpload: (
    name: string,
    tag: string,
    body: { name: string; content_base64: string },
    opts?: NexhubOpts,
  ): Promise<{ ok: boolean; asset: ReleaseAsset }> =>
    request(
      `/api/v1/coderepo/repos/${encodeURIComponent(name)}/releases/${encodeURIComponent(tag)}/assets`,
      { method: 'POST', body, headers: nexhubAuthHeaders(opts) },
    ),
  /**
   * 流式上传附件（POST .../assets?stream=1&name=，application/octet-stream 直收；
   * owner/admin；v0.1.53 大载荷通道）。fetch 直接以 File 为 body——浏览器从磁盘
   * 流式发送，不再 FileReader→base64（峰值内存省一倍+）；进度条 fetch 无原生
   * 上传进度，未实现（上传中 spinner 即反馈）。超时 10min（100MB 慢链路余量，
   * 同 IM 文件上传的放宽先例）。
   */
  codeRepoReleaseAssetUploadStream: async (
    name: string,
    tag: string,
    assetName: string,
    file: File,
    ...
    try {
      ...
      if (opts?.nexhubToken) {
        headers['Authorization'] = `Bearer ${opts.nexhubToken}`;
      }
      const resp = await fetch(
        `/api/v1/coderepo/repos/${encodeURIComponent(name)}/releases/${encodeURIComponent(tag)}` +
          `/assets?stream=1&name=${encodeURIComponent(assetName)}`,
      ...
      if (!resp.ok) {
        ...
        throw new ApiError(
          `${resp.status} ${resp.statusText}${detail ? ` — ${detail}` : ''}`,
          { status: resp.status, path: 'assets?stream=1' },
        );
      }
      const text = await resp.text();
      return (text ? JSON.parse(text) : null) as { ok: boolean; asset: ReleaseAsset };
    } finally {
      clearTimeout(timer);
    }
  },
  /** 删除附件（DELETE .../assets/:aid；owner/admin）。 */
  codeRepoReleaseAssetDelete: (
    name: string,
    tag: string,
    aid: string,
    opts?: NexhubOpts,
    ...
    request(
      `/api/v1/coderepo/repos/${encodeURIComponent(name)}/releases/${encodeURIComponent(tag)}/assets/${encodeURIComponent(aid)}`,
      { method: 'DELETE', headers: nexhubAuthHeaders(opts) },
    ),
  /**
   * 附件下载直链（GET .../assets/:aid，octet-stream 直传）。
   * 供 <a href> / window.open 直接下载（同源公开端点，无需 fetch 信封）。
   */
  codeRepoReleaseAssetUrl: (name: string, tag: string, aid: string): string =>
    `/api/v1/coderepo/repos/${encodeURIComponent(name)}/releases/${encodeURIComponent(tag)}/assets/${encodeURIComponent(aid)}`,
