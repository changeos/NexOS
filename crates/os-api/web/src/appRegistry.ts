...
>({
    ...
    // 监控摄像头（surveillance）已随 v0.1.51 三引擎剥离删除出主前端——
    // 摄像头监控能力由独立产品线 StreamingStudio 承接（surveillance 已并入
    // 流媒体中心独立化决策）。
...
export const APP_CATEGORY: Record<string, AppCategory> = {
    ...
    containers: 'devtools', cloudsync: 'files', notes: 'system',
    // streaming/qrtransfer 已剥离为应用包：分类随注册声明（media/devtools），不在内置表；
    // surveillance 已随 v0.1.51 三引擎剥离删除（独立产品线 StreamingStudio 承接）
