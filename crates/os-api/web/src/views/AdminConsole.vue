...
function activeTab(): SessionTab | null {
  return tabs.value.find((tb) => tb.session.session_id === activeId.value) ?? null;
}
...
function setFont(delta: number): void {
  ...
  for (const tb of tabs.value) {
    tb.terminal.options.fontSize = fontSize.value;
  }
  const tab = activeTab();
...
function attachWs(tab: SessionTab): void {
  ...
  ws.onmessage = (ev) => {
    ...
    switch (frame.type) {
      ...
      case 'exit':
        ...
        tab.terminal.write(
          `\r\n\x1b[90m${t('admin.termExited', { code: tab.exitCode })}\x1b[0m\r\n`,
        );
        ws.close();
        break;
      case 'error':
        if (frame.msg) {
          tab.terminal.write(`\r\n\x1b[31m${t('admin.termError', { msg: frame.msg })}\x1b[0m\r\n`);
        }
        break;
      default:
        break;
  ...
  ws.onerror = () => {
    ...
    tab.error = t('admin.wsFail');
  };
  ws.onclose = () => {
    if (tab.state === 'open') {
      tab.state = 'closed';
      tab.terminal.write(`\r\n\x1b[90m${t('admin.termClosed')}\x1b[0m\r\n`);
    } else if (tab.state === 'connecting') {
      tab.state = 'closed';
      tab.error = tab.error || t('admin.wsConnectFail');
    }
  };
...
async function switchTab(id: string): Promise<void> {
  activeId.value = id;
  await nextTick();
  const tab = tabs.value.find((tb) => tb.session.session_id === id);
  if (tab) {
    scheduleFit(tab);
    tab.terminal.focus();
  }
...
async function closeTab(id: string): Promise<void> {
  const idx = tabs.value.findIndex((tb) => tb.session.session_id === id);
  if (idx === -1) return;
...
async function createSession(body: {
  ...
  try {
    ...
    msg.value =
      body.kind === 'local'
        ? { kind: 'ok', text: t('admin.localOpened') }
        : { kind: 'ok', text: t('admin.sshOpened', { target: session.target }) };
  } catch (e) {
    msg.value = { kind: 'err', text: t('admin.createFailed', { msg: friendlyError(e) }) };
  } finally {
...
async function loadSshTargets(): Promise<void> {
  ...
  try {
    ...
    sshTargets.value = arr
      .filter((tb) => typeof tb.id === 'string')
      .map((tb) => ({
        id: String(tb.id),
        name: String(tb.name ?? tb.id),
        host: String(tb.host ?? ''),
        port: Number(tb.port ?? 22),
        user: String(tb.user ?? 'root'),
      }));
...
async function quickConnect(): Promise<void> {
  ...
  if (!host) {
    msg.value = { kind: 'err', text: t('admin.quickHostRequired') };
    return;
  }
...
async function refreshSessions(): Promise<void> {
  try {
    ...
    for (const s of list) {
      if (!tabs.value.some((tb) => tb.session.session_id === s.session_id)) {
        await openTab(s);
      }
    }
  } catch {
...
/** 分类显示名（i18n 键 admin.cat.<category>，语言切换实时生效）。 */
function categoryLabel(k: QuickCategory): string {
  return t(`admin.cat.${k}`);
}

/** 预置命令集（分类 pill；命令缺失时由 shell 自己报错，如非 Docker 环境）。 */
const PRESET_COMMANDS: Omit<QuickCommand, 'name'>[] = [
  // —— 系统（name 经 i18n 键 admin.cmd.<id> 渲染）——
  { id: 'sys-htop', command: 'htop', category: 'system' },
  { id: 'sys-df', command: 'df -h', category: 'system' },
  { id: 'sys-free', command: 'free -h', category: 'system' },
  { id: 'sys-uname', command: 'uname -a', category: 'system' },
  { id: 'sys-uptime', command: 'uptime', category: 'system' },
  { id: 'sys-svc', command: 'systemctl status os-api', category: 'system' },
  {
    id: 'sys-log',
  ...
  // —— 网络 ——
  { id: 'net-ip', command: 'ip a', category: 'network' },
  { id: 'net-ss', command: 'ss -tlnp', category: 'network' },
  { id: 'net-ping', command: 'ping -c 3 203.0.113.2', category: 'network' },
  {
    id: 'net-health',
  ...
  { id: 'nx-zpool', command: 'zpool status', category: 'nexos' },
  { id: 'nx-zfs', command: 'zfs list', category: 'nexos' },
  { id: 'nx-gpu', command: 'nvidia-smi', category: 'nexos' },
  // —— Docker（非容器环境命令报错属预期，静默换下一招）——
  { id: 'dk-ps', command: 'docker ps', category: 'docker' },
];
...
const QUICK_CATEGORIES: QuickCategory[] = ['system', 'network', 'nexos', 'docker', 'custom'];

const categoryPills = computed(() => [
  { key: 'all' as const, label: t('admin.cat.all') },
...
const allCommands = computed<QuickCommand[]>(() => [
  ...PRESET_COMMANDS.map((c) => ({ ...c, name: t(`admin.cmd.${c.id}`) })),
  ...customCmds.value,
]);
...
function sendCommand(command: string): void {
  const tab = activeTab();
  if (!tab) {
    msg.value = { kind: 'err', text: t('admin.quickNeedSession') };
    return;
  }
  if (!tab.ws || tab.ws.readyState !== WebSocket.OPEN) {
    msg.value = { kind: 'err', text: t('admin.quickNotConnected') };
    return;
  }
...
function addCustomCmd(): void {
  ...
  if (!name || !command) {
    msg.value = { kind: 'err', text: t('admin.customNeedBoth') };
    return;
  }
  if (customCmds.value.some((c) => c.name === name)) {
    msg.value = { kind: 'err', text: t('admin.customDup', { name }) };
    return;
  }
  ...
  cmdSearch.value = '';
  msg.value = { kind: 'ok', text: t('admin.customAdded', { name }) };
}
...
function clearActive(): void {
  const tab = activeTab();
  if (!tab) {
    msg.value = { kind: 'err', text: t('admin.needSession') };
    return;
  }
  tab.terminal.clear();
  tab.terminal.focus();
...
async function copyOutput(): Promise<void> {
  ...
  if (!text.trim()) {
    msg.value = { kind: 'info', text: t('admin.copyEmpty') };
    return;
  }
  const ok = await copyText(text);
  msg.value = ok
    ? { kind: 'ok', text: t('admin.copyDone', { n: lines.length }) }
    : { kind: 'err', text: t('admin.copyDenied') };
}
...
function fmtUptime(secs: number): string {
  ...
  if (d > 0) return t('admin.uptimeDH', { d, h });
  if (h > 0) return t('admin.uptimeHM', { h, m });
  return t('admin.uptimeM', { m });
}
...
function tabLabel(tab: SessionTab): string {
  ...
  return tab.session.kind === 'local' ? t('admin.localTab') : tab.session.target;
}
  ...
  <div class="admin-page">
    <div class="page-head">
      <div>
        ...
        <div class="page-sub muted">{{ t('admin.sub') }}</div>
      </div>
      <div class="head-actions">
        <button class="btn btn-small" :disabled="creating" @click="refreshSessions">{{ t('admin.refresh') }}</button>
      </div>
    </div>
    ...
    <div v-if="snapshot" class="snapshot-bar">
      <span class="snap-label muted">{{ t('admin.snapLabel') }}</span>
      <button
        class="snap-chip"
        :title="t('admin.snapClick', { cmd: 'curl -s localhost:8558/api/v1/update/status | head -c 200' })""
        @click="sendCommand('curl -s localhost:8558/api/v1/update/status | head -c 200')"
      >
        <span class="snap-k">{{ t('admin.snapVersion') }}</span><span class="snap-v mono">v{{ snapshot.version }}</span>
      </button>
      <button class="snap-chip" :title="t('admin.snapClick', { cmd: 'uptime' })" @click="sendCommand('uptime')">
        <span class="snap-k">{{ t('admin.snapUptime') }}</span><span class="snap-v mono">{{ fmtUptime(snapshot.uptime_secs) }}</span>
      </button>
      <button
        class="snap-chip"
        :title="t('admin.snapClick', { cmd: 'curl -s localhost:8558/api/v1/p2p/peers' })""
        @click="sendCommand('curl -s localhost:8558/api/v1/p2p/peers')"
      >
        <span class="snap-k">P2P</span>
        <span class="snap-v mono">{{
          snapshot.p2p_connected === null ? t('admin.p2pDisabled') : t('admin.p2pNodes', { n: snapshot.p2p_connected })
        }}</span>
      </button>
      <button class="snap-chip" :title="t('admin.snapClick', { cmd: 'df -h' })" @click="sendCommand('df -h')">
        <span class="snap-k">{{ t('admin.snapDisk') }}</span>
        <span class="snap-v mono" :class="{ 'snap-warn': snapshot.disk_use_pct >= 85 }"
          >{{ snapshot.disk_use_pct }}%</span
        >
      </button>
      <button class="snap-chip" :title="t('admin.snapClick', { cmd: 'free -h' })" @click="sendCommand('free -h')">
        <span class="snap-k">{{ t('admin.snapMem') }}</span>
    ...
    <div class="console-wrap">
      ...
      <aside class="side">
        ...
        <div class="side-block">
          <div class="side-title">{{ t('admin.sshTargets') }}</div>
          <select v-model="pickedTargetId" class="input select" :disabled="targetsLoading">
            <option value="" disabled>{{ targetsLoading ? t('admin.loading') : t('admin.pickTarget') }}</option>
            <option v-for="tg in sshTargets" :key="tg.id" :value="tg.id">
              {{ tg.name }}（{{ tg.user }}@{{ tg.host }}:{{ tg.port }}）
            </option>
          </select>
          <button
            class="btn"
        ...
        <div class="side-block">
          <div class="side-title">{{ t('admin.quickTitle') }}</div>
          <input v-model="quick.host" class="input" :placeholder="t('admin.quickHostPh')" spellcheck="false" />
          <div class="quick-row">
            <input v-model="quick.user" class="input" :placeholder="t('admin.quickUserPh')" spellcheck="false" />
            <input v-model="quick.port" class="input" type="number" min="1" max="65535" :placeholder="t('admin.quickPortPh')" />
          </div>
          <input
            v-model="quick.key_path"
            class="input mono"
            :placeholder="t('admin.quickKeyPh')"
            spellcheck="false"
          />
          <button class="btn" :disabled="creating" @click="quickConnect">{{ t('admin.connect') }}</button>
          <div class="side-hint muted">{{ t('admin.quickPasswordHint') }}</div>
        </div>

        <div class="side-block grow">
          <div class="side-title">{{ t('admin.limitTitle') }}</div>
      ...
      <section class="term-area">
        <div v-if="tabs.length === 0" class="term-empty">
          <div class="term-empty-title">{{ t('admin.emptyTitle') }}</div>
          <div class="muted">
            {{ t('admin.emptyHint') }}<br />
            {{ t('admin.emptyArch') }}
          </div>
        </div>
        <template v-else>
          ...
          <div class="quick-cmds" :class="{ collapsed: !panelOpen }">
            <div class="qc-head">
              <button class="qc-toggle" @click="panelOpen = !panelOpen">
                {{ panelOpen ? '▾' : '▸' }} {{ t('admin.quickPanel') }}
              </button>
              <input
                v-if="panelOpen"
                v-model="cmdSearch"
                class="qc-search"
                :placeholder="t('admin.searchCmdPh')""
                spellcheck="false"
              />
              <label v-if="panelOpen" class="qc-pin" :title="t('admin.pinTitle')">
                <input v-model="panelPinned" type="checkbox" /> {{ t('admin.pin') }}
              </label>
              <button
                v-if="panelOpen"
                class="qc-add"
                :title="t('admin.addCustomTitle')""
                @click="showCustomForm = !showCustomForm"
              >
                {{ t('admin.addCustom') }}
              </button>
              <div class="qc-tools">
                <button class="qc-tool" :title="t('admin.fontSmaller')" @click="setFont(-1)">A-</button>
                <span class="qc-font mono">{{ fontSize }}px</span>
                <button class="qc-tool" :title="t('admin.fontBigger')" @click="setFont(1)">A+</button>
                <button class="qc-tool" :title="t('admin.clearTitle')" @click="clearActive">
                  {{ t('admin.clear') }}
                </button>
                <button
                  class="qc-tool"
            ...
            <div v-if="panelOpen" class="qc-body">
              <div v-if="showCustomForm" class="qc-form">
                <input
                  v-model="newCmd.name"
                  class="input"
                  :placeholder="t('admin.customNamePh')""
                  maxlength="30"
                  spellcheck="false"
                />
                <input
                  ...
                  class="input mono"
                  :placeholder="t('admin.customCmdPh')""
                  spellcheck="false"
                  @keyup.enter="addCustomCmd"
                />
                <button class="btn btn-small" @click="addCustomCmd">{{ t('admin.add') }}</button>
                <button class="btn btn-small" @click="showCustomForm = false">{{ t('admin.cancel') }}</button>
              </div>
              <div class="qc-cats">
                <button
              ...
              <div class="qc-list">
                ...
                >
                  ...
                  <span class="qc-cmd-cat">{{ categoryLabel(c.category) }}</span>
                  <span
                    v-if="c.category === 'custom'"
                    class="qc-cmd-del"
                    :title="t('admin.deleteCustom')""
