...
// =============================================================================
// Monitor.vue —— 系统监控（实时指标 + 告警 + ZFS 池 + GPU 显存）
//
// 功能：
...
//   5. GPU 显存卡（vraminfo 探测式：颗粒厂商/类型徽章 + 显存结温阈值着色 +
//      逐颗粒迷你条；无工具/无传感器降级文案，i18n ×4）
//   6. onMounted 加载，可选 5s 定时刷新
//
// 后端：
//   GET /api/v1/monitor/metrics / services / alerts / zpools / stats / history
//   GET /api/v1/monitor/vram（需 admin；vraminfo 采集，10s 缓存）
...
// GPU 显存（GET /api/v1/monitor/vram；字段见后端 VramSnapshot/VramGpu DTO）
interface VramGpu {
  bdf?: string;
  board_vendor?: string | null;
  memory_maker?: string | null;
  memory_type?: string | null;
  memory_temp_c?: number | null;
  memory_temp_modules_c?: number[] | null;
  note?: string | null;
  [k: string]: unknown;
}
...
const vram = ref<VramSnapshot>({});
/** /vram 加载失败（401/403 非 admin / 网络错）——与"工具未装"分开显示。 */
const vramLoadError = ref('');
...
async function loadVram(): Promise<void> {
  // 静默降级：非 admin（401/403）/后端旧版本（404）只降级这一块面板，不报错
  vramLoadError.value = '';
  try {
    const raw = await endpoints.monitorVram();
    vram.value = (raw ?? {}) as VramSnapshot;
  } catch (e) {
    vram.value = {};
    const m = e instanceof Error ? e.message : String(e);
    vramLoadError.value = /401|403/.test(m) ? t('monitor.gpuAuthFailed') : m;
  }
}

async function refreshAll(): Promise<void> {
  await Promise.all([loadMetrics(), loadServices(), loadAlerts(), loadZpools(), loadVram()]);
}
...
// —— GPU 显存（/vram）：阈值着色与降级判定（100/110 与后端告警阈值缺省一致）——
const VRAM_TEMP_WARN = 100;
const VRAM_TEMP_CRIT = 110;

function vramTempColor(temp: number | null | undefined): string {
  if (temp == null) return 'var(--text-muted, #6b7280)';
  if (temp >= VRAM_TEMP_CRIT) return '#C7162B';
  if (temp >= VRAM_TEMP_WARN) return '#F99B11';
  return '#0E8420';
}

/** 温度读数状态徽章（≥110 过热 / ≥100 偏高；正常/无读数不挂徽章）。 */
function vramTempBadge(temp: number | null | undefined): { cls: string; label: string | null } {
  if (temp == null) return { cls: 'pill-muted', label: null };
  if (temp >= VRAM_TEMP_CRIT) return { cls: 'pill-err', label: t('monitor.gpuCritBadge') };
  if (temp >= VRAM_TEMP_WARN) return { cls: 'pill-warn', label: t('monitor.gpuWarnBadge') };
  return { cls: 'pill-ok', label: null };
}

/** 无传感器判定：note 表明该 DRAM 类型硬件上没有传感器（GDDR6/5，3060 语义）。 */
function vramNoSensor(g: VramGpu): boolean {
  return (
    g.memory_temp_c == null &&
    !!g.note &&
    g.note.includes('no memory temperature sensor')
  );
}

/** 逐颗粒迷你条高度（0..130°C 线性映射到 15%..100%）。 */
function vramModuleHeight(temp: number): string {
  const pct = Math.max(15, Math.min(100, Math.round((temp / 130) * 100)));
  return `${pct}%`;
}
...
<template>
  <div class="monitor-page">
    ...
    <!-- GPU 显存（vraminfo 探测式；缺失/无传感器降级文案） -->
    <section class="panel">
      <div class="card card-table">
        <div class="card-title">
          {{ t('monitor.gpuTitle') }}
          <span v-if="vram.age_secs != null" class="card-sub-inline muted">
            · {{ t('monitor.gpuAge', { s: vram.age_secs }) }}
          </span>
        </div>

        <!-- 工具缺失：安装指引（降级不报错，监控其余部分不受影响） -->
        <div v-if="vram.available === false" class="vram-degraded muted">
          {{ vram.hint || t('monitor.gpuNoTool') }}
        </div>

        <template v-else>
          <!-- 请求失败（401/403 非 admin / 网络错）与工具执行失败分开显示 -->
          <div v-if="vramLoadError" class="vram-degraded muted">{{ vramLoadError }}</div>
          <div v-else-if="vram.error" class="vram-degraded muted mono">{{ vram.error }}</div>

          <table v-if="vram.available === true && (vram.gpus?.length ?? 0) > 0" class="data-table">
            <thead>
              <tr>
                <th>GPU</th>
                <th>{{ t('monitor.gpuBoard') }}</th>
                <th>{{ t('monitor.gpuMemory') }}</th>
                <th class="col-center">{{ t('monitor.gpuMemTemp') }}</th>
                <th>{{ t('monitor.gpuModules') }}</th>
                <th>{{ t('monitor.gpuNote') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="g in vram.gpus" :key="g.bdf ?? ''">
                <td class="mono">{{ g.bdf ?? '—' }}</td>
                <td>{{ g.board_vendor ?? '—' }}</td>
                <td>
                  <span
                    v-if="g.memory_maker || g.memory_type"
                    class="pill pill-blue vram-mem-pill"
                  >{{ [g.memory_maker, g.memory_type].filter(Boolean).join(' · ') }}</span>
                  <span v-else class="muted">—</span>
                </td>
                <td class="col-center">
                  <div v-if="g.memory_temp_c != null" class="vram-temp">
                    <span class="vram-temp-value" :style="{ color: vramTempColor(g.memory_temp_c) }">
                      {{ g.memory_temp_c }}<span class="vram-temp-unit">°C</span>
                    </span>
                    <span
                      v-if="vramTempBadge(g.memory_temp_c).label"
                      class="pill"
                      :class="vramTempBadge(g.memory_temp_c).cls"
                    >{{ vramTempBadge(g.memory_temp_c).label }}</span>
                  </div>
                  <!-- 无传感器（GDDR6/GDDR5 硬件属性）：如实标注，不算故障 -->
                  <span v-else-if="vramNoSensor(g)" class="pill pill-muted">{{ t('monitor.gpuNoSensor') }}</span>
                  <span v-else class="muted">—</span>
                </td>
                <td>
                  <div
                    v-if="g.memory_temp_modules_c?.length"
                    class="vram-mods"
                    :title="(g.memory_temp_modules_c ?? []).join(' / ') + ' °C'"
                  >
                    <span
                      v-for="(m, i) in g.memory_temp_modules_c"
                      :key="i"
                      class="vram-mod"
                      :style="{ height: vramModuleHeight(m), background: vramTempColor(m) }"
                    ></span>
                  </div>
                  <span v-else class="muted">—</span>
                </td>
                <td class="mono vram-note">{{ g.note ?? '—' }}</td>
              </tr>
            </tbody>
          </table>
          <div
            v-else-if="vram.available === true && !vram.error && !vramLoadError"
            class="vram-degraded muted"
          >{{ t('monitor.gpuNoGpu') }}</div>
        </template>
      </div>
    </section>
...
/* —— GPU 显存卡（/vram）—— */
.card-sub-inline { font-size: 12px; margin-left: 6px; font-weight: 400; }
.vram-degraded { padding: 18px 16px; font-size: 13px; line-height: 1.6; }
.vram-mem-pill { font-size: 11.5px; }
.vram-temp { display: flex; align-items: center; justify-content: center; gap: 8px; flex-wrap: wrap; }
.vram-temp-value { font-size: 18px; font-weight: 700; letter-spacing: -0.02em; }
.vram-temp-unit { font-size: 12px; font-weight: 600; margin-left: 1px; }
.vram-mods { display: flex; align-items: flex-end; gap: 2px; height: 28px; }
.vram-mod { display: inline-block; width: 7px; min-height: 4px; border-radius: 2px 2px 0 0; }
.vram-note { max-width: 260px; white-space: normal; word-break: break-word; font-size: 11.5px; }
