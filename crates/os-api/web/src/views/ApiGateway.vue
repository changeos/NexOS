...
const groupTabs: Record<TabGroup, { key: TabKey; label: string }[]> = {
  gateway: [
    ...
    { key: 'channels', label: t('gw.tabChannels') },
    { key: 'llmmon', label: t('gw.tabInstances') },
    { key: 'tokens', label: t('gw.tabTokens') },
    { key: 'logs', label: t('gw.tabLogs') },
  ],
  lobby: [
    { key: 'market', label: t('gw.tabMarket') },
    { key: 'mylistings', label: t('gwMine.tab') },
  ],
  ops: [
    { key: 'payments', label: t('gw.tabPayments') },
...
const billingModeOptions: { value: 'free' | 'per_token' | 'per_image' | 'credits'; label: string; hint: string }[] = [
  { value: 'free', label: t('gw.bmFree'), hint: t('gw.bmFreeHint') },
  { value: 'per_token', label: t('gw.bmPerToken'), hint: t('gw.bmPerTokenHint') },
  { value: 'per_image', label: t('gw.bmPerImage'), hint: t('gw.bmPerImageHint') },
  { value: 'credits', label: t('gw.bmCredits'), hint: t('gw.bmCreditsHint') },
];
...
async function importFromExternalApi(row: ExternalApiRow): Promise<void> {
  ...
  try {
    ...
    msg.value = {
      kind: 'ok',
      text:
        t('gw.extImported', { name: created.name ?? row.name ?? row.id, n: n > 0 ? t('gw.modelsCountParen', { n }) : '' }) +
    ...
    msg.value = { kind: 'err', text: t('gw.importFailed', { err: e instanceof Error ? e.message : String(e) }) };
  } finally {
...
function prefillFromDiscovery(e: DiscoveryEntry): void {
  const port = e.port ?? 0;
  channelForm.value.name = e.name ?? t('gw.discoveredName', { port });
...
const channelColumns: Column<Channel>[] = [
  { key: 'name', title: t('gw.colName') },
  { key: 'provider', title: 'Provider' },
  { key: 'base_url', title: 'Base URL' },
  { key: 'models', title: t('gw.colModel') },
  { key: 'priority', title: t('gw.colPriority') },
  { key: 'status', title: t('gw.colStatus') },
  { key: 'request_count', title: t('gw.colRequests') },
  { key: 'actions', title: t('gw.colActions') },
];
...
const tokenColumns: Column<ApiToken>[] = [
  ...
  { key: 'status', title: t('gw.colStatus') },
  { key: 'billing_mode', title: t('gw.colBilling') },
  { key: 'quota', title: t('gw.colQuota') },
  { key: 'allowed_models', title: t('gw.colAllowedModels') },
  { key: 'request_count', title: t('gw.colRequests') },
  { key: 'created_at', title: t('gw.colCreatedAt') },
  { key: 'actions', title: t('gw.colActions') },
];

const logColumns: Column<CallLog>[] = [
  { key: 'created_at', title: t('gw.colTime') },
  { key: 'token_name', title: t('gw.colToken') },
  { key: 'channel_name', title: t('gw.tabChannels') },
  { key: 'model', title: t('gw.colModel') },
  { key: 'prompt_tokens', title: 'Prompt' },
  { key: 'completion_tokens', title: 'Completion' },
  { key: 'total_tokens', title: t('gw.colTotalTokens') },
  { key: 'latency_ms', title: t('gw.colLatency') },
  { key: 'status', title: t('gw.colStatus') },
];

const paymentColumns: Column<PaymentOrder>[] = [
  { key: 'created_at', title: t('gw.colTime') },
  { key: 'token_id', title: t('gw.colToken') },
  { key: 'currency', title: t('gw.colCurrency') },
  { key: 'amount_crypto', title: t('gw.colAmount') },
  { key: 'credits', title: t('gw.creditsLabel') },
  { key: 'status', title: t('gw.colStatus') },
  { key: 'txid', title: 'TxID' },
  { key: 'actions', title: t('gw.colActions') },
];
...
const quotaUsagePct = (tok: ApiToken): number => {
  const limit = tok.quota_limit ?? 0;
  if (!limit) return 0;
  return Math.min(100, Math.round(((tok.quota_used ?? 0) / limit) * 100));
};
/** 积分余量（quota_limit - quota_used；无上限时为 null）。 */
function tokenQuotaRemaining(tok: ApiToken): number | null {
  if (!tok.quota_limit) return null;
  return tok.quota_limit - (tok.quota_used ?? 0);
}
...
function tokenStatusClass(tok: ApiToken): string {
  if (!tok.enabled || tok.status === 'disabled') return 'pill pill-muted';
  if (tok.status === 'expired') return 'pill pill-err';
  return 'pill pill-ok';
}
function tokenStatusLabel(tok: ApiToken): string {
  return tok.status ?? (tok.enabled ? 'active' : 'disabled');
}
...
// —— 计费模式 ——
function billingModeOf(tok: ApiToken): string {
  return tok.billing_mode ?? 'per_token';
}
...
function billingModeLabelOf(mode: string): string {
  switch (mode) {
    case 'free':
      return t('gw.bmFree');
    case 'per_image':
      return t('gw.bmPerImage');
    case 'credits':
      return t('gw.bmCredits');
    case 'per_token':
    default:
      return t('gw.bmPerToken');
  }
}
...
function billingModeLabel(tok: ApiToken): string {
  return billingModeLabelOf(billingModeOf(tok));
}
function billingModeClass(tok: ApiToken): string {
  return billingModeClassOf(billingModeOf(tok));
}
...
function payStatusLabel(s?: string): string {
  if (s === 'confirmed') return t('gw.payConfirmed');
  if (s === 'rejected') return t('gw.payRejected');
  return t('gw.payPending');
}
...
function amountUnitHint(c?: string): string {
  switch (c) {
    case 'usdt':
      return t('gw.unitUsdt');
    case 'btc':
      return t('gw.unitSat');
    case 'evm':
      return 'wei（1 ETH / 1 BNB = 1e18 wei）';
    default:
      return '';
  }
}
...
async function copyWithToast(text: string): Promise<void> {
  if (await copyText(text)) {
    msg.value = { kind: 'ok', text: t('gw.copied') };
  } else {
    msg.value = { kind: 'err', text: t('gw.copyFailed') };
  }
}
...
async function submitChannel(): Promise<void> {
  ...
  try {
    if (editingChannel.value?.id) {
      ...
      msg.value = { kind: 'ok', text: t('gw.channelUpdated') };
    } else {
      await endpoints.createGatewayChannel(body);
      msg.value = { kind: 'ok', text: t('gw.channelAdded') };
    }
    showChannelDialog.value = false;
    editingChannel.value = null;
    await loadChannels();
...
async function removeChannel(c: Channel): Promise<void> {
  if (!c.id) return;
  if (!confirm(t('gw.channelDeleteConfirm', { name: c.name ?? c.id }))) return;
  busyId.value = c.id;
  msg.value = null;
  try {
    await endpoints.deleteGatewayChannel(c.id);
    msg.value = { kind: 'ok', text: t('gw.channelDeleted') };
    await loadChannels();
    await loadStats();
    await loadModels();
  } catch (e) {
...
async function testChannel(c: Channel): Promise<void> {
  ...
  try {
    ...
    msg.value = {
      kind: 'ok',
      text: t('gw.testOk', { n: r.models_count ?? 0 }),
    };
    await loadChannels();
  } catch (e) {
    msg.value = {
      kind: 'err',
      text: t('gw.testFailed', { err: e instanceof Error ? e.message : String(e) }),
    };
    await loadChannels();
  } finally {
    busyId.value = '';
...
async function disableToken(tok: ApiToken): Promise<void> {
  if (!tok.id) return;
  busyId.value = tok.id;
  msg.value = null;
  try {
    await endpoints.disableGatewayToken(tok.id); // t: ApiToken 行参
    msg.value = { kind: 'ok', text: t('gw.tokenDisabled') };
    await loadTokens();
    await loadStats();
  } catch (e) {
    msg.value = { kind: 'err', text: e instanceof Error ? e.message : String(e) };
...
async function enableToken(tok: ApiToken): Promise<void> {
  if (!tok.id) return;
  busyId.value = tok.id;
  msg.value = null;
  try {
    await endpoints.enableGatewayToken(tok.id);
    msg.value = { kind: 'ok', text: t('gw.tokenEnabled') };
    await loadTokens();
    await loadStats();
  } catch (e) {
    msg.value = { kind: 'err', text: e instanceof Error ? e.message : String(e) };
...
async function removeToken(tok: ApiToken): Promise<void> {
  if (!tok.id) return;
  if (!confirm(t('gw.tokenDeleteConfirm', { name: tok.name ?? tok.id }))) return;
  busyId.value = tok.id;
  msg.value = null;
  try {
    await endpoints.deleteGatewayToken(tok.id);
    msg.value = { kind: 'ok', text: t('gw.tokenDeleted') };
    await loadTokens();
    await loadStats();
  } catch (e) {
    msg.value = { kind: 'err', text: e instanceof Error ? e.message : String(e) };
...
async function submitMapping(): Promise<void> {
  ...
  try {
    ...
    msg.value = { kind: 'ok', text: t('gw.mappingAdded') };
    showMappingDialog.value = false;
    await loadMappings();
  } catch (e) {
    msg.value = { kind: 'err', text: e instanceof Error ? e.message : String(e) };
...
async function removeMapping(m: ModelMapping): Promise<void> {
  ...
  if (!confirm(t('gw.mappingDeleteConfirm', { name: m.public_name }))) return;
  msg.value = null;
  try {
    await endpoints.deleteGatewayMapping(m.public_name);
    msg.value = { kind: 'ok', text: t('gw.mappingDeleted') };
    await loadMappings();
  } catch (e) {
    msg.value = { kind: 'err', text: e instanceof Error ? e.message : String(e) };
  }
...
async function submitPayment(): Promise<void> {
  ...
  if (!paymentForm.value.token_id) {
    msg.value = { kind: 'err', text: t('gw.payNeedToken') };
    return;
  }
  submitting.value = true;
  try {
    ...
    msg.value = { kind: 'ok', text: t('gw.payCreated') };
    await loadPayments();
  } catch (e) {
    msg.value = { kind: 'err', text: e instanceof Error ? e.message : String(e) };
  } finally {
...
async function submitConfirmPayment(): Promise<void> {
  ...
  try {
    ...
    msg.value = {
      kind: 'ok',
      text: t('gw.payConfirmedOk', { name: tokenNameOf(confirmingOrder.value.token_id), n: r.added_credits ?? confirmingOrder.value.credits ?? 0 }),
    };
    showConfirmPayDialog.value = false;
    confirmingOrder.value = null;
    ...
    if (e instanceof ApiError && e.status === 409) {
      msg.value = { kind: 'info', text: t('gw.payAlreadyConfirmed') };
      showConfirmPayDialog.value = false;
      confirmingOrder.value = null;
      await loadPayments();
    } else {
...
async function submitRejectPayment(): Promise<void> {
  ...
  try {
    ...
    msg.value = { kind: 'ok', text: t('gw.orderRejected') };
    showRejectPayDialog.value = false;
    rejectingOrder.value = null;
    await loadPayments();
  } catch (e) {
...
function loadBadgeText(id?: string): string {
  ...
  if (!m || !m.reachable || !m.metrics) return t('gw.unreachable');
  const mt = m.metrics;
  ...
  if (mt.running !== null && mt.running !== undefined) {
    ...
    return t('gw.loadRunningQueued', { running: Math.round(mt.running), waiting: w ? t('gw.loadWaiting', { n: Math.round(w) }) : '' });
  }
  return '—';
}
function loadBadgeTitle(id?: string): string {
  const m = metricsOf(id);
  if (!m) return t('gw.loadFetching');
  const src = m.source === 'heartbeat' ? t('gw.srcHeartbeat') : m.source === 'metrics_url' ? t('gw.srcProxy') : t('gw.srcNone');
  const fresh = m.reachable && !m.stale ? t('gw.fresh') : m.reachable ? t('gw.maybeStale') : t('gw.unreachable');
  return t('gw.loadBadge', { src, fresh });
}
...
function priceBadge(l: MarketListing): { text: string; cls: string } {
  ...
  switch (p.mode) {
    case 'free':
      return { text: t('gw.bmFree'), cls: 'pill pill-ok' };
    case 'per_image':
      return { text: t('gw.pricePerImage', { p: p.price_per_1k_tokens ?? '—', cur }), cls: 'pill pill-purple' };
    case 'per_token':
    default:
  ...
  if (cards.length > 0) {
    ...
    const cap = first.unified
      ...
      : first.vram
        ...
        : '';
    const per = cap
      ? ` · ${first.unified ? t('gw.unifiedMemPrefix') : ''}${cap}${!first.unified && cards.length > 1 ? t('gw.perCardSuffix') : ''}`
      : '';
    if (cards.length === 1) return `${first.name}${per}`;
    const sameModel = cards.every((g) => g.name === first.name);
    return sameModel ? t('gw.gpuSame', { name: first.name, n: cards.length, per }) : t('gw.gpuMulti', { name: first.name, n: cards.length, per });
  }
  const n = Math.max(c.gpu_count ?? 0, c.gpu_name ? 1 : 0);
  if (!c.gpu_name) return '—';
  const per = c.gpu_vram_mb ? ` · ${vramLabel(c.gpu_vram_mb)}${n > 1 ? t('gw.perCardSuffix') : ''}` : '';
...
function cpuSummary(c: MarketServerConfig): string {
  if (c.cpu_model) return c.cpu_cores ? t('gw.cpuModelCores', { model: c.cpu_model, n: c.cpu_cores }) : c.cpu_model;
  return c.cpu_cores ? t('gw.cpuCores', { n: c.cpu_cores }) : '—';
}
...
function configRows(l: MarketListing): { k: string; v: string }[] {
  ...
  const rows = [
    ...
    { k: t('gw.hwRam'), v: c.ram_gb ? `${c.ram_gb} GB` : '—' },
    { k: t('gw.hwModel'), v: c.model_name ?? '—' },
    { k: t('gw.hwCtx'), v: ctx !== null ? ctx.toLocaleString() : '—' },
  ];
  if (c.quantization) rows.push({ k: t('gw.hwQuant'), v: c.quantization });
  if (c.region) rows.push({ k: t('gw.hwRegion'), v: c.region });
  return rows;
}
...
async function federateMarket(l: MarketListing): Promise<void> {
  ...
  try {
    ...
    msg.value = {
      kind: 'ok',
      text: t('gw.fedPushed', { again: r.first_push === false ? t('gw.fedPushedAgain') : '', name: l.api_name ?? l.id, note: r.note ? t('gw.noteParen', { note: r.note }) : '' }),
    };
    await loadMarket();
  } catch (e) {
    msg.value = { kind: 'err', text: marketWriteErr(t('gw.actFedPush'), e) };
  } finally {
    busyId.value = '';
  }
}
...
const priceFieldLabel = computed(() =>
  marketPublishForm.value.pricing_mode === 'per_image' ? t('gw.pricePerImageLabel') : t('gw.pricePer1kLabel'),
);
...
function marketWriteErr(action: string, e: unknown): string {
  if (e instanceof ApiError && e.status === 401) {
    forceNexhubReauth();
    return t('gw.mktErr401', { action });
  }
  if (e instanceof ApiError && e.status === 403) {
    return t('gw.mktErr403', { action });
  }
  return t('gw.mktErr', { action, err: e instanceof Error ? e.message : String(e) });
}
...
async function submitMarketPublish(): Promise<void> {
  ...
  msg.value = null;
  if (!f.api_name.trim()) {
    msg.value = { kind: 'err', text: t('gw.needApiName') };
    return;
  }
  if (!f.endpoint_url.trim()) {
    msg.value = { kind: 'err', text: t('gw.needEndpoint') };
    return;
  }
  if (!f.model_name.trim()) {
    msg.value = { kind: 'err', text: t('gw.needModelName') };
    return;
  }
  if (f.pricing_mode !== 'free' && (!f.price || f.price <= 0)) {
    msg.value = { kind: 'err', text: t('gw.needPrice') };
    return;
  }
  if (!marketHasIdentity.value) {
    msg.value = { kind: 'err', text: t('gw.needIdentity') };
    return;
  }
  submitting.value = true;
  try {
    ...
    msg.value = {
      kind: 'ok',
      text: t('gw.published', { verb: r.refreshed ? t('gw.pubRefreshed') : t('gw.pubPublished'), name: r.api_name ?? f.api_name, publisher: r.publisher_display ?? identityEvm.value ?? t('gw.chainIdentity') }),
    };
    showMarketPublish.value = false;
    await loadMarket();
  } catch (e) {
    msg.value = { kind: 'err', text: marketWriteErr(t('gw.actPublish'), e) };
  } finally {
    submitting.value = false;
  }
}
...
async function sendHeartbeat(l: MarketListing): Promise<void> {
  ...
  try {
    ...
    msg.value = {
      kind: 'ok',
      text: t('gw.heartbeatOk', { time: fmtTime(r.heartbeat_at) }),
    };
    await fetchMarketMetrics(l.id);
  } catch (e) {
    msg.value = { kind: 'err', text: marketWriteErr(t('gw.actHeartbeat'), e) };
  } finally {
    busyId.value = '';
  }
}
...
async function unlistMarket(l: MarketListing): Promise<void> {
  if (!l.id) return;
  if (!confirm(t('gw.unlistConfirm', { name: l.api_name ?? l.id }))) return;
  busyId.value = l.id;
  msg.value = null;
  try {
    const opts = await requireMarketIdentity();
    await endpoints.apiMarketUnpublish(l.id, opts);
    msg.value = { kind: 'ok', text: t('gw.unlisted', { name: l.api_name ?? l.id }) };
    await loadMarket();
  } catch (e) {
    msg.value = { kind: 'err', text: marketWriteErr(t('gw.actUnlist'), e) };
  } finally {
    busyId.value = '';
  }
}
...
<template>
  <div class="gw-page">
    <div class="page-head">
      <div>
        <h2 class="page-title">{{ t('gw.title') }}</h2>
        <div class="page-sub muted">{{ t('gw.subtitle') }}</div>
      </div>
    ...
    <nav class="tabs sub-tabs" role="tablist" :aria-label="t('gw.subTabs')">
      <button
    ...
    <section v-show="activeTab === 'channels'" class="tab-panel">
      ...
      <section class="stat-grid">
        <div class="card stat-card">
          <div class="stat-label">{{ t('gw.statChannels') }}</div>
          <div class="stat-value">{{ stats.channels_total ?? 0 }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('gw.statEnabled') }}</div>
          <div class="stat-value">{{ stats.channels_enabled ?? 0 }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('gw.statErrors') }}</div>
          <div class="stat-value stat-err">{{ channelErrorCount }}</div>
        </div>
      </section>

      <div class="panel-head">
        <span class="panel-title">{{ t('gw.channelList') }}</span>
        <button class="btn btn-small btn-primary" @click="openCreateChannel">＋ {{ t('gw.addChannel') }}</button>
      </div>
        ...
        <div class="card card-table">
          ...
          >
            <template #cell-name="{ row }">
              <span class="ch-name-cell">
                ...
                <span
                  v-if="row.via_node"
                  class="pill pill-fed"
                  :title="t('gw.relayChannelTitle', { node: row.via_node.slice(0, 10) })"
                >🌐 {{ t('gw.relayBadge') }}</span>
              </span>
            </template>
            ...
            <template #cell-models="{ row }">
              <span class="small">{{ t('gw.countUnit', { n: (row.models ?? []).length }) }}</span>
            </template>
      ...
      <div v-if="showChannelDialog" class="modal-backdrop" @click.self="closeChannelDialog">
        <div class="modal" role="dialog" aria-modal="true" aria-labelledby="gw-ch-title">
          <div class="modal-head">
            <h3 id="gw-ch-title">{{ editingChannel ? t('gw.editChannel') : t('gw.addChannel') }}</h3>
          ...
          <form class="modal-body" @submit.prevent="submitChannel">
            ...
            <div v-if="!editingChannel" class="discovery-box">
              <div class="discovery-head">
                <span class="discovery-title">{{ t('gw.discoveryTitle') }}</span>
                <span class="muted small">{{ t('gw.discoveryHint') }}</span>
              </div>
              <div v-if="discoveryLoading" class="muted small">{{ t('gw.discovering') }}</div>
              <div v-else-if="discoveryError" class="muted small">
                {{ t('gw.discoveryFailed', { err: discoveryError }) }}
              </div>
              ...
              <div v-else class="discovery-list">
                ...
                >
                  ...
                  <span class="pill pill-cyan">{{ t('gw.modelsCount', { n: (e.model_ids ?? []).length }) }}</span>
                  <span v-if="e.discovered" class="pill pill-warning">{{ t('gw.scanFound') }}</span>
                </button>
              </div>
            </div>
            ...
            <div v-if="!editingChannel" class="discovery-box">
              <div class="discovery-head">
                <span class="discovery-title">{{ t('gw.extImportTitle') }}</span>
                <span class="muted small">{{ t('gw.extImportHint') }}</span>
              </div>
              <div v-if="externalLoading" class="muted small">{{ t('gw.extLoading') }}</div>
              <div v-else-if="externalError" class="muted small">
                {{ t('gw.extFailed', { err: externalError }) }}
              </div>
              ...
              <div v-else class="discovery-list">
                ...
                >
                  ...
                  <span v-if="api.via_node" class="pill pill-fed" :title="t('gw.relayEntryTitle')">🌐 {{ t('gw.relayBadge') }}</span>
                  <span class="pill pill-cyan">{{ t('gw.modelsCount', { n: (api.models ?? []).length }) }}</span>
                  <span class="muted small">{{ importingExtId === api.id ? t('gw.importing') : t('gw.importOne') }}</span>
                </button>
              </div>
            </div>
            <div class="field">
              <label for="gw-ch-name">{{ t('gw.channelNameLabel') }}</label>
              <input id="gw-ch-name" v-model="channelForm.name" type="text" :placeholder="t('gw.channelNamePh')" :disabled="submitting" />
            </div>
            <div class="field-row">
              <div class="field">
            ...
            <div class="field">
              <label for="gw-ch-key">{{ t('gw.keyLabel') }}</label>
              <input id="gw-ch-key" v-model="channelForm.api_key" type="text" :placeholder="t('gw.keyPh')" :disabled="submitting" />
            </div>
            <div class="field">
              <label for="gw-ch-models">{{ t('gw.modelsLabel') }}</label>
              <input id="gw-ch-models" v-model="channelForm.models" type="text" placeholder="gpt-4o, gpt-4o-mini" :disabled="submitting" />
            </div>
            <div class="field-row">
              <div class="field">
                <label for="gw-ch-prio">{{ t('gw.priorityLabel') }}</label>
                <input id="gw-ch-prio" v-model.number="channelForm.priority" type="number" min="0" :disabled="submitting" />
              </div>
              <div class="field">
                <label for="gw-ch-w">{{ t('gw.weightLabel') }}</label>
                <input id="gw-ch-w" v-model.number="channelForm.weight" type="number" min="1" :disabled="submitting" />
              </div>
            </div>
            <div class="field">
              <label for="gw-ch-via">{{ t('gw.viaLabel') }}</label>
              <input
                id="gw-ch-via"
                v-model="channelForm.via_node"
                type="text"
                class="mono"
                :placeholder="t('gw.viaPh')"
                :disabled="submitting"
              />
            ...
            <div class="form-actions">
              <button type="button" class="btn" :disabled="submitting" @click="closeChannelDialog">{{ t('gw.cancel') }}</button>
              <button type="submit" class="btn btn-primary" :disabled="submitting">
                {{ submitting ? t('gw.saving') : t('gw.save') }}
              </button>
            </div>
          </form>
        </div>
    ...
    <section v-show="activeTab === 'tokens'" class="tab-panel">
      ...
      <section class="stat-grid">
        <div class="card stat-card">
          <div class="stat-label">{{ t('gw.statTokens') }}</div>
          <div class="stat-value">{{ stats.tokens_total ?? 0 }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('gw.statActive') }}</div>
          <div class="stat-value">{{ stats.tokens_active ?? 0 }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('gw.statTotalReqs') }}</div>
          <div class="stat-value">{{ stats.total_requests ?? 0 }}</div>
        </div>
      </section>

      <div class="panel-head">
        <span class="panel-title">{{ t('gw.tokenList') }}</span>
        <button class="btn btn-small btn-primary" @click="openCreateToken">＋ {{ t('gw.createToken') }}</button>
      </div>
          ...
          >
            ...
            <template #cell-quota="{ row }">
              <div v-if="billingModeOf(row) === 'free'" class="quota-cell">
                ...
                <span class="muted small">{{ t('gw.freeUnlimited') }}</span>
              </div>
              <div v-else-if="billingModeOf(row) === 'credits'" class="quota-cell">
                <span class="mono strong">{{ t('gw.creditsRemaining', { n: tokenQuotaRemaining(row) ?? '—' }) }}</span>
              ...
              <div v-else class="quota-cell">
                ...
                <span v-else class="muted small">{{ t('gw.unlimited') }}</span>
              </div>
            </template>
            <template #cell-allowed_models="{ row }">
              <span class="small">{{ (row.allowed_models ?? []).length ? (row.allowed_models ?? []).join(', ') : t('gw.allModels') }}</span>
            </template>
            <template #cell-request_count="{ row }">
              <span class="mono">{{ row.request_count ?? 0 }}</span>
            </template>
      ...
      <div v-if="showTokenDialog" class="modal-backdrop" @click.self="closeTokenDialog">
        <div class="modal" role="dialog" aria-modal="true" aria-labelledby="gw-tok-title">
          <div class="modal-head">
            <h3 id="gw-tok-title">{{ t('gw.createTokenTitle') }}</h3>
          ...
          <form class="modal-body" @submit.prevent="submitToken">
            <div class="field">
              <label for="gw-tok-name">{{ t('gw.tokenNameLabel') }}</label>
              <input id="gw-tok-name" v-model="tokenForm.name" type="text" :placeholder="t('gw.tokenNamePh')" :disabled="submitting" />
            </div>
            <div class="field">
              <label for="gw-tok-billing">{{ t('gw.billingLabel') }}</label>
    ...
    <section v-show="activeTab === 'logs'" class="tab-panel">
      <div class="panel-head">
        <span class="panel-title">{{ t('gw.logsTitle', { n: logLimit }) }}</span>
        <div class="head-actions">
          <label class="small muted">{{ t('gw.perPage') }}</label>
    ...
    <section v-show="activeTab === 'payments'" class="tab-panel">
      ...
      <section class="stat-grid">
        <div class="card stat-card">
          <div class="stat-label">{{ t('gw.payPending') }}</div>
          <div class="stat-value stat-warn">{{ pendingPaymentCount }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('gw.payConfirmed') }}</div>
          <div class="stat-value">{{ confirmedPaymentCount }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('gw.payRejected') }}</div>
          <div class="stat-value stat-err">{{ rejectedPaymentCount }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('gw.statOrders') }}</div>
      ...
      <!-- 创建订单表单 -->
      <div class="panel-head">
        <span class="panel-title">{{ t('gw.createOrderTitle') }}</span>
        <span class="muted small">{{ t('gw.orderSub') }}</span>
      </div>
      <div class="card pay-form-card">
        <form class="pay-form" @submit.prevent="submitPayment">
          <div class="field">
            <label for="gw-pay-token">{{ t('gw.payTokenLabel') }}</label>
            <select id="gw-pay-token" v-model="paymentForm.token_id" :disabled="submitting">
              <option value="">{{ t('gw.selectToken') }}</option>
          ...
          <div class="field">
            <label for="gw-pay-cur">{{ t('gw.currencyLabel') }}</label>
          ...
          <div class="field">
            <label for="gw-pay-credits">{{ t('gw.creditsAmountLabel') }}</label>
          ...
          <div class="field pay-form-actions">
            <button type="submit" class="btn btn-primary" :disabled="submitting || paymentsLoading">
              {{ submitting ? t('gw.creating') : t('gw.createOrder') }}
            </button>
          </div>
        </form>
      </div>
      ...
      <div v-if="createdPayment" class="card pay-info-card">
        <div class="pay-info-head">
          <h3 class="pay-info-title">{{ t('gw.payInfoTitle', { id: createdPayment.id ?? '—' }) }}</h3>
        ...
        <div class="pay-info-grid">
          <div class="pay-info-item">
            <div class="pay-info-label">{{ t('gw.amountLabel', { cur: currencyLabel(createdPayment.currency) }) }}</div>
            <div class="pay-info-value mono">{{ payAmount(createdPayment) }}</div>
            <div class="muted small">{{ t('gw.unitLabel') }}{{ amountUnitHint(createdPayment.currency) }}</div>
          </div>
          <div class="pay-info-item">
            <div class="pay-info-label">{{ t('gw.creditsLabel') }}</div>
            <div class="pay-info-value mono">{{ createdPayment.credits ?? 0 }}</div>
            <div class="muted small">{{ t('gw.creditsPoolHint', { name: tokenNameOf(createdPayment.token_id) }) }}</div>
          </div>
        </div>
        <div class="pay-info-item pay-info-addr">
          <div class="pay-info-label">{{ t('gw.addressLabel', { cur: currencyLabel(createdPayment.currency) }) }}</div>
          <!-- 占位收款地址（env 未配置真实钱包）→ 红色醒目警示 + 禁用复制 -->
          <div v-if="isPlaceholderPayAddress(createdPayment.address)" class="pay-placeholder-banner">
            {{ t('gw.placeholderAddressWarn') }}
          </div>
          ...
          <div v-else class="error-box">{{ t('gw.noAddressErr') }}</div>
        </div>
        <p v-if="createdPayment.memo" class="muted small pay-memo">{{ t('gw.memoLabel') }}{{ createdPayment.memo }}</p>
      </div>

      <!-- 订单列表 -->
      <div class="panel-head">
        <span class="panel-title">{{ t('gw.ordersTitle') }}</span>
      ...
      <div v-if="showConfirmPayDialog" class="modal-backdrop" @click.self="closeConfirmPayDialog">
        <div class="modal" role="dialog" aria-modal="true" aria-labelledby="gw-payc-title">
          <div class="modal-head">
            <h3 id="gw-payc-title">{{ t('gw.confirmPayTitle') }}</h3>
          ...
          <form class="modal-body" @submit.prevent="submitConfirmPayment">
            <p class="muted small pay-confirm-summary">
              {{ t('gw.orderLabel') }}<span class="mono">{{ confirmingOrder?.id ?? '—' }}</span> ·
              {{ currencyLabel(confirmingOrder?.currency) }}
              <span class="mono">{{ confirmingOrder ? payAmount(confirmingOrder) : '—' }}</span> →
              {{ t('gw.confirmPayBody', { name: confirmingOrder ? tokenNameOf(confirmingOrder.token_id) : '—', n: confirmingOrder?.credits ?? 0 }) }}
              <strong>{{ t('gw.confirmPayStrong') }}</strong>
            </p>
            <div class="field">
              <label for="gw-payc-txid">{{ t('gw.txidLabel') }}</label>
              <input id="gw-payc-txid" v-model="confirmPayTxid" type="text" :placeholder="t('gw.txidPh')" :disabled="submitting" />
            </div>
            <p v-if="msg" :class="['form-msg', `is-${msg.kind}`]">{{ msg.text }}</p>
            <div class="form-actions">
              <button type="button" class="btn" :disabled="submitting" @click="closeConfirmPayDialog">{{ t('gw.cancel') }}</button>
              <button type="submit" class="btn btn-primary" :disabled="submitting">
                {{ submitting ? t('gw.confirming') : t('gw.confirmPaid') }}
              </button>
            </div>
          </form>
        </div>
      ...
      <div v-if="showRejectPayDialog" class="modal-backdrop" @click.self="closeRejectPayDialog">
        <div class="modal" role="dialog" aria-modal="true" aria-labelledby="gw-payr-title">
          <div class="modal-head">
            <h3 id="gw-payr-title">{{ t('gw.rejectTitle') }}</h3>
          ...
          <form class="modal-body" @submit.prevent="submitRejectPayment">
            <p class="muted small pay-confirm-summary">
              {{ t('gw.rejectBody', { id: rejectingOrder?.id ?? '—' }) }}
              （{{ rejectingOrder ? tokenNameOf(rejectingOrder.token_id) : '—' }} ·
              {{ t('gw.rejectBody2', { n: rejectingOrder?.credits ?? 0 }) }}
            </p>
            <div class="field">
              <label for="gw-payr-reason">{{ t('gw.rejectReasonLabel') }}</label>
              <input id="gw-payr-reason" v-model="rejectPayReason" type="text" :placeholder="t('gw.rejectReasonPh')" :disabled="submitting" />
            </div>
            <p v-if="msg" :class="['form-msg', `is-${msg.kind}`]">{{ msg.text }}</p>
            <div class="form-actions">
              <button type="button" class="btn" :disabled="submitting" @click="closeRejectPayDialog">{{ t('gw.cancel') }}</button>
              <button type="submit" class="btn btn-danger" :disabled="submitting">
                {{ submitting ? t('gw.submitting') : t('gw.confirmReject') }}
              </button>
            </div>
          </form>
        </div>
    ...
    <section v-show="activeTab === 'overview'" class="tab-panel">
      ...
      <!-- ④ 可用模型聚合（原「总览」既有功能保留） -->
      <div class="panel-head">
        <span class="panel-title">{{ t('gw.aggregatedTitle') }}</span>
        <span class="muted small">{{ t('gw.aggregatedCount', { n: aggregatedModels.count ?? 0 }) }}</span>
      </div>
      <div class="card models-card">
        <div v-if="modelsLoading" class="empty-inline muted small">{{ t('gw.loading') }}</div>
      ...
      <!-- ⑤ 模型映射管理（原「总览」既有功能保留） -->
      <div class="panel-head">
        <span class="panel-title">{{ t('gw.mappingTitle') }}</span>
        <button class="btn btn-small btn-primary" @click="openCreateMapping">＋ {{ t('gw.addMapping') }}</button>
      </div>
      ...
      <div class="panel">
        <div class="card card-table">
          <DataTable
            :columns="[
              { key: 'public_name', title: t('gw.colPublicName') },
              { key: 'channel_id', title: t('gw.tabChannels') },
              { key: 'upstream_model', title: t('gw.colUpstreamModel') },
              { key: 'actions', title: t('gw.colActions') },
            ]"
            :rows="mappings"
            :loading="mappingsLoading"
            :empty-text="t('gw.mappingsEmpty')"
          >
      ...
      <div v-if="showMappingDialog" class="modal-backdrop" @click.self="closeMappingDialog">
        <div class="modal" role="dialog" aria-modal="true" aria-labelledby="gw-map-title">
          <div class="modal-head">
            <h3 id="gw-map-title">{{ t('gw.addMappingTitle') }}</h3>
    ...
    <section v-show="activeTab === 'market'" class="tab-panel">
      ...
      <div class="market-view-switch" role="tablist" :aria-label="t('gw.marketSourceSwitch')">
        <button
          type="button"
          class="seg-btn"
      ...
      <div class="panel-head market-toolbar">
        <span class="panel-title">{{ t('gw.marketTitle') }}</span>
        <div class="head-actions">
          <input
            v-model="marketQ"
            class="market-search"
            type="text"
            :placeholder="t('gw.marketSearchPh')"
          ...
          <select
            ...
            @change="loadMarket"
          >
            <option value="recent">{{ t('gw.sortRecent') }}</option>
            <option value="price">{{ t('gw.sortPrice') }}</option>
          </select>
          ...
          <button class="btn btn-small btn-primary" @click="openMarketPublish">＋ {{ t('gw.publishApi') }}</button>
        </div>
      </div>
      ...
      <div v-if="marketLoading && marketListings.length === 0" class="card market-empty">
        <span class="muted small">{{ t('gw.loading') }}</span>
      </div>
      <div v-else-if="!marketError && visibleMarketListings.length === 0" class="card market-empty">
        ...
        <span v-else class="muted small">{{ t('gw.marketEmpty') }}</span>
      </div>
          ...
          <div class="mkt-main">
            <div class="mkt-title-row">
              ...
              <span
                ...
                :title="t('gw.mineTitle')"
              >
                <img v-if="publisherIdenticon(l)" class="identicon" :src="publisherIdenticon(l) ?? ''" alt="" />{{ t('gw.mineBadge') }}
              </span>
              <span
                v-if="isRemoteListing(l)"
                class="pill pill-fed"
                :title="t('gw.fedEntryTitle', { node: l.source_node })"
              >🌐 {{ l.source_node }}</span>
          ...
          <div class="market-card-foot">
            ...
            >
              <img
                ...
                alt=""
              />{{ t('gw.publisherPrefix') }} {{ shortEvm(l.publisher_display) }}
            </span>
            ...
            <TipButton
              ...
              size="small"
            />
            <span class="mono small muted">{{ t('gw.downloads', { n: l.download_count ?? 0 }) }}</span>
    ...
    <section v-show="activeTab === 'mylistings'" class="tab-panel">
      ...
      <div v-if="showMarketPublish" class="modal-backdrop" @click.self="closeMarketPublish">
        <div class="modal market-publish-modal" role="dialog" aria-modal="true" aria-labelledby="gw-mkt-title">
          <div class="modal-head">
            <h3 id="gw-mkt-title">{{ t('gw.publishTitle') }}</h3>
          ...
          <form class="modal-body" @submit.prevent="submitMarketPublish">
            ...
            <div v-if="marketHasIdentity" class="mkt-identity mkt-identity-ok">
              {{ t('gw.publisherLabel') }}<code class="mono">{{ identityEvm || identityPubkey }}</code>
              <span v-if="identityAuthing" class="muted small">{{ t('gw.authingParen') }}</span>
            </div>
            <div v-else class="mkt-identity mkt-identity-warn">
              {{ t('gw.noIdentityHint1') }}<strong>{{ t('gw.noIdentityHintStrong') }}</strong>{{ t('gw.noIdentityHint2') }}
            </div>

            <div class="field">
              <label for="gw-mkt-name">{{ t('gw.apiNameLabel') }}</label>
              <input id="gw-mkt-name" v-model="marketPublishForm.api_name" type="text" placeholder="qwen3.5-9b chat" :disabled="submitting" />
            </div>
            <div class="field">
              <label for="gw-mkt-url">{{ t('gw.endpointLabel') }}</label>
              <input id="gw-mkt-url" v-model="marketPublishForm.endpoint_url" type="text" :disabled="submitting" />
            </div>
            <div class="field-row">
              <div class="field">
                <label for="gw-mkt-model">{{ t('gw.modelNameLabel') }}</label>
                <input id="gw-mkt-model" v-model="marketPublishForm.model_name" type="text" placeholder="Qwen3.5-9B" :disabled="submitting" />
              </div>
              <div class="field">
                <label for="gw-mkt-ctx">{{ t('gw.ctxLenLabel') }}</label>
                <input id="gw-mkt-ctx" v-model.number="marketPublishForm.context_len" type="number" min="0" :placeholder="t('gw.ctxLenPh')" :disabled="submitting" />
              </div>
            </div>
            <div class="field-row">
              <div class="field">
                <label for="gw-mkt-desc">{{ t('gw.descLabel') }}</label>
                <input id="gw-mkt-desc" v-model="marketPublishForm.description" type="text" :placeholder="t('gw.descPh')" :disabled="submitting" />
              </div>
              <div class="field">
                <label for="gw-mkt-tags">{{ t('gw.tagsLabel') }}</label>
                <input id="gw-mkt-tags" v-model="marketPublishForm.tags" type="text" placeholder="llm, chat" :disabled="submitting" />
              </div>
            </div>
            <div class="field-row">
              <div class="field">
                <label for="gw-mkt-mode">{{ t('gw.pricingLabel') }}</label>
                <select id="gw-mkt-mode" v-model="marketPublishForm.pricing_mode" :disabled="submitting">
                  <option value="free">{{ t('gw.bmFree') }}</option>
                  <option value="per_token">{{ t('gw.bmPerToken') }}</option>
                  <option value="per_image">{{ t('gw.bmPerImageShort') }}</option>
                </select>
              </div>
            ...
            <div class="field">
              <label for="gw-mkt-murl">{{ t('gw.metricsUrlLabel') }}</label>
              <input id="gw-mkt-murl" v-model="marketPublishForm.metrics_url" type="text" :placeholder="localMetricsUrlHint" :disabled="submitting" />
              <span class="muted small">{{ t('gw.metricsUrlHint', { url: localMetricsUrlHint }) }}</span>
            </div>
            <!-- 接入信息（可选）：消费者直连凭据——仅发布者本人/admin 可见明文，其他身份脱敏 -->
            <div class="field">
              <label for="gw-mkt-akey">{{ t('gw.apiKeyLabel') }}</label>
              ...
              <span class="muted small">{{ t('gw.apiKeyHint') }}</span>
            </div>
            <div class="field-row">
              <div class="field">
                <label for="gw-mkt-ahdr">{{ t('gw.authHeaderLabel') }}</label>
                ...
                <span class="muted small">{{ t('gw.authHeaderHint') }}</span>
              </div>
              <div class="field">
                <label for="gw-mkt-anote">{{ t('gw.accessNoteLabel') }}</label>
                <input id="gw-mkt-anote" v-model="marketPublishForm.access_notes" type="text" :placeholder="t('gw.accessNotePh')" :disabled="submitting" />
              </div>
            </div>
            <div class="mkt-server-cfg">
              <div class="head-actions">
                <button type="button" class="btn btn-small" @click="showServerOverride = !showServerOverride">
                  {{ showServerOverride ? t('gw.collapseHwOverride') : t('gw.overrideHw') }}
                </button>
                <span class="muted small">{{ t('gw.hwAutoHint') }}</span>
              </div>
              <div v-if="showServerOverride" class="mkt-override-grid">
                <div class="field">
                  <label for="gw-mkt-gpu">{{ t('gw.gpuModelLabel') }}</label>
            ...
            <p v-if="msg" :class="['form-msg', `is-${msg.kind}`]">{{ msg.text }}</p>
            <div class="form-actions">
              <button type="button" class="btn" :disabled="submitting" @click="closeMarketPublish">{{ t('gw.cancel') }}</button>
              <button type="submit" class="btn btn-primary" :disabled="submitting">
                {{ submitting ? t('gw.publishing') : t('gw.publish') }}
              </button>
            </div>
          </form>
        </div>
      ...
      <div v-if="metricsDetail" class="modal-backdrop" @click.self="closeMetricsDetail">
        <div class="modal" role="dialog" aria-modal="true" aria-labelledby="gw-mx-title">
          <div class="modal-head">
            <h3 id="gw-mx-title">{{ t('gw.metricsTitle', { name: metricsDetail.listing.api_name ?? '—' }) }}</h3>
            ...
            <template v-if="metricsDetail.resp">
              <div class="mx-meta">
                <span class="load-badge" style="cursor: default">
                  ...
                  {{ metricsDetail.resp.reachable ? (metricsDetail.resp.stale ? t('gw.maybeStale') : t('gw.freshOnline')) : t('gw.unreachable') }}
                </span>
                <span class="muted small">{{ t('gw.sourceLabel') }}{{ metricsDetail.resp.source === 'heartbeat' ? t('gw.srcHeartbeat') : metricsDetail.resp.source === 'metrics_url' ? t('gw.srcProxyFull') : t('gw.srcNone') }}</span>
                <span class="muted small">{{ t('gw.dataTimeLabel') }}{{ fmtTime(metricsDetail.resp.ts) }}</span>
              </div>
              ...
              <div v-if="metricsDetail.resp.metrics" class="mx-table">
                <div class="mx-row"><span class="muted">{{ t('gw.mxLoad') }}</span><span class="mono strong">{{ metricsFmt(metricsDetail.resp.metrics.load_pct) }}%</span></div>
                <div class="mx-row"><span class="muted">{{ t('gw.mxRunning') }}</span><span class="mono">{{ metricsFmt(metricsDetail.resp.metrics.running) }}</span></div>
                <div class="mx-row"><span class="muted">{{ t('gw.mxWaiting') }}</span><span class="mono">{{ metricsFmt(metricsDetail.resp.metrics.waiting) }}</span></div>
                <div class="mx-row"><span class="muted">{{ t('gw.mxGpuCache') }}</span><span class="mono">{{ metricsFmt(metricsDetail.resp.metrics.gpu_cache) }}%</span></div>
                <div class="mx-row"><span class="muted">{{ t('gw.mxThroughput') }}</span><span class="mono">{{ metricsFmt(metricsDetail.resp.metrics.tokens_per_sec) }} tok/s</span></div>
                <div class="mx-row"><span class="muted">{{ t('gw.mxLatency') }}</span><span class="mono">{{ metricsFmt(metricsDetail.resp.metrics.latency_ms) }} ms</span></div>
              </div>
              <p v-else class="muted small" style="margin: 0">
                {{ t('gw.metricsEmpty') }}
              </p>
            </template>
            <div v-else class="muted small">{{ t('gw.fetching') }}</div>
            <div class="form-actions">
              <button type="button" class="btn" @click="closeMetricsDetail">{{ t('gw.close') }}</button>
            </div>
          </div>
        </div>
      </div>
