...
function relTime(ts: number): string {
  if (!ts) return t('imChat.relNever');
  const sec = Math.max(0, Math.floor(Date.now() / 1000 - ts));
  if (sec < 60) return t('imChat.relJustNow');
  if (sec < 3600) return t('imChat.relMinAgo', { n: Math.floor(sec / 60) });
  if (sec < 86_400) return t('imChat.relHourAgo', { n: Math.floor(sec / 3600) });
  return t('imChat.relDayAgo', { n: Math.floor(sec / 86_400) });
}
...
const NEXOS_ASSISTANT = 'NexOS助手'; // 内置助手名（产品名，不翻译）
...
async function onFileChosen(e: Event): Promise<void> {
  ...
  if (file.size > MAX_FILE_BYTES) {
    attachError.value = t('imChat.attachTooLarge', { size: fmtSize(file.size) });
    return;
  }
  ...
  try {
    ...
    attachError.value = t('imChat.attachUploadFailed') + (e instanceof Error ? e.message : String(e));
  } finally {
...
function fileToBase64(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(new Error(t('imChat.readLocalFailed')));
    reader.onload = () => {
      try {
...
const fedToggleTitle = computed(() =>
  fedReceiveEnabled.value === false
    ? t('imChat.fedPausedTitle')
    : t('imChat.fedActiveTitle'),
);
...
async function copyAgentSnippet(key: AgentSnippetKey): Promise<void> {
  ...
  if (!ok) {
    toast.error(t('imChat.copyFailed'));
    return;
  }
...
const remotePhaseNotice = computed(() => {
  switch (activeRemoteState.value.phase) {
    case 'denied':
      return t('imChat.remoteDenied');
    case 'unreachable':
      return t('imChat.remoteNoAnswer');
    case 'error':
      return t('imChat.remoteFetchFailed', { err: activeRemoteState.value.error });
    case 'loading':
      return t('imChat.remoteQuerying');
    default:
      return '';
  }
});
...
async function toggleFedReceive(): Promise<void> {
  ...
  try {
    ...
    fedReceiveEnabled.value = !!r.enabled;
    toast.success(r.enabled ? t('imChat.fedOnToast') : t('imChat.fedOffToast'));
  } catch (e) {
...
async function toggleLobbyAccess(): Promise<void> {
  ...
  try {
    ...
    lobbyAccess.value = r;
    toast.success(
      r.lobby_public ? t('imChat.lobbyOpenToast') : t('imChat.lobbyClosedToast'),
    );
  } catch (e) {
    lobbyAccessError.value = e instanceof Error ? e.message : String(e);
  } finally {
...
async function sendMessage(): Promise<void> {
  ...
  try {
    ...
    messagesError.value = t('imChat.sendFailed') + (e instanceof Error ? e.message : String(e));
  } finally {
    sending.value = false;
  }
}
...
async function sendLobbyMessage(): Promise<void> {
  ...
  try {
    ...
    draft.value = prevDraft; // 失败回滚草稿；附件保持暂存可重发
    lobbyError.value = t('imChat.sendFailed') + (e instanceof Error ? e.message : String(e));
  } finally {
    sending.value = false;
  }
}
...
async function sendFedLobbyMessage(): Promise<void> {
  ...
  try {
    ...
    fedLobbyError.value = t('imChat.sendFailed') + (e instanceof Error ? e.message : String(e));
  } finally {
    sending.value = false;
  }
}
  ...
  try {
    ...
    const hint =
      e instanceof ApiError && e.status === 403
        ? t('imChat.dmClosedHint')
        : e instanceof Error
          ? e.message
          : String(e);
    messagesError.value = t('imChat.dmSendFailed') + hint;
  } finally {
    sending.value = false;
  }
}
...
async function runSearch(): Promise<void> {
  ...
  try {
    ...
    searchScopeName.value = inLobbyView.value
      ? (inFedLobbyView.value ? t('imChat.fedLobby') : t('imChat.myLobby'))
...
async function submitCreateGroup(): Promise<void> {
  ...
  if (!name) {
    createGroupMsg.value = { kind: 'err', text: t('imChat.needGroupName') };
    return;
  }
  createGroupSubmitting.value = true;
  createGroupMsg.value = { kind: 'info', text: t('imChat.creating') };
  try {
    ...
    createGroupMsg.value = {
      kind: 'err',
      text: t('imChat.createFailed') + (e instanceof Error ? e.message : String(e)),
    };
  } finally {
...
function startGenerateIdentity(): void {
  try {
    ...
    authPhase.value = 'error';
    authError.value = t('imChat.genKeyFailed') + (e instanceof Error ? e.message : String(e));
  }
}
...
function switchIdentity(): void {
  const ok = window.confirm(
    t('imChat.clearIdentityConfirm', { key: shortPubkey.value || pubkey.value }),
  );
  if (!ok) return;
  teardownChat();
  clearIdentity();
...
const authPhaseText = computed(() => {
  if (authPhase.value === 'ok') return t('imChat.authOk');
  if (authPhase.value === 'authing' || imAuthenticating.value) return t('imChat.authing');
  return t('imChat.authNone');
});
...
function replyPreview(m: ImMessageExt): string {
  ...
  if (!target) return t('imChat.replyOriginal');
...
async function fetchImFileEnvelope(fileId: string, imToken: string): Promise<ImFileDownloadEnvelope> {
  ...
  try {
    ...
    if (resp.status === 401) {
      throw new ApiError(t('imChat.tokenExpired'), { status: 401, path: resp.url });
    }
    if (!resp.ok) {
      let detail = '';
      try {
    ...
    if (e instanceof DOMException && e.name === 'AbortError') {
      throw new Error(t('imChat.downloadTimeout'));
    }
    throw e;
  } finally {
    clearTimeout(timer);
...
async function openAttachment(a: ImMessageAttachment | null | undefined): Promise<void> {
  ...
  try {
    ...
    attachError.value = t('imChat.downloadFailed') + (e instanceof Error ? e.message : String(e));
  } finally {
  ...
  <div class="chat-root">
    ...
    <aside class="chat-sidebar">
      ...
      <div v-if="!hasIdentity" class="identity-card">
        <div class="id-head">
          ...
          <div class="id-main">
            <span class="id-name">{{ t('imChat.idTitle') }}</span>
            <span class="id-sub">{{ t('imChat.idSub') }}</span>
          </div>
        </div>
        <div class="id-actions">
          ...
          <button class="btn btn-small" type="button" @click="openImportKey">{{ t('imChat.importKey') }}</button>
        </div>
      </div>
      <div v-else class="identity-card ok" :title="pubkey">
        <span class="id-avatar" :title="t('imChat.avatarTitle')">
        ...
        <button
          class="id-switch"
          type="button"
          :title="t('imChat.clearIdentity')"
          @click="switchIdentity"
        >
          {{ t('imChat.switchIdentity') }}
        </button>
      </div>
      <!-- 认证失败：错误 + 重试 -->
      <div v-if="authPhase === 'error'" class="auth-err">
        <span class="auth-err-text">{{ t('imChat.authFailed') }}{{ authError }}</span>
        <button class="btn btn-small btn-primary" type="button" @click="bootstrapAuth">{{ t('imChat.retry') }}</button>
      </div>
      ...
      <div class="group-list lobby-list">
        ...
        >
          ...
          <span class="g-main">
            <span class="g-name">{{ t('imChat.myLobby') }}</span>
            <span class="g-sub">{{ t('imChat.myLobbySub') }}</span>
          </span>
          <span
            v-if="myLobbyUnread > 0"
            class="unread-badge"
            :title="t('imChat.unread')"
          >{{ myLobbyUnread }}</span>
          <span
            v-else
            class="online-badge"
            :title="t('imChat.onlineCount')"
        ...
        <button
          ...
          :title="t('imChat.fedLobbyTitle')"
          @click="selectFedLobby"
        >
          <span class="g-avatar fed-avatar">🌐</span>
          <span class="g-main">
            <span class="g-name">{{ t('imChat.fedLobby') }}</span>
            <span class="g-sub">{{ t('imChat.fedLobbySub') }}</span>
          </span>
          <span v-if="fedLobbyUnread > 0" class="unread-badge" :title="t('imChat.unread')">{{
            fedLobbyUnread
          }}</span>
        </button>
      </div>
      ...
      <div class="sidebar-head">
        <h3>{{ t('imChat.conversations') }}</h3>
      </div>

      <div v-if="groupsError" class="error-box">{{ t('imChat.groupsLoadFailed', { err: groupsError }) }}</div>

      <div class="group-list">
        <p v-if="groupsLoading && !groups.length" class="muted hint-pad">{{ t('imChat.loading') }}</p>
        <p v-else-if="!groups.length && !groupsError" class="muted hint-pad">
          {{ t('imChat.noConversations') }}
        </p>
        <button
        ...
        >
          ...
          <span class="g-main">
            ...
            <span class="g-sub">
              <span v-if="g.id.startsWith('dm-')">{{ t('imChat.dmBadge') }}</span>
              <span v-else-if="g.kind === 'direct'">{{ t('imChat.directBadge') }}</span>
              <span v-else>{{ t('imChat.groupBadge') }}</span>
              <span v-if="!g.id.startsWith('dm-') && g.members && g.members.length"> · {{ t('imChat.membersCount', { n: g.members.length }) }}</span>
            </span>
          </span>
      ...
      <div v-show="!collapsedNodeLobbies" class="group-list">
        ...
        >
          <button
            ...
            :title="t('imChat.remoteLobbyTitle', { name: remoteLabelOf(r.node_id) })"
            @click="selectRemoteLobby(r.node_id)"
          >
            <span class="g-avatar remote-avatar">🏨</span>
            <span class="g-main">
              <span class="g-name">{{ t('imChat.remoteLobbyName', { name: remoteLabelOf(r.node_id) }) }}</span>
              <span class="g-sub">{{ t('imChat.remoteSub') }}</span>
            </span>
            <span
              v-if="remoteStates[r.node_id]?.phase === 'open'"
              class="online-badge"
              :title="t('imChat.remoteOpen')"
            >{{ t('imChat.open') }}</span>
            <span
              v-else-if="remoteStates[r.node_id]?.phase === 'denied'"
              class="remote-badge muted-badge"
              :title="t('imChat.remoteClosed')"
            >{{ t('imChat.notOpen') }}</span>
          </button>
          <button
            type="button"
            class="remote-lobby-remove"
            :title="t('imChat.removeRemote')"
      ...
      <div v-show="!collapsedPeers" class="group-list">
        ...
        <button
          ...
          :title="t('imChat.enterLobbyTitle', { name: shortIdentity(n.id), score: nodeScore(n) })"
          @click="openNodeLobby(n)"
        >
          <span class="g-avatar">🟢</span>
          <span class="g-main">
            <span class="g-name mono">{{ shortIdentity(n.id) }}</span>
            <span class="g-sub">{{ t('imChat.lastSeen', { time: relTime(n.last_seen) }) }}</span>
          </span>
    ...
    <section class="chat-main">
      ...
      <header class="main-head" :class="{ 'fed-off': inLobbyView && fedReceiveEnabled === false }">
        <div class="main-title">
          ...
          <template v-if="inMyLobbyView">
            ...
            <div>
              <div class="t-name">{{ t('imChat.myLobby') }}</div>
              <div class="t-sub">
                {{ t('imChat.myLobbyMeta', { members: lobbyInfo?.member_count ?? 0, online: lobbyInfo?.online_count ?? 0 }) }}
              </div>
            </div>
          </template>
          <!-- 联邦大厅：跨节点共享频道（可写） -->
          <template v-else-if="inFedLobbyView">
            ...
            <div>
              <div class="t-name">{{ t('imChat.fedLobby') }}</div>
              <div class="t-sub">
                {{ t('imChat.fedLobbyMeta', { n: fedLobbyMessages.length }) }}
              </div>
            </div>
          </template>
          ...
          <template v-else-if="inRemoteLobbyView">
            ...
            <div>
              <div class="t-name">{{ t('imChat.remoteLobbyName', { name: remoteLabelOf(activeRemoteNodeId) }) }}</div>
              <div class="t-sub">
                {{ t('imChat.remoteLobbyMeta', { n: activeRemoteState.messages.length }) }}
              </div>
            </div>
          </template>
          <!-- 群组/私聊 -->
          <template v-else-if="currentGroup">
            ...
            <div>
              ...
              <div class="t-sub">
                <span v-if="currentGroup.id.startsWith('dm-')">{{ t('imChat.dmHeader') }}</span>
                <span v-else-if="currentGroup.kind === 'direct'">{{ t('imChat.directBadge') }}</span>
                <span v-else>{{ t('imChat.groupHeader') }}</span>
                <span v-if="!currentGroup.id.startsWith('dm-') && currentGroup.members && currentGroup.members.length">
                  · {{ t('imChat.membersCount', { n: currentGroup.members.length }) }}
                </span>
              </div>
            </div>
          </template>
        ...
        <div v-if="inMyLobbyView" class="lobby-presence">
          <button
            ...
            :title="t('imChat.dmMemberTitle', { name: lobbyMemberTitle(m) })"
            @click="openDm(m.user_id, m.display_name || '')"
          >
            <img :src="lobbyMemberIdenticon(m)" alt="" />
          </button>
          <span v-if="!lobbyOnlineMembers.length" class="presence-empty">{{ t('imChat.noOnlineMembers') }}</span>
        </div>
        ...
        <div class="head-tools">
          ...
          <div v-if="!inRemoteLobbyView" class="head-search">
            <input
              ...
              class="search-input"
              type="text"
              :placeholder="inGroupView ? t('imChat.searchGroupPh') : t('imChat.searchLobbyPh')"
          ...
          <button
            ...
            type="button"
            :title="t('imChat.settingsTitle')"
            @click="openImSettings"
          >⚙️ {{ t('imChat.settings') }}</button>
        </div>
      </header>
      ...
      <div v-if="inLobbyView && (fedReceiveEnabled === false || fedError)" class="fed-paused-banner">
        <span v-if="fedReceiveEnabled === false"
          >🌐 {{ t('imChat.fedPausedBanner') }}</span
        >
        <span v-if="fedError" class="fed-err">{{ t('imChat.toggleFailed', { err: fedError }) }}</span>
      </div>

      <div v-if="inMyLobbyView && lobbyError" class="error-box thin">{{ t('imChat.lobbyLoadFailed', { err: lobbyError }) }}</div>
      <div v-if="inFedLobbyView && fedLobbyError" class="error-box thin">{{ t('imChat.fedLoadFailed', { err: fedLobbyError }) }}</div>
      <div v-if="inGroupView && messagesError" class="error-box thin">{{ t('imChat.msgLoadFailed', { err: messagesError }) }}</div>

      <!-- 搜索结果面板（替换消息列表；点「清空返回」回正常消息流） -->
      <div v-if="searchMode" ref="messageListEl" class="message-list search-results">
        <div class="sr-head">
          <span class="sr-title">
            {{ searching ? t('imChat.searching') : t('imChat.searchResults', { n: searchResults.length }) }}
            <span v-if="!searching" class="sr-scope">· {{ searchScopeName }} ·「{{ searchQuery }}」</span>
          </span>
          <button class="btn btn-small" type="button" @click="clearSearch">× {{ t('imChat.clearBack') }}</button>
        </div>
        <p v-if="!searching && !searchResults.length" class="muted hint-pad">{{ t('imChat.noResults') }}</p>
        <button
          v-for="m in searchResults"
          :key="m.id"
          type="button"
          class="search-item"
          :title="t('imChat.jumpToMessage')"
      ...
      <div v-else ref="messageListEl" class="message-list">
        ...
        <template v-if="inLobbyView">
          <p v-if="!hasIdentity" class="muted hint-pad">
            {{ t('imChat.noIdentityHint') }}
          </p>
          <p v-else-if="authPhase === 'error'" class="muted hint-pad">
            {{ t('imChat.authFailedHint', { err: authError }) }}
          </p>
              ...
              <div class="msg-bubble" :class="{ agent: isAgentMsg(m) }">
                <div v-if="!isOwnMsg(m)" class="msg-sender" :title="m.sender_name || m.sender_id">
                  {{ senderDisplay(m) }}<span
                    v-if="isAgentMsg(m)"
                    class="agent-badge"
                    :title="t('imChat.agentMsg')"
                    >🤖</span
                  ><span
                    v-if="isFedMsg(m)"
                    class="fed-badge"
                    :title="t('imChat.fedMsgTitle', { node: fedNodeOf(m) })"
                    >🌐 {{ t('imChat.fedFrom', { node: fedNodeOf(m) }) }}</span
                  ><button
                    v-if="dmTargetOf(m)"
                    type="button"
                    class="dm-launch-btn"
                    :title="t('imChat.dmBtnTitle')"
                    @click="openDm(dmTargetOf(m), m.sender_id.startsWith('fed:') ? '' : m.sender_name || '')"
                  >✉️ {{ t('imChat.dm') }}</button
                  ><TipButton
                ...
                >
                  ...
                  <span class="att-dl">{{
                    downloadingFileId === m.attachment.file_id ? t('imChat.downloading') : t('imChat.download')
                  }}</span>
                </button>
        ...
        <template v-else>
          ...
          <p v-else-if="!messages.length" class="muted hint-pad">{{ t('imChat.noMessages') }}</p>
      ...
      <footer class="composer">
        ...
        <div class="composer-quick">
          <span
            ...
            class="attach-hint"
            :title="t('imChat.remoteSendHint')"
          >
            🌐 {{ t('imChat.sendFederated', { name: remoteLabelOf(activeRemoteNodeId) }) }}
          </span>
          <span
            v-if="inFedLobbyView"
            class="attach-hint"
            :title="t('imChat.fedSendHint')"
          >
            🌐 {{ t('imChat.sendBroadcast') }}
          </span>
          <span v-if="uploading" class="attach-hint">📎 {{ t('imChat.uploading') }}</span>
          <span
        ...
        <div class="composer-row">
          ...
          <textarea
            ...
            :placeholder="
              inRemoteLobbyView
                ? t('imChat.phRemote', { name: remoteLabelOf(activeRemoteNodeId) })
                : inFedLobbyView
                  ? t('imChat.phFed')
                  : inMyLobbyView
                    ? t('imChat.phMyLobby')
          ...
          <button
            ...
            @click="sendMessage"
          >
            {{ sending ? t('imChat.sending') : t('imChat.send') }}
          </button>
        </div>
    ...
    <div v-if="showCreateGroup" class="modal-backdrop" @click.self="closeCreateGroup">
      <div class="modal" role="dialog" aria-modal="true" aria-labelledby="cg-title">
        <div class="modal-head">
          <h3 id="cg-title">{{ t('imChat.newGroupTitle') }}</h3>
        ...
        <form class="modal-body" @submit.prevent="submitCreateGroup">
          <div class="field">
            <label for="cg-name">{{ t('imChat.groupNameLabel') }}</label>
            <input
              id="cg-name"
              v-model="newGroupName"
              type="text"
              :placeholder="t('imChat.groupNamePh')"
              :disabled="createGroupSubmitting"
            />
          </div>
          <p v-if="createGroupMsg" :class="['form-msg', `is-${createGroupMsg.kind}`]">{{ createGroupMsg.text }}</p>
          <div class="form-actions">
            <button type="button" class="btn" :disabled="createGroupSubmitting" @click="closeCreateGroup">{{ t('imChat.cancel') }}</button>
            <button type="submit" class="btn btn-primary" :disabled="createGroupSubmitting">{{ t('imChat.submit') }}</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div v-if="showGenKey" class="modal-backdrop">
      <div class="modal" role="dialog" aria-modal="true" aria-labelledby="gk-title">
        <div class="modal-head">
          <h3 id="gk-title">{{ t('imChat.keyGenTitle') }}</h3>
        ...
        <div class="modal-body">
          ...
          <div class="field">
            <label>{{ t('imChat.privkeyLabel') }}</label>
            <div class="key-box mono">{{ generatedPrivkey }}</div>
          </div>
          <div class="field">
            <label>{{ t('imChat.pubkeyLabel') }}</label>
            <div class="key-box mono">{{ generatedPubkey }}</div>
          </div>
          <div class="form-actions">
            <button type="button" class="btn" @click="copyGeneratedPrivkey">
              {{ privkeyCopied ? t('imChat.copiedMark') : t('imChat.copyPrivkey') }}
            </button>
            <button type="button" class="btn" @click="closeGenKey">{{ t('imChat.skip') }}</button>
            <button type="button" class="btn btn-primary" @click="closeGenKey">{{ t('imChat.savedKey') }}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showImportKey" class="modal-backdrop" @click.self="closeImportKey">
      <div class="modal" role="dialog" aria-modal="true" aria-labelledby="ik-title">
        ...
        <form class="modal-body" @submit.prevent="submitImportKey">
          ...
          <div class="form-actions">
            <button type="button" class="btn" @click="closeImportKey">{{ t('imChat.cancel') }}</button>
            <button type="submit" class="btn btn-primary">{{ t('imChat.importAuth') }}</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div v-if="showImSettings" class="modal-backdrop" @click.self="closeImSettings">
      <div class="modal im-settings-modal" role="dialog" aria-modal="true" aria-labelledby="ims-title">
        <div class="modal-head">
          <h3 id="ims-title">{{ t('imChat.settings') }}</h3>
          <button class="modal-close" type="button" @click="closeImSettings">×</button>
        </div>
        <!-- 选项卡条（CodeHub.vue .tabs 同款：文字 Tab + 底部高亮条） -->
        <nav class="im-settings-tabs" role="tablist" :aria-label="t('imChat.settingsTabs')">
          <button
            type="button"
        ...
        <div class="modal-body">
          ...
          <div class="im-setting-row">
            <div class="im-setting-main">
              <div class="im-setting-name">
                🌐 {{ t('imChat.fedReceive') }}
                <span v-if="fedToggleBusy" class="im-setting-busy">{{ t('imChat.toggling') }}</span>
                <span v-else-if="fedReceiveEnabled === null" class="im-setting-busy">{{ t('imChat.loading') }}</span>
              </div>
              <div class="im-setting-desc">
                {{ t('imChat.fedReceiveHint') }}
              </div>
            </div>
            <button
              type="button"
          ...
          <p v-if="fedError" class="form-msg is-err">{{ t('imChat.fedToggleFailed', { err: fedError }) }}</p>

          <!-- ② 允许浏览本机大厅（原系统设置「IM 联邦」卡片迁入） -->
          <div class="im-setting-row">
            <div class="im-setting-main">
              <div class="im-setting-name">
                🏛 {{ t('imChat.lobbyBrowse') }}
                <span v-if="lobbyAccessBusy" class="im-setting-busy">{{ t('imChat.toggling') }}</span>
                <span v-else-if="!lobbyAccess && !lobbyAccessError" class="im-setting-busy">{{ t('imChat.loading') }}</span>
              </div>
              <div class="im-setting-desc">
                {{ t('imChat.lobbyBrowseHint1') }}
                {{ t('imChat.lobbyBrowseHint2') }}
              </div>
            </div>
            <button
              type="button"
              ...
              :title="t('imChat.lobbyBrowseToggle')"
          ...
          <p v-if="lobbyAccessError" class="form-msg is-err">
            {{ t('imChat.lobbyToggleFailed', { err: lobbyAccessError }) }}
          </p>
          ...
          <div class="im-setting-row">
            <div class="im-setting-main">
              <div class="im-setting-name">
                ✉️ {{ t('imChat.dmAllow') }}
                <span v-if="dmAccessBusy" class="im-setting-busy">{{ t('imChat.toggling') }}</span>
                <span v-else-if="!dmAccess && !dmAccessError" class="im-setting-busy">{{ t('imChat.loading') }}</span>
              </div>
              <div class="im-setting-desc">
                {{ t('imChat.dmAllowHint1') }}
                {{ t('imChat.dmAllowHint2') }}
              </div>
            </div>
            <button
              type="button"
          ...
          <p v-if="dmAccessError" class="form-msg is-err">
            {{ t('imChat.dmToggleFailed', { err: dmAccessError }) }}
          </p>
          </div><!-- /Tab1 设置 -->
          ...
          <div class="form-actions">
            <button type="button" class="btn" @click="closeImSettings">{{ t('imChat.close') }}</button>
          </div>
        </div>
      </div>
    </div>
