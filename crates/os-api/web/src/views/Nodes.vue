<!--
  ...
  数据来源：os-api GET /api/v1/nodes/combined（NodeViewRouteHandler 聚合）：
    ...
    - 每节点行元数据富化字段（os-p2p meta 组件——节点存活判定的唯一账本）：
      ...
      心跳新鲜度）/ meta_source（direct=本机直连观测 / gossip=他节点转述）；
      lan/p2p 组内按 score 降序展示（心跳一直正常的排前面）；
    - 每节点行节点自述（联邦节点信息协议，2026-09-14 v0.1.45 批）：对端经
      心跳 gossip 自广播的 agent_version/hostname/platform/gpu——版本徽章 +
      机器名/系统/GPU 摘要行；四字段全缺 = 对端旧版未自述 → 「待升级自述」
      （升级后自动补上，无需手动操作）；
...
const selfName = computed(
  () => selfInfo.value?.name || selfInfo.value?.hostname || t('nodes.unnamed'),
);
...
const ladderLabel = computed(() => {
  ...
  return `Direct ${l.direct} / Punched ${l.punched} / Relayed ${l.relayed}` +
    (l.punch_failed > 0 ? ` / ${t('nodes.punchFailed', { n: l.punch_failed })}` : '');
});
...
function relTime(ts: number): string {
  if (!ts) return t('nodes.rel.never');
  const sec = Math.max(0, Math.floor(Date.now() / 1000 - ts));
  if (sec < 60) return t('nodes.rel.justNow');
  if (sec < 3600) return t('nodes.rel.minAgo', { n: Math.floor(sec / 60) });
  if (sec < 86_400) return t('nodes.rel.hourAgo', { n: Math.floor(sec / 3600) });
  return t('nodes.rel.dayAgo', { n: Math.floor(sec / 86_400) });
}
...
function metaSourceLabel(src: 'direct' | 'gossip' | null): string {
  if (src === 'direct') return t('nodes.src.direct');
  if (src === 'gossip') return t('nodes.src.gossip');
  return '';
}

// —— 节点自述展示（联邦节点信息协议，2026-09-14 v0.1.45 批）——
// 对端经心跳 gossip 自广播的四字段：agent_version/hostname/platform/gpu——
// 版本徽章 + 摘要行；四字段全缺 = 对端旧版未自述 → 「待升级自述」。

/** 自述字段承载面（lan/p2p/inactive 三组行的公共子集）。 */
interface DescLike {
...
/** 是否有任何自述（全缺 = 旧版节点未自述——待升级提示）。 */
function hasDesc(n: DescLike): boolean {
  return Boolean(n.agent_version || n.hostname || n.platform || n.gpu);
}

/** 自述摘要段（hostname · platform · gpu，缺段跳过）。 */
function descParts(n: DescLike): string[] {
  return [n.hostname, n.platform, n.gpu].filter((x): x is string => Boolean(x));
}
...
function imBtnTitle(n: { node_id: string; im_public: boolean | null }): string {
  switch (imBtnState(n)) {
    case 'open':
      return t('nodes.im.titleOpen', { id: shortIdentity(n.node_id) });
    case 'denied':
      return t('nodes.im.titleDenied');
    case 'loading':
      return t('nodes.im.titleLoading');
    default:
      return t('nodes.im.titleUnknown');
  }
}
...
async function heartbeat(n: CombinedInactiveNode): Promise<void> {
  ...
  hbOk.value = null;
  hbMsg.value = t('nodes.hb.probing', { id: shortIdentity(n.node_id) });
  try {
    const r = await endpoints.p2pNodeMetaReactivate(n.node_id);
    if (r.probed) {
      hbOk.value = true;
      hbMsg.value = t('nodes.hb.ok', { id: shortIdentity(n.node_id) });
      await load();
    } else {
      hbOk.value = false;
      hbMsg.value = t('nodes.hb.unreachable', { id: shortIdentity(n.node_id) });
    }
  } catch (e) {
    hbOk.value = false;
    hbMsg.value = t('nodes.hb.fail', { msg: errMsg(e) });
  } finally {
...
async function addPeer(): Promise<void> {
  ...
  manualMsg.value = t('nodes.add.connecting', { addr });
  manualOk.value = null;
  try {
    ...
    manualMsg.value =
      r.note === 'already-connected'
        ? t('nodes.add.already', { addr: r.addr, id: shortIdentity(r.node_id) })
        : t('nodes.add.ok', { id: shortIdentity(r.node_id), addr: r.addr });
    manualAddr.value = '';
    await load();
  } catch (e) {
    manualOk.value = false;
    manualMsg.value = t('nodes.add.fail', { msg: errMsg(e) });
  } finally {
...
<template>
  <div class="nodes-page">
      ...
      <div>
        ...
        <div class="page-sub">
          ...
          <span v-if="lastUpdatedLabel" class="muted small">
            {{ t('nodes.lastUpdated', { time: lastUpdatedLabel }) }}
          </span>
        </div>
      </div>
    ...
    <div v-if="loading && !combined" class="loading">{{ t('nodes.loading') }}</div>
    <div v-else-if="errorMsg && !combined" class="error">
      {{ t('nodes.loadFail', { msg: errorMsg }) }}
    </div>
      ...
      <div class="card self-bar">
        ...
        <div class="self-main">
          <div class="self-name-row">
            ...
            <span class="muted small">{{ t('nodes.self') }}</span>
            <span
          ...
          <div class="muted mono small self-id-row">
            <template v-if="selfInfo?.node_id">
              ...
              <span class="self-listen">{{ t('nodes.listen', { addr: selfInfo.listen || '—' }) }}</span>
            </template>
            <template v-else>
              {{ selfInfo?.hostname || '—' }}{{ t('nodes.enableHint') }}
            </template>
          </div>
        </div>
      </div>
      ...
      <div class="zone-grid">
        ...
        <section class="card zone-card zone-lan">
          <header class="zone-head">
            ...
            <h3 class="zone-title">{{ t('nodes.lanTitle') }}</h3>
            <span class="muted small">
              {{ t('nodes.lanCount', { total: lanNodes.length, online: lanOnline }) }}
            </span>
          </header>
            ...
            <li v-for="n in lanNodes" :key="n.node_id" class="node-row">
              ...
              <div class="node-main">
                ...
                <div class="muted small meta-line">
                  <span class="status-badge" :title="t('nodes.metaStatus', { status: n.status })">
                    <span class="dot dot-on meta-dot"></span>{{ t('nodes.active') }}
                  </span>
                  <span class="score-badge" :class="scoreClass(n.score)">
                    {{ n.score > 0 ? t('nodes.scoreVal', { score: n.score }) : t('nodes.unscored') }}
                  </span>
                  <span :title="`last_seen: ${n.last_seen}`">
                    {{ t('nodes.lastSeen', { t: relTime(n.last_seen) }) }}
                  </span>
                  <span v-if="n.meta_source">{{ metaSourceLabel(n.meta_source) }}</span>
                </div>
                <div
                  class="muted small desc-line"
                  :title="hasDesc(n) ? undefined : t('nodes.descPendingTip')"
                >
                  <span v-if="n.agent_version" class="ver-badge" :title="t('nodes.version')">
                    v{{ n.agent_version }}
                  </span>
                  <span v-if="descParts(n).length" class="mono">{{ descParts(n).join(' · ') }}</span>
                  <span v-if="!hasDesc(n)" class="desc-pending">{{ t('nodes.descPending') }}</span>
                </div>
              </div>
              <span
                :class="['dot', n.connected ? 'dot-on' : 'dot-off']"
                :title="n.connected ? t('nodes.connected') : t('nodes.notConnected')"
              ></span>
              <span class="node-state muted small">
                {{ n.connected ? t('nodes.online') : t('nodes.offline') }}
              </span>
              <span
        ...
        <section class="card zone-card zone-p2p">
          <header class="zone-head">
            ...
            <h3 class="zone-title">{{ t('nodes.p2pTitle') }}</h3>
            <span class="muted small">
              {{ t('nodes.p2pCount', { total: p2pNodes.length, online: p2pOnline }) }}
            </span>
          </header>
            ...
            <li v-for="n in p2pNodes" :key="`${n.source}:${n.node_id}`" class="node-row">
              ...
              <div class="node-main">
                ...
                <div class="muted mono small">
                  {{ n.addr || t('nodes.natAddr') }}
                  <template v-if="n.route_via">
                    {{ t('nodes.routeVia', { id: shortIdentity(n.route_via) }) }}
                  </template>
                </div>
                <div class="muted small meta-line">
                  <span class="status-badge" :title="t('nodes.metaStatus', { status: n.status })">
              ...
              <span
                :class="['dot', n.connected ? 'dot-on' : 'dot-off']"
                :title="n.connected ? t('nodes.connected') : t('nodes.notDirect')"
              ></span>
              <span class="node-state muted small">
                {{ n.connected ? t('nodes.connected') : t('nodes.notDirect') }}
              </span>
              <span
                v-if="n.public"
                class="badge badge-info"
                :title="t('nodes.publicTip')"
              >
                public
              </span>
              <span
          ...
          <footer v-if="ladderLabel" class="zone-foot muted small">
            {{ t('nodes.ladder', { label: ladderLabel }) }}
          </footer>
        </section>
        ...
        <section v-if="inactiveNodes.length || hbMsg" class="card zone-card zone-inactive">
          <header class="zone-head">
            ...
            <h3 class="zone-title">{{ t('nodes.inactiveTitle') }}</h3>
            <span class="muted small">{{ t('nodes.inactiveCount', { n: inactiveNodes.length }) }}</span>
          </header>
          ...
          <ul v-if="inactiveNodes.length" class="node-list">
            ...
            >
              ...
              <div class="node-main">
                ...
                <div class="muted mono small">
                  {{ n.addrs[0] || t('nodes.noAddr') }}
                  <template v-if="n.addrs.length > 1">
                    {{ t('nodes.addrHistory', { n: n.addrs.length }) }}
                  </template>
                </div>
                <div class="muted small meta-line">
                  <span class="status-badge status-inactive" :title="t('nodes.inactiveStatusTip')">
                    <span class="dot dot-off meta-dot"></span>{{ t('nodes.inactive') }}
                  </span>
                  <span :title="`last_seen: ${n.last_seen}`">
                    {{ t('nodes.lastSeen', { t: relTime(n.last_seen) }) }}
                  </span>
                  <span :title="`since: ${n.since}`">
                    {{ t('nodes.downSince', { t: relTime(n.since) }) }}
                  </span>
              ...
              <button
                ...
                :title="t('nodes.hb.tip')"
                @click="heartbeat(n)"
              >
                {{ heartbeatBusy[n.node_id] ? t('nodes.hb.busy') : t('nodes.hb.run') }}
              </button>
            </li>
          </ul>
          <div v-else class="zone-empty muted center">
            {{ t('nodes.inactiveEmpty') }}
          </div>
    ...
    <section class="card add-card">
      <header class="zone-head">
        ...
        <h3 class="zone-title">{{ t('nodes.addTitle') }}</h3>
        <span class="muted small">{{ t('nodes.addSub') }}</span>
      </header>
      <div class="add-row">
        <input
          v-model="manualAddr"
        ...
        <button
          ...
          @click="addPeer"
        >
          {{ manualBusy ? t('nodes.add.busy') : t('nodes.add.connect') }}
        </button>
      </div>
      <div
        v-if="manualMsg"
...
/* —— 节点自述行（联邦节点信息协议：版本徽章 + hostname·platform·GPU 摘要；
      四字段全缺 = 对端旧版未自述 → 「待升级自述」虚线徽标）—— */
.desc-line {
  display: flex;
  align-items: center;
  gap: 6px;
...
.ver-badge {
  ...
  color: var(--info);
  border: 1px solid rgba(51, 82, 128, 0.4);
  background: rgba(51, 82, 128, 0.08);
}
.desc-pending {
  display: inline-block;
  padding: 0 6px;
