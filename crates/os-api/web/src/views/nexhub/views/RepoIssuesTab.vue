    ...
    <div v-else class="collab-list">
      ...
      >
        <div class="collab-row-line" :title="t('nexhub.collab.toggleDetail')" @click="toggleIssue(i)">
          ...
          <!-- 交叉引用徽章：引用本 issue 的 PR（fixes #n；merge 时自动关闭） -->
          <span v-if="i.referencing_pulls?.length" class="collab-labels">
            <span
              v-for="n in i.referencing_pulls"
              :key="n"
              class="meta-chip ref-chip"
              :title="t('nexhub.issues.refBadgeTitle')"
            >⇄ !{{ n }}</span>
          </span>
