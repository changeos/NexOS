...
/** 待确认合并的 PR（合并对话框数据：策略选择 + 可选信息，2026-09-24 §top4）。 */
const mergePending = ref<RepoPull | null>(null);
/** 合并策略（merge=双 parent 合并提交 / squash=压单提交 / rebase=线性变基）。 */
const mergeStrategy = ref<'merge' | 'squash' | 'rebase'>('merge');
/** 自定义合并提交信息（merge/squash 生效；空=缺省——merge=`Merge PR #n: 标题`、
 * squash=`标题 (#n)`）。 */
const mergeMessage = ref('');

/** 打开合并对话框（策略回缺省 merge）。 */
function openMergeDialog(p: RepoPull): void {
  mergeStrategy.value = 'merge';
  mergeMessage.value = '';
  mergePending.value = p;
}
...
/** 确认弹窗点「合并」：真实执行 merge（三策略，仅 admin/仓库 owner 入口可见）。
 * body/评论里 fixes #n 引用的 open issue 由服务端联动关闭（closed_issues 回显）。 */
async function doMerge(): Promise<void> {
  ...
  try {
    ...
    const r = await endpoints.codeRepoPullMerge(
      repo,
      p.number,
      {
    ...
    const closed = (r.closed_issues ?? []).length;
    ctx.showMsg(
      'ok',
      t('nexhub.pulls.merged', { n: p.number, sha: shortHash(r.merged_sha) }) +
        (closed > 0 ? ` · ${t('nexhub.pulls.closedIssues', { n: closed })}` : ''),
    );
...
<template>
  <section class="pulls-tab">
    ...
    <div v-else class="collab-list">
      ...
      >
        <div class="collab-row-line" :title="t('nexhub.collab.toggleDetail')" @click="togglePull(p)">
          ...
          <!-- 交叉引用徽章：body/评论 fixes #n 引用的 issue（merge 时自动关闭） -->
          <span v-if="p.referenced_issues?.length" class="collab-labels">
            <span
              v-for="n in p.referenced_issues"
              :key="n"
              class="meta-chip ref-chip"
              :title="t('nexhub.pulls.refBadgeTitle')"
            >🔗 #{{ n }}</span>
          </span>
    ...
    <!-- 合并 PR 对话框（三策略选择 + 可选信息；替代原生 confirm，2026-09-24 §top4） -->
    <div v-if="mergePending" class="modal-overlay" @click.self="mergePending = null">
      <div class="card modal-card">
        <div class="modal-head">
          <h3 class="modal-title">{{ t('nexhub.pulls.mergeConfirmTitle', { n: mergePending.number }) }}</h3>
          <button class="btn btn-small btn-ghost" type="button" @click="mergePending = null">✕</button>
        </div>
        <p class="muted small merge-body-hint">
          {{ t('nexhub.pulls.mergeConfirmBody', { from: mergePending.from_branch, to: mergePending.to_branch }) }}
        </p>
        <div class="form-row">
          <label class="form-label">{{ t('nexhub.pulls.strategyLabel') }}</label>
          <div class="strategy-options">
            <label
              v-for="s in (['merge', 'squash', 'rebase'] as const)"
              :key="s"
              class="strategy-option"
              :class="{ active: mergeStrategy === s }"
            >
              <input v-model="mergeStrategy" type="radio" :value="s" name="merge-strategy" />
              <span class="strategy-name">{{ t(`nexhub.pulls.strategy_${s}`) }}</span>
              <span class="muted small">{{ t(`nexhub.pulls.strategyHint_${s}`) }}</span>
            </label>
          </div>
        </div>
        <div v-if="mergeStrategy !== 'rebase'" class="form-row">
          <label class="form-label" for="merge-msg">{{ t('nexhub.pulls.mergeMsgLabel') }}</label>
          <textarea
            id="merge-msg"
            v-model="mergeMessage"
            class="search-input collab-input"
            rows="2"
            :placeholder="mergeStrategy === 'squash'
              ? t('nexhub.pulls.mergeMsgPlaceholderSquash')
              : t('nexhub.pulls.mergeMsgPlaceholderMerge')"
          ></textarea>
        </div>
        <div class="modal-actions">
          <button class="btn btn-small" type="button" @click="mergePending = null">{{ t('common.cancel') }}</button>
          <button
            class="btn btn-small btn-primary"
            type="button"
            :disabled="ctx.actionLoading.value"
            @click="doMerge"
          >{{ t('nexhub.pulls.mergeAction') }}</button>
        </div>
      </div>
    </div>
  </section>
...
.ref-chip {
  background: rgba(63, 127, 191, 0.12); color: #3573b9; cursor: default;
}
.merge-body-hint { margin: 0; line-height: 1.6; }
