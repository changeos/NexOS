...
// octet-stream 直链）。上传走 b64-JSON 信封（FileReader→base64，≤100MB，
// 同 IM 附件 / downloads 种子先例——网关契约无 multipart）。
// =============================================================================
import { ref, watch } from 'vue';
import { useI18n } from 'vue-i18n';
import { endpoints, fileToBase64 } from '@/api/client';
...
/** 上传附件（owner/admin）：FileReader→base64 → POST b64 信封。 */
async function uploadAsset(): Promise<void> {
  ...
  try {
    ...
    const opts = await idctx.requireNexhubOpts();
    const r = await endpoints.codeRepoReleaseAssetUpload(
      repo,
      tag,
