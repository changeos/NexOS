...
const groupTabs: Record<Exclude<TabGroup, 'repo'>, { key: TabKey; label: string }[]> = {
  inference: [
    { key: 'instances', label: t('llmTab.instances') },
    { key: 'envs', label: t('llmEnv.tab') },
    { key: 'recipes', label: t('llmTab.recipes') },
    { key: 'generate', label: t('llmTab.generate') },
    { key: 'modelchat', label: t('llmChat.tab') },
    { key: 'external', label: t('llmExt.tab') },
  ],
  diag: [
    { key: 'metrics', label: t('llmTab.metrics') },
    { key: 'gpu', label: t('llmTab.gpu') },
    { key: 'params', label: t('llmTab.params') },
  ],
};
...
function friendlyError(e: unknown): string {
  ...
  if (/502/.test(m)) return t('llmInst.err502');
  return m;
}
...
const instColumns: Column<ModelInstance>[] = [
  { key: 'name', title: t('llmInst.colName'), sortable: true },
  { key: 'model', title: t('llmInst.colModel') },
  { key: 'source_type', title: t('llmInst.colSource'), width: '110px' },
  { key: 'port', title: t('llmInst.colPort'), width: '80px' },
  { key: 'status', title: t('llmInst.colStatus'), width: '100px' },
  { key: 'pid', title: 'PID', width: '80px' },
  { key: 'actions', title: t('llmInst.colActions'), width: '400px', align: 'right' },
];
...
async function startInst(id: string): Promise<void> {
  ...
  try {
    ...
    await loadInstances();
    msg.value = { kind: 'ok', text: t('llmInst.startSent') };
  } catch (e) {
    msg.value = { kind: 'err', text: t('llmInst.startFailed') + friendlyError(e) };
  } finally {
    busyId.value = '';
  }
}
async function stopInst(id: string): Promise<void> {
  if (!window.confirm(t('llmInst.stopConfirm'))) return;
  busyId.value = id;
  msg.value = null;
  try {
    await endpoints.stopLlmInstance(id);
    await loadInstances();
    msg.value = { kind: 'ok', text: t('llmInst.stopped') };
  } catch (e) {
    msg.value = { kind: 'err', text: t('llmInst.stopFailed') + friendlyError(e) };
  } finally {
    busyId.value = '';
  }
}
async function healthInst(id: string): Promise<void> {
  ...
  try {
    ...
    await loadInstances();
    if (h.alive) {
      msg.value = { kind: 'ok', text: t('llmInst.healthAlive', { n: h.models?.length ?? 0 }) };
    } else {
      msg.value = { kind: 'err', text: t('llmInst.healthDead') };
    }
  } catch (e) {
    msg.value = { kind: 'err', text: t('llmInst.healthFailed') + friendlyError(e) };
  } finally {
    busyId.value = '';
  }
}
async function removeInst(id: string): Promise<void> {
  if (!window.confirm(t('llmInst.deleteConfirm'))) return;
  busyId.value = id;
  msg.value = null;
  try {
    await endpoints.deleteLlmInstance(id);
    await loadInstances();
    msg.value = { kind: 'ok', text: t('llmInst.deleted') };
  } catch (e) {
    msg.value = { kind: 'err', text: t('llmInst.deleteFailed') + friendlyError(e) };
  } finally {
    busyId.value = '';
  }
}
...
async function submitCreate(): Promise<void> {
  if (!form.value.name.trim()) {
    msg.value = { kind: 'err', text: t('llmInst.needName') };
    return;
  }
  if (!form.value.model.trim()) {
    msg.value = { kind: 'err', text: t('llmInst.needModel') };
    return;
  }
  ...
  try {
    ...
    msg.value = form.value.autostart
      ? {
          kind: 'ok',
          text: t('llmInst.createdAuto', { port: finalPort }),
        }
      : { kind: 'ok', text: t('llmInst.createdStopped') };
    showCreate.value = false;
  } catch (e) {
    msg.value = { kind: 'err', text: t('llmInst.createFailed') + friendlyError(e) };
  } finally {
...
async function copyRecipeCommand(): Promise<void> {
  ...
  if (!cmd) {
    msg.value = { kind: 'err', text: t('llmRecipes.noCommand') };
    return;
  }
  if (await copyText(cmd)) {
    ...
    msg.value = { kind: 'err', text: t('llmRecipes.copyFailed') };
  }
}

async function copySavedCommand(cmd: string): Promise<void> {
  msg.value = (await copyText(cmd))
    ? { kind: 'ok', text: t('llmRecipes.copied') }
    : { kind: 'err', text: t('llmRecipes.copyFailed') };
}
...
function saveRecipeToLocal(): void {
  ...
  if (savedRecipes.value.some((s) => s.hf_id === hfId)) {
    msg.value = { kind: 'info', text: t('llmRecipes.alreadySaved') };
    return;
  }
  ...
  msg.value = {
    kind: 'ok',
    text: t('llmRecipes.savedLocal', { id: hfId }),
  };
}
...
function difficultyLabel(d?: string): string {
  switch (d) {
    case 'beginner':
      return t('llmRecipes.diffBeginner');
    case 'intermediate':
      return t('llmRecipes.diffIntermediate');
    case 'advanced':
      return t('llmRecipes.diffAdvanced');
    default:
      return d ?? '—';
  }
}
...
async function generateImage(): Promise<void> {
  ...
  if (!prompt) {
    imgError.value = t('llmGen.needPrompt');
    return;
  }
...
const recentColumns: Column<ImageRecentItem>[] = [
  { key: 'created_at', title: t('llmGen.colTime'), width: '170px' },
  { key: 'prompt_summary', title: 'Prompt' },
  { key: 'width', title: t('llmGen.colSize'), width: '100px' },
  { key: 'steps', title: t('llmGen.colSteps'), width: '70px' },
  { key: 'elapsed_ms', title: t('llmGen.colElapsed'), width: '90px', align: 'right' },
];
...
async function createVideoTask(): Promise<void> {
  ...
  if (!prompt) {
    videoError.value = t('llmGen.needPrompt');
    return;
  }
...
function videoStatusLabel(s: string): string {
  switch (s) {
    case 'completed':
      return t('llmGen.stDone');
    case 'processing':
      return t('llmGen.stRunning');
    case 'queued':
      return t('llmGen.stQueued');
    case 'failed':
      return t('llmGen.stFailed');
    default:
      return s;
  }
}
...
function statusLabel(s?: string): string {
  switch (s) {
    case 'running':
      return t('llmInst.stRunning');
    case 'starting':
      return t('llmInst.stStarting');
    case 'error':
      return t('llmInst.stError');
    case 'stopped':
      return t('llmInst.stStopped');
    default:
      return s ?? '—';
  }
}
...
function gpuMemBadge(d: GpuDevice): string {
  if (d.unified_memory) {
    ...
    return gb ? t('llmInst.unifiedMem', { gb }) : t('llmInst.unifiedMemUnknown');
  }
...
<template>
  <div class="llm-page">
    <div class="page-head">
      <div>
        <h2 class="page-title">{{ t('llmInst.title') }}</h2>
        <div class="page-sub muted">{{ t('llmInst.subtitle') }}</div>
      </div>
    ...
    <section v-show="activeTab === 'instances'" class="tab-panel">
      ...
      <div class="card gpu-summary-card">
        <div class="gpu-summary-head">
          ...
          <span v-else class="pill pill-muted">{{ t('llmInst.noGpu') }}</span>
        </div>
        <div v-if="gpu.available && (gpu.devices?.length ?? 0) > 0" class="gpu-mini-list">
          <div v-for="d in gpu.devices" :key="d.index" class="gpu-mini-item">
            ...
            <span v-if="d.unified_memory" class="pill pill-purple">{{ t('llmInst.unifiedMemPill') }}</span>
      ...
      <section class="stat-grid">
        <div class="card stat-card">
          <div class="stat-label">{{ t('llmInst.statTotal') }}</div>
          <div class="stat-value">{{ stats.instances_total ?? 0 }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('llmInst.statRunning') }}</div>
          <div class="stat-value">{{ stats.running ?? 0 }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('llmInst.statStopped') }}</div>
          <div class="stat-value">{{ stats.stopped ?? 0 }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('llmInst.statGpu') }}</div>
          <div class="stat-value">{{ stats.gpu_devices ?? 0 }}</div>
        </div>
      </section>

      <div class="panel-head">
        <span class="panel-title">{{ t('llmInst.listTitle') }}</span>
        <button class="btn btn-small btn-primary" @click="openCreate">＋ {{ t('llmInst.createBtn') }}</button>
      </div>
      ...
      <div v-if="showCreate" class="modal-backdrop" @click.self="closeCreate">
        <div class="modal" role="dialog" aria-modal="true" aria-labelledby="llm-create-title">
          <div class="modal-head">
            <h3 id="llm-create-title">{{ t('llmInst.createTitle') }}</h3>
          ...
          <form class="modal-body" @submit.prevent="submitCreate">
            ...
            <div class="preset-box">
              <div class="preset-row">
                <span class="preset-label">{{ t('llmInst.presetTitle') }}</span>
                <div class="preset-btns">
                  ...
                  >{{ t('llmInst.presetClear') }}</button>
                </div>
              </div>
              <p class="preset-hint muted small">
                {{ t('llmInst.presetHint') }}
              </p>
            </div>
            <div class="field">
              <label for="llm-name">{{ t('llmInst.nameLabel') }}</label>
              <input id="llm-name" v-model="form.name" type="text" :placeholder="t('llmInst.namePh')" :disabled="submitting" />
            </div>
            <div class="field">
              <label for="llm-model">{{ t('llmInst.modelLabel') }}</label>
              <input
                id="llm-model"
                v-model="form.model"
                type="text"
                list="llm-model-path-options"
                :placeholder="t('llmInst.modelPh')"
                :disabled="submitting"
              />
            ...
            <div class="field-row">
              <div class="field">
                <label for="llm-src">{{ t('llmInst.srcLabel') }}</label>
                <select id="llm-src" v-model="form.source_type" :disabled="submitting">
                  <option value="huggingface">{{ t('llmInst.srcHf') }}</option>
                  <option value="local">{{ t('llmInst.srcLocal') }}</option>
                </select>
              </div>
              <div class="field">
            ...
            <div class="field-row">
              ...
              <div class="field">
                <label>{{ t('llmInst.trustLabel') }}</label>
                <label class="switch">
            ...
            <div class="field-row">
              <div class="field">
                ...
                <input id="llm-smn" v-model="form.served_model_name" type="text" :placeholder="t('llmInst.smnPh')" :disabled="submitting" />
              </div>
              ...
              <div class="field">
                ...
                <select id="llm-env" v-model="form.env_name" :disabled="submitting">
                  ...
                  <option v-for="e in envs" :key="e.name" :value="e.name">
                    {{ e.name }}（{{ envStatusLabel(e.status) }}{{ e.is_default ? ' · ' + t('llmInst.defaultSuffix') : '' }}）
                  </option>
                </select>
              </div>
            </div>
            ...
            <div class="field">
              ...
              >
                {{ showExtraArgs ? t('llmInst.extraCollapse') : t('llmInst.extraExpand') }} extra_args{{ extraArgsList.length ? t('llmInst.extraCount', { n: extraArgsList.length }) : '' }} {{ showExtraArgs ? '▴' : '▾' }}
              </button>
              <textarea
                v-if="showExtraArgs"
                id="llm-extra"
                ...
                :disabled="submitting"
              />
              <span v-if="showExtraArgs" class="muted small">{{ t('llmInst.extraHint') }}</span>
            </div>
            <p class="muted small">{{ t('llmLog.portFooterHint') }}</p>
            <p v-if="msg" :class="['form-msg', `is-${msg.kind}`]">{{ msg.text }}</p>
            <div class="form-actions">
              <button type="button" class="btn" :disabled="submitting" @click="closeCreate">{{ t('llmInst.cancel') }}</button>
              <button type="submit" class="btn btn-primary" :disabled="submitting">
                {{ submitting ? t('llmInst.creating') : t('llmInst.createBtnShort') }}
              </button>
            </div>
          </form>
        </div>
    ...
    <section v-show="activeTab === 'recipes'" class="tab-panel">
      ...
      <div class="card rc-intro">
        <div class="rc-intro-text">
          <span class="panel-title">{{ t('llmRecipes.officialTitle') }}</span>
          <span class="muted small">
        ...
        <div class="head-actions">
          <a class="btn btn-small" :href="RECIPES_SITE" target="_blank" rel="noopener">{{ t('llmRecipes.viewSite') }}</a>
      ...
      <div v-if="recipesError" class="error-box">{{ t('llmRecipes.fetchFailed', { err: recipesError }) }}</div>

      <!-- 官方同款树状目录：左侧可折叠提供方树 + 右侧选中速览 -->
      <div class="panel-head">
        <span class="panel-title">{{ t('llmRecipes.catalogTitle', { shown: filteredRecipes.length, total: recipes.length }) }}</span>
        <input
          v-model="recipeSearch"
          class="rc-search"
          type="search"
          :placeholder="t('llmRecipes.searchPh')"
        />
      </div>
      ...
      <div class="rc-tree-layout panel">
        ...
        <div class="card rc-tree">
          <div v-if="recipesLoading" class="rc-tree-loading muted">{{ t('llmRecipes.loading') }}</div>
          <template v-else-if="recipeTree.length">
            <div v-for="g in recipeTree" :key="g.provider" class="rc-tree-group">
              ...
              >
                ...
                <span class="rc-tree-provider">{{ g.provider || t('llmRecipes.noProvider') }}</span>
          ...
          <template v-if="selectedRecipe">
            ...
            <div class="rc-quick-actions">
              ...
              <a
                ...
                target="_blank"
                rel="noopener"
              >{{ t('llmRecipes.viewSite') }}</a>
            </div>
            <div class="muted small rc-quick-hint">
              {{ t('llmRecipes.quickHint') }}
            </div>
          </template>
          <div v-else class="rc-quick-empty">
            <div class="rc-quick-empty-title">{{ t('llmRecipes.pickModel') }}</div>
            <div class="muted small">
              {{ t('llmRecipes.treeHint1') }}
              <a :href="RECIPES_SITE" target="_blank" rel="noopener">recipes.vllm.ai</a>
              {{ t('llmRecipes.treeHint2') }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="panel-head">
        <span class="panel-title">{{ t('llmRecipes.savedTitle', { n: savedRecipes.length }) }}</span>
        <span class="muted small">{{ t('llmRecipes.savedSub') }}</span>
      </div>
      <div v-if="savedRecipes.length === 0" class="card empty-card">
        {{ t('llmRecipes.savedEmpty') }}
      </div>
      <ul v-else class="rc-saved-list">
        <li v-for="s in savedRecipes" :key="s.hf_id" class="rc-saved-item">
          ...
          <div class="rc-saved-actions">
            <button class="btn btn-small" @click="copySavedCommand(s.command)">{{ t('llmRecipes.copyCmd') }}</button>
            <a class="btn btn-small" :href="recipeSiteUrl(s.hf_id)" target="_blank" rel="noopener">{{ t('llmRecipes.viewSite') }}</a>
            <button class="btn btn-small btn-danger" @click="removeSavedRecipe(s.hf_id)">{{ t('llmRecipes.remove') }}</button>
          </div>
        </li>
      </ul>
      ...
      <div v-if="showRecipeDetail" class="modal-backdrop" @click.self="closeRecipe">
        <div class="modal rc-modal" role="dialog" aria-modal="true" aria-labelledby="rc-detail-title">
          ...
          <div class="modal-body rc-detail-body">
            <div v-if="recipeDetailLoading" class="empty-card">{{ t('llmRecipes.detailLoading') }}</div>
            ...
            <template v-else-if="recipeDetail">
              ...
              <div class="rc-meta-row">
                ...
                <span v-if="recipeDetail.meta?.date_updated" class="muted small">
                  {{ t('llmRecipes.updated', { date: recipeDetail.meta.date_updated }) }}
                </span>
              </div>
              ...
              <div v-if="recipeDetail.recommended_command" class="rc-section">
                <div class="rc-section-head">
                  <span class="panel-title">{{ t('llmRecipes.recommendTitle') }}</span>
                  <div class="head-actions">
                    ...
                    <button class="btn btn-small" @click="copyRecipeCommand">
                      {{ recipeCopied ? t('llmRecipes.copiedMark') : t('llmRecipes.copyCmdBtn') }}
                    </button>
                  </div>
                </div>
                ...
                <template v-if="recipeDetail.recommended_command.docker_command">
                  <span class="muted small">
                    Docker（{{ recipeDetail.recommended_command.docker_image || t('llmRecipes.officialImage') }}）
                  </span>
              ...
              <div v-if="recipeVariants.length" class="rc-section">
                <span class="panel-title">{{ t('llmRecipes.variantsTitle') }}</span>
                <table class="rc-variants">
                  <thead>
                    <tr><th>{{ t('llmRecipes.thVariant') }}</th><th>{{ t('llmRecipes.thPrecision') }}</th><th>{{ t('llmRecipes.thVram') }}</th><th>{{ t('llmRecipes.thNote') }}</th></tr>
                  </thead>
                  <tbody>
              ...
              <details v-if="recipeGuideHtml" class="rc-guide">
                <summary>{{ t('llmRecipes.guideSummary') }}</summary>
              ...
              <div class="form-actions rc-footer">
                <a class="btn" :href="recipeSiteUrl(recipeDetailHfId)" target="_blank" rel="noopener">
                  {{ t('llmRecipes.viewSite') }}
                </a>
                <button class="btn btn-primary" @click="saveRecipeToLocal">{{ t('llmRecipes.saveLocal') }}</button>
              </div>
            </template>
          </div>
        </div>
    ...
    <section v-show="activeTab === 'generate'" class="tab-panel">
      ...
      <div class="card gen-card">
        <div class="panel-head gen-card-head">
          <span class="panel-title">{{ t('llmGen.imgTitle') }}</span>
          <span class="muted small">{{ t('llmGen.imgSub') }}</span>
        </div>
        <div class="gen-form">
          <div class="field">
            ...
            <textarea
              ...
              rows="3"
              :placeholder="t('llmGen.imgPromptPh')"
              :disabled="imgGenerating"
            />
          </div>
          <div class="field-row">
            <div class="field">
              <label for="gen-width">{{ t('llmGen.width') }}</label>
              <input
                id="gen-width"
                v-model="imgForm.width"
                type="number" min="256" max="1024" step="64"
                :placeholder="t('llmGen.widthPh')"
                :disabled="imgGenerating"
              />
            </div>
            <div class="field">
              <label for="gen-height">{{ t('llmGen.height') }}</label>
              <input
                id="gen-height"
                v-model="imgForm.height"
                type="number" min="256" max="1024" step="64"
                :placeholder="t('llmGen.heightPh')"
                :disabled="imgGenerating"
              />
            </div>
            <div class="field">
              <label for="gen-steps">{{ t('llmGen.steps') }}</label>
              <input
                id="gen-steps"
                v-model="imgForm.steps"
                type="number" min="1" max="8"
                :placeholder="t('llmGen.stepsPh')"
                :disabled="imgGenerating"
              />
            </div>
            <div class="field gen-submit-field">
              <label class="gen-hidden-label">{{ t('llmGen.actions') }}</label>
              <button
              ...
              >
                <span
                  ...
                  aria-hidden="true"
                >↻</span>
                {{ imgGenerating ? t('llmGen.generating') : t('llmGen.generate') }}
              </button>
            </div>
          </div>
          <p class="muted small">
        ...
        <div v-if="imgResult" class="gen-result">
          <img
            ...
            :alt="t('llmGen.resultAlt')"
          />
          <div class="gen-result-meta">
            <span class="muted small">
              {{ t('llmGen.resultMeta', { w: imgResult.width, h: imgResult.height, ms: fmtMs(imgResult.elapsed_ms) }) }}
            </span>
            <a
              class="btn btn-small gen-download-btn"
              :href="imgResultUrl"
              :download="`nexos-${imgResult.id}.png`"
            >{{ t('llmGen.downloadPng') }}</a>
          </div>
        </div>
      </div>
      ...
      <!-- —— 最近生成（环形 50 条，无图，仅信息）—— -->
      <div class="panel-head">
        <span class="panel-title">{{ t('llmGen.recentTitle') }}</span>
      ...
      <div class="panel">
        <div class="card card-table">
          <DataTable
            ...
            :empty-text="t('llmGen.recentEmpty')"
          >
      ...
      <div class="card gen-card">
        <div class="panel-head gen-card-head">
          <span class="panel-title">{{ t('llmGen.videoTitle') }}</span>
        ...
        <div class="gen-form">
          <div class="field">
            ...
            <textarea
              ...
              class="gen-prompt-input"
              rows="2"
              :placeholder="t('llmGen.videoPromptPh')"
              :disabled="videoCreating"
            />
          </div>
          <div class="field-row">
            <div class="field">
              <label for="vid-duration">{{ t('llmGen.durationLabel') }}</label>
              <input
                id="vid-duration"
                v-model="videoForm.duration_secs"
                type="number" min="1" max="30"
                :placeholder="t('llmGen.durationPh')"
                :disabled="videoCreating"
              />
            </div>
            <div class="field">
              <label for="vid-backend">{{ t('llmGen.backendLabel') }}</label>
              <select id="vid-backend" v-model="videoForm.backend" :disabled="videoCreating">
                <option value="external">{{ t('llmGen.backendExternal') }}</option>
                <option value="local">{{ t('llmGen.backendLocal') }}</option>
              </select>
            </div>
            <div class="field gen-submit-field">
              <label class="gen-hidden-label">{{ t('llmGen.actions') }}</label>
              ...
              >{{ videoCreating ? t('llmGen.creating') : t('llmGen.createTask') }}</button>
            </div>
          </div>
        </div>
        ...
        <ul v-else class="gen-task-list">
          <li v-for="vt in videoTasks" :key="vt.id" class="gen-task-item">
            <div class="gen-task-head">
              <span class="pill" :class="videoStatusClass(vt.status)">{{ videoStatusLabel(vt.status) }}</span>
              <span class="gen-task-prompt">{{ vt.prompt }}</span>
              <span class="muted small mono">{{ vt.id }}</span>
            </div>
            <div class="gen-task-meta muted small">
              <span>{{ vt.duration_secs }}s · {{ vt.backend }} · {{ fmtDateTime(vt.created_at) }}</span>
              <a v-if="vt.video_url" :href="vt.video_url" target="_blank" rel="noopener">{{ t('llmGen.viewVideo') }}</a>
            </div>
            <p v-if="vt.status === 'failed' && vt.error" class="form-msg is-err gen-task-error">
              {{ vt.error }}
            </p>
          </li>
        </ul>
          ...
          <template v-else-if="mcKind === 'extapi'">
            <select v-model="mcExtApiId" class="mc-select" :disabled="mcSending">
              ...
              <option v-for="a in extApis" :key="a.id" :value="a.id">
                {{ a.name }}<template v-if="a.models?.length"> · {{ t('llmChat.modelsShort', { n: a.models.length }) }}</template><template v-if="a.via_node"> · 🌐</template>
              </option>
    ...
    <section v-show="activeTab === 'gpu'" class="tab-panel">
      <div class="panel-head">
        <span class="panel-title">{{ t('llmInst.gpuTitle') }}</span>
      ...
      <div v-if="gpuLoading && !(gpu.devices?.length)" class="card empty-card">{{ t('llmInst.loading') }}</div>
      <div v-else-if="!gpu.available" class="card empty-card">
        {{ t('llmInst.gpuEmpty1') }}<br />
        {{ t('llmInst.gpuEmpty2') }}
      </div>
      <section v-else class="gpu-grid">
        <div v-for="d in gpu.devices" :key="d.index" class="card gpu-card">
          ...
          <template v-if="d.unified_memory">
            <div class="gpu-row">
              <span class="muted small">{{ t('llmInst.dedicatedVram') }}</span>
              <span class="mono">{{ t('llmInst.unifiedNa') }}</span>
            </div>
            <div class="gpu-row">
              <span class="muted small">{{ t('llmInst.unifiedMemLabel') }}</span>
              <span class="mono">{{ mibToGB(d.unified_memory_total_mib) || '—' }}</span>
            </div>
            <div class="gpu-row">
              <span class="muted small">{{ t('llmInst.usedShared') }}</span>
              <span class="mono">{{ d.unified_memory_used_mib ?? '—' }} MiB</span>
            </div>
            <div class="gpu-row">
              <span class="muted small">{{ t('llmInst.availShared') }}</span>
              <span class="mono">{{ d.unified_memory_free_mib ?? '—' }} MiB</span>
            </div>
            <div class="gpu-row">
              <span class="muted small">{{ t('llmInst.memUsage') }}</span>
              <span class="prog-wrap">
                <span class="prog-bar"><span class="prog-fill" :style="{ width: memPct(d) + '%' }" /></span>
                <span class="prog-text">{{ Math.round(memPct(d)) }}%</span>
              </span>
          ...
          <template v-else>
            <div class="gpu-row">
              <span class="muted small">{{ t('llmInst.totalVram') }}</span>
              <span class="mono">{{ d.memory_total_mib ?? 0 }} MiB</span>
            </div>
            <div class="gpu-row">
              <span class="muted small">{{ t('llmInst.used') }}</span>
              <span class="mono">{{ d.memory_used_mib ?? 0 }} MiB</span>
            </div>
            <div class="gpu-row">
              <span class="muted small">{{ t('llmInst.free') }}</span>
              <span class="mono">{{ d.memory_free_mib ?? 0 }} MiB</span>
            </div>
            <div class="gpu-row">
              <span class="muted small">{{ t('llmInst.vramUsage') }}</span>
              <span class="prog-wrap">
                <span class="prog-bar"><span class="prog-fill" :style="{ width: memPct(d) + '%' }" /></span>
                <span class="prog-text">{{ Math.round(memPct(d)) }}%</span>
              </span>
          ...
          <div class="gpu-row">
            <span class="muted small">{{ t('llmInst.gpuUsage') }}</span>
            <span class="prog-wrap">
    ...
    <section v-show="activeTab === 'params'" class="tab-panel">
      <div class="card params-card">
        <h3 class="params-title">{{ t('llmInst.paramsTitle') }}</h3>
        <p class="muted small">{{ t('llmInst.paramsSub') }}</p>
        ...
        <dl class="param-list">
          ...
          <div class="param-item">
            <dt>quantization</dt>
            <dd>
              {{ t('llmInst.paramQuant1') }}
              {{ t('llmInst.paramQuant2') }}
            </dd>
          </div>
          <div class="param-item">
            <dt>dtype</dt>
            <dd>
              {{ t('llmInst.paramDtype1') }}
              {{ t('llmInst.paramDtype2') }}
            </dd>
          </div>
          <div class="param-item">
          ...
          <div class="param-item">
            ...
            <dd>
              {{ t('llmInst.paramTrust1') }}
              {{ t('llmInst.paramTrust2') }}
            </dd>
          </div>
        </dl>
      </div>
