...
const tabLabels = computed<Record<TabKey, string>>(() => ({
  nodes: t('bcn.tabNodes'),
  explorers: t('bcn.tabExplorers'),
  runtime: t('bcn.tab'),
  overview: t('bcn.tabOverview'),
}));
...
async function loadChainPresets(): Promise<void> {
  try {
    ...
    msg.value = { kind: 'err', text: t('bcn.loadChainPresetsFailed', { msg: friendlyError(e) }) };
  }
}
...
async function loadClients(): Promise<void> {
  try {
    ...
    msg.value = { kind: 'err', text: t('bcn.loadClientsFailed', { msg: friendlyError(e) }) };
  }
}
...
// 徽章辅助
// =============================================================================
function chainTypePill(ct: string): string {
  switch (ct) {
    case 'ethereum':
      return 'pill-blue';
    case 'dev':
...
function statusLabel(s: NodeStatus): string {
  switch (s) {
    case 'running':
      return t('bcn.statusRunning');
    case 'error':
      return t('bcn.statusError');
    case 'syncing':
      return t('bcn.statusSyncing');
    case 'stopped':
      return t('bcn.statusStopped');
    default:
      return s;
  }
}
...
// 节点表格列定义
// =============================================================================
const nodeCols = computed<Column<NodeInstance>[]>(() => [
  { key: 'name', title: t('bcn.colName'), sortable: true },
  { key: 'chain_type', title: t('bcn.colChainType'), width: '110px' },
  { key: 'network', title: t('bcn.colNetwork'), width: '120px' },
  { key: 'client', title: t('bcn.colClient'), width: '120px' },
  { key: 'chain_id', title: 'Chain ID', width: '90px', align: 'right' },
  { key: 'rpc_port', title: t('bcn.fRpcPort'), width: '90px', align: 'right' },
  { key: 'status', title: t('bcn.colStatus'), width: '100px' },
  { key: 'actions', title: t('bcn.colActions'), width: '200px', align: 'right' },
]);

const explorerCols = computed<Column<ExplorerInstance>[]>(() => [
  { key: 'name', title: t('bcn.colName'), sortable: true },
  { key: 'node_id', title: t('bcn.colLinkedNode'), width: '160px' },
  { key: 'web_port', title: t('bcn.colWebPort'), width: '100px', align: 'right' },
  { key: 'status', title: t('bcn.colStatus'), width: '100px' },
  { key: 'url', title: t('bcn.colUrl'), width: '220px' },
  { key: 'actions', title: t('bcn.colActions'), width: '180px', align: 'right' },
]);
...
async function submitNode(): Promise<void> {
  ...
  try {
    ...
    msg.value = { kind: 'ok', text: t('bcn.nodeCreated', { name: created.name }) };
    await reloadAll();
  } catch (e) {
    msg.value = { kind: 'err', text: t('bcn.createFailed', { msg: friendlyError(e) }) };
  } finally {
...
async function startNode(n: NodeInstance): Promise<void> {
  ...
  try {
    ...
    msg.value = {
      kind: 'ok',
      text: t('bcn.startSentDocker', { name: n.name }),
    };
    await reloadAll();
    ...
    msg.value = {
      kind: 'err',
      text: t('bcn.startFailedDocker', { msg: friendlyError(e) }),
    };
  } finally {
    acting.value = null;
  }
...
async function stopNode(n: NodeInstance): Promise<void> {
  ...
  try {
    ...
    msg.value = { kind: 'err', text: t('bcn.stopFailed', { msg: friendlyError(e) }) };
  } finally {
    acting.value = null;
  }
}
...
async function deleteNode(n: NodeInstance): Promise<void> {
  if (!confirm(t('bcn.confirmDeleteNode', { name: n.name }))) return;
  acting.value = n.id;
  msg.value = null;
  try {
    await endpoints.deleteBlockchainNode(n.id);
    msg.value = { kind: 'ok', text: t('bcn.nodeDeleted', { name: n.name }) };
    if (viewingNode.value?.id === n.id) viewingNode.value = null;
    await reloadAll();
  } catch (e) {
    msg.value = { kind: 'err', text: t('bcn.deleteFailed', { msg: friendlyError(e) }) };
  } finally {
    acting.value = null;
  }
}
...
async function submitExplorer(): Promise<void> {
  if (!explorerForm.value.name.trim()) {
    msg.value = { kind: 'err', text: t('bcn.errExplorerName') };
    return;
  }
  if (!explorerForm.value.node_id) {
    msg.value = { kind: 'err', text: t('bcn.errPickNode') };
    return;
  }
  submitting.value = true;
  msg.value = null;
  try {
    ...
    msg.value = {
      kind: 'ok',
      text: t('bcn.explorerCreated', { name: created.name, url: created.url ?? '' }),
    };
    await reloadAll();
  } catch (e) {
    msg.value = { kind: 'err', text: t('bcn.createFailed', { msg: friendlyError(e) }) };
...
async function startExplorer(e: ExplorerInstance): Promise<void> {
  ...
  try {
    ...
    msg.value = { kind: 'ok', text: t('bcn.explorerStartSent', { name: e.name }) };
    await reloadAll();
  } catch (err) {
    msg.value = { kind: 'err', text: t('bcn.startFailed', { msg: friendlyError(err) }) };
  } finally {
    acting.value = null;
  }
}
...
async function deleteExplorer(e: ExplorerInstance): Promise<void> {
  if (!confirm(t('bcn.confirmDeleteExplorer', { name: e.name }))) return;
  acting.value = e.id;
  msg.value = null;
  try {
    await endpoints.deleteBlockchainExplorer(e.id);
    msg.value = { kind: 'ok', text: t('bcn.explorerDeleted', { name: e.name }) };
    await reloadAll();
  } catch (err) {
    msg.value = { kind: 'err', text: t('bcn.deleteFailed', { msg: friendlyError(err) }) };
  } finally {
    acting.value = null;
  }
}
...
async function copyWithToast(text: string): Promise<void> {
  if (await copyText(text)) {
    msg.value = { kind: 'ok', text: t('bcn.copied') };
  } else {
    msg.value = { kind: 'err', text: t('bcn.copyUnsupported') };
  }
}
...
<template>
  <div class="page">
    ...
    <div class="page-head">
      <div>
        <div class="page-title">{{ t('bcn.title') }}</div>
        <div class="page-sub muted">{{ t('bcn.sub') }}</div>
      </div>
      <div class="head-actions">
        <button class="btn btn-small" type="button" @click="reloadAll">{{ t('bcn.refresh') }}</button>
        <button
    ...
    <div v-if="activeTab === 'nodes'" class="tab-panel">
      ...
      <div class="stat-grid">
        <div class="card stat-card">
          <div class="stat-label">{{ t('bcn.statNodes') }}</div>
          <div class="stat-value">{{ stats.nodes_total }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('bcn.statusRunning') }}</div>
          <div class="stat-value">{{ stats.running }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('bcn.statusStopped') }}</div>
      ...
      <div class="card card-table">
        <div v-if="nodesLoading" class="table-empty">{{ t('bcn.loading') }}</div>
    ...
    <div v-else-if="activeTab === 'explorers'" class="tab-panel">
      ...
      <div class="card card-table">
        <div v-if="explorersLoading" class="table-empty">{{ t('bcn.loading') }}</div>
        <div v-else-if="!explorers.length" class="table-empty">
          {{ t('bcn.explorersEmpty') }}
        </div>
      ...
      <div v-if="viewingExplorer" class="modal-backdrop" @click.self="viewingExplorer = null">
        <div class="modal" role="dialog" aria-modal="true">
          <div class="modal-head">
            <h3>{{ t('bcn.explorerDetail', { name: viewingExplorer.name }) }}</h3>
            <button class="modal-close" type="button" @click="viewingExplorer = null">×</button>
          </div>
          <div class="modal-body">
            <div class="detail-grid">
              <div><span class="muted small">{{ t('bcn.colLinkedNode') }}</span> {{ nodeName(viewingExplorer.node_id) }}</div>
              <div><span class="muted small">{{ t('bcn.colWebPort') }}</span> {{ viewingExplorer.web_port }}</div>
              <div><span class="muted small">{{ t('bcn.colStatus') }}</span> <span :class="['pill', statusPill(viewingExplorer.status)]">{{ statusLabel(viewingExplorer.status) }}</span></div>
              <div>
                <span class="muted small">{{ t('bcn.colUrl') }}</span>
            ...
            <pre class="code-block">{{ viewingExplorer.compose_yaml ?? t('bcn.noCompose') }}</pre>
            <div class="compose-note muted small">
              {{ t('bcn.composeNoteExplorer') }}
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-else class="tab-panel">
      <div class="stat-grid">
        <div class="card stat-card">
          <div class="stat-label">{{ t('bcn.tabNodes') }}</div>
          <div class="stat-value">{{ stats.nodes_total }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('bcn.tabExplorers') }}</div>
          <div class="stat-value">{{ stats.explorers_total }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('bcn.statChains') }}</div>
      ...
      <div class="card overview-card">
        <div class="panel-head">
          <div class="panel-title">{{ t('bcn.chainPresetsTitle') }}</div>
        </div>
        <div class="preset-grid">
          <div v-for="p in chainPresets" :key="p.network" class="preset-item">
            ...
            <div class="muted small">
              {{ t('bcn.presetLine', { network: p.network, chainId: p.chain_id, sync: p.default_sync }) }}
            </div>
            <div class="client-list muted small">
              {{ t('bcn.recClients', { clients: p.clients.join(' / ') }) }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="card overview-card">
        <div class="panel-head">
          <div class="panel-title">{{ t('bcn.clientsTitle') }}</div>
        </div>
    ...
    <div v-if="showNodeDialog" class="modal-backdrop" @click.self="showNodeDialog = false">
      <div class="modal" role="dialog" aria-modal="true" aria-labelledby="bc-node-title">
        <div class="modal-head">
          <h3 id="bc-node-title">{{ t('bcn.createNodeTitle') }}</h3>
          <button
            class="modal-close"
            type="button"
            :disabled="submitting"
        ...
        <form class="modal-body" @submit.prevent="submitNode">
          <div class="field">
            <label for="bc-name">{{ t('bcn.fName') }}</label>
            <input
              id="bc-name"
              v-model="nodeForm.name"
              type="text"
              :placeholder="t('bcn.nodeNamePh')"
            />
          </div>

          <div class="form-row">
            <div class="field">
              <label for="bc-type">{{ t('bcn.colChainType') }}</label>
              <select id="bc-type" v-model="nodeForm.chain_type" @change="onChainTypeChange">
                <option value="ethereum">{{ t('bcn.typeEthereum') }}</option>
                <option value="dev">{{ t('bcn.typeDev') }}</option>
                <option value="l2">{{ t('bcn.typeL2') }}</option>
                <option value="custom">{{ t('bcn.typeCustom') }}</option>
              </select>
            </div>
            <div class="field">
              <label for="bc-net">{{ t('bcn.fNetworkPreset') }}</label>
          ...
          <div class="form-row">
            <div class="field">
              <label for="bc-client">{{ t('bcn.colClient') }}</label>
          ...
          <div class="form-row">
            <div class="field">
              <label for="bc-rpc">{{ t('bcn.fRpcPort') }}</label>
              <input id="bc-rpc" v-model.number="nodeForm.rpc_port" type="number" min="1" max="65535" />
            </div>
            <div class="field">
              <label for="bc-ws">{{ t('bcn.fWsPort') }}</label>
              <input id="bc-ws" v-model.number="nodeForm.ws_port" type="number" min="1" max="65535" />
            </div>
          </div>

          <div class="field">
            <label for="bc-dir">{{ t('bcn.fDataDir') }}</label>
            <input
              id="bc-dir"
              v-model="nodeForm.data_dir"
              type="text"
              :placeholder="t('bcn.nodeDirPh')"
            />
          </div>

          <div v-if="!isDevChain" class="field">
            <label for="bc-sync">{{ t('bcn.fSyncMode') }}</label>
            <select id="bc-sync" v-model="nodeForm.sync_mode">
              <option value="snap">{{ t('bcn.syncSnap') }}</option>
              <option value="full">{{ t('bcn.syncFull') }}</option>
              <option value="archive">{{ t('bcn.syncArchive') }}</option>
            </select>
          </div>
          <div v-else class="form-msg is-info small">{{ t('bcn.devFullNote') }}</div>
          ...
          <div class="form-actions">
            ...
            <button class="btn btn-primary" type="submit" :disabled="submitting">
              {{ submitting ? t('bcn.creating') : t('bcn.create') }}
            </button>
          </div>
        </form>
      </div>
    ...
    <div v-if="showExplorerDialog" class="modal-backdrop" @click.self="showExplorerDialog = false">
      <div class="modal" role="dialog" aria-modal="true" aria-labelledby="bc-exp-title">
        <div class="modal-head">
          <h3 id="bc-exp-title">{{ t('bcn.createExplorerTitle') }}</h3>
          <button
            class="modal-close"
            type="button"
            :disabled="submitting"
        ...
        <form class="modal-body" @submit.prevent="submitExplorer">
          <div class="field">
            <label for="bc-exp-name">{{ t('bcn.explorerName') }}</label>
            <input id="bc-exp-name" v-model="explorerForm.name" type="text" :placeholder="t('bcn.explorerNamePh')" />
          </div>
          <div class="field">
            <label for="bc-exp-node">{{ t('bcn.colLinkedNode') }}</label>
          ...
          <div class="field">
            <label for="bc-exp-port">{{ t('bcn.colWebPort') }}</label>
            <input
              id="bc-exp-port"
    ...
    <div v-if="viewingNode && 'chain' in viewingNode" class="modal-backdrop" @click.self="viewingNode = null">
      <div class="modal" role="dialog" aria-modal="true">
        <div class="modal-head">
          <h3>{{ t('bcn.nodeDetail', { name: viewingNode.name }) }}</h3>
          <button class="modal-close" type="button" @click="viewingNode = null">×</button>
        </div>
        <div class="modal-body">
          <div class="detail-grid">
            <div><span class="muted small">{{ t('bcn.colChainType') }}</span> <span :class="['pill', chainTypePill(viewingNode.chain.chain_type)]">{{ viewingNode.chain.chain_type }}</span></div>
            <div><span class="muted small">{{ t('bcn.colNetwork') }}</span> {{ viewingNode.chain.network }}</div>
            <div><span class="muted small">Chain ID</span> {{ viewingNode.chain.chain_id }}</div>
            <div><span class="muted small">{{ t('bcn.colClient') }}</span> <span class="pill pill-cyan">{{ viewingNode.client }}</span></div>
            <div><span class="muted small">{{ t('bcn.fRpcPort') }}</span> {{ viewingNode.rpc_port }}</div>
            <div><span class="muted small">{{ t('bcn.fWsPort') }}</span> {{ viewingNode.ws_port ?? '—' }}</div>
            <div><span class="muted small">{{ t('bcn.fSyncMode') }}</span> {{ viewingNode.sync_mode }}</div>
            <div><span class="muted small">{{ t('bcn.colStatus') }}</span> <span :class="['pill', statusPill(viewingNode.status)]">{{ statusLabel(viewingNode.status) }}</span></div>
            <div class="detail-full"><span class="muted small">{{ t('bcn.fDataDir') }}</span> {{ viewingNode.data_dir }}</div>
            <div v-if="viewingNode.start_cmd" class="detail-full">
              <span class="muted small">{{ t('bcn.startCmd') }}</span>
          ...
          <pre class="code-block">{{ viewingNode.compose_yaml ?? t('bcn.noCompose') }}</pre>
          <div class="compose-note muted small">
            {{ t('bcn.composeNoteNode') }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="createdCompose" class="modal-backdrop" @click.self="createdCompose = null">
      <div class="modal" role="dialog" aria-modal="true">
        <div class="modal-head">
          <h3>{{ t('bcn.composeGenerated') }}</h3>
          <button class="modal-close" type="button" @click="createdCompose = null">×</button>
        </div>
        <div class="modal-body">
          <div class="compose-actions">
            <button class="btn btn-small" type="button" @click="copyWithToast(createdCompose)">{{ t('bcn.copyBtn') }}</button>
          </div>
          <pre class="code-block">{{ createdCompose }}</pre>
          <div class="compose-note muted small">
            {{ t('bcn.composeNoteCreated') }}
          </div>
        </div>
      </div>
    </div>
