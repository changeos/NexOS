...
const {
  ...
  onRegisterFailed: async (name, e) => {
    ...
    msg.value = {
      kind: 'info',
      text: t('apps.pkgInstallDone', { name }) + t('apps.pkgRegisterPartial', { err: friendlyError(e) }),
    };
    await loadPkgCatalog();
    await loadInstalledPkgs();
  },
...
const installedPkgColumns = computed<Column<InstalledPkgApp>[]>(() => [
  { key: 'name', title: t('apps.colName'), sortable: true },
  { key: 'version', title: t('apps.colVersion'), width: '110px' },
  { key: 'category', title: t('apps.colCategory'), width: '110px' },
  { key: 'id', title: 'ID', width: '130px' },
  { key: 'installed_at', title: t('apps.colInstalledAt'), width: '180px' },
  { key: 'actions', title: t('apps.colActions'), width: '110px', align: 'right' },
]);
...
function fmtDownloads(n?: number): string {
  // 按 locale 紧凑计数（zh: 1.2亿/3.4万；en: 120M/34K；ja: 1.2億/3.4万）
  return new Intl.NumberFormat(locale.value || 'zh-CN', { notation: 'compact' }).format(n ?? 0);
}
...
async function installApp(app: StoreApp): Promise<void> {
  ...
  try {
    ...
    msg.value = {
      kind: 'ok',
      text: t('apps.installOk', { name: app.name ?? id }),
    };
    activeTab.value = 'tasks';
    startPolling();
  } catch (e) {
    msg.value = { kind: 'err', text: t('apps.installFail', { msg: friendlyError(e) }) };
  } finally {
    busyAppId.value = '';
  }
}
...
const installedColumns = computed<Column<InstalledApp>[]>(() => [
  { key: 'display_name', title: t('apps.colName'), sortable: true },
  { key: 'version', title: t('apps.colVersion'), width: '140px' },
  { key: 'source', title: t('apps.colType'), width: '100px' },
  { key: 'actions', title: t('apps.colActions'), width: '120px', align: 'right' },
]);
...
async function uninstallApp(row: InstalledApp): Promise<void> {
  ...
  if (!name) return;
  if (!window.confirm(t('apps.uninstallConfirm', { name }))) return;
  uninstalling.value = name;
  msg.value = null;
  try {
    await endpoints.appStoreUninstall(name, source);
    msg.value = { kind: 'ok', text: t('apps.uninstallOk', { name }) };
    activeTab.value = 'tasks';
    startPolling();
  } catch (e) {
    msg.value = { kind: 'err', text: t('apps.uninstallFail', { msg: friendlyError(e) }) };
  } finally {
    uninstalling.value = '';
  }
}
...
function startPolling(): void {
  ...
  pollTimer = setInterval(async () => {
    const hasActive = tasks.value.some(
      (tk) => tk.status === 'installing' || tk.status === 'pending',
    );
    await loadTasks();
...
const taskColumns = computed<Column<InstallTask>[]>(() => [
  { key: 'app_name', title: t('apps.colApp'), sortable: true },
  { key: 'install_type', title: t('apps.colType'), width: '90px' },
  { key: 'status', title: t('apps.colStatus'), width: '110px' },
  { key: 'pid', title: 'PID', width: '80px' },
  { key: 'created_at', title: t('apps.colCreatedAt'), width: '180px' },
  { key: 'actions', title: t('apps.colActions'), width: '100px', align: 'right' },
]);
...
const CATEGORY_VALUES = ['custom', 'media', 'dev', 'office', 'internet', 'system'] as const;
const categoryOptions = computed(() =>
  CATEGORY_VALUES.map((value) => ({ value, label: t(`apps.cat.${value}`) })),
);

async function submitPublish(): Promise<void> {
  if (!publishForm.value.name.trim()) {
    msg.value = { kind: 'err', text: t('apps.nameRequired') };
    return;
  }
  if (!publishForm.value.install_target.trim()) {
    msg.value = { kind: 'err', text: t('apps.targetRequired') };
    return;
  }
  ...
  try {
    ...
    msg.value = { kind: 'ok', text: t('apps.publishOk') };
    // 重置表单（保留分类/类型选择）
    ...
    msg.value = { kind: 'err', text: t('apps.publishFail', { msg: friendlyError(e) }) };
  } finally {
...
function statusLabel(s?: string): string {
  switch (s) {
    ...
    case 'completed':
      return t('apps.statusCompleted');
    case 'failed':
      return t('apps.statusFailed');
    default:
      return s ?? '—';
  }
}
...
const hasActiveTasks = computed(() =>
  tasks.value.some((tk) => tk.status === 'installing' || tk.status === 'pending'),
);
  ...
  <div class="as-page">
    <div class="page-head">
      <div>
        ...
        <div class="page-sub muted">{{ t('apps.sub') }}</div>
      </div>
    ...
    <section v-show="activeTab === 'store'" class="tab-panel">
      ...
      <section class="stat-grid">
        <div class="card stat-card">
          <div class="stat-label">{{ t('apps.statTotal') }}</div>
          <div class="stat-value">{{ stats.total_apps ?? 0 }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('apps.statInstalled') }}</div>
          <div class="stat-value">{{ stats.installed ?? 0 }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('apps.statCategories') }}</div>
      ...
      <div class="store-layout">
        ...
        <aside class="cat-sidebar card">
          ...
          >
            <span class="cat-name">{{ t('apps.catAll') }}</span>
        ...
        <div class="store-main">
          <div class="store-toolbar">
            ...
            />
            <span class="muted small">{{ t('apps.appCount', { n: filteredApps.length }) }}</span>
          </div>

          <div v-if="appsError" class="error-box">{{ appsError }}</div>
          <div v-if="appsLoading && !apps.length" class="card empty-card">{{ t('apps.loading') }}</div>
          <div v-else-if="!filteredApps.length" class="card empty-card">
            {{ t('apps.emptyPre') }}<a class="link" @click="activeTab = 'publish'">{{ t('apps.emptyLink') }}</a>{{ t('apps.emptyPost') }}
          </div>
          <div v-else class="app-grid">
            <div v-for="a in filteredApps" :key="a.id" class="card app-card">
              ...
              <div class="app-meta">
                <span class="app-stars" :title="t('apps.ratingTitle', { n: a.rating ?? 0 })">{{ ratingStars(a.rating) }}</span>
                <span class="muted small">{{ t('apps.downloads', { n: fmtDownloads(a.downloads) }) }}</span>
              </div>
              <div class="app-target-row">
                <span class="muted small">{{ t('apps.target') }}</span>
                <code class="mono app-target">{{ a.install_target ?? '—' }}</code>
              </div>
              <div class="app-card-actions">
                <span v-if="a.installed" class="pill pill-ok">{{ t('apps.installedBuiltin') }}</span>
                <button
                  v-else
                  class="btn btn-small btn-primary"
                ...
                >
                  {{ busyAppId === a.id ? t('apps.creating') : t('apps.installBtn') }}
                </button>
              </div>
            </div>
          </div>
    ...
    <section v-show="activeTab === 'installed'" class="tab-panel">
      <div class="panel-head">
        <span class="panel-title">{{ t('apps.installedTitle') }}</span>
      </div>
      ...
      <div class="panel">
        <div class="card card-table">
          <DataTable
            ...
            :empty-text="t('apps.installedEmpty')"
          >
    ...
    <section v-show="activeTab === 'tasks'" class="tab-panel">
      <div class="panel-head">
        <span class="panel-title">{{ t('apps.tasksTitle') }}</span>
        ...
        <div class="card card-table">
          ...
          >
            ...
            <template #cell-actions="{ row }">
              ...
              >
                {{ expandedTask === row.id ? t('apps.collapse') : t('apps.logBtn') }}
              </button>
            </template>
          </DataTable>
        </div>
      ...
      <div v-if="expandedTask" class="card task-detail">
        <div class="task-detail-head">
          <span class="panel-title">{{ t('apps.logTitle', { id: expandedTask }) }}</span>
          <button
            class="btn btn-small"
            @click="refreshTaskDetail(expandedTask)"
          >{{ t('apps.refresh') }}</button>
        </div>
        <template v-for="tk in tasks" :key="tk.id">
          <template v-if="tk.id === expandedTask">
            <div v-if="tk.error" class="task-error">{{ t('apps.taskErrorLine', { err: tk.error }) }}</div>
            <pre v-if="tk.log_tail" class="task-log">{{ tk.log_tail }}</pre>
            <p v-if="!tk.error && !tk.log_tail" class="muted small">{{ t('apps.noLog') }}</p>
          </template>
        </template>
      </div>
    ...
    <section v-show="activeTab === 'publish'" class="tab-panel">
      <div class="panel-head">
        <span class="panel-title">{{ t('apps.publishTitle') }}</span>
      </div>
      <div class="card publish-card">
        <form class="publish-form" @submit.prevent="submitPublish">
          <div class="field">
            <label for="as-name">{{ t('apps.fName') }}</label>
            <input
              id="as-name"
          ...
          <div class="field">
            <label for="as-desc">{{ t('apps.fDesc') }}</label>
            <input
              id="as-desc"
              v-model="publishForm.description"
              type="text"
              :placeholder="t('apps.descPh')"
              :disabled="publishing"
            />
          </div>
          <div class="field-row">
            <div class="field">
              <label for="as-cat">{{ t('apps.fCategory') }}</label>
            ...
            <div class="field">
              <label>{{ t('apps.fInstallType') }}</label>
              <div class="fixed-type">
                <span class="pill pill-purple">NexOS</span>
                <span class="muted small">{{ t('apps.builtinOnly') }}</span>
              </div>
            </div>
          </div>
          <div class="field">
            <label for="as-target">{{ t('apps.colTargetStar') }}</label>
            <input
              id="as-target"
              v-model="publishForm.install_target"
              type="text"
              :placeholder="t('apps.targetPh')"
              :disabled="publishing"
            />
          </div>
          <p class="muted small">
            {{ t('apps.publishNoteA') }}<code class="mono">apt</code> / <code class="mono">snap</code> /
            <code class="mono">flatpak</code>{{ t('apps.publishNoteB') }}
          </p>
          <p v-if="msg" :class="['form-msg', `is-${msg.kind}`]">{{ msg.text }}</p>
          <div class="form-actions">
            <button type="submit" class="btn btn-primary" :disabled="publishing">
              {{ publishing ? t('apps.publishing') : t('apps.publishBtn') }}
            </button>
          </div>
        </form>
      </div>
