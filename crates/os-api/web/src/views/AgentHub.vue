...
function toolchainOk(name: string): boolean {
  return toolchains.value.some((tl) => tl.name === name && tl.available);
}
...
const CATEGORY_CHIP_KEYS = ['', 'coding', 'assistant', 'custom'] as const;
const categoryChips = computed(() => CATEGORY_CHIP_KEYS.map((key) => ({
  key,
  label: key === '' ? t('agentHub.catAll') : t(`agentHub.cat.${key}`),
})));
...
function channelReady(ch?: string): boolean {
  switch (ch) {
    case 'npm':
...
function channelHint(ch?: string): string {
  switch (ch) {
    case 'npm':
      return toolchainOk('npm') ? 'npm install -g' : t('agentHub.chNeedNode');
    case 'script':
      return toolchainOk('curl') ? 'curl -fsSL … | bash' : t('agentHub.chNeedCurl');
    case 'uv':
      return toolchainOk('uv') ? 'uv tool install' : t('agentHub.chNeedUv');
    case 'cargo':
      return toolchainOk('cargo') ? 'cargo install' : t('agentHub.chNeedCargo');
    default:
      return t('agentHub.chUnknown');
  }
}
...
async function installAgent(a: CatalogAgent): Promise<void> {
  ...
  try {
    ...
    msg.value = { kind: 'ok', text: t('agentHub.installOk', { name: a.name ?? id }) };
    activeTab.value = 'tasks';
    await loadTasks();
    startPolling();
  } catch (e) {
    msg.value = { kind: 'err', text: t('agentHub.installFail', { msg: friendlyError(e) }) };
  } finally {
    busyAgentId.value = '';
    busyAction.value = '';
  }
...
async function uninstallAgent(a: CatalogAgent): Promise<void> {
  ...
  if (!window.confirm(t('agentHub.uninstallConfirm', { name: a.name ?? id, bin }))) return;
  busyAgentId.value = id;
  busyAction.value = 'uninstall';
  msg.value = null;
  try {
    await endpoints.agentHubUninstall(id);
    msg.value = { kind: 'ok', text: t('agentHub.uninstallOk', { name: a.name ?? id }) };
    activeTab.value = 'tasks';
    await loadTasks();
    startPolling();
  } catch (e) {
    msg.value = { kind: 'err', text: t('agentHub.uninstallFail', { msg: friendlyError(e) }) };
  } finally {
    busyAgentId.value = '';
    busyAction.value = '';
  }
...
const taskColumns = computed<Column<AgentTask>[]>(() => [
  { key: 'agent_name', title: 'Agent', sortable: true },
  { key: 'action', title: t('agentHub.colAction'), width: '90px' },
  { key: 'install_type', title: t('agentHub.colChannel'), width: '90px' },
  { key: 'status', title: t('agentHub.colStatus'), width: '100px' },
  { key: 'pid', title: 'PID', width: '80px' },
  { key: 'created_at', title: t('agentHub.colCreatedAt'), width: '170px' },
  { key: 'actions', title: t('agentHub.colOperations'), width: '100px', align: 'right' },
]);

async function refreshTaskDetail(taskId: string): Promise<void> {
  try {
    const dt = (await endpoints.agentHubTaskDetail(taskId)) as AgentTask;
    const i = tasks.value.findIndex((x) => x.id === taskId);
    if (i >= 0 && dt) tasks.value[i] = dt;
  } catch {
    /* 详情拉取失败保持现状 */
  }
}
...
function statusLabel(s?: string): string {
  switch (s) {
    case 'running':
      return t('agentHub.statusRunning');
    case 'pending':
      return t('agentHub.statusPending');
    case 'completed':
      return t('agentHub.statusCompleted');
    case 'failed':
      return t('agentHub.statusFailed');
    default:
      return s ?? '—';
  }
}
function actionLabel(a?: string): string {
  return a === 'uninstall' ? t('agentHub.actionUninstall') : t('agentHub.actionInstall');
}

function installTypeBadge(ch?: string): { cls: string; label: string } {
  switch (ch) {
    case 'npm':
      return { cls: 'pill-cyan', label: 'npm' };
    case 'script':
      return { cls: 'pill-purple', label: t('agentHub.badgeScript') };
    case 'uv':
      return { cls: 'pill-blue', label: 'uv' };
    case 'cargo':
      return { cls: 'pill-muted', label: 'cargo' };
    default:
      return { cls: 'pill-muted', label: ch ?? '—' };
  }
}

const hasActiveTasks = computed(() =>
  tasks.value.some((tk) => tk.status === 'running' || tk.status === 'pending'),
);
...
async function submitPublish(): Promise<void> {
  ...
  if (!f.name.trim() || !f.install_target.trim() || !f.check_binary.trim()) {
    msg.value = { kind: 'err', text: t('agentHub.publishErrRequired') };
    return;
  }
  ...
  try {
    ...
    msg.value = {
      kind: 'ok',
      text: t('agentHub.publishOk', { name: (created as CatalogAgent)?.name ?? f.name }),
    };
    publishForm.value = {
      name: '',
      description: '',
    ...
    msg.value = { kind: 'err', text: t('agentHub.publishFail', { msg: friendlyError(e) }) };
  } finally {
...
async function deleteUserAgent(a: CatalogAgent): Promise<void> {
  const id = String(a.id ?? '');
  if (!id) return;
  if (!window.confirm(t('agentHub.deleteConfirm', { name: a.name ?? id }))) return;
  try {
    await endpoints.deleteAgentHubPublished(id);
    msg.value = { kind: 'ok', text: t('agentHub.deleteOk', { name: a.name ?? id }) };
    await loadAgents();
    await loadStats();
  } catch (e) {
    msg.value = { kind: 'err', text: t('agentHub.deleteFail', { msg: friendlyError(e) }) };
  }
}
...
const targetPlaceholder = computed(() => {
  switch (publishForm.value.install_type) {
    case 'npm':
      return t('agentHub.targetPhNpm');
    case 'script':
      return 'https://example.com/install.sh';
    case 'uv':
      return t('agentHub.targetPhUv');
    case 'cargo':
      return t('agentHub.targetPhCargo');
    default:
      return t('agentHub.colTarget');
  }
});

const userColumns = computed<Column<CatalogAgent>[]>(() => [
  { key: 'name', title: t('agentHub.colName'), sortable: true },
  { key: 'install_type', title: t('agentHub.colChannel'), width: '90px' },
  { key: 'install_target', title: t('agentHub.colTarget') },
  { key: 'check_binary', title: t('agentHub.colCmd'), width: '120px' },
  { key: 'actions', title: t('agentHub.colOperations'), width: '90px', align: 'right' },
]);
...
<template>
  <div class="ah-page">
    <div class="page-head">
      <div>
        <h2 class="page-title">{{ t('agentHub.title') }}</h2>
        <div class="page-sub muted">{{ t('agentHub.sub') }}</div>
      </div>
    ...
    <section v-show="activeTab === 'catalog'" class="tab-panel">
      ...
      <section class="stat-grid">
        <div class="card stat-card">
          <div class="stat-label">{{ t('agentHub.statTotal') }}</div>
          <div class="stat-value">{{ stats.total_agents ?? 0 }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('agentHub.statInstalled') }}</div>
          <div class="stat-value">{{ stats.installed ?? 0 }}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('agentHub.statToolchains') }}</div>
          <div class="stat-value">{{ stats.toolchains_ready ?? 0 }}<span class="stat-unit">/ 5</span></div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">{{ t('agentHub.statTasks') }}</div>
      ...
      <div class="toolchain-row">
        <span
          v-for="tl in toolchains"
          :key="tl.name"
          class="pill"
          :class="tl.available ? 'pill-ok' : 'pill-muted'"
          :title="tl.available ? tl.version : t('agentHub.toolchainUnavailable')"
        >{{ tl.name }} {{ tl.available ? '✓' : '✗' }}</span>
        <template v-if="hasMissingToolchain">
          <button
            ...
            :title="tc === 'node' ? t('agentHub.tcNodeTitle') : t('agentHub.tcInstallTitle', { tc })"
        ...
        <span v-else class="muted small">{{ t('agentHub.readyHint') }}</span>
      </div>
      ...
      <div class="store-toolbar">
        ...
        <input
          ...
          class="search-input"
          type="text"
          :placeholder="t('agentHub.searchPh')"
        />
        <span class="muted small">{{ t('agentHub.agentCount', { n: filteredAgents.length }) }}</span>
      </div>

      <div v-if="agentsError" class="error-box">{{ agentsError }}</div>
      <div v-if="agentsLoading && !agents.length" class="card empty-card">{{ t('agentHub.loading') }}</div>
      <div v-else-if="!filteredAgents.length" class="card empty-card">
        {{ t('agentHub.emptyPre') }}<a class="link" @click="activeTab = 'publish'">{{ t('agentHub.emptyLink') }}</a>{{ t('agentHub.emptyPost') }}
      </div>
      <div v-else class="agent-grid">
        <div v-for="a in filteredAgents" :key="a.id" class="card agent-card">
          ...
          <div v-if="a.tags && a.tags.length" class="agent-tags">
            <span v-for="tag in a.tags" :key="tag" class="tag-chip">{{ tag }}</span>
          </div>
          <div class="agent-target-row">
            <span class="muted small">{{ t('agentHub.colCmd') }}</span>
            <code class="mono agent-target">{{ a.check_binary ?? '—' }}</code>
            <span class="muted small">{{ t('agentHub.colTarget') }}</span>
            <code class="mono agent-target">{{ a.install_target ?? '—' }}</code>
          </div>
          <div class="agent-card-actions">
            <template v-if="a.installed">
              <span class="pill pill-ok">{{ t('agentHub.installed') }}</span>
              ...
              >{{ busyAgentId === a.id && busyAction === 'uninstall' ? t('agentHub.creating') : t('agentHub.actionUninstall') }}</button>
              <span v-else class="muted small" :title="t('agentHub.scriptNoUninstallTitle')">{{ t('agentHub.scriptNoUninstall') }}</span>
            </template>
            <template v-else>
              <button
                class="btn btn-small btn-primary"
              ...
              >{{ busyAgentId === a.id && busyAction === 'install' ? t('agentHub.creating') : t('agentHub.installBtn') }}</button>
    ...
    <section v-show="activeTab === 'tasks'" class="tab-panel">
      <div class="panel-head">
        <span class="panel-title">{{ t('agentHub.tasksTitle') }}</span>
      ...
      <div class="panel">
        <div class="card card-table">
          <DataTable
            ...
            :empty-text="t('agentHub.tasksEmpty')"
          >
            ...
            <template #cell-actions="{ row }">
              <button class="btn btn-small" @click.stop="toggleTaskLog(String(row.id ?? ''))">
                {{ expandedTask === row.id ? t('agentHub.collapse') : t('agentHub.logBtn') }}
              </button>
            </template>
          </DataTable>
        </div>
      ...
      <div v-if="expandedTask" class="card task-detail">
        <div class="task-detail-head">
          <span class="panel-title">{{ t('agentHub.logTitle', { id: expandedTask }) }}</span>
          <button class="btn btn-small" @click="refreshTaskDetail(expandedTask)">{{ t('agentHub.refresh') }}</button>
        </div>
        <template v-for="tk in tasks" :key="tk.id">
          <template v-if="tk.id === expandedTask">
            <div v-if="tk.error" class="task-error">{{ t('agentHub.taskErrorLine', { err: tk.error }) }}</div>
            <pre v-if="tk.log_tail" class="task-log">{{ tk.log_tail }}</pre>
            <p v-if="!tk.error && !tk.log_tail" class="muted small">{{ t('agentHub.noLog') }}</p>
          </template>
        </template>
      </div>

      <p v-if="hasActiveTasks" class="muted small">{{ t('agentHub.autoRefresh') }}</p>
    </section>

    <!-- =================== Tab3 自定义 =================== -->
    <section v-show="activeTab === 'publish'" class="tab-panel">
      <div class="panel-head">
        <span class="panel-title">{{ t('agentHub.publishTitle') }}</span>
      </div>
      <div class="card publish-card">
        <form class="publish-form" @submit.prevent="submitPublish">
          <div class="field-row">
            <div class="field">
              <label for="ah-name">{{ t('agentHub.fName') }}</label>
              <input id="ah-name" v-model="publishForm.name" type="text" :placeholder="t('agentHub.namePh')" :disabled="publishing" />
            </div>
            <div class="field">
              <label for="ah-type">{{ t('agentHub.fChannel') }}</label>
          ...
          <div class="field-row">
            <div class="field">
              <label for="ah-target">{{ t('agentHub.colTarget') }} *</label>
              <input id="ah-target" v-model="publishForm.install_target" type="text" :placeholder="targetPlaceholder" :disabled="publishing" />
            </div>
            <div class="field">
              <label for="ah-bin">{{ t('agentHub.fBin') }}</label>
              <input id="ah-bin" v-model="publishForm.check_binary" type="text" :placeholder="t('agentHub.binPh')" :disabled="publishing" />
            </div>
          </div>
          <div class="field-row">
            <div class="field">
              <label for="ah-home">{{ t('agentHub.homepage') }}</label>
              <input id="ah-home" v-model="publishForm.homepage" type="text" placeholder="https://…" :disabled="publishing" />
            </div>
            <div class="field">
              <label for="ah-desc">{{ t('agentHub.fDesc') }}</label>
              <input id="ah-desc" v-model="publishForm.description" type="text" :placeholder="t('agentHub.descPh')" :disabled="publishing" />
            </div>
          </div>
          <div class="form-actions">
            <button class="btn btn-primary" type="submit" :disabled="publishing">
              {{ publishing ? t('agentHub.publishing') : t('agentHub.publishBtn') }}
            </button>
          </div>
        </form>
      </div>
      ...
      <div class="panel-head">
        <span class="panel-title">{{ t('agentHub.mineTitle', { n: userAgents.length }) }}</span>
      </div>
      <div class="panel">
        <div class="card card-table">
          <DataTable
            :columns="userColumns"
            :rows="userAgents"
            :empty-text="t('agentHub.mineEmpty')"
          >
            <template #cell-install_type="{ row }">
              <span class="pill" :class="installTypeBadge(row.install_type).cls">
                {{ installTypeBadge(row.install_type).label }}
            ...
            <template #cell-actions="{ row }">
              <button class="btn btn-small btn-danger" @click.stop="deleteUserAgent(row)">{{ t('agentHub.deleteBtn') }}</button>
            </template>
          </DataTable>
        </div>
      </div>
