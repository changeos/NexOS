...
const tabs: { key: Tab; label: string }[] = [
  { key: 'pools', label: t('storage.tabPools') },
  { key: 'datasets', label: t('storage.tabDatasets') },
  { key: 'snapshots', label: t('storage.tabSnapshots') },
  { key: 'files', label: t('storage.tabFiles') },
];
...
const MODE_META: Record<RaidMode, { label: string; desc: string }> = {
  single: { label: t('storage.modeSingle'), desc: t('storage.modeSingleDesc') },
  raid0: { label: t('storage.modeRaid0'), desc: t('storage.modeRaid0Desc') },
  raid1: { label: t('storage.modeRaid1'), desc: t('storage.modeRaid1Desc') },
  raid5: { label: 'RAID5', desc: t('storage.modeRaid5Desc') },
  raid10: { label: 'RAID10', desc: t('storage.modeRaid10Desc') },
};
...
function signaturesText(d: DiskInfo): string {
  ...
  return sigs.length ? sigs.join(' / ') : t('storage.unknownSignature');
}

/** 故障容忍的人类可读描述。 */
function faultText(mode: RaidMode, fault: number): string {
  if (mode === 'raid10') return t('storage.faultRaid10');
  if (fault === 0) return t('storage.faultNone');
  return t('storage.faultTolerates', { fault });
}
...
async function importPool(
  ...
  const confirmed = window.confirm(
    t('storage.importConfirm', { name }) + ' ' + t('storage.importNameClashHint'),
  );
  if (!confirmed) return;
  importingPool.value = name;
  importMsg.value = { kind: 'info', text: t('storage.importing', { name }) };
  try {
    await endpoints.importPool(name);
    importMsg.value = { kind: 'ok', text: t('storage.importDone', { name }) };
    await loadPools();
    ...
    importMsg.value = {
      kind: 'err',
      text: t('storage.importFailed', { name }) + (e instanceof Error ? e.message : String(e)),
    };
  } finally {
    importingPool.value = '';
  }
...
const canConfirmDelete = computed(() => {
  ...
  return (
    !!tgt &&
    deleteConfirmName.value.trim() === (tgt.name || tgt.id).trim() &&
...
async function submitDeletePool(): Promise<void> {
  ...
  if (!pool || !canConfirmDelete.value) return;
  const name = pool.name || pool.id;
  const wipe = deleteMode.value === 'wipe';
  deleteSubmitting.value = true;
  deleteMsg.value = {
    kind: 'info',
    text: wipe ? t('storage.deleteWiping') : t('storage.deleteExporting'),
  };
  try {
    ...
    void loadSnapshots();
    let text = wipe
      ? t('storage.deleteWiped', { name, count: resp.wiped_disks.length, disks: resp.wiped_disks.map(bareName).join(' / ') })
      : t('storage.deleteExported', { name });
    if (resp.warning) text += t('storage.deleteWarningSuffix', { warning: resp.warning });
    poolDeleteMsg.value = { kind: 'ok', text };
    if (wipe) importMsg.value = null; // 清掉旧导入提示，避免误导（wipe 后无可导入池）
  } catch (e) {
    deleteMsg.value = {
      kind: 'err',
      text: t('storage.deleteFailed') + (e instanceof Error ? e.message : String(e)),
    };
  } finally {
...
async function submitInit(): Promise<void> {
  const disk = initTarget.value;
  if (!disk || !canConfirmInit.value) return;
  initSubmitting.value = true;
  initMsg.value = { kind: 'info', text: t('storage.initWiping') };
  try {
    const resp = await endpoints.initializeDisk(bareName(disk.name));
    initTarget.value = null;
    ...
    createPoolMsg.value = {
      kind: 'ok',
      text: t('storage.initDone', { name: disk.name, wiped: wiped ? t('storage.initWipedCount', { count: wiped }) : '' }),
    };
    // 清理该盘的分区详情缓存（数据已过期）
    const next = { ...partitionsCache.value };
    delete next[disk.name];
    partitionsCache.value = next;
    detailDiskName.value = '';
  } catch (e) {
    initMsg.value = {
      kind: 'err',
      text: t('storage.initFailed') + (e instanceof Error ? e.message : String(e)),
    };
  } finally {
...
async function submitCreatePool(): Promise<void> {
  ...
  if (!name) {
    createPoolMsg.value = { kind: 'err', text: t('storage.needPoolName') };
    return;
  }
  if (confirmName.value.trim() !== name) {
    createPoolMsg.value = { kind: 'err', text: t('storage.confirmNameMismatch') };
    return;
  }
  const dataDisks = Array.from(selectedDiskNames.value);
  if (!dataDisks.length) {
    createPoolMsg.value = { kind: 'err', text: t('storage.needOneDisk') };
    return;
  }
  ...
  createPoolMsg.value = { kind: 'info', text: t('storage.creating') };
  try {
    ...
    createPoolMsg.value = {
      kind: 'err',
      text: t('storage.createFailed') + (e instanceof Error ? e.message : String(e)),
    };
  } finally {
...
async function submitCreateDataset(): Promise<void> {
  ...
  if (!nameFrag) {
    createDsMsg.value = { kind: 'err', text: t('storage.needDsName') };
    return;
  }
  if (!pool) {
    createDsMsg.value = { kind: 'err', text: t('storage.needDsPool') };
    return;
  }
  ...
  if (nameFrag.includes('/')) {
    createDsMsg.value = {
      kind: 'err',
      text: t('storage.dsNameSegmentHint'),
    };
    return;
  }
  ...
  if (quotaRaw) {
    ...
    if (!Number.isFinite(gib) || gib <= 0) {
      createDsMsg.value = { kind: 'err', text: t('storage.quotaPositive') };
      return;
    }
  ...
  createDsMsg.value = { kind: 'info', text: t('storage.creating') };
  try {
...
async function submitCreateSnapshot(): Promise<void> {
  ...
  if (!dataset) {
    createSnapMsg.value = { kind: 'err', text: t('storage.needSnapDataset') };
    return;
  }
  if (!name) {
    createSnapMsg.value = { kind: 'err', text: t('storage.needSnapName') };
    return;
  }

  createSnapSubmitting.value = true;
  createSnapMsg.value = { kind: 'info', text: t('storage.creating') };
  try {
    ...
    const friendly = /404|405|未匹配|not found|method not allowed/i.test(m)
      ? t('storage.snapCreateUnsupported')
      : t('storage.createFailed') + m;
...
async function deleteSnapshot(s: Snapshot): Promise<void> {
  const id = s.id;
  if (!id) return;
  if (!window.confirm(t('storage.snapDeleteConfirm', { id }))) return;
  ...
  try {
    ...
    const m = e instanceof Error ? e.message : String(e);
    const friendly = /404|405|未匹配|not found|method not allowed/i.test(m)
      ? t('storage.snapDeleteUnsupported')
      : t('storage.deleteFailed') + m;
...
const poolColumns: Column<Pool>[] = [
  { key: 'name', title: t('storage.colPoolName'), accessor: (p) => p.name || p.id },
  { key: 'health', title: t('storage.colHealth'), width: '100px' },
  { key: 'capacity', title: t('storage.colCapacity'), width: '360px' },
  { key: 'vdevs', title: t('storage.colVdevs'), width: '90px', align: 'right', sortable: true,
    accessor: (p) => (p.vdevs || []).length },
  { key: 'actions', title: t('storage.colActions'), width: '110px', align: 'right' },
];

const datasetColumns: Column<Dataset>[] = [
  { key: 'name', title: t('storage.colName'), accessor: (d) => d.name || d.id },
  { key: 'pool', title: t('storage.colPool'), width: '120px', sortable: true },
  { key: 'used', title: t('storage.colUsed'), width: '120px', align: 'right', sortable: true,
    accessor: (d) => d.used_bytes },
  { key: 'avail', title: t('storage.colAvail'), width: '120px', align: 'right', sortable: true,
    accessor: (d) => d.avail_bytes },
  { key: 'mounted', title: t('storage.colMounted'), width: '100px' },
  { key: 'encryption', title: t('storage.colEncryption'), width: '110px' },
];

const snapshotColumns: Column<Snapshot>[] = [
  { key: 'id', title: t('storage.colName') },
  { key: 'dataset', title: t('storage.colDataset'), width: '180px' },
  { key: 'created', title: t('storage.colCreated'), width: '180px', sortable: true,
    accessor: (s) => s.created },
  { key: 'used', title: t('storage.colSize'), width: '120px', align: 'right', sortable: true,
...
function encryptionMeta(enc: string): { health: 'healthy' | 'unhealthy' | 'unknown'; label: string } {
  switch ((enc || '').toLowerCase()) {
    case 'unlocked':
      return { health: 'healthy', label: t('storage.encUnlocked') };
    case 'locked':
      return { health: 'unhealthy', label: t('storage.encLocked') };
    default:
      return { health: 'unknown', label: t('storage.encPlain') };
  }
}
...
<template>
  <div class="storage-page">
    ...
    <section v-show="activeTab === 'pools'" class="panel">
      ...
      <div v-if="importablePools.length" class="card import-banner">
        <div class="import-banner-head">
          💾 {{ t('storage.importableFound', { count: importablePools.length }) }}
          <span v-if="importableLoading" class="hint">{{ t('storage.probingSuffix') }}</span>
        </div>
        <div
        ...
        >
          ...
          >
            {{ importingPool === p.name ? t('storage.importingShort') : t('storage.importPool') }}
          </button>
        </div>
      ...
      <div v-if="poolsError" class="error-box">{{ t('storage.loadFailed', { err: poolsError }) }}</div>
      ...
      <div class="card card-table">
        <DataTable
          ...
          :empty-text="
            zfsUnavailable ? t('storageZfs.poolsEmpty') : t('storage.poolsEmptyHint')
          "
        >
          ...
          <template #cell-actions="{ row }">
            <button class="btn btn-small" @click.stop="openPoolDetail(row)">{{ t('storage.detail') }}</button>
          </template>
        </DataTable>
      </div>
    </section>
    ...
    <section v-show="activeTab === 'datasets'" class="panel">
      <div class="panel-head">
        <h3>{{ t('storage.datasetsTitle') }}</h3>
        <div class="head-actions">
          <div class="filter-group">
            <label for="ds-pool-filter">{{ t('storage.filterByPool') }}</label>
            <select id="ds-pool-filter" v-model="poolFilter" @change="onFilterChange">
              <option value="">{{ t('storage.filterAll') }}</option>
      ...
      <div v-if="datasetsError" class="error-box">{{ t('storage.loadFailed', { err: datasetsError }) }}</div>
      ...
      <div class="card card-table">
        <DataTable
          ...
          :empty-text="zfsUnavailable ? t('storageZfs.datasetsEmpty') : t('storage.datasetsEmptyHint')"
        >
          ...
          <template #cell-mounted="{ row }">
            <span :class="['pill', row.mounted ? 'pill-ok' : 'pill-muted']">
              {{ row.mounted ? t('storage.mountedYes') : t('storage.mountedNo') }}
            </span>
          </template>
    ...
    <section v-show="activeTab === 'snapshots'" class="panel">
      <div class="panel-head">
        <h3>{{ t('storage.snapshotsTitle') }}</h3>
        <button
          class="btn btn-primary"
          :disabled="zfsUnavailable"
          :title="zfsUnavailable ? t('storageZfs.createDisabledTip') : undefined"
      ...
      <div class="card card-table">
        <DataTable
          ...
          :empty-text="zfsUnavailable ? t('storageZfs.snapshotsEmpty') : t('storage.snapshotsEmptyHint')"
        >
          ...
          <template #cell-actions="{ row }">
            ...
            >
              {{ deletingSnapshotId === row.id ? t('storage.deleting') : t('storage.delete') }}
            </button>
          </template>
        </DataTable>
      </div>
    ...
    <section v-if="activeTab === 'files'" class="panel">
      <div class="panel-head">
        <h3>{{ t('storage.filesTitle') }}</h3>
      </div>
      <p class="hint">
        {{ t('storage.filesHint1') }} <code>/tank</code> {{ t('storage.filesHint2') }}
      </p>
    ...
    <div v-if="showCreatePool" class="modal-backdrop" @click.self="closeCreatePool">
      <div class="modal modal-wizard" role="dialog" aria-modal="true" aria-labelledby="create-pool-title">
        <div class="modal-head">
          <h3 id="create-pool-title">{{ t('storage.wizardTitle') }}</h3>
        ...
        <div class="stepper">
          <div
            v-for="s in [{n:1,t:t('storage.stepSelect')},{n:2,t:t('storage.stepConfigure')},{n:3,t:t('storage.stepConfirm')}]"
            :key="s.n"
            class="step"
        ...
        <div class="modal-body">
          ...
          <div v-if="wizardStep === 1" class="wizard-split">
            ...
            <div class="split-left">
              <div class="split-head">
                <span class="split-title">{{ t('storage.availableDisks') }}</span>
                <button
                  type="button"
                  class="btn btn-small"
                ...
                >
                  {{ selectedDiskNames.size === usableDataDisks.length && usableDataDisks.length
                    ? t('storage.clearSel') : t('storage.selectAll') }}
                </button>
              </div>
              <p v-if="disksLoading" class="hint">{{ t('storage.detectingDisks') }}</p>
              <p v-else-if="disksError" class="hint hint-warn">{{ t('storage.detectDisksFailed', { err: disksError }) }}</p>
              <p v-else-if="!dataDiskOptions.length" class="hint hint-warn">
                    {{ t('storage.noDataDisks') }}
                  </p>
                ...
                <div v-for="d in dataDiskOptions" :key="d.name" class="disk-item">
                    ...
                    <div class="disk-card-main">
                      <div class="disk-card-row">
                        ...
                        <span v-if="memberOfPool(d)" class="init-flag member-flag">
                          {{ t('storage.diskInPool', { pool: memberOfPool(d) }) }}
                        </span>
                        <span v-else-if="importableHint(d)" class="init-flag hint-flag">
                          {{ t('storage.diskInImportable', { pool: importableHint(d) }) }}
                        </span>
                        <span v-else-if="needsInit(d)" class="init-flag">{{ t('storage.needsInit') }}</span>
                      </div>
                  ...
                  <div v-if="memberOfPool(d)" class="init-warn member-warn">
                    <div class="init-warn-text">
                      {{ t('storage.diskActivePoolPrefix', { name: d.name }) }}
                      <strong>{{ memberOfPool(d) }}</strong>
                      {{ t('storage.diskActivePoolHint') }}
                    </div>
                    <div class="init-warn-actions">
                      ...
                      >
                        {{ detailDiskName === d.name ? t('storage.collapseDetail') : t('storage.viewDetail') }}
                      </button>
                    </div>
                  </div>
                  ...
                  <div v-else-if="importableHint(d)" class="init-warn importable-warn">
                    <div class="init-warn-text">
                      {{ t('storage.diskExportedPoolPrefix', { name: d.name }) }}
                      <strong>{{ importableHint(d) }}</strong>
                      {{ t('storage.diskExportedPoolHint') }}
                    </div>
                    <div class="init-warn-actions">
                      <button
                        type="button"
                      ...
                      >
                        {{ importingPool === importableHint(d) ? t('storage.importingShort') : t('storage.importPool') }}
                      </button>
                    </div>
                  </div>
                  ...
                  <div v-else-if="needsInit(d)" class="init-warn">
                    <div class="init-warn-text">
                      ⚠️ {{ t('storage.diskHasSignatures', { name: d.name, sigs: signaturesText(d) }) }}
                      {{ t('storage.diskHasSignaturesHint') }}
                    </div>
                    <div class="init-warn-actions">
                      <button
                        type="button"
                  ...
                  <div v-if="detailDiskName === d.name" class="part-detail">
                    <p v-if="partitionsLoading === d.name" class="hint">{{ t('storage.readingPartitions') }}</p>
                    <template v-else-if="partitionsCache[d.name]">
                      ...
                      <p v-else class="hint">{{ t('storage.noPartitions') }}</p>
                    </template>
                  </div>
                </div>
              </div>
            ...
            <div class="split-right">
              <div class="split-head">
                <span class="split-title">{{ t('storage.selectedSummary') }}</span>
              </div>
              <div class="summary-card">
                <div class="summary-row big">
                  <span class="summary-label">{{ t('storage.selDisks') }}</span>
                  <span class="summary-val">{{ t('storage.disksCount', { count: selectedDiskNames.size }) }}</span>
                </div>
                <div class="summary-row">
                  <span class="summary-label">{{ t('storage.totalCapacity') }}</span>
                  <span class="summary-val mono">{{ formatBytes(capacity.total) }}</span>
                </div>
                <div class="summary-row">
                  <span class="summary-label">{{ t('storage.minDisk') }}</span>
                  <span class="summary-val mono">{{ formatBytes(selectedMinMax.min) }}</span>
                </div>
                <div class="summary-row">
                  <span class="summary-label">{{ t('storage.maxDisk') }}</span>
                  <span class="summary-val mono">{{ formatBytes(selectedMinMax.max) }}</span>
                </div>
              </div>

              <div v-if="selectedDiskNames.size" class="mode-hints">
                <p class="hint">{{ t('storage.availableModes') }}</p>
                <ul class="hint-list">
              ...
              <p v-else class="hint hint-warn">{{ t('storage.pickDisksFirst') }}</p>
            </div>
          </div>
          ...
          <div v-else-if="wizardStep === 2" class="wizard-split">
            ...
            <div class="split-left">
              <div class="split-head">
                <span class="split-title">{{ t('storage.storageMode') }}</span>
                <span class="hint">{{ t('storage.basedOnDisks', { count: selectedDiskNames.size }) }}</span>
              </div>
              <div class="mode-grid">
                <button
                ...
                >
                  ...
                  <div class="mode-cap">
                    {{ t('storage.usablePrefix') }} <strong class="mono">{{ formatBytes(c.cap.usable) }}</strong>
                  </div>
            ...
            <div class="split-right">
              <div class="split-head">
                <span class="split-title">{{ t('storage.advancedOptions') }}</span>
              </div>
              <div class="field">
                ...
                <div class="radio-row">
                  ...
                  <label class="radio-item disabled">
                    ...
                    <span>ext4 <em>{{ t('storage.comingSoon') }}</em></span>
                  </label>
                  <label class="radio-item disabled">
                    <input type="radio" value="btrfs" disabled />
                    <span>Btrfs <em>{{ t('storage.comingSoon') }}</em></span>
                  </label>
                </div>
                <p class="hint">{{ t('storage.fsZfsOnly') }}</p>
              </div>

              <div class="field">
                <label for="pool-name">{{ t('storage.poolNameLabel') }}</label>
                <input
                  id="pool-name"
                  v-model="poolName"
                  type="text"
              ...
              <div class="advanced-toggle">
                <button type="button" class="link-btn" @click="showAdvanced = !showAdvanced">
                  {{ showAdvanced ? '▾' : '▸' }} {{ t('storage.cacheDisksTitle') }}
                </button>
              </div>
              <div v-if="showAdvanced" class="advanced-box">
                <div class="field">
                  <label>{{ t('storage.l2arcLabel') }}</label>
                  <p v-if="!l2arcDiskOptions.length" class="hint hint-warn">{{ t('storage.noCacheDisks') }}</p>
                ...
                <div class="field">
                  <label>{{ t('storage.zilLabel') }}</label>
                  <p v-if="!logDiskOptions.length" class="hint hint-warn">{{ t('storage.noLogDisks') }}</p>
                  <div v-else class="mini-disk-list">
                    <label
          ...
          <div v-else class="confirm-wrap">
            <div class="confirm-card">
              <div class="confirm-section">
                <div class="confirm-label">{{ t('storage.confirmDisks', { count: selectedDiskNames.size }) }}</div>
                <div class="chip-row">
              ...
              <div class="confirm-grid">
                <div class="confirm-row">
                  <span class="confirm-label">{{ t('storage.storageMode') }}</span>
                  <span>{{ MODE_META[raidMode].label }}</span>
                </div>
                <div class="confirm-row">
                  <span class="confirm-label">{{ t('storage.filesystem') }}</span>
                  <span>ZFS</span>
                </div>
                <div class="confirm-row">
                  <span class="confirm-label">{{ t('storage.poolNameShort') }}</span>
                  <span class="mono">{{ poolName || t('storage.notFilled') }}</span>
                </div>
                <div class="confirm-row">
                  <span class="confirm-label">{{ t('storage.estTotal') }}</span>
                  <span class="mono">{{ formatBytes(capacity.total) }}</span>
                </div>
                <div class="confirm-row">
                  <span class="confirm-label">{{ t('storage.estUsable') }}</span>
                  <span class="mono strong">{{ formatBytes(capacity.usable) }}</span>
                </div>
                <div class="confirm-row">
                  <span class="confirm-label">{{ t('storage.redundancy') }}</span>
                  <span>{{ faultText(raidMode, capacity.faultTolerance) }}</span>
                </div>
              </div>
              <div v-if="logDiskNames.size || l2arcDiskNames.size" class="confirm-section">
                <div class="confirm-label">{{ t('storage.cacheDisks') }}</div>
            ...
            <div class="warn-box">
              {{ t('storage.wizardWarnPrefix') }}<strong>{{ t('storage.wizardWarnStrong') }}</strong>{{ t('storage.wizardWarnSuffix') }}
            </div>

            <div class="field">
              <label for="confirm-name">{{ t('storage.confirmPoolNamePrompt', { name: poolName || '...' }) }}</label>
              <input
                id="confirm-name"
                v-model="confirmName"
                type="text"
          ...
          <div class="form-actions">
            ...
            >
              {{ createPoolSubmitting ? t('storage.creating') : t('storage.createBtn') }}
            </button>
          </div>
        </div>
      </div>
    ...
    <div v-if="initTarget" class="modal-backdrop" @click.self="closeInit">
      <div class="modal modal-init" role="dialog" aria-modal="true" aria-labelledby="init-disk-title">
        <div class="modal-head">
          <h3 id="init-disk-title">{{ t('storage.initTitle', { name: initTarget.name }) }}</h3>
        ...
        <div class="modal-body">
          ...
          <template v-if="initStep === 1">
            <div class="warn-box">
              {{ t('storage.initWarn1', { name: initTarget.name, sigs: signaturesText(initTarget) }) }}
              <strong>{{ t('storage.initWarn2') }}</strong>
            </div>
            <p class="hint">
              {{ t('storage.initWarnForeign') }}
            </p>
          </template>
          <!-- 第二步：输入磁盘名确认 -->
          <template v-else>
            <div class="warn-box">
              {{ t('storage.initConfirmPrompt', { name: bareName(initTarget.name) }) }}
            </div>
            <div class="field">
              <label for="init-confirm-name">{{ t('storage.initConfirmLabel') }}</label>
              <input
          ...
          <div class="form-actions">
            ...
            <button
              ...
              @click="submitInit"
            >
              {{ initSubmitting ? t('storage.initializing') : t('storage.confirmInit') }}
            </button>
          </div>
        </div>
      </div>
    ...
    <div v-if="deleteTarget" class="modal-backdrop" @click.self="closeDeletePool">
      <div class="modal modal-init" role="dialog" aria-modal="true" aria-labelledby="del-pool-title">
        <div class="modal-head">
          <h3 id="del-pool-title">{{ t('storage.delTitle', { name: deleteTarget.name || deleteTarget.id }) }}</h3>
        ...
        <div class="modal-body">
          <!-- 摘要：成员盘 + 容量 -->
          <div class="warn-box">
            {{ t('storage.delIntro', { name: deleteTarget.name || deleteTarget.id }) }}
            {{ t('storage.delCapacity') }}
            <span class="mono">{{ formatBytes(deleteTarget.capacity?.total_bytes) }}</span>{{ t('storage.delUsedInfix', { used: formatBytes(deleteTarget.capacity?.used_bytes) }) }}
            <span class="mono">{{ formatBytes(deleteTarget.capacity?.used_bytes) }}</span>），
            {{ t('storage.delMembers') }}
            <span v-if="poolMemberDisks(deleteTarget).length" class="mono">
              {{ poolMemberDisks(deleteTarget).join(' / ') }}
            </span>
            <span v-else class="muted">{{ t('storage.delMembersUnknown') }}</span>
            {{ t('storage.delDatasetsNote') }}
          </div>

          <!-- 两种删除模式单选（默认保留标签，可重新导入） -->
          <div class="del-modes" role="radiogroup" :aria-label="t('storage.deleteMode')">
            <label class="del-mode" :class="{ checked: deleteMode === 'export' }">
              ...
              <div class="del-mode-body">
                <strong>{{ t('storage.delModeExport') }}</strong>
                <p class="hint">
                  {{ t('storage.delModeExportDesc1') }}
                  {{ t('storage.delModeExportDesc2') }}
                </p>
              </div>
            </label>
            <label class="del-mode is-danger" :class="{ checked: deleteMode === 'wipe' }">
              ...
              />
              <div class="del-mode-body">
                <strong class="danger-text">{{ t('storage.delModeWipe') }}</strong>
                <p class="hint danger-text">
                  {{ t('storage.delModeWipeDesc1') }}<strong>{{ t('storage.delModeWipeDesc2') }}</strong>{{ t('storage.delModeWipeDesc3') }}
                </p>
              </div>
            </label>
          </div>
          ...
          <div class="field">
            <label for="del-confirm-name">
              {{ t('storage.delConfirmPrompt', { name: deleteTarget.name || deleteTarget.id }) }}
            </label>
            <input
              id="del-confirm-name"
    ...
    <div v-if="showCreateDataset" class="modal-backdrop" @click.self="closeCreateDataset">
      <div class="modal" role="dialog" aria-modal="true" aria-labelledby="create-ds-title">
        <div class="modal-head">
          <h3 id="create-ds-title">{{ t('storage.createDsTitle') }}</h3>
        ...
        <form class="modal-body" @submit.prevent="submitCreateDataset">
          <div class="field">
            <label for="ds-name">{{ t('storage.dsNameLabel') }}</label>
            <input
              id="ds-name"
              v-model="createDsForm.name"
              type="text"
              :placeholder="t('storage.dsNamePlaceholder')"
              :disabled="createDsSubmitting"
            />
            <p class="hint">{{ t('storage.dsPathHint') }}</p>
          </div>
          <div class="field">
            <label for="ds-pool">{{ t('storage.dsPoolLabel') }}</label>
            <select id="ds-pool" v-model="createDsForm.pool" :disabled="createDsSubmitting">
              <option value="" disabled>{{ t('storage.selectPoolPrompt') }}</option>
              <option v-for="name in poolFilterOptions" :key="name" :value="name">{{ name }}</option>
            </select>
            <p v-if="!poolFilterOptions.length" class="hint hint-warn">{{ t('storage.noPoolsYet') }}</p>
          </div>
          <div class="field">
            <label for="ds-quota">{{ t('storage.quotaLabel') }}</label>
            <input
              id="ds-quota"
          ...
          <div class="form-actions">
            ...
            <button type="submit" class="btn btn-primary" :disabled="createDsSubmitting">{{ t('storage.submit') }}</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div v-if="showCreateSnapshot" class="modal-backdrop" @click.self="closeCreateSnapshot">
      <div class="modal" role="dialog" aria-modal="true" aria-labelledby="create-snap-title">
        <div class="modal-head">
          <h3 id="create-snap-title">{{ t('storage.createSnapTitle') }}</h3>
        ...
        <form class="modal-body" @submit.prevent="submitCreateSnapshot">
          <div class="field">
            <label for="snap-dataset">{{ t('storage.snapDatasetLabel') }}</label>
            <select id="snap-dataset" v-model="createSnapForm.dataset" :disabled="createSnapSubmitting">
              <option value="" disabled>{{ t('storage.selectDsPrompt') }}</option>
              <option v-for="d in datasets" :key="d.id" :value="d.name">{{ d.name }}</option>
            </select>
            <p v-if="!datasets.length" class="hint hint-warn">{{ t('storage.noDatasetsYet') }}</p>
          </div>
          <div class="field">
            <label for="snap-name">{{ t('storage.snapNameLabel') }}</label>
            <input
              id="snap-name"
              v-model="createSnapForm.name"
              type="text"
              :placeholder="t('storage.snapNamePlaceholder')"
          ...
          <div class="form-actions">
            ...
            <button type="submit" class="btn btn-primary" :disabled="createSnapSubmitting">{{ t('storage.submit') }}</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div v-if="detailPool" class="modal-backdrop" @click.self="closePoolDetail">
      <div class="modal" role="dialog" aria-modal="true" aria-labelledby="pool-detail-title">
        <div class="modal-head">
          <h3 id="pool-detail-title">{{ t('storage.poolDetailTitle', { name: detailPool.name || detailPool.id }) }}</h3>
          <button class="modal-close" type="button" @click="closePoolDetail">×</button>
        </div>
        <div class="modal-body">
          <div class="detail-row">
            <span class="detail-label">{{ t('storage.detailHealth') }}</span>
            <HealthBadge :health="detailPool.health" />
          </div>
          <div class="detail-row">
            <span class="detail-label">{{ t('storage.detailUsage') }}</span>
            <CapacityBar :capacity="detailPool.capacity" :show-text="true" />
          </div>
          <div class="detail-row">
            <span class="detail-label">{{ t('storage.detailTotal') }}</span>
            <span class="mono">{{ formatBytes(detailPool.capacity?.total_bytes) }}</span>
          </div>
          <div class="detail-row">
            <span class="detail-label">{{ t('storage.colUsed') }}</span>
            <span class="mono">{{ formatBytes(detailPool.capacity?.used_bytes) }}（{{ poolPct(detailPool) }}%）</span>
          </div>
          <div class="detail-block">
            <div class="detail-label">{{ t('storage.detailVdevLayout') }}</div>
              ...
              <div v-for="(v, i) in detailPool.vdevs" :key="i" class="vdev-item">
                <div class="vdev-head">
                  ...
                  <span class="vdev-errs" v-if="v.read_errors || v.write_errors || v.cksum_errors">
                    {{ t('storage.vdevErrors', { read: v.read_errors, write: v.write_errors, cksum: v.cksum_errors }) }}
                  </span>
                </div>
                <div class="vdev-disks mono">
                  <span v-for="(dsk, j) in v.disks" :key="j" class="disk-chip">{{ dsk }}</span>
                  <span v-if="!v.disks || !v.disks.length" class="muted">{{ t('storage.noDisks') }}</span>
                </div>
              </div>
            </div>
            <p v-else class="muted">{{ t('storage.noVdevInfo') }}</p>
          </div>
          <div class="form-actions">
            <button
              type="button"
            ...
            >
              {{ t('storage.deletePoolDots') }}
            </button>
            <button type="button" class="btn" @click="closePoolDetail">{{ t('storage.close') }}</button>
          </div>
        </div>
      </div>
    </div>
