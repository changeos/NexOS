...
async fn cli_script_served_public_through_gateway() {
    ...
    let script = resp
        .body
        .as_str()
        .expect("body 应为脚本原文（Value::String）");
    assert!(
        script.starts_with("#!/usr/bin/env"),
        "脚本原文（非 JSON 信封）"
    );
...
async fn cli_script_host_derivation_through_gateway() {
    ...
    let forwarded = g
        ...
        .0;
    assert!(
        node_default_line(forwarded.body.as_str().unwrap()).contains("'http://203.0.113.77:8558'")
    );
    ...
    let host_only = g
        ...
        .0;
    assert!(
        node_default_line(host_only.body.as_str().unwrap())
            .contains("'http://hub.example.com:8558'"),
        "无端口补 8558"
    );

    let none = g.dispatch(req(serde_json::json!({}))).await.0;
    assert!(node_default_line(none.body.as_str().unwrap()).contains("'http://127.0.0.1:8558'"));
}
