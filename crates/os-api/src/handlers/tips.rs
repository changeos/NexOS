...
fn open_source_ro(path: &str) -> Result<Connection, String> {
    ...
    // 只读连接同样补 busy_timeout（非 WAL 模式下读也会撞写锁）
...
///
/// 持有 `Arc<Mutex<Connection>>`（短锁快放，不跨 `.await` 持锁；Arc 化仅为
/// `spawn_blocking` 闭包可持 'static 句柄——锁结构不变，仍是组件级单连接
/// 单互斥，审计 A1-1/Top1 批 1）+ 可选的两只 [`ChainAuth`]（IM /
/// nexhub+api-market，main.rs 装配时注入共享实例——handler 自验链上 token
/// 反查 from pubkey）。
...
impl TipsRouteHandler {
    ...
    /// DB 短事务统一入口（审计 A1-1/Top1 批 1，2026-09-25，与 im.rs /
    /// api_market.rs 同款）：闭包在 `spawn_blocking` 线程里拿锁执行——锁等待
    /// 与 SQLite 同步 IO 离开 tokio worker。锁语义不变：同一把组件级互斥锁、
    /// 同一条连接、短锁快放不跨 `.await`；panic 原样上抛（等价原
    /// `expect("tips db poisoned")`）。
        ...
        F: FnOnce(&Connection) -> T + Send + 'static,
    {
        ...
        match tokio::task::spawn_blocking(move || {
            let conn = db.lock().expect("tips db poisoned");
            f(&conn)
        })
        .await
        {
    ...
    async fn handle_create(&self, req: ApiRequest) -> Result<ApiResponse, ApiGatewayError> {
        ...
        let id = self
            .db_call(move |conn| {
                insert_tip(conn, &row_for_db).map_err(|e| {
                    ApiGatewayError::Internal(format!("打赏落账失败: {e}"))
                })
            })
            .await?;
        eprintln!(
    ...
    async fn handle_target(&self, kind: &str, ref_: &str) -> Result<ApiResponse, ApiGatewayError> {
        ...
        let (total, count, recent) = {
            let kind = kind.to_string();
            let ref_owned = ref_.to_string();
            self.db_call(move |conn| {
                let (total, count) = aggregate_target(conn, &kind, &ref_owned)
                    .map_err(|e| ApiGatewayError::Internal(format!("聚合查询失败: {e}")))?;
                let recent = recent_for_target(conn, &kind, &ref_owned, RECENT_LIMIT).map_err(
                    |e| ApiGatewayError::Internal(format!("recent 查询失败: {e}")),
                )?;
                Ok::<_, ApiGatewayError>((total, count, recent))
            })
            .await?
        };
    ...
    async fn handle_me(&self, identity: &str) -> Result<ApiResponse, ApiGatewayError> {
        let (((recv_total, recv_count), (give_total, give_count)), received, given) = {
            let identity = identity.to_string();
            self.db_call(move |conn| {
                let agg = aggregate_identity(conn, &identity).map_err(|e| {
                    ApiGatewayError::Internal(format!("身份聚合查询失败: {e}"))
                })?;
                let received = recent_received(conn, &identity, ME_RECENT_LIMIT).map_err(|e| {
                    ApiGatewayError::Internal(format!("received 查询失败: {e}"))
                })?;
                let given = recent_given(conn, &identity, ME_RECENT_LIMIT).map_err(|e| {
                    ApiGatewayError::Internal(format!("given 查询失败: {e}"))
                })?;
                Ok::<_, ApiGatewayError>((agg, received, given))
            })
            .await?
        };
        Ok(ok_json(serde_json::json!({
...
mod tests {
    ...
    #[test]
    fn open_ledger_sets_busy_timeout_pragma() {
        // 审计 E#6（2026-09-24 批 1/3）：全部文件型连接补 busy_timeout=3000ms
        // 防 SQLITE_BUSY 立败——此处以 tips open 点为代表断言 PRAGMA 生效
        //（其余 17 处连接点同一插入手法，见交付报告）。
        let path = tmp_db_path("busy");
        let conn = open_ledger(path.to_str().unwrap()).expect("打开测试账本");
        let ms: i64 = conn
            .query_row("PRAGMA busy_timeout", [], |r| r.get(0))
            .expect("读 busy_timeout pragma");
        assert_eq!(ms, 3000, "busy_timeout 应为 3000ms");
