...
/// LLM DB 短事务统一入口（DB 收敛第三批，2026-09-25，照抄 im.rs `im_db_call`
...
///
/// 锁语义与原先完全一致：同一把组件级互斥锁（`llm.db` 单连接，与
/// llm_envs / llm_external 子模块共享）、短锁快放不跨 `.await`；闭包 panic
/// 经 `resume_unwind` 原样上抛（等价原 `expect`）。供 [`LlmRouteHandler::db_call`]
/// 与持有 `Arc<Mutex<Connection>>` 的 spawn 监测任务（换口重试落库）共用。
async fn llm_db_call<T, F>(db: Arc<Mutex<Connection>>, f: F) -> T
where
    T: Send + 'static,
...
impl LlmRouteHandler {
    ...
    /// 注入统一任务登记表到推理环境子模块（v0.1.54 统一任务框架：环境
    /// 创建/更新任务双写 unified_tasks，kind=llm_env；轻旁路）。须在
    /// `Arc::new` 之前调（main.rs 装配序）。
    pub fn set_env_unified_tasks(
        &mut self,
        store: std::sync::Arc<crate::handlers::tasks_common::UnifiedTaskStore>,
    ) {
        self.env_state.set_unified_tasks(store);
    }
    ...
    /// LLM DB 短事务统一入口（DB 收敛第三批，2026-09-25）：闭包在
    /// `spawn_blocking` 线程里拿锁执行——锁等待与 SQLite 同步 IO 全部离开
    /// tokio worker（批 1 im.rs / 批 2 api_gateway 同款手法；锁结构不变，
    /// 仍是组件级单连接单互斥，与 llm_envs / llm_external 共享同一把锁）。
    async fn db_call<T, F>(&self, f: F) -> T
    where
        T: Send + 'static,
        F: FnOnce(&Connection) -> T + Send + 'static,
    {
        llm_db_call(Arc::clone(&self.db), f).await
    }

    /// 实例定义变更后同步落表（写失败仅影响重启恢复，不影响当次请求——
    /// 内存态才是运行时真值；错误吞掉并继续，与 forwarding 同款容错）。
    ///
    /// DB 收敛第三批（2026-09-25）起 async：落库在 `spawn_blocking` 线程
    /// 拿锁执行（[`Self::db_call`]）。
    async fn persist(&self, inst: &ModelInstance) {
        let inst = inst.clone();
    ...
    /// 删除实例时同步删表行（同上容错；第三批起 spawn_blocking）。
    async fn persist_remove(&self, id: &str) {
        let id = id.to_string();
        self.db_call(move |conn| {
    ...
    /// 2026-08-31 起机器 venv 由推理环境注册表（[`llm_envs`] 子模块）管理：
    /// 查 `llm_environments` 表 is_default=1 且 status=ready 的行 → 其
    /// `<path>/bin/vllm` 与 `<path>/bin`。（DB 收敛第三批起查表在
    /// [`Self::db_call`] 的 spawn_blocking 线程里拿锁执行。）
    async fn default_env_bin(&self) -> (String, String) {
        let resolved = self
            .db_call(|conn| {
                llm_envs::default_ready_env(conn).map(|r| {
                    (
    ...
    async fn env_bin_for(&self, env_name: Option<&str>) -> Result<(String, String), String> {
        let Some(name) = env_name else {
            return Ok(self.default_env_bin().await);
        };
        let name = name.to_string();
        let name_for_db = name.clone();
        let row = self
            .db_call(move |conn| llm_envs::env_row_by_name(conn, &name_for_db))
            .await;
        match row {
    ...
    async fn spawn_vllm(&self, inst: &ModelInstance) -> Result<(u32, String), String> {
        let (vllm_bin, vllm_bin_dir) = self.env_bin_for(inst.env_name.as_deref()).await?;
        ...
        let real_command = format!(
            "{vllm_bin} {}",
            build_vllm_serve_cmd(&inst.model, &config).join(" ")
        );
        let pid = spawned.pid;
    ...
    pub(crate) async fn chat_complete(
        ...
        let mut payload =
            serde_json::to_value(&outbound).map_err(|e| format!("构造推理请求体失败: {e}"))?;
    ...
    /// 失败 502 带原因。
    async fn recipes_recipe_body(&self, hf_id: &str) -> Result<serde_json::Value, (u16, String)> {
    ...
    async fn gateway_models_body(&self) -> GatewayModelsResponse {
        ...
        self.apply_status_correction(&demoted).await;
    ...
    /// 第三批起落库走 [`Self::persist`] 的 spawn_blocking）。
    async fn apply_status_correction(&self, updated: &[ModelInstance]) {
        if updated.is_empty() {
            return;
        }
        {
    ...
    async fn reconcile_instance_statuses(&self) -> Vec<ModelInstance> {
        ...
        let targets: Vec<&ModelInstance> = port_fixed
            ...
            .collect();
        if targets.is_empty() {
            self.apply_status_correction(&need_port_persist).await;
            return port_fixed;
        }
        let probes = targets
            .iter()
        ...
        self.apply_status_correction(&corrected).await;
        out
    }
...
/// 清 error，并落库。（DB 收敛第三批：落库段在 `spawn_blocking` 里拿锁——
/// `llm_db_call`；实例内存态仍在调用方线程短锁完成。）
async fn apply_spawn_retry_to_row(
...
/// （第三批：落库段同样走 `llm_db_call` 的 spawn_blocking。）
async fn mark_instance_error(
    instances: &Mutex<Vec<ModelInstance>>,
    db: Arc<Mutex<Connection>>,
    instance_id: &str,
    err: &str,
        ...
        match spawned.child.try_wait() {
            ...
            Ok(Some(_)) => {
                ...
                if retried {
                    ...
                    mark_instance_error(&instances, Arc::clone(&db), &instance_id, &err).await;
                    return;
                }
                ...
                match spawn_fn(&config) {
                    Ok(sp2) => {
                        ...
                        apply_spawn_retry_to_row(&instances, Arc::clone(&db), &instance_id, new_port, sp2.pid)
                            .await;
                        spawned = sp2;
                        retried = true;
                    }
        ...
        match (req.method, segs.as_slice()) {
            ...
            (HttpMethod::Post, ["api", "v1", "llm", "instances"]) => {
                ...
                if let Some(n) = &env_name {
                    let n_for_db = n.clone();
                    let exists = self
                        .db_call(move |conn| llm_envs::env_row_by_name(conn, &n_for_db).is_some())
                        .await;
                    if !exists {
                        return Ok(error_response(
                            400,
                ...
                self.persist(&instance).await; // 定义持久化（重启恢复用）
                self.instances
                    .lock()
                    .expect("instances poisoned")
                    .push(instance);
            ...
            (HttpMethod::Post, ["api", "v1", "llm", "instances", id, "start"]) => {
                ...
                self.persist(&inst).await; // 启停同步落表
            ...
            (HttpMethod::Post, ["api", "v1", "llm", "instances", id, "stop"]) => {
                // 锁内完成状态翻转（快照即释放；持久化 await 不占 instances 锁）
                let snapshot = {
                    let mut instances = self.instances.lock().expect("instances poisoned");
                    let Some(i) = instances.iter_mut().find(|i| i.id == *id) else {
                        return Ok(error_response(404, &format!("实例不存在: {id}")));
                    };
            ...
            (HttpMethod::Delete, ["api", "v1", "llm", "instances", id]) => {
                ...
                self.persist_remove(id).await; // 删除同步落表
                Ok(ok_json(
            ...
            (HttpMethod::Post, ["api", "v1", "llm", "instances", id, "health"]) => {
                ...
                let snapshot: Option<ModelInstance> = {
                    let mut instances = self.instances.lock().expect("instances poisoned");
                ...
                if let Some(inst) = snapshot {
                    self.persist(&inst).await; // status 翻转（starting→running）同步落表
                }
...
fn create_schema(conn: &Connection) -> rusqlite::Result<()> {
    ...
    let _ = conn.execute(
        "ALTER TABLE llm_instances ADD COLUMN launch_command TEXT",
        [],
    );
    // 推理环境注册表（子模块管理；同库同连接）
...
mod tests {
    ...
    fn apply_unified_meminfo_noop_without_unified() {
        let mut devices =
            vec![
                parse_nvidia_smi_line("0, NVIDIA GeForce RTX 3090, 24576, 1024, 23552, 5").unwrap(),
            ];
    ...
    async fn routes_declares_all_llm_endpoints() {
        ...
        assert_eq!(
            routes.len(),
            30,
            "应有 30 条路由（17 实例 + 7 环境 + 6 外部 API）: {routes:?}"
        );
        assert!(
    ...
    async fn env_bin_for_named_env_requires_exists_and_ready() {
        ...
        assert!(h.env_bin_for(Some("ghost")).await.is_err(), "不存在应 Err");
        assert!(
            h.env_bin_for(Some("staging")).await.is_err(),
            "非 ready 应 Err"
        );
        // 未指定 → 回退语义 Ok
        assert!(h.env_bin_for(None).await.is_ok());
        // ready 后可用
        {
            let conn = h.db.lock().unwrap();
    ...
    async fn instance_responses_expose_launch_command_constructed_when_never_spawned() {
        ...
        assert!(
            cmd.contains("--swap-space 4"),
            "extra_args 应原样追加: {cmd}"
        );
    ...
    fn effective_launch_command_falls_back_to_construction() {
        ...
        assert!(
            cmd.starts_with("vllm serve Qwen/Qwen2.5-7B-Instruct"),
            "{cmd}"
        );
    ...
    async fn recipes_catalog_refresh_failure_keeps_cached_copy() {
        ...
        assert!(
            failed.body["error"].as_str().unwrap().contains("拉取失败"),
            "错误应带原因"
        );
    ...
    async fn recipes_catalog_refresh_clears_recipe_detail_cache() {
        ...
        let mutated_recipe = {
            let mut v: serde_json::Value = serde_json::from_str(&sample_recipe_json()).unwrap();
