...
//!   ratio(16:9|9:16|1:1)/style_hint/status/dir/export_dir/created_at/updated_at。
...
/// 画幅白名单与生图尺寸（宽高均为 8 的倍数——sd-turbo/diffusers 要求）。
const RATIOS: [(&str, u32, u32); 3] = [("16:9", 1272, 720), ("9:16", 720, 1272), ("1:1", 720, 720)];
        ...
        match (req.method, segs.as_slice()) {
            ...
            (HttpMethod::Post, ["api", "v1", "film", "projects"]) => {
                ...
                if ratio_dims(&body.ratio).is_none() {
                    return Ok(error_response(
                        400,
                        &format!("ratio 必须是 16:9 / 9:16 / 1:1（当前 {}）", body.ratio),
                    ));
                }
                let id = self.next_project_id();
                let dir = format!("{}/{}", self.root_dir.trim_end_matches('/'), id);
                if let Err(e) = tokio::fs::create_dir_all(&dir).await {
                    return Ok(error_response(
                        500,
            ...
            (HttpMethod::Put, ["api", "v1", "film", "projects", id]) => {
                ...
                if let Some(r) = &ratio {
                    if ratio_dims(r).is_none() {
                        return Ok(error_response(
                            400,
                            &format!("ratio 必须是 16:9 / 9:16 / 1:1（当前 {r}）"),
                        ));
                    }
                }
            ...
            (HttpMethod::Delete, ["api", "v1", "film", "projects", id]) => {
                ...
                let dir_removed = match tokio::fs::remove_dir_all(&project.dir).await {
                    Ok(()) => true,
...
mod tests {
    ...
    async fn routes_declares_twentyone_film_endpoints_with_auth_matrix() {
        ...
        assert_eq!(
            routes.len(),
            42,
            "应有 42 条路由（21 基线 + 21 FilmHub 新链）: {routes:?}"
        );
        assert!(
    ...
    #[test]
    fn ratio_dims_covers_three_ratios() {
        assert_eq!(ratio_dims("16:9"), Some((1272, 720)));
        assert_eq!(ratio_dims("9:16"), Some((720, 1272)));
        assert_eq!(ratio_dims("1:1"), Some((720, 720)));
        assert_eq!(ratio_dims("4:3"), None, "非法画幅应 None");
    }
    ...
    async fn project_crud_roundtrip_with_artifacts_and_dir_cleanup() {
        ...
        let resp = h
            .handle(post_req(
                "/api/v1/film/projects",
                serde_json::json!({"title":"t","idea":"i","ratio":"4:3"}),
            ))
            .await
            .unwrap();
    ...
    async fn compose_fake_ffmpeg_argv_and_filters_recorded() {
        ...
        assert!(pass1.contains("scale=1272:720"), "16:9 统一尺寸: {pass1}");
