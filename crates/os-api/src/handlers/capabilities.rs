...
pub trait CapabilitySources: Send + Sync {
    ...
    /// ffmpeg 可用性（生产走 probe_utils.rs `detect_ffmpeg()` 的 env→PATH→常规路径链）。
...
impl CapabilitySources for RealSources {
    ...
    fn ffmpeg_available(&self) -> bool {
        crate::handlers::probe_utils::detect_ffmpeg().is_some()
    }
}
...
impl CapabilitiesRouteHandler {
    ...
    #[must_use]
    pub fn new(sources: RealSources, p2p: Option<os_p2p::Handle>) -> Self {
        Self {
...
mod tests {
    ...
    fn pure_build_p2p_enabled_shape() {
        ...
        assert_eq!(v["apps"], serde_json::json!(["demo-app"]));
    ...
    async fn served_through_full_axum_router() {
        ...
        let tmp = std::env::temp_dir().join(format!("nexos-caps-e2e-{}", std::process::id()));
