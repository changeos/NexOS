...
fn latest_snapshot(repo_dir: &std::path::Path) -> Option<std::path::PathBuf> {
    ...
    for entry in read.flatten() {
        ...
        let better = match &best {
            None => true,
            Some((bt, bn, _)) => mtime > *bt || (mtime == *bt && name > *bn),
        };
        if better {
...
pub fn scan_model_weight_detail(root: &str, name: &str) -> Result<ModelWeightDetail, String> {
    ...
    let dir = if own.is_dir() {
        ...
        resolve_hf_snapshot_by_id(name).ok_or_else(|| format!("本地模型不存在: {name}"))?
    };
...
fn hf_delete_guard(root: &str, name: &str) -> Option<String> {
    let in_library = std::fs::symlink_metadata(std::path::Path::new(root).join(name)).is_ok();
    if in_library {
        return None;
    }
...
pub fn validate_repo_id(repo_id: &str) -> Result<(String, String), String> {
    ...
    let Some((org, model)) = id.split_once('/') else {
        return Err(format!(
            "仓库 id 须为 org/model 形态（如 Qwen/Qwen3-VL-8B-Instruct）: {id}"
        ));
    };
    let seg_ok = |s: &str| {
        !s.is_empty()
...
fn remote_dl_concurrency(raw: &str) -> usize {
    let n = raw
        .trim()
        .parse::<usize>()
        .unwrap_or(REMOTE_DL_CONCURRENCY_DEFAULT);
...
async fn run_remote_file_with_retries(
    ...
    for attempt in 1..=REMOTE_FILE_RETRIES {
        ...
        match download_remote_file(kind, repo_id, dir, &mf.name, mf.size_bytes, tasks, task_id)
            .await
        {
            Ok(bytes) => {
...
async fn run_remote_download(
    ...
    let outcomes: Vec<(ManifestFile, Result<(), String>)> =
        futures::stream::iter(plan_files.into_iter().map(|mf| {
            ...
            async move {
                let outcome =
                    run_remote_file_with_retries(kind, &repo_id, &dir, &mf, &tasks, &task_id).await;
                (mf, outcome)
            }
        }))
...
pub fn builtin_spark_zone_entries() -> Vec<SparkZoneEntry> {
    [
        (
            "nv-community/Qwen3.6-27B-NVFP4",
            "27B",
            "Qwen3.6 27B NVFP4（ModelOpt 量化）——27B 级均衡之选，Spark 上留足 KV cache 空间",
        ),
        (
            "unsloth/Qwen3.8-27B-NVFP4",
            "27B",
            "Qwen3.8 27B NVFP4（compressed-tensors 量化）——双源可下，最新一代 27B",
        ),
        (
            "unsloth/Qwen3.6-27B-NVFP4",
            "27B",
            "Qwen3.6 27B NVFP4 unsloth 复刻——与 nv-community 版同源模型的另一发布渠道",
        ),
        (
            "nv-community/Qwen3.6-35B-A3B-NVFP4",
            "35B-A3B",
            "Qwen3.6 35B MoE（激活 3B）NVFP4——MoE 低激活高吞吐，Spark 单机长文本优选",
        ),
        (
            "unsloth/Qwen3.6-35B-A3B-NVFP4",
            "35B-A3B",
            "Qwen3.6 35B-A3B NVFP4 unsloth 复刻——双源可下",
        ),
        (
            "RedHatAI/Qwen3.6-35B-A3B-NVFP4",
            "35B-A3B",
            "Qwen3.6 35B-A3B NVFP4 RedHatAI 发行——NeMo/ModelOpt 生态量化，双源可下",
        ),
        (
            "nv-community/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4",
            "30B-A3B",
            "NVIDIA Nemotron 3.5 Lightning 30B-A3B NVFP4——NVIDIA 官方系 MoE，SM120 原生适配",
        ),
    ]
    .into_iter()
...
fn parse_spark_zone_env(body: &str) -> Result<(Vec<SparkZoneEntry>, bool), String> {
    ...
    match &v {
        serde_json::Value::Array(_) => {
            let entries =
                serde_json::from_value(v).map_err(|e| format!("条目反序列化失败: {e}"))?;
            Ok((entries, false))
        }
...
pub fn spark_zone_entries() -> (Vec<SparkZoneEntry>, String) {
    ...
    let body = match std::fs::read_to_string(&path) {
        ...
        Err(e) => {
            eprintln!(
                "[modelhub] spark-zone: 读取 NEXOS_SPARK_ZONE_FILE={path} 失败（{e}），回退内置表"
            );
    ...
    match parse_spark_zone_env(&body) {
        ...
        Ok((env_entries, replace)) => {
            ...
            for b in &bad {
                eprintln!(
                    "[modelhub] spark-zone: 剔除非法 repo 条目 {}（须为 org/model）",
                    b.repo
                );
            }
            let out = if replace {
...
impl ModelHubRouteHandler {
    ...
    async fn create_remote_task(
        ...
        let plan_files: Vec<ManifestFile> = match files {
            ...
            Some(sel) => {
                ...
                if !unknown.is_empty() {
                    return Ok(error_response(
                        400,
                        &format!("files 含清单外路径: {}", unknown.join(", ")),
                    ));
                }
                probe
                    .files
        ...
        let concurrency = remote_dl_concurrency(
            &std::env::var("NEXOS_MODELHUB_DL_CONCURRENCY").unwrap_or_default(),
        );
        ...
        match (req.method, segs.as_slice()) {
            ...
            (HttpMethod::Get, ["api", "v1", "models", "local", id]) => {
                ...
                let dir = if dir_path.is_dir() {
                    ...
                    match resolve_hf_snapshot_by_id(&id_owned) {
                        Some(p) => p,
                        None => {
                            return Ok(error_response(404, &format!("本地模型不存在: {id_owned}")))
                        }
                    }
                };
            ...
            (HttpMethod::Get, ["api", "v1", "models", "remote", kind, org, model]) => {
                let Some(k) = RemoteRepoKind::parse(kind) else {
                    return Ok(error_response(400, "kind 须为 modelscope 或 hf（HF 镜像）"));
                };
                let repo_id = format!("{org}/{model}");
            ...
            (HttpMethod::Post, ["api", "v1", "models", "remote", "downloads"]) => {
                ...
                let body: CreateRemoteBody = serde_json::from_value(req.body).map_err(|e| {
            ...
            (HttpMethod::Get, ["api", "v1", "models", "spark-zone"]) => {
                let probe = query_param(&req.path, "probe")
                    .map(|v| v != "0")
                    .unwrap_or(true);
...
mod tests {
    ...
    fn latest_snapshot_falls_back_to_mtime_when_no_refs() {
        ...
        assert!(
            models[0].path.ends_with("snapshots/zzz"),
            "应取 mtime 更新的"
        );
        let _ = std::fs::remove_dir_all(&hub);
    }
    ...
    async fn list_local_models_merges_hf_cache_e2e() {
        ...
        // 块作用域保证守卫先于清理 drop。
        {
            ...
            assert!(hub
                .join("models--nvidia--Qwen3.6-27B-NVFP4/snapshots/deadbeef")
                .is_dir());
    ...
    fn encode_path_pure() {
        ...
        assert_eq!(
            encode_path("中文/model.bin"),
            "%E4%B8%AD%E6%96%87/model.bin"
        );
    ...
    fn remote_repo_kind_parse_and_slug() {
        assert_eq!(
            RemoteRepoKind::parse("modelscope"),
            Some(RemoteRepoKind::Modelscope)
        );
        assert_eq!(
            RemoteRepoKind::parse("ModelScope"),
            Some(RemoteRepoKind::Modelscope)
        ...
        assert_eq!(
            RemoteRepoKind::parse("hf_mirror"),
            Some(RemoteRepoKind::HfMirror)
        );
    ...
    fn fake_remote_respond(
        ...
        if path.contains("/repo/files") {
            let mut entries =
                vec![r#"{"Name":"sub","Path":"sub","Size":0,"Type":"tree"}"#.to_string()];
            entries.extend(files.iter().map(|(n, d)| {
                format!(
                    r#"{{"Name":"{n}","Path":"{n}","Size":{},"Type":"blob"}}"#,
                    d.len()
                )
            }));
    ...
    async fn remote_probe_get_endpoint_routing() {
        let fake = FakeRemoteRepo::start(vec![("config.json".into(), b"{}".to_vec())], vec![]);
    ...
    async fn remote_download_e2e_selected_files_range_resume() {
        let f1 = (
            ...
            (0..=255u8).cycle().take(1200).collect::<Vec<u8>>(),
        );
        let f2 = (
            "config.json".to_string(),
            br#"{"model_type":"fake"}"#.to_vec(),
        );
    ...
    async fn remote_download_file_retry_then_fail() {
        ...
        let err = last["error"].as_str().unwrap();
        assert!(
            err.contains("model.safetensors") && err.contains("3 次"),
            "错误应指明文件与次数: {err}"
        );
        assert_eq!(last["files_done"], 0);
        let _ = std::fs::remove_dir_all(&root);
    ...
    async fn remote_download_parallel_failure_isolation() {
        ...
        assert_eq!(
            std::fs::read(dir.join(&fa.0)).unwrap(),
            fa.1,
            "成功件 a 保留"
        );
        assert_eq!(
            std::fs::read(dir.join(&fc.0)).unwrap(),
            fc.1,
            "成功件 c 保留"
        );
    ...
    fn builtin_spark_zone_entries_valid_and_serializable() {
        ...
        for e in &entries {
            ...
            assert!(
                !e.params.is_empty() && !e.note.is_empty(),
                "参数量/简述非空: {}",
                e.repo
            );
        }
        ...
        assert!(
            repos.contains(&"nv-community/Qwen3.6-27B-NVFP4"),
            "缺用户点名仓: {repos:?}"
        );
        assert!(
            repos.contains(&"unsloth/Qwen3.8-27B-NVFP4"),
            "缺用户点名仓: {repos:?}"
        );
        // 无重复 repo
    ...
    fn parse_spark_zone_env_shapes() {
        ...
        assert!(
            parse_spark_zone_env(r#"[{"repo":"a/One"}]"#).is_err(),
            "条目缺字段应 Err"
        );
    }
    ...
    fn spark_zone_env_file_merge_replace_fallback() {
        ...
        let touched = merged
            .iter()
            .find(|e| e.repo == "unsloth/Qwen3.8-27B-NVFP4")
            .unwrap();
        ...
        let _envs3 = ScopedEnvs::set(&[("NEXOS_SPARK_ZONE_FILE", "/no/such/spark-zone-file.json")]);
    ...
    async fn spark_zone_endpoint_probe_semantics_mock() {
        ...
        {
            let fake2 = FakeRemoteRepo::start(vec![("config.json".into(), b"{}".to_vec())], vec![]);
