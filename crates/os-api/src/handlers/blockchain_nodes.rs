...
fn home_dir() -> String {
    ...
    if let Some(u) = std::env::var("USER")
        .ok()
        .filter(|v| v != "root" && !v.is_empty())
    {
...
pub fn resolve_client_bin(client: &str, overrides: &HashMap<String, String>) -> Option<String> {
        ...
        .filter(|v| !v.trim().is_empty())
    {
        ...
        return if is_executable(Path::new(&p)) {
            Some(p)
        } else {
            None
        };
    ...
    candidates.into_iter().find(|c| is_executable(Path::new(c)))
}
...
pub fn default_data_root() -> String {
    if let Some(r) = std::env::var(ENV_DATA_ROOT)
        .ok()
        .filter(|v| !v.trim().is_empty())
    {
            ...
            if let Some(mut child) = guard.take() {
                match tokio::time::timeout(Duration::from_secs(STOP_GRACE_SECS), child.wait()).await
                {
                    ...
                    Err(_) => {
                        ...
                        note = format!(
                            "SIGTERM {}s 未退出，已 SIGKILL（pid={}）",
                            STOP_GRACE_SECS, p.pid
                        );
                    }
                }
            }
...
fn create_schema(conn: &Connection) -> rusqlite::Result<()> {
    ...
    let _ = conn.execute(
        "ALTER TABLE blockchain_nodes ADD COLUMN updated_at TEXT",
        [],
    );
    let _ = conn.execute(
    ...
    let _ = conn.execute(
        "ALTER TABLE blockchain_nodes ADD COLUMN last_command TEXT",
        [],
    );
    Ok(())
}
...
pub fn route_specs() -> Vec<RouteSpec> {
    ...
    vec![
        ...
        spec(
            HttpMethod::Get,
            "/api/v1/blockchain/chain-nodes/presets",
            false,
            vec![],
        ),
        spec(
            HttpMethod::Get,
            "/api/v1/blockchain/chain-nodes/space-check",
            false,
            vec![],
        ),
        spec(
        ...
        spec(
            HttpMethod::Get,
            "/api/v1/blockchain/chain-nodes/:id/logs",
            false,
            vec![],
        ),
    ]
...
pub async fn handle(
    ...
    match (method, segs) {
        ...
        (HttpMethod::Get, ["space-check"]) => {
            ...
            let txindex = q.get("txindex").is_some_and(|v| v == "1" || v == "true");
            ...
            if mode != "fast" && mode != "full" {
                return Ok(error_response(
                    400,
                    &format!("mode 非法（fast|full）: {mode}"),
                ));
            }
            if data_dir.is_empty() || !valid_data_dir(&data_dir) {
                return Ok(error_response(
                    400,
                    "data_dir 非法（须为绝对路径且不含 ..）",
                ));
            }
        ...
        (HttpMethod::Post, []) => {
            let body: CreateNodeBody = serde_json::from_value(body)
                .map_err(|e| ApiGatewayError::Internal(format!("解析创建节点请求体失败: {e}")))?;
            ...
            let mode = match body.mode.as_deref().map(str::trim) {
                ...
                Some(m) => return Ok(error_response(400, &format!("mode 非法（fast|full）: {m}"))),
            };
            let client = body
                .client
            ...
            let expected_client = if kind == "ethereum" {
                "geth"
            } else {
                "bitcoind"
            };
...
mod tests {
    ...
    fn geth_argv_fast_vs_full_vs_networks() {
        ...
        assert!(
            joined.contains("--gcmode full"),
            "fast 应 gcmode full: {joined}"
        );
        assert!(
            !joined.contains("--mainnet"),
            "geth 主网无网络旗标: {joined}"
        );
        assert!(
            joined.contains("--http.addr 127.0.0.1"),
            "RPC 绑回环: {joined}"
        );

        let full = build_geth_argv(&sample_node("ethereum", "mainnet", "full"));
        assert!(
            full.join(" ").contains("--gcmode archive"),
            "full 应 archive"
        );
    ...
    async fn create_eth_mainnet_carries_cl_warning() {
        ...
        assert!(
            ws.iter()
                .any(|w| w.as_str().unwrap().contains("共识客户端")),
            "应带 CL 提示: {ws:?}"
        );
    }
    ...
    async fn create_rejects_bad_inputs() {
        ...
        let cases = vec![
            (
                serde_json::json!({"name": "", "kind": "ethereum", "network": "mainnet"}),
                400,
            ),
            (
                serde_json::json!({"name": "x", "kind": "solana", "network": "mainnet"}),
                400,
            ),
            (
    ...
    async fn lifecycle_start_logs_stop_delete_with_fake_bin() {
        ...
        for _ in 0..20 {
            ...
            if lines
                .iter()
                .any(|l| l.as_str().unwrap().contains("fake-geth"))
            {
                saw_fake_line = true;
                break;
            }
        }
        assert!(saw_fake_line, "日志应含假二进制启动行（2s 轮询后仍未出现）");
