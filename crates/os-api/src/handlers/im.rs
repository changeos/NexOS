        ...
        match (req.method, segs.as_slice()) {
            ...
            (HttpMethod::Post, ["api", "v1", "im", "groups", id, "join"]) => {
                ...
                let mut group = {
                    ...
                    match self
                        .db_call(move |conn| find_group(conn, &gid))
                        .await?
                    {
                        Some(g) => g,
            ...
            (HttpMethod::Post, ["api", "v1", "im", "lobby", "messages"]) => {
                ...
                let is_member = {
                    let pubkey = caller.pubkey.clone();
                    self.db_call(move |conn| lobby_is_member(conn, &pubkey)).await
                };
                if !is_member {
                    return Ok(error_response(
                        403,
            ...
            (HttpMethod::Get, ["api", "v1", "im", "files", file_id]) => {
                ...
                let record = {
                    let fid = fid.clone();
                    self.db_call(move |conn| find_file_record(conn, &fid)).await?
                };
            ...
            (HttpMethod::Post, ["api", "v1", "im", "notify", "register"]) => {
                    ...
                    .filter(|s| !s.is_empty())
                {
                    let access = {
                        ...
                        let pubkey = caller.pubkey.clone();
                        self.db_call(move |conn| {
                            Self::conversation_readable(conn, &cid, &pubkey)
                        })
                        .await
                    };
                    match access {
...
impl ImShared {
    ...
    async fn dispatch_webhooks(self: &Arc<Self>, msg: &Message) {
