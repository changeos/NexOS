...
impl ToolchainState {
    ...
    fn register_task(&self, toolchain: &str) -> String {
        ...
        self.tasks.lock().expect("tctasks poisoned").insert(
            task_id.clone(),
            ToolchainTask {
                id: task_id.clone(),
...
fn install_node(ctx: &TaskCtx) -> Result<(), String> {
    ...
    if Path::new(&nvm_sh).exists() {
        task_log(
            ctx,
            "~/.nvm/nvm.sh 已存在，跳过 nvm 安装（幂等），直接 nvm install",
        );
    } else {
        ...
        if let Some(url) = override_url {
            ...
            task_log(
                ctx,
                &format!("使用 {ENV_NVM_INSTALL_URL} 指定的 nvm 安装脚本"),
            );
...
fn install_uv(ctx: &TaskCtx) -> Result<(), String> {
    ...
    if let Err(e1) = first {
        ...
        task_log(
            ctx,
            &format!("官方源尝试失败（{e1}），回退 ghfast 镜像重试一次"),
        );
        run_installer_script(
            ctx,
...
pub async fn handle(
    ...
    match (method, segs) {
        ...
        (HttpMethod::Post, ["install"]) => {
            let body: InstallBody = serde_json::from_value(body)
                .map_err(|e| ApiGatewayError::Internal(format!("解析工具链安装请求体失败: {e}")))?;
            let name = body.name.trim().to_string();
            if !INSTALLABLE_TOOLCHAINS.contains(&name.as_str()) {
                return Ok(error_response(
                    400,
                    &format!("name 必须是 {INSTALLABLE_TOOLCHAINS:?} 之一（node 覆盖 node+npm）"),
                ));
            }
            ...
            let (task_id, status) = match available {
                Some(path) => {
                    ...
                    let id = state
                        .register_done(&name, &format!("已安装（探测命中 {path}），无需重复安装"));
                    (id, "done".to_string())
                }
                None => {
        ...
        (HttpMethod::Get, ["install", "tasks", id]) => {
            ...
            match found {
                Some(v) => Ok(ApiResponse {
                    status: 200,
                    body: serde_json::to_value(v)
                        .map_err(|e| ApiGatewayError::Internal(format!("响应序列化失败: {e}")))?,
                    headers: serde_json::json!({}),
                }),
...
mod tests {
    ...
    async fn node_install_prefers_ghfast_mirror_then_official_fallback() {
        ...
        assert!(
            log.contains(&format!(
                "curl -o- {} | bash",
                ghfast(NVM_INSTALL_URL_OFFICIAL)
            )),
            "应优先 ghfast 镜像: {log}"
        );
        // 镜像失败 → 回退官方源一次
    ...
    async fn uv_install_official_first_then_ghfast_env_fallback() {
        ...
        let state =
            ToolchainState::with_executor(home.path(), Arc::new(UvFallback { home: home.path() }));
    ...
    fn toolchain_available_requires_both_node_and_npm() {
        ...
        let found = toolchain_available(&home.path(), "node").expect("node+npm 应命中 nvm 兜底");
