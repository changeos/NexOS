...
//!   `null`（无条目）；
//! - `agent_version` / `hostname` / `platform` / `gpu`（联邦节点信息协议，
//!   2026-09-14 v0.1.45 批）：节点自述四字段随 meta 条目透传——来源仅对端
//!   digest 自广播/转述（版本不可从外部确证）。全缺 = 对端旧版未自述（前端
//!   显示「待升级自述」）；序列化 None 省略（旧前端线格式零变化）。
//!
...
struct LanNodeDto {
    ...
    /// 节点自述：agent 版本（如 "0.1.45"；None = 对端旧版未自述——前端显示
    /// 「待升级自述」）。来源：对端 digest 自广播/转述（meta 注册条目透传）。
...
struct P2pNodeDto {
    ...
    /// 恒 None——无从寻址查询）。
    im_public: Option<bool>,
    /// 节点自述四字段（语义同 [`LanNodeDto::agent_version`] 等——meta 注册
    /// 条目透传；bucket 短 ID 无从匹配时恒 None）。
    #[serde(skip_serializing_if = "Option::is_none")]
    agent_version: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    hostname: Option<String>,
...
struct InactiveNodeDto {
    ...
    since: u64,
    /// 节点自述四字段（语义同 [`LanNodeDto::agent_version`] 等——出局前学到的
    /// 自述照常展示，复活/重连后经新鲜报告刷新）。
    #[serde(skip_serializing_if = "Option::is_none")]
    agent_version: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    hostname: Option<String>,
...
struct MetaFields {
    ...
    /// 节点自述四字段（联邦节点信息协议，2026-09-14 v0.1.45 批）：agent 版本 /
    /// 机器名 / 系统 / GPU 摘要——来源仅对端 digest 自广播/转述（版本不可从
    /// 外部确证——Spark 踩坑）。全 None = 对端旧版未自述（前端显示「待升级
    /// 自述」）。
    agent_version: Option<String>,
    hostname: Option<String>,
    platform: Option<String>,
...
/// 从注册表条目取展示字段（Active 带分数；Inactive 分数记 0；自述四字段随
/// 条目透传——gossip 学到的知识，展示层不自判）。
...
/// 死亡——meta 引擎可能尚未建档），score/last_seen 记 0、来源 null、自述全
/// None（无条目即无自述知识——等对端自广播经 gossip 到达/洗白）。
...
///    `14.145.78.57`）。**展示地址**经 [`lan_display_addr`] 规整：入站观测的
...
fn inactive_group(
    ...
    meta.iter()
        ...
        .map(|e| InactiveNodeDto {
            ...
            // 节点自述透传（出局前学到的版本/硬件照常展示——复活/重连后经
            // 新鲜报告刷新）
            agent_version: e.agent_version.clone(),
            hostname: e.hostname.clone(),
            platform: e.platform.clone(),
...
mod tests {
    ...
    /// 自述四字段默认 None——带自述的场景用 struct 更新语法覆盖）。
    fn meta_entry(
        id: os_p2p::NodeId,
        addrs: &[&str],
        last_seen: u64,
    ...
    fn partition_direct_observed_overrides_polluted_endpoint_book() {
        ...
        // 端点簿里 113 的观测地址被公网锚点 gossip 覆盖成公网 14.145.78.57
        ...
        let wan_hex = wan_id.to_hex();
        let endpoints = vec![
            endpoint(lan_id.clone(), "14.145.78.57:55637"), // gossip 污染的公网观测
    ...
    // —— 5c-2) 节点自述透传（联邦节点信息协议，2026-09-14 v0.1.45 批）：
    //          meta 条目携带的自述四字段（agent 版本/hostname/platform/gpu）
    //          富化到 lan/p2p/inactive 三组行；无条目/无自述 = 全 None（前端
    //          显示「待升级自述」）；序列化 None 省略（旧前端线格式零变化）——

    #[test]
    fn partition_and_inactive_group_relay_self_desc_fields() {
        let described = random_id(); // Active + 直连观测私网 → lan（带全量自述）
        let relayed = random_id(); // Active + 公网 underlay → p2p（gossip 学的自述）
        let plain = random_id(); // Active 无自述（旧版节点）→ p2p（None 全缺）
        let gone = random_id(); // Inactive meta-only → inactive 分组（自述照带）
        let described_hex = described.to_hex();
        let relayed_hex = relayed.to_hex();
        let plain_hex = plain.to_hex();
        let gone_hex = gone.to_hex();
        let with_desc = |mut e: NodeMetaEntry| {
            e.agent_version = Some("0.1.45".into());
            e.hostname = Some("nexos-a".into());
            e.platform = Some("Ubuntu 24.04.1 LTS".into());
            e.gpu = Some("NVIDIA GeForce RTX 3060 ×2".into());
            e
        };
        let meta = vec![
            with_desc(meta_entry(
                described.clone(),
                &["192.168.1.113:7070"],
                1_700_000_100,
                os_p2p::MetaState::Active {
            ...
            // 旧版节点：有 meta 条目但无自述（digest 无 desc 字段 → None）
            meta_entry(
                plain.clone(),
                &["203.0.113.10:40022"],
                1_700_000_030,
            ...
            // 出局节点的自述照常透传（出局前学到的版本/硬件）
            with_desc(meta_entry(
                gone.clone(),
                &["203.0.113.11:40022"],
                900,
                os_p2p::MetaState::Inactive { since: 1_000 },
                os_p2p::MetaSource::Gossip,
            )),
        ];
        let peers = vec![
            PeerInfo {
                observed_addr: Some("192.168.1.113:49730".parse().unwrap()),
                ..peer(described, None, true, false)
            },
            peer(relayed, Some("203.0.113.9:40022"), false, true),
            peer(plain, Some("203.0.113.10:40022"), false, true),
        ];
        let idx = MetaIndex::build(&meta);
        let (lan, p2p) = partition(&peers, &[], &[], &idx, "");
        // lan 行：自述四字段全量透出
        assert_eq!(lan.len(), 1);
        let l = &lan[0];
        assert_eq!(l.node_id, described_hex);
        assert_eq!(l.agent_version.as_deref(), Some("0.1.45"), "lan 行版本透传");
        assert_eq!(l.hostname.as_deref(), Some("nexos-a"));
        assert_eq!(l.platform.as_deref(), Some("Ubuntu 24.04.1 LTS"));
        assert_eq!(l.gpu.as_deref(), Some("NVIDIA GeForce RTX 3060 ×2"));
        // p2p 行：gossip 学的自述照常；旧版节点全 None
        let r = p2p.iter().find(|p| p.node_id == relayed_hex).unwrap();
        assert_eq!(r.agent_version.as_deref(), Some("0.1.45"), "p2p 行版本透传");
        assert_eq!(r.gpu.as_deref(), Some("NVIDIA GeForce RTX 3060 ×2"));
        let n = p2p.iter().find(|p| p.node_id == plain_hex).unwrap();
        assert!(
            n.agent_version.is_none()
                && n.hostname.is_none()
                && n.platform.is_none()
                && n.gpu.is_none(),
            "旧版节点无自述 = 全 None（前端显示待升级）"
        );
        // inactive 行：出局节点的自述照带
        let inactive = inactive_group(&meta, &lan, &p2p);
        let g = inactive.iter().find(|n| n.node_id == gone_hex).unwrap();
        assert_eq!(
            g.agent_version.as_deref(),
            Some("0.1.45"),
            "inactive 行版本透传"
        );
        assert_eq!(g.hostname.as_deref(), Some("nexos-a"));
        // 序列化：None 字段省略（旧前端线格式零变化）；Some 字段在线
        let json = serde_json::to_value(&n).unwrap();
        for f in ["agent_version", "hostname", "platform", "gpu"] {
            assert!(
                json.get(f).is_none(),
                "None 自述字段 {f} 不序列化（旧前端线格式零变化）"
            );
        }
        let json = serde_json::to_value(&l).unwrap();
        assert_eq!(json["agent_version"], serde_json::json!("0.1.45"));
        assert_eq!(json["gpu"], serde_json::json!("NVIDIA GeForce RTX 3060 ×2"));
    }
