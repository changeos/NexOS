...
impl CiCore {
    ...
    pub fn list_runs(&self, repo: &str) -> Vec<CiRun> {
        ...
        stmt.query_map(params![repo, LIST_LIMIT], map_run_row)
            .map(|it| it.filter_map(Result::ok).filter_map(row_to_run).collect())
            .unwrap_or_default()
    }
...
impl RouteHandler for NexhubCiRouteHandler {
    ...
    async fn handle(&self, req: ApiRequest) -> Result<ApiResponse, ApiGatewayError> {
        ...
        match (req.method, segs.as_slice()) {
            ...
            (HttpMethod::Get, ["api", "v1", "coderepo", "ci", "latest"]) => Ok(ok_json(
                serde_json::json!({ "latest": self.core.latest_per_repo() }),
            )),
...
mod tests {
    ...
    async fn endpoints_list_limit_delete_and_latest() {
        ...
        let mut tries = 0;
        while slow_core.db_get(&running_id).map(|r| r.status).as_deref() != Some("running")
            && tries < 150
        {
