...
fn inject_monorepo_skeleton(work_dir: &Path, repo: &str, ring: &mut LogRing) -> Result<(), String> {
...
pub struct CiCore {
    /// runs 表持久化（ci.db 单连接；`Arc<Mutex<Connection>>`——DB 收敛第三批
    /// （2026-09-25，im.rs `im_db_call` 同款手法）：锁结构不变（组件级单连接
    /// 单互斥），Arc 化仅为 `spawn_blocking` 闭包可持 `'static` 句柄，把
    /// 「async 上下文里直接 `db.lock()` + 同步 SQLite IO」挪出 tokio worker。
    ...
    /// 统一任务登记表（v0.1.54 双写旁路；Mutex<Option>——open_default 返回
    /// Arc 后装配层再注，与既有字段零构造耦合）。None=未注入，零旁路。
    unified: Mutex<Option<Arc<crate::handlers::tasks_common::UnifiedTaskStore>>>,
}

/// CI DB 短事务统一入口（DB 收敛第三批，2026-09-25，照抄 im.rs `im_db_call`
/// 手法——审计 A1-1/Top1 续批）：把「async 上下文里直接 `db.lock()` + 同步
/// SQLite IO」挪进 `tokio::task::spawn_blocking`——锁等待与 SQLite 同步 IO
/// 全部离开 tokio worker，async 任务只 `.await` 结果。
///
/// 锁语义与原先完全一致：同一把组件级互斥锁（ci.db 单连接）、短锁快放不跨
/// `.await`；闭包 panic 经 `resume_unwind` 原样上抛（等价原
/// `expect("ci db lock")`）。供 [`CiCore`] 的 runs 读写方法共用。
async fn ci_db_call<T, F>(db: Arc<Mutex<Connection>>, f: F) -> T
where
    T: Send + 'static,
    F: FnOnce(&Connection) -> T + Send + 'static,
{
    match tokio::task::spawn_blocking(move || {
        let conn = db.lock().expect("ci db lock");
        f(&conn)
    })
    .await
    {
...
impl CiCore {
    ...
    pub fn with_paths(db_path: &str, repos_dir: &str, work_root: &Path) -> Self {
        let conn =
            open_ci_db(db_path).unwrap_or_else(|e| panic!("打开 ci.db（{db_path}）失败: {e}"));
    ...
    /// 注入统一任务登记表（v0.1.54 统一任务框架：runs 入队/终态双写
    /// `unified_tasks`，kind=ci；ci_runs 专表保留不动；轻旁路）。
    pub fn set_unified_tasks(&self, store: Arc<crate::handlers::tasks_common::UnifiedTaskStore>) {
        *self.unified.lock().expect("ci unified slot") = Some(store);
    }

    /// 统一登记快照（空 = 未注入，调用方跳过）。
    fn unified_store(&self) -> Option<Arc<crate::handlers::tasks_common::UnifiedTaskStore>> {
        self.unified.lock().expect("ci unified slot").clone()
    }
    ...
    // ---- DB 原子操作（短锁）----
    //
    // DB 收敛第三批（2026-09-25，照抄 im.rs `im_db_call` 手法）：runs 表读写
    // 全部经 `ci_db_call`——闭包在 `spawn_blocking` 线程里拿锁执行，锁等待与
    // SQLite 同步 IO 离开 tokio worker；同一把组件级互斥锁、锁内无 await，
    // 语义与原先「async 里直接 `self.db.lock()`」一比一相同。

    async fn db_insert(&self, run: &CiRunRow) -> rusqlite::Result<()> {
        let run = run.clone();
        ci_db_call(
            Arc::clone(&self.db),
            move |conn| {
    ...
    async fn db_update_status(
        ...
        let pipeline = pipeline.map(String::from);
        ci_db_call(
            Arc::clone(&self.db),
            move |conn| {
                let _ = conn.execute(
    ...
    /// 环形日志整环刷盘（攒批满与状态切换时调用）。ring 的清脏标与渲染在
    /// 调用方线程完成（纯内存操作），仅 SQL 落库进 spawn_blocking。
    async fn db_flush_log(&self, id: &str, ring: &mut LogRing) {
        ring.dirty = 0;
        let text = ring.render();
        let id = id.to_string();
        ci_db_call(Arc::clone(&self.db), move |conn| {
    ...
    async fn db_get(&self, id: &str) -> Option<CiRun> {
        let id = id.to_string();
        ci_db_call(Arc::clone(&self.db), move |conn| {
            conn.query_row(
    ...
    async fn db_delete(&self, id: &str) -> usize {
        let id = id.to_string();
        ci_db_call(Arc::clone(&self.db), move |conn| {
    ...
    #[must_use]
    pub async fn list_runs(&self, repo: &str) -> Vec<CiRun> {
    ...
    #[must_use]
    pub async fn latest_per_repo(&self) -> Vec<CiRun> {
        ci_db_call(Arc::clone(&self.db), move |conn| {
            let mut stmt = match conn.prepare(
                "SELECT id, repo_name, trigger, status, pipeline, exit_code, created_ms, \
    ...
    ///
    /// DB 收敛第三批（2026-09-25）起 async：`db_insert` 走 `ci_db_call` 的
    /// spawn_blocking（锁等待与 SQLite IO 离开 tokio worker）。push 钩子侧
    /// 经 [`push_hook_with`] 的 fire-and-forget `tokio::spawn` 调用（CI 本就是
    /// 旁路，入队结果只记日志——与 webhooks::fire_push 同款形态）。
    pub async fn enqueue(
        ...
        // 统一任务框架旁路登记（queued；ci_runs 专表保留不动）
        if let Some(store) = self.unified_store() {
            store.register(
                &crate::handlers::tasks_common::UnifiedTask {
                    id: run_id.clone(),
                    kind: "ci".into(),
                    status: crate::handlers::tasks_common::UnifiedStatus::Queued,
                    label: format!("CI run {repo}（trigger={trigger}）"),
                    stage: None,
                    log_tail: String::new(),
                    output: None,
                    created_at: ms_to_iso(Some(row.created_ms)).unwrap_or_default(),
                    finished_at: None,
                    error: None,
                },
            );
...
async fn worker(core: Arc<CiCore>, repo: String) {
    loop {
        ...
        let next = {
            ...
            let head = inner.queues.get_mut(&repo).and_then(|q| q.pop_front());
            if head.is_none() {
...
async fn run_one(core: &CiCore, run: &PendingRun) {
    ...
    core.db_update_status(run_id, "running", None, None, false).await;
    // 统一登记：running（stage=pipeline 探测中）
    let unified = core.unified_store();
    let ci_label = format!("CI run {}", run.repo);
    if let Some(store) = &unified {
        store.progress("ci", run_id, Some("running"), "");
    }
    ...
    core.db_update_status(run_id, &status, pipeline.as_deref(), exit_code, true)
        .await;
    core.db_flush_log(run_id, &mut ring).await;
    // 统一登记：终态（passed→done / failed→error / skipped→skipped）
    if let Some(store) = &unified {
        store.finish(
            "ci",
            run_id,
            crate::handlers::tasks_common::UnifiedStatus::from_source("ci", &status),
            &ci_label,
            Some(&format!(
                "status={status} exit={} pipeline={}",
                exit_code
                    .map(|c| c.to_string())
                    .unwrap_or_else(|| "-".into()),
                pipeline.as_deref().unwrap_or("-")
            )),
            None,
        );
    }
...
async fn ci_run_inner(
    ...
    match clone {
        ...
        Ok(Ok(_)) => {
            ring.push("[clone ok]");
            core.db_flush_log(run_id, ring).await;
        }
    }
...
///
/// DB 收敛第三批（2026-09-25）起：入队（含 `ci_db_call` 的 spawn_blocking 落库）
/// 经 `tokio::spawn` fire-and-forget 执行——CI 是旁路、入队结果本就只记日志，
/// 与相邻 webhooks::fire_push 同款形态；push 响应路径不再被 SQLite 同步 IO
/// 阻塞。须在 tokio 运行时上下文调用（http.rs 的 axum handler / 测试均满足）。
pub fn push_hook(repo: &str) {
    push_hook_with(global_core(), repo);
}

/// [`push_hook`] 的可注入形态（测试用）。env 门控与仓库名校验同步先行
/// （关了/非法直接 no-op 不起任务），入队异步起任务。
pub fn push_hook_with(core: Option<Arc<CiCore>>, repo: &str) {
    ...
    let repo = repo.trim().trim_end_matches(".git").to_string();
    tokio::spawn(async move {
        match core.enqueue(&repo, "push").await {
...
impl NexhubCiRouteHandler {
    ...
    /// 注入统一任务登记表（v0.1.54 统一任务框架：runs 双写 unified_tasks；
    /// builder 风格——core 已 Arc 化，经 set_unified_tasks 内部注入）。
    #[must_use]
    pub fn with_unified_tasks(
        self,
        store: std::sync::Arc<crate::handlers::tasks_common::UnifiedTaskStore>,
    ) -> Self {
        self.core.set_unified_tasks(store);
        self
    }
}
    ...
    async fn handle(&self, req: ApiRequest) -> Result<ApiResponse, ApiGatewayError> {
        ...
        match (req.method, segs.as_slice()) {
            ...
            (HttpMethod::Post, ["api", "v1", "coderepo", "repos", name, "ci"]) => {
                match self.core.enqueue(name, "manual").await {
                    Ok(id) => {
                        let run = self.core.db_get(&id).await;
                        Ok(ApiResponse {
                            status: 202,
            ...
            (HttpMethod::Get, ["api", "v1", "coderepo", "repos", name, "ci"]) => {
                let runs = self.core.list_runs(name).await;
            ...
            (HttpMethod::Get, ["api", "v1", "coderepo", "repos", name, "ci", run_id]) => {
                match self.core.db_get(run_id).await {
            ...
            (HttpMethod::Delete, ["api", "v1", "coderepo", "repos", name, "ci", run_id]) => {
                match self.core.db_get(run_id).await {
                    ...
                    Some(run) if run.status == "queued" || run.status == "running" => Ok(
                        error_response(409, &format!("run 进行中（{}），不可删除", run.status)),
                    ),
                    Some(run) => {
                        let n = self.core.db_delete(&run.id).await;
                        if n == 0 {
            ...
            (HttpMethod::Get, ["api", "v1", "coderepo", "ci", "latest"]) => {
                let latest = self.core.latest_per_repo().await;
                Ok(ok_json(serde_json::json!({ "latest": latest })))
            }
...
mod tests {
    ...
    /// 轮询等待某仓 run 行数达到 `want`（push 钩子 fire-and-forget 入队后，
    /// 行落库在 spawn 的异步任务里完成——第三批起不能同步断言）。
    async fn wait_run_count(core: &CiCore, repo: &str, want: usize, timeout_ms: u64) -> usize {
        let started = std::time::Instant::now();
        loop {
            let n = core.list_runs(repo).await.len();
            if n >= want || started.elapsed().as_millis() as u64 > timeout_ms {
                return n;
            }
    ...
    async fn full_run_passes_with_streamed_log() {
        ...
        let id = core.enqueue("demo", "manual").await.expect("入队");
    ...
    async fn failed_run_records_exit_code() {
        ...
        core.enqueue("bad", "manual").await.unwrap();
    ...
    async fn skipped_when_no_pipeline_detected() {
        ...
        let id = core.enqueue("docs", "push").await.unwrap();
        ...
        // 详情日志写明原因，且未执行任何步骤
        let run = core.db_get(&id).await.unwrap();
    ...
    async fn timeout_kills_step_and_marks_failed_124() {
        ...
        core.enqueue("slow", "manual").await.unwrap();
    ...
    async fn program_resolution_and_override() {
        ...
        core.enqueue("npmonly", "manual").await.unwrap();
    ...
    fn skeleton_layout_path_assembly() {
        ...
        assert_eq!(layout.sdk_src, Path::new("/mono/crates/os-api/web/src/sdk"));
        assert_eq!(
            layout.sdk_dest,
    ...
    async fn skeleton_injection_makes_relative_sdk_reachable() {
        ...
        let id = core.enqueue("nexos-app-x", "push").await.unwrap();
    ...
    async fn skeleton_missing_monorepo_or_sdk_fails_with_guidance() {
        ...
        let mut id = core.enqueue("nexos-app-y", "manual").await.unwrap();
        ...
        assert!(
            log.contains("本机无 monorepo，应用构建需 NexOS 环境"),
            "{log}"
        );
        ...
        id = core.enqueue("nexos-app-y", "manual").await.unwrap();
    ...
    async fn same_repo_runs_execute_serially() {
        ...
        for _ in 0..3 {
            core.enqueue("one", "manual").await.unwrap();
        }
    ...
    async fn global_concurrency_capped_at_two() {
        ...
        for r in repos {
            core.enqueue(r, "manual").await.unwrap();
        }
        for r in repos {
    ...
    async fn endpoints_list_limit_delete_and_latest() {
        ...
        for _ in 0..25 {
            last_id = core.enqueue("ep", "manual").await.unwrap();
        }
        // 等最后入队的一条到达终态（bounded，防意外挂死测试进程）
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(30);
        loop {
            let runs = core.list_runs("ep").await;
        ...
        let other_id = core.enqueue("other", "manual").await.unwrap();
        ...
        let running_id = slow_core.enqueue("slowrepo", "manual").await.unwrap();
        let mut tries = 0;
        while slow_core.db_get(&running_id).await.map(|r| r.status).as_deref() != Some("running")
            && tries < 150
        {
    ...
    async fn enqueue_validates_repo() {
        ...
        assert_eq!(core.enqueue("../evil", "manual").await.unwrap_err().0, 400);
        assert_eq!(core.enqueue("a/b", "manual").await.unwrap_err().0, 400);
        assert_eq!(core.enqueue("ghost", "manual").await.unwrap_err().0, 404);
        make_bare_repo(&root.join("repos"), "ok", &[("README.md", "x")]);
        let id = core.enqueue("ok", "manual").await.unwrap();
    ...
    async fn push_hook_gating() {
        ...
        // 1) 缺省开 → 入队（第三批起入队在 spawn 的异步任务里，轮询等行落库）
        std::env::remove_var(ENV_AUTO_PUSH);
        push_hook_with(Some(Arc::clone(&core)), "hooked");
        assert_eq!(
            wait_run_count(&core, "hooked", 1, 5_000).await,
            1,
            "缺省应自动触发"
        );
        // 2) =0 → 不触发（同步短路，不起任务——立即可断言）
        std::env::set_var(ENV_AUTO_PUSH, "0");
        push_hook_with(Some(Arc::clone(&core)), "hooked");
        assert_eq!(core.list_runs("hooked").await.len(), 1, "=0 应关闭");
        // 3) =false → 不触发
        std::env::set_var(ENV_AUTO_PUSH, "false");
        push_hook_with(Some(Arc::clone(&core)), "hooked");
        assert_eq!(core.list_runs("hooked").await.len(), 1);
        // 4) =1 → 恢复触发
        std::env::set_var(ENV_AUTO_PUSH, "1");
        push_hook_with(Some(Arc::clone(&core)), "hooked");
        assert_eq!(
            wait_run_count(&core, "hooked", 2, 5_000).await,
            2,
            "=1 应恢复"
        );
        ...
        assert_eq!(core.list_runs("hooked").await.len(), 2);
        // 6) 未装配核心 → 静默
    ...
    // ---- 统一任务框架双写（v0.1.54）：enqueue → queued；run_one 终态落表 ----

    #[tokio::test]
    async fn unified_dual_write_covers_enqueue_and_terminal() {
        let root = temp_root("unified");
        let stub = make_stub(&root, "cargo", 1, "0", 0, "");
        let core = Arc::new(
            CiCore::with_paths(
                root.join("ci.db").to_str().unwrap(),
                root.join("repos").to_str().unwrap(),
        ...
        // 注入统一登记表（与 ci.db 分离的临时库）
        let unified = Arc::new(crate::handlers::tasks_common::UnifiedTaskStore::with_path(
            root.join("tasks.db").to_str().unwrap(),
        ));
        core.set_unified_tasks(Arc::clone(&unified));

        let id = core.enqueue("demo", "manual").await.expect("入队");
        // 入队即登记（queued 或已被 worker 推进——不锁中间态，只验行存在）
        let runs = wait_finished(&core, "demo", 1, 30_000).await;
        assert_eq!(runs[0].status, "passed");
        // 终态落表：kind=ci、id=run id、status=done（passed 归一）
        let rows = unified.list(Some("ci"), None, 50);
        assert_eq!(rows.len(), 1, "{rows:?}");
        assert_eq!(rows[0].id, id);
        assert_eq!(rows[0].status, crate::handlers::tasks_common::UnifiedStatus::Done);
        assert!(rows[0].finished_at.is_some(), "终态补记 finished");
        assert!(rows[0].label.contains("demo"), "label 含仓库名");
        assert!(
            rows[0].output.as_deref().unwrap_or("").contains("passed"),
            "output 摘要含终态: {:?}",
            rows[0].output
        );
        // 未注入时零旁路（同 core 不再写——另起一个 core 不注 unified 跑通即可，
        // 此处只验注入面存在；enqueued 行为一致性由其余既有测试兜底）
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    async fn write_endpoints_enforced_by_gateway_auth() {
        ...
        assert!(core.list_runs("gwrepo").await.is_empty(), "401 不得入队");
        ...
        assert_eq!(
            route.map(|r| r.handler_component).as_deref(),
            Some("nexhub_ci")
        );
        assert_eq!(core.list_runs("gwrepo").await.len(), 1);
        let _ = std::fs::remove_dir_all(&root);
    }
}
