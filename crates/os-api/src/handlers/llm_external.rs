...
fn load_all(conn: &Connection) -> rusqlite::Result<Vec<ExternalApi>> {
    let sql = format!("SELECT {SELECT_COLS} FROM llm_external_apis ORDER BY created_at, id");
...
pub fn route_specs() -> Vec<RouteSpec> {
    ...
    vec![
        ...
        spec(HttpMethod::Post, "/api/v1/llm/external-apis/:id/test", true),
        spec(HttpMethod::Post, "/api/v1/llm/external-apis/:id/chat", true),
    ]
}
...
pub async fn handle(
    ...
    match (method, segs) {
        ...
        (HttpMethod::Post, []) => {
            ...
            // ——错误值在此挡下，不留到执行层才炸。
            let via_node = body.via_node.unwrap_or_default().trim().to_string();
        ...
        (HttpMethod::Post, [id, "chat"]) => {
            let chat_body: ChatBody = serde_json::from_value(body)
                .map_err(|e| ApiGatewayError::Internal(format!("解析对话请求体失败: {e}")))?;
...
/// error 态——零编造）。
async fn relay_test_connectivity(state: &LlmExternalState, api: &ExternalApi) -> TestOutcome {
    let started = std::time::Instant::now();
    let outcome = async {
...
async fn relay_chat_forward_once(
    ...
    serde_json::from_slice(&done.body).map_err(|e| relay_fail(api, format!("上游响应非 JSON: {e}")))
}
...
async fn relay_chat_stream_response(
    ...
    let Some(ep) = state.relay_endpoint() else {
        return json_error_response(
            502,
            &format!(
                "经 {node_label} 中继失败: P2P 通道未装配（NEXOS_P2P_ENABLE=1 且对端组网后可用）"
            ),
        );
    };
    ...
    let first = match tokio::time::timeout(STREAM_FIRST_BYTE_TIMEOUT, stream.next_chunk()).await {
        ...
        Ok(None) => return json_error_response(502, "中继上游流在首字节前结束（空响应体）"),
        Err(_) => return json_error_response(504, "中继上游首字节超时（120s 无数据）"),
    };
    eprintln!(
...
pub async fn chat_stream_handler(
    ...
    let principal =
        crate::http::extract_principal(&headers, state.jwt.as_ref(), state.admin_token.as_ref())
            .await;
    let path = parts
        .uri
        .path_and_query()
...
mod tests {
    ...
    async fn edit_updates_each_field_and_preserves_unprovided() {
        ...
        assert_eq!(
            resp.body["base_url"], "http://192.0.2.107:8000/v1",
            "尾斜杠收敛"
        );
    ...
    fn edit_persists_across_state_reopen() {
        let dir =
            std::env::temp_dir().join(format!("nexos-llm-ext-edit-{}", os_core::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let db = dir.join("llm.db");
        let id = {
            let state = LlmExternalState::new(Arc::new(Mutex::new(Connection::open(&db).unwrap())));
            let api = ExternalApi {
                id: "xapi-1".into(),
                name: "旧名".into(),
    ...
    async fn chat_nonstream_passes_through_upstream_body_and_usage() {
        ...
        let seen = seen.lock().unwrap().clone();
        assert!(
            seen.contains("\"model\":\"qwen3.5-9b\""),
            "model 未透传: {seen}"
        );
        assert!(
        ...
        assert!(
            seen.to_ascii_lowercase()
                .contains("authorization: bearer sk-up"),
            "缺鉴权头: {seen}"
        );
    }
    ...
    async fn edit_upgrades_legacy_direct_row_via_fed_reimport() {
        ...
        assert_eq!(
            row.base_url, "http://192.168.5.11:8558/v1",
            "未提供字段保留"
        );
    }
    ...
    async fn stream_via_fake_relay_passes_chunks_and_errors() {
        ...
        std::thread::spawn(move || {
            use std::io::{Read, Write};
            let Ok((mut s, _)) = listener2.accept() else {
                return;
            };
            let mut buf = [0u8; 4096];
            let _ = s.read(&mut buf);
