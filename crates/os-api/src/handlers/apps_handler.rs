...
//! 4. 已剥离引擎拦截（v0.1.51 三引擎剥离批）：manifest 的 `engine` 字段或
//!    `id` 命中 film / streaming（[`STRIPPED_ENGINES`]）→ 400「引擎已剥离，
//!    请使用独立产品线 FilmStudio/StreamingStudio」——历史 `nexos-app-film`
//!    / `nexos-app-streaming` 仓仍在 NexHub（历史发布线保留），但内置引擎
//!    已删除出 os-api，装了也没有后端，直接拦截更诚实。
//!
...
use crate::handlers::tasks_common::{UnifiedStatus, UnifiedTaskStore};
...
/// 已剥离引擎名单（v0.1.51 三引擎剥离批）：这些引擎的内置实现已删除出
/// os-api，由独立产品线（FilmStudio / StreamingStudio，各自 NexHub 仓）
/// 承接。历史 `nexos-app-*` 仓仍在（历史发布线保留）——安装时按 manifest
/// 的 `engine` 字段（及 `id`，与 [`AppRegistry::is_engine_enabled`] 的
/// `id=? OR engine=?` 门控口径一致）命中即 400 拦截并引导独立版。
pub const STRIPPED_ENGINES: [&str; 2] = ["film", "streaming"];

/// 剥离拦截文案（安装 400 响应共用）。
#[must_use]
pub fn stripped_engine_hint(engine: &str) -> String {
    let product = match engine {
        "film" => "FilmStudio",
        "streaming" => "StreamingStudio",
        _ => "对应独立版产品",
    };
    format!("引擎 {engine} 已剥离出 NexOS（v0.1.51 三引擎剥离批）：内置引擎已删除，请使用独立产品线 {product}（各自 NexHub 仓）")
}

/// manifest 是否命中已剥离引擎（engine 字段或 id——与引擎门控同口径）。
#[must_use]
pub fn is_stripped_engine_manifest(engine: Option<&str>, id: &str) -> bool {
    engine
        .map(str::trim)
        .filter(|e| !e.is_empty())
        .is_some_and(|e| STRIPPED_ENGINES.contains(&e))
        || STRIPPED_ENGINES.contains(&id)
}
...
pub struct AppRegistry {
    ...
    /// 统一任务登记表（v0.1.54 双写旁路；None=未注入，行为与历史完全一致）。
...
impl AppRegistry {
    ...
    pub fn with_paths(db_path: &str, apps_dir: &str, repos_dir: &str) -> Self {
        ...
        // 防 SQLITE_BUSY 立败（审计 E#6）；内存降级连接上设置同样无害
    ...
    /// 注入统一任务登记表（v0.1.54 统一任务框架：安装任务终态双写
    /// `unified_tasks`；轻旁路——登记失败仅日志，安装主路径零感知）。
    #[must_use]
    ...
    ///
    /// v0.1.54 统一任务框架：同步双写 `unified_tasks`（kind=apps；completed
    /// → done / failed → error）。轻旁路——登记失败仅日志，不影响本表记录。
    fn record_task(&self, task: AppInstallTask) {
        if let Some(store) = &self.unified {
            let status = UnifiedStatus::from_source("apps", &task.status);
            store.finish(
                "apps",
                &task.id,
                status,
                &format!("应用安装 {}（{}）", task.app_id, task.repo),
                task.log_tail.as_deref(),
                task.error.as_deref(),
            );
        }
        self.tasks.lock().expect("apps tasks poisoned").push(task);
    }
    ...
    /// 卸载成功的统一登记（v0.1.54）：kind=apps、done、id 独立序列
    /// `app-uninstall-N`（与安装任务 id 空间区分）。不进内存 tasks Vec
    /// （既有 /apps/tasks 契约保留不动），只在 unified_tasks 留痕。
    fn record_uninstall_unified(&self, rec: &AppRecord) {
        let Some(store) = &self.unified else {
            return;
        };
        let id = {
            let mut c = self.counter.lock().expect("apps counter poisoned");
            *c += 1;
            format!("app-uninstall-{}", *c)
        };
        store.finish(
            "apps",
            &id,
            UnifiedStatus::Done,
            &format!("应用卸载 {}（{}）", rec.id, rec.repo),
            Some(&format!("已删除 {}", rec.dir)),
            None,
        );
    }
    ...
    pub async fn install(&self, repo: &str) -> Result<(String, AppRecord), (u16, String)> {
        ...
        // 2.5 已剥离引擎拦截（v0.1.51 三引擎剥离批）：manifest 声明 film/
        //     streaming 引擎（或 id 即引擎名）→ 400 引导独立产品线，不落盘
        if is_stripped_engine_manifest(manifest.engine.as_deref(), &manifest.id) {
            let engine = manifest
                .engine
                .clone()
            ...
            let _ = std::fs::remove_dir_all(&tmp);
            eprintln!("[apps] 安装拦截（引擎已剥离）：{repo}（engine={engine}）");
            return Err((400, stripped_engine_hint(&engine)));
        }
        // 3. min_os_api 下限
        if let Some(req) = manifest
            .min_os_api
            .as_deref()
            .map(str::trim)
            .filter(|s| !s.is_empty())
...
fn load_apps(conn: &Connection) -> Vec<AppRecord> {
    let Ok(mut stmt) = conn.prepare(&format!("SELECT {APP_COLS} FROM apps ORDER BY id")) else {
        return vec![];
    };
...
fn read_manifest_at(root: &Path) -> Result<AppManifest, String> {
    ...
    let raw =
        std::fs::read_to_string(&path).map_err(|e| format!("读取 manifest.json 失败: {e}"))?;
    let m: AppManifest =
...
pub fn serve_asset_blocking(
    ...
    let bytes = std::fs::read(&target_canon).map_err(|e| (404, format!("资源读取失败: {e}")))?;
...
impl RouteHandler for AppsRouteHandler {
    ...
    async fn handle(&self, req: ApiRequest) -> Result<ApiResponse, ApiGatewayError> {
        ...
        match (req.method, segs.as_slice()) {
            ...
            // —— DELETE /api/v1/apps/:id —— 卸载（删目录 + 注销 + 统一登记）
            (HttpMethod::Delete, ["api", "v1", "apps", id]) => match self.registry.uninstall(id) {
                Ok(rec) => {
                    // 统一任务框架旁路登记（卸载不进内存 Vec——历史行为保留，
                    // 但 unified_tasks 留痕；同步终态直接 finish）
                    self.registry.record_uninstall_unified(&rec);
            ...
            (HttpMethod::Get, ["api", "v1", "apps", "tasks"]) => Ok(ok_json(
                serde_json::json!({ "tasks": self.registry.tasks_snapshot() }),
            )),

            // —— GET /apps-assets/:id/* —— 应用静态资源（web/ 下，防穿越）
            (HttpMethod::Get, ["apps-assets", id, rest @ ..]) => {
                let sub = rest.join("/");
                // Cache-Control 同批覆盖（审计 §G / 快赢#1）：应用包升级原地
                // 覆盖同名文件，与主前端同一策略——assets/ 哈希名 immutable、
                // 其余（entry.js / standalone.html 等入口与非哈希名）no-cache
                // 保升级即时生效。剥 web/ 前缀与 serve_asset_blocking 同一口径。
                let trimmed = sub.trim_start_matches('/');
                let cache_control = crate::http::cache_control_for_static(
                    trimmed.strip_prefix("web/").unwrap_or(trimmed),
                );
                let apps_dir = self.registry.apps_dir.clone();
                let id = id.to_string();
                let res =
                    tokio::task::spawn_blocking(move || serve_asset_blocking(&apps_dir, &id, &sub))
                        .await
                        .map_err(|e| {
                            ApiGatewayError::Internal(format!("资源读取任务 join 失败: {e}"))
                        })?;
                match res {
                    Ok((bytes, mime)) => {
...
mod tests {
    ...
    fn make_app_repo(
        ...
        assert!(
            run_git(&["git", "init", "--bare", bare.to_str().unwrap()]),
            "init bare"
        );
    ...
    async fn install_uninstall_roundtrip_with_real_git_repo() {
        ...
        // 卸载 → 目录删 + 行删
        let resp = h.handle(del_req("/api/v1/apps/demo-app")).await.unwrap();
    ...
    async fn install_resolves_dist_publish_root_for_dual_layout_repo() {
        ...
        assert!(
            Path::new(app_dir).join("web/entry.js").is_file(),
            "{app_dir}"
        );
        assert!(Path::new(app_dir).join("manifest.json").is_file());
        assert!(!Path::new(app_dir).join("src").exists(), "src 不应拷贝");
        assert!(
            !Path::new(app_dir).join("dist").exists(),
            "dist 前缀不应保留"
        );
        // 静态托管两写法均命中
        for url in [
            "/apps-assets/dual-app/entry.js",
            "/apps-assets/dual-app/web/entry.js",
        ] {
    ...
    async fn install_rejects_bad_manifests() {
        ...
        let _ = make_app_repo(
            &dir,
            "nexos-app-badentry",
            serde_json::json!({"entry": "web/nope.js"}),
        );
        // id 非法（大写）
        ...
        for repo in [
            ...
            assert!(
                tasks
                    .iter()
                    .any(|t| t["repo"] == repo && t["status"] == "failed"),
                "{repo} 应有 failed 任务记录"
            );
        }
        // 无一入表
    ...
    async fn assets_serve_and_block_traversal() {
        ...
        for url in [
            "/apps-assets/webapp/entry.js",
            "/apps-assets/webapp/web/entry.js",
        ] {
            let resp = h.handle(get_req(url)).await.unwrap();
            assert_eq!(resp.status, 200, "{url}: {resp:?}");
    ...
    #[tokio::test]
    async fn assets_cache_control_policy() {
        // 审计 §G / 快赢#1（2026-09-24 批 1/3）：/apps-assets 独立路由同批覆盖
        // Cache-Control——assets/ 哈希名产物 immutable；入口与非哈希名
        //（entry.js / standalone.html）no-cache 保应用升级即时生效。
        let dir = temp_dir_for("cachec");
        let _ = make_app_repo(&dir, "nexos-app-web", serde_json::json!({"id": "webapp"}));
        let h = AppsRouteHandler::new(Arc::new(AppRegistry::with_paths(
            dir.join("apps.db").to_str().unwrap(),
            dir.join("apps-root").to_str().unwrap(),
        ...
        let resp = h
            ...
            .unwrap();
        assert_eq!(resp.status, 201, "{resp:?}");
        // 追加入口 html 与哈希产物（应用包真实形态：standalone.html + assets/ 哈希名）
        let web = dir.join("apps-root").join("webapp").join("web");
        std::fs::write(web.join("standalone.html"), "<html>film standalone</html>").unwrap();
        std::fs::create_dir_all(web.join("assets")).unwrap();
        std::fs::write(web.join("assets/main-Bk3xQ9.js"), "export default {}").unwrap();

        for (url, want) in [
            ("/apps-assets/webapp/entry.js", "no-cache"),
            ("/apps-assets/webapp/standalone.html", "no-cache"),
            (
                "/apps-assets/webapp/assets/main-Bk3xQ9.js",
            ...
            assert_eq!(resp.status, 200, "{url}: {resp:?}");
            assert_eq!(
                resp.headers["cache-control"], want,
                "{url} Cache-Control 应为 {want}"
            );
        }
    }
    ...
    async fn is_engine_enabled_matches_id_or_engine() {
        // v0.1.51 三引擎剥离后 fixture 改用 qrtransfer（film/streaming 会被
        // 安装拦截 400，走不到登记——见下方 stripped_engine 测试）
        let dir = temp_dir_for("gate");
        let _ = make_app_repo(
            &dir,
            "nexos-app-qrtransfer",
            serde_json::json!({"id": "qrtransfer", "engine": "qrtransfer"}),
        );
        let reg = Arc::new(AppRegistry::with_paths(
            dir.join("apps.db").to_str().unwrap(),
            dir.join("apps-root").to_str().unwrap(),
        ...
        assert!(!reg.is_engine_enabled("qrtransfer"), "未装 → 关");
        let h = AppsRouteHandler::new(Arc::clone(&reg));
        let resp = h
            .handle(post_req(
                "/api/v1/apps/install",
                serde_json::json!({"repo": "nexos-app-qrtransfer"}),
            ))
            .await
            .unwrap();
        assert_eq!(resp.status, 201);
        assert!(
            reg.is_engine_enabled("qrtransfer"),
            "装了（id 或 engine 命中）→ 开"
        );
        reg.uninstall("qrtransfer").unwrap();
        assert!(!reg.is_engine_enabled("qrtransfer"), "卸载 → 关");
    }

    // ---- 已剥离引擎安装拦截（v0.1.51 三引擎剥离批）----

    #[test]
    fn stripped_engine_manifest_pure_checks() {
        // engine 字段命中
        assert!(is_stripped_engine_manifest(Some("film"), "whatever"));
        assert!(is_stripped_engine_manifest(Some("streaming"), "whatever"));
        // id 命中（无 engine 字段的旧仓形态）
        assert!(is_stripped_engine_manifest(None, "film"));
        assert!(is_stripped_engine_manifest(None, "streaming"));
        // 正常应用不拦
        assert!(!is_stripped_engine_manifest(Some("qrtransfer"), "qrtransfer"));
        assert!(!is_stripped_engine_manifest(None, "demo-app"));
        assert!(!is_stripped_engine_manifest(Some(""), "demo-app"));
        // 文案含独立产品线引导
        assert!(stripped_engine_hint("film").contains("FilmStudio"));
        assert!(stripped_engine_hint("streaming").contains("StreamingStudio"));
    }

    #[tokio::test]
    async fn install_intercepts_stripped_engine_with_400() {
        // 历史 nexos-app-film / nexos-app-streaming 仓形态（manifest 带 engine
        // 字段）——安装 400 + 引导独立版，不落盘不登记
        for repo in ["nexos-app-film", "nexos-app-streaming"] {
            let dir = temp_dir_for("intercept");
            let engine = repo.trim_start_matches("nexos-app-");
            let _ = make_app_repo(
                &dir,
                repo,
                serde_json::json!({"id": engine, "engine": engine}),
            );
            let reg = Arc::new(AppRegistry::with_paths(
                dir.join("apps.db").to_str().unwrap(),
                dir.join("apps-root").to_str().unwrap(),
            ...
            let resp = h
                ...
                .await
                .unwrap();
            assert_eq!(resp.status, 400, "{repo}: {resp:?}");
            let msg = resp.body["error"].as_str().unwrap();
            assert!(msg.contains("已剥离"), "{repo} 文案: {msg}");
            assert!(msg.contains("Studio"), "{repo} 应引导独立产品线: {msg}");
            // 未登记（已装清单空）+ 未落盘
            assert!(reg.installed_apps().is_empty(), "{repo} 不应登记");
            // 失败任务如实记录（appstore 同款观测面）
            let tasks = reg.tasks_snapshot();
            assert_eq!(tasks.len(), 1);
    ...
    #[tokio::test]
    async fn install_intercepts_stripped_engine_by_id_only() {
        // 旧仓形态兜底：无 engine 字段但 id 即引擎名（与门控 id=? 口径一致）
        let dir = temp_dir_for("intercept-id");
        let _ = make_app_repo(&dir, "nexos-app-legacy", serde_json::json!({"id": "film"}));
        let h = AppsRouteHandler::new(Arc::new(AppRegistry::with_paths(
            dir.join("apps.db").to_str().unwrap(),
            dir.join("apps-root").to_str().unwrap(),
            dir.join("repos").to_str().unwrap(),
        ...
        let resp = h
            .handle(post_req(
                "/api/v1/apps/install",
                serde_json::json!({"repo": "nexos-app-legacy"}),
            ))
            .await
            .unwrap();
        assert_eq!(resp.status, 400, "{resp:?}");
        assert!(resp.body["error"].as_str().unwrap().contains("已剥离"));
    }
    ...
    // ---- 统一任务框架双写（v0.1.54）：装/卸终态 + 失败均落 unified_tasks ----

    #[tokio::test]
    async fn install_uninstall_dual_write_to_unified_tasks() {
        use crate::handlers::tasks_common::{UnifiedStatus, UnifiedTaskStore};

        let dir = temp_dir_for("unified");
        let _ = make_app_repo(&dir, "nexos-app-demo", serde_json::json!({}));
        let unified = Arc::new(UnifiedTaskStore::with_path(
            dir.join("tasks.db").to_str().unwrap(),
        ));
        let h = AppsRouteHandler::new(Arc::new(
            AppRegistry::with_paths(
                dir.join("apps.db").to_str().unwrap(),
                dir.join("apps-root").to_str().unwrap(),
                dir.join("repos").to_str().unwrap(),
            )
        ...
        // 安装成功 → unified 落 done 行（kind=apps、id=app-task-1）
        let resp = h
            .handle(post_req(
                "/api/v1/apps/install",
                serde_json::json!({"repo": "nexos-app-demo"}),
            ...
            .unwrap();
        assert_eq!(resp.status, 201, "{resp:?}");
        let rows = unified.list(Some("apps"), None, 50);
        assert_eq!(rows.len(), 1, "{rows:?}");
        assert_eq!(rows[0].id, "app-task-1");
        assert_eq!(rows[0].status, UnifiedStatus::Done);
        assert!(rows[0].finished_at.is_some(), "同步安装即时终态");
        assert!(rows[0].label.contains("demo-app"), "label 含应用 id");

        // 内存任务面不受影响（保留不动：/apps/tasks 仍是既有 1 条 completed）
        let resp = h.handle(get_req("/api/v1/apps/tasks")).await.unwrap();
        let tasks = resp.body["tasks"].as_array().unwrap();
        assert_eq!(tasks.len(), 1);
        assert_eq!(tasks[0]["status"], "completed");

        // 卸载成功 → unified 追加 app-uninstall-N done 行（内存 Vec 不加行）
        let resp = h.handle(del_req("/api/v1/apps/demo-app")).await.unwrap();
        assert_eq!(resp.status, 200, "{resp:?}");
        let rows = unified.list(Some("apps"), None, 50);
        assert_eq!(rows.len(), 2, "卸载留痕: {rows:?}");
        assert!(rows
            .iter()
            .any(|r| r.id.starts_with("app-uninstall-") && r.status == UnifiedStatus::Done));
        let resp = h.handle(get_req("/api/v1/apps/tasks")).await.unwrap();
        assert_eq!(resp.body["tasks"].as_array().unwrap().len(), 1, "内存面不变");

        // 安装失败（仓库不存在）→ unified 落 error 行
        let resp = h
            .handle(post_req(
                "/api/v1/apps/install",
                serde_json::json!({"repo": "nexos-app-nope"}),
            ...
            .unwrap();
        assert_eq!(resp.status, 404);
        let rows = unified.list(Some("apps"), Some("error"), 50);
        assert_eq!(rows.len(), 1, "失败如实落 error: {rows:?}");
        assert!(rows[0].error.is_some());
    }

    #[tokio::test]
    async fn install_without_unified_store_behaves_unchanged() {
        // 未注入（None）→ 零旁路：安装/任务面行为与历史完全一致
        let dir = temp_dir_for("no-unified");
        let _ = make_app_repo(&dir, "nexos-app-demo", serde_json::json!({}));
        let h = AppsRouteHandler::new(Arc::new(AppRegistry::with_paths(
            dir.join("apps.db").to_str().unwrap(),
            dir.join("apps-root").to_str().unwrap(),
