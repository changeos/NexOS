...
impl ForwardingRouteHandler {
    ...
    ///
    /// 直锁短查保留（批 2 有意不动）：消费方是测试断言——async 端点读走
    /// [`Self::db_call`] 同款闭包。
    #[must_use]
    ...
    /// 当前全量 RDP 转发快照（从 DB 查）。同 [`Self::ssh_tunnels_snapshot`]：
    /// 直锁保留（测试消费），端点走 [`Self::db_call`]。
    #[must_use]
    ...
    /// 锁语义不变：同一把组件级互斥锁、同一条连接（`rdp_accept_loop` 等持
    /// `Arc` 的并发方经同一把锁互斥）、短锁快放不跨 `.await`；panic 经
    /// `resume_unwind` 原样上抛（等价原 `expect("db poisoned")`）。
    async fn db_call<T, F>(&self, f: F) -> T
    where
        T: Send + 'static,
        F: FnOnce(&Connection) -> T + Send + 'static,
    {
        forwarding_db_call(Arc::clone(&self.db), f).await
    }
    ...
    async fn start_tunnel(&self, id: &str) -> Result<SshTunnel, String> {
        let mut tunnel = {
            ...
            self.db_call(move |conn| {
                find_tunnel(conn, &tid).ok_or_else(|| format!("隧道不存在: {tid}"))
            })
            .await?
        };
        ...
        let tunnel_for_db = tunnel.clone();
        self.db_call(move |conn| {
            let _ = update_tunnel(conn, &tunnel_for_db);
        })
        .await;
        Ok(tunnel)
    }
    ...
    /// 已死（stale）→ stopped 并回写 DB。批 2 起 async（两段 DB 访问走
    /// [`Self::db_call`]，中间的 children 锁仍是同步短锁）。
    async fn tunnel_detail(&self, id: &str) -> Option<SshTunnel> {
        let mut tunnel = {
            let tid = id.to_string();
            self.db_call(move |conn| find_tunnel(conn, &tid)).await?
        };
    ...
    async fn start_rdp(&self, id: &str) -> Result<RdpForward, String> {
        let mut fwd = {
            ...
            self.db_call(move |conn| {
                find_rdp(conn, &rid).ok_or_else(|| format!("RDP 转发不存在: {rid}"))
            })
            .await?
        };
        if self
        ...
        let fwd_for_db = fwd.clone();
        self.db_call(move |conn| {
            let _ = update_rdp(conn, &fwd_for_db);
        })
        .await;
        Ok(fwd)
    }
    ...
    async fn watchdog_scan_once(&self) {
        let tunnels = self
            .db_call(move |conn| load_all_tunnels(conn).unwrap_or_default())
            .await;
        for t in tunnels {
            if !t.watchdog {
                continue;
            ...
            if restarted {
                ...
                if give_up {
                    let tid = t.id.clone();
                    self.db_call(move |conn| {
                        if let Some(mut t2) = find_tunnel(conn, &tid) {
...
/// forwarding DB 短事务统一入口（见 [`ForwardingRouteHandler::db_call`] 文档；
/// 批 2 与批 1 的 `market_db_call` / `im_db_call` 同款）：独立成自由函数，供
/// `rdp_accept_loop` 等持 `Arc<Mutex<Connection>>` 的非方法上下文共用。
async fn forwarding_db_call<T, F>(db: Arc<Mutex<Connection>>, f: F) -> T
where
    T: Send + 'static,
    F: FnOnce(&Connection) -> T + Send + 'static,
{
    ...
    async fn handle(&self, req: ApiRequest) -> Result<ApiResponse, ApiGatewayError> {
        ...
        match (req.method, segs.as_slice()) {
            ...
            (HttpMethod::Post, ["api", "v1", "forwarding", "ssh"]) => {
                ...
                let tunnel_for_db = tunnel.clone();
                self.db_call(move |conn| insert_tunnel(conn, &tunnel_for_db))
                    .await?;
                Ok(ApiResponse {
                    status: 201,
            ...
            (HttpMethod::Get, ["api", "v1", "forwarding", "ssh", id]) => {
                match self.tunnel_detail(id).await {
            ...
            (HttpMethod::Delete, ["api", "v1", "forwarding", "ssh", id]) => {
                let existed = {
                    let tid = (*id).to_string();
                    self.db_call(move |conn| find_tunnel(conn, &tid).is_some()).await
                };
                if !existed {
                ...
                {
                    let tid = (*id).to_string();
                    self.db_call(move |conn| delete_tunnel(conn, &tid)).await?;
                }
            ...
            (HttpMethod::Get, ["api", "v1", "forwarding", "rdp"]) => {
                let list = self
                    .db_call(move |conn| load_all_rdp(conn).unwrap_or_default())
                    .await;
                Ok(ok_json(to_value(&list)?))
            }
            ...
            (HttpMethod::Post, ["api", "v1", "forwarding", "rdp"]) => {
                ...
                let fwd_for_db = fwd.clone();
                self.db_call(move |conn| insert_rdp(conn, &fwd_for_db)).await?;
                Ok(ApiResponse {
                    status: 201,
                    body: to_value(&fwd)?,
                    headers: serde_json::json!({}),
            ...
            (HttpMethod::Delete, ["api", "v1", "forwarding", "rdp", id]) => {
                let existed = {
                    let rid = (*id).to_string();
                    self.db_call(move |conn| find_rdp(conn, &rid).is_some()).await
                };
                if !existed {
                ...
                {
                    let rid = (*id).to_string();
                    self.db_call(move |conn| delete_rdp(conn, &rid)).await?;
                }
                Ok(ok_json(serde_json::json!({
                    "deleted": id,
                    "last_status": stopped.status,
            ...
            (HttpMethod::Get, ["api", "v1", "forwarding", "rdp", id, "rdp-file"]) => {
                let fwd = {
                    let rid = (*id).to_string();
                    self.db_call(move |conn| find_rdp(conn, &rid)).await
                };
            ...
            Ok((mut inbound, _peer)) => {
                {
                    let db = Arc::clone(&db);
                    let id = id.clone();
                    forwarding_db_call(db, move |conn| {
