...
/// 调用日志（logs 表）保留天数缺省（v0.1.49 快赢 E.3 / 审查 A2-1：无界增长
/// 的结构性封顶——查询与写入双端惰性裁剪，见
/// [`ApiGatewayRouteHandler::prune_logs_if_due`]）。
pub const GW_LOG_RETAIN_DAYS_DEFAULT: u64 = 30;

/// 解析 env `NEXOS_GW_LOG_RETAIN_DAYS`（纯函数，可单测，同
/// [`parse_upstream_timeout_secs`] 手法）：`Some(合法非负整数)` → 该值
/// （`0` = 显式关闭裁剪，保留全部）；缺省 / 空串 / 非数字 →
/// [`GW_LOG_RETAIN_DAYS_DEFAULT`]（30 天——解析失败不猜更激进的值）。
#[must_use]
pub fn parse_log_retain_days(raw: Option<&str>) -> u64 {
    match raw.map(str::trim).filter(|s| !s.is_empty()) {
        None => GW_LOG_RETAIN_DAYS_DEFAULT,
        Some(s) => s.parse::<u64>().unwrap_or(GW_LOG_RETAIN_DAYS_DEFAULT),
    }
}

/// logs 保留裁剪的最小触发间隔（节流窗：写入/查询端点每次都检查，但实际
/// DELETE 至多每小时一轮——「写入时惰性删过期，别每次全删」）。
const LOG_PRUNE_MIN_INTERVAL: Duration = Duration::from_secs(3600);
...
/// 所有 DB 访问用短锁快查快放（同步执行，不跨 `.await` 持锁）。`db` 取
/// `Arc<Mutex<Connection>>`（审计 A1-1/Top1 批 2，2026-09-25）：锁结构不变
/// ——仍是组件级单连接单互斥，Arc 化仅为 `spawn_blocking` 闭包可持 `'static`
/// 句柄；**media_gen / llm_external / http.rs SSE 特挂共享本实例的计费原子面
/// 依赖同一把锁**（见 [`Self::try_charge_image`] 与模块级"查-检-扣"注记），
/// Arc 化不改互斥语义；计费原子面（try_charge_image / redeem / payments
/// confirm）自计费专项批 3（2026-09-25）起整段包 `db_call`——闭包内同一把
/// 锁一次持锁，原子边界与互斥性不变。
pub struct ApiGatewayRouteHandler {
    db: Arc<Mutex<Connection>>,
    counter: Mutex<u64>,
    /// logs 保留裁剪节流（上次裁剪时刻；None=本实例尚未裁过）。写入
    /// （record_success/record_failure）与查询（GET logs/stats）双端共用
    /// 同一节流窗——先到者裁一轮，间隔内其余调用直接跳过（别每次全删）。
    /// Arc 化同 `db`（批 2）：`spawn_blocking` 闭包持 clone 进裁剪点。
    log_pruned_at: Arc<Mutex<Option<std::time::Instant>>>,
...
impl ApiGatewayRouteHandler {
    ...
    pub fn with_db_path(path: &str) -> Self {
        ...
        Self {
            ...
            log_pruned_at: Arc::new(Mutex::new(None)),
    ...
    /// 当前全量渠道快照（从 DB 查）。
    ///
    /// 直锁短查保留（批 2 有意不动）：消费方是 capabilities 的**同步** trait
    /// 刷新面 + 测试——async 端点读走 [`Self::db_call`] 同款闭包，别复用本函数。
    #[must_use]
    ...
    /// 当前全量令牌快照（从 DB 查）。
    ///
    /// 直锁短查保留（批 2 有意不动）：消费方是测试辅助——async 端点读走
    /// [`Self::db_call`]。
    #[must_use]
    ...
    /// DB 短事务统一入口（审计 A1-1/Top1 批 2，2026-09-25，与批 1 的 im.rs /
    /// api_market.rs / tips.rs 同款）：闭包在 `spawn_blocking` 线程里拿锁执行
    /// ——锁等待与 SQLite 同步 IO 离开 tokio worker，async 任务只 `.await` 结果。
    ///
    /// 锁语义与原先「async 里直接 `self.db.lock()`」完全一致：
    /// - 同一把组件级互斥锁、同一条连接（WAL）——读写仍互斥串行，原子边界
    ///   不变（不引连接池；rusqlite `Connection` 非 `Sync` 仍单连接共享）；
    ///   media_gen / http.rs SSE 特挂共享本实例——它们与计费原子面（如
    ///   [`Self::try_charge_image`] 的查-检-扣，批 3 起同样走 `db_call` 闭包）
    ///   经**同一把锁**互斥，搬运不改变互斥性；
    /// - 短锁快放：闭包返回即解锁，绝不跨 `.await` 持锁；
    /// - panic 面一致：锁中毒/闭包 panic 经 `resume_unwind` 原样上抛（等价原
    ///   `expect("db poisoned")`），不静默吞错。
    async fn db_call<T, F>(&self, f: F) -> T
    where
        T: Send + 'static,
        F: FnOnce(&Connection) -> T + Send + 'static,
    {
        gw_db_call(Arc::clone(&self.db), f).await
    }
    ...
    ///
    /// 批 2（2026-09-25）起 async：两段 DB 快查走 [`Self::db_call`]（读类，
    /// 各自单锁快照——与原先两次独立短锁的语义一致，本函数不含扣减，配额
    /// 扣减在转发成功后的 [`Self::record_success`]）。
    pub async fn resolve_forward_plan(
        &self,
        ...
        // 2. 查令牌（DB 快查，拿到快照后释放锁做异步转发）
        let token_snap = {
            let key = key.clone();
            self.db_call(move |conn| find_token_by_key(conn, &key).unwrap_or(None))
                .await
        };
        ...
        // 4. 查模型映射 → 候选渠道（DB 快查，快照后锁外做纯计算排序）
    ...
    async fn proxy_forward(
        ...
        let plan = match self.resolve_forward_plan(&req.headers, &req.body).await {
            Ok(p) => p,
        ...
        for ch in ordered {
            ...
            match self.forward_channel(ch, suffix, &fwd_body).await {
                ...
                Err(e) => {
                    ...
                    self.record_failure(token, ch, model, &last_err).await;
                }
            }
        }
        // 6. 全失败 → 502
    ...
    /// logs 保留裁剪（v0.1.49 快赢 E.3 / 审查 A2-1）：`DELETE FROM logs
    /// WHERE created_at < now - NEXOS_GW_LOG_RETAIN_DAYS`（缺省 30 天，0=关闭）。
    ///
    /// **双端惰性触发**：写入（[`Self::record_success`] / [`Self::record_failure`]）
    /// 与查询（GET /gateway/logs、/gateway/stats）路径都调本方法，但同一节流窗
    /// （[`LOG_PRUNE_MIN_INTERVAL`]，默认 1h）内只执行一轮 DELETE——调用频次
    /// 高的端点不因此每次多付一次全表扫描。`min_interval` 参数供测试注入
    /// （`Duration::ZERO` = 强制触发）。失败仅日志（保留策略绝不拦业务写）。
    /// （`&self` 形态的裁剪入口：批 2 后生产路径全部走 [`Self::prune_logs_locked`]
    /// 进 `db_call` 闭包，本包装仅剩测试直调——`#[cfg(test)]` 消除非测试构建的
    /// dead_code。）
    #[cfg(test)]
    fn prune_logs_if_due(&self, conn: &Connection, min_interval: Duration) {
        Self::prune_logs_locked(&self.log_pruned_at, conn, min_interval);
    }

    /// [`Self::prune_logs_if_due`] 的核心体：节流时刻锁单独收参——批 2 起
    /// 写入/查询端点把整段（含本裁剪）挪进 [`Self::db_call`] 闭包，闭包不能
    /// 借 `&self`，持 `log_pruned_at` 的 Arc clone 调这里（同锁同语义）。
    fn prune_logs_locked(
        last: &Mutex<Option<std::time::Instant>>,
        conn: &Connection,
        min_interval: Duration,
    ) {
        let mut last = last.lock().expect("log prune state poisoned");
        ...
        *last = Some(std::time::Instant::now());
        let retain =
            parse_log_retain_days(std::env::var("NEXOS_GW_LOG_RETAIN_DAYS").ok().as_deref());
        if retain == 0 {
            return; // 显式关闭（保留全部——env 调用点求值，热改生效）
        }
        let cutoff = (chrono::Local::now() - chrono::Duration::days(retain as i64))
            .format("%Y-%m-%dT%H:%M:%S%:z")
            .to_string();
        match conn.execute("DELETE FROM logs WHERE created_at < ?", params![cutoff]) {
            Ok(n) if n > 0 => {
                eprintln!("[api-gateway] logs 保留裁剪：删除 {n} 行过期日志（retain={retain}d）");
            }
            Ok(_) => {}
            Err(e) => eprintln!("[api-gateway] logs 保留裁剪失败（忽略）: {e}"),
        }
    }
    ...
    /// 真实数据铁律：不估算不编造）。
    ///
    /// 批 2（2026-09-25）起 async：整段写库（insert + 惰性裁剪 + 渠道计数 +
    /// 计费扣减）在 [`Self::db_call`] 闭包里**单锁一次**完成——与原先单次锁持
    /// 的全部步骤一比一相同（独立写类：扣减额由 usage 快照算出，锁内无
    /// 查-检-扣分支——那是 [`Self::try_charge_image`] 的原子面，批 3 已同款
    /// 包 `db_call`）。
    ...
    pub async fn record_success(
        ...
        let model_for_db = model.to_string();
        let now_for_db = now;
        let pruned_at = Arc::clone(&self.log_pruned_at);
        self.db_call(move |conn| {
            let _ = insert_log(conn, &log);
            // 写入端惰性裁剪（保留策略见 prune_logs_if_due——节流窗内零成本跳过）
            Self::prune_logs_locked(&pruned_at, conn, LOG_PRUNE_MIN_INTERVAL);
            // 渠道计数 + last_used
            let _ = bump_channel(conn, &channel_id, &now_for_db);
            ...
            let cost = match token_for_db.billing_mode.as_str() {
                "free" => 0,
                _ => {
                    let model_ratio = get_model_ratio(conn, &model_for_db);
                    let group_ratio = get_group_ratio(conn, &token_for_db.group_name);
                    calc_cost(tt as u64, model_ratio, group_ratio)
                }
            };
            let _ = bump_token(conn, &token_for_db.id, cost, &now_for_db);
        })
        .await;
    }

    /// 记录失败调用日志（写 DB，短锁快放；流式转发的渠道级失败同样走这里）。
    /// 批 2 起 async：整段（insert + 惰性裁剪）在 [`Self::db_call`] 单锁完成。
    pub async fn record_failure(&self, token: &ApiToken, channel: &Channel, model: &str, err: &str) {
        let log = CallLog {
            id: self.next_id("log"),
            token_id: token.id.clone(),
            token_name: token.name.clone(),
    ...
    ///
    /// **计费专项批 3（2026-09-25）落地**：整段（查-检-扣）包进 [`Self::db_call`]
    /// 闭包——闭包在 `spawn_blocking` 线程里**单锁一次持锁**跑完全部步骤，与原先
    /// async 里直接 `self.db.lock()` 的临界区一比一相同（同一把锁、锁内无 await，
    /// 原子性不破，并发不超扣不变），锁等待与 SQLite 同步 IO 全部离开 tokio
    /// worker。与 media_gen / llm_external 共享实例的互斥性不变（同一把锁）。
    pub async fn try_charge_image(&self, bearer: &str) -> Result<ChargeOutcome, String> {
        let key = bearer.trim().to_string();
        // 单次 DB 锁内完成：查令牌 → 校验 → 余额预检 → 扣费（原子，防并发超扣；
        // 全程无 await，锁不跨异步点）
        self.db_call(move |conn| -> Result<ChargeOutcome, String> {
...
/// api-gateway DB 短事务统一入口（见 [`ApiGatewayRouteHandler::db_call`] 文档；
/// 批 2 与批 1 的 `market_db_call` / `im_db_call` 同款）：独立成自由函数，
/// 供持 `Arc<Mutex<Connection>>` 的非方法上下文共用。
async fn gw_db_call<T, F>(db: Arc<Mutex<Connection>>, f: F) -> T
where
    T: Send + 'static,
    F: FnOnce(&Connection) -> T + Send + 'static,
{
    ...
    async fn handle(&self, req: ApiRequest) -> Result<ApiResponse, ApiGatewayError> {
        ...
        match (req.method, segs.as_slice()) {
            ...
            (HttpMethod::Get, ["api", "v1", "gateway", "channels"]) => {
                ...
                let snap = self
                    .db_call(move |conn| load_all_channels(conn).unwrap_or_default())
                    .await;
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "channels"]) => {
                    ...
                    .filter(|s| !s.is_empty())
                {
                    ...
                    self.db_call(move |conn| insert_channel(conn, &channel)).await?;
                    return Ok(ApiResponse {
                        status: 201,
                        body: resp,
                        headers: serde_json::json!({}),
                ...
                self.db_call(move |conn| insert_channel(conn, &channel))
                    .await?;
                Ok(ApiResponse {
                    status: 201,
                    body: resp,
            ...
            (HttpMethod::Put, ["api", "v1", "gateway", "channels", id]) => {
                ...
                // 查-改-写整段单锁完成（与原先同一把锁同一批步骤；独立写类，
                // 批 2 搬 spawn_blocking——锁等待与 SQLite IO 离开 tokio worker）
                let cid = (*id).to_string();
                let c = self
                    .db_call(move |conn| -> Result<Option<Channel>, rusqlite::Error> {
                        let Some(mut c) = find_channel(conn, &cid)? else {
                            return Ok(None);
                        };
            ...
            (HttpMethod::Delete, ["api", "v1", "gateway", "channels", id]) => {
                let cid = (*id).to_string();
                let affected = self
                    .db_call(move |conn| delete_channel(conn, &cid))
                    .await?;
                if affected == 0 {
                    return Ok(error_response(404, &format!("渠道不存在: {id}")));
                }
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "channels", id, "test"]) => {
                ...
                match outcome {
                    Ok(models) => {
                        ...
                        let cid = (*id).to_string();
                        let _ = self
                            .db_call(move |conn| update_channel_status(conn, &cid, "enabled"))
                            .await;
                    ...
                    Err(e) => {
                        ...
                        let cid = (*id).to_string();
                        let _ = self
                            .db_call(move |conn| update_channel_status(conn, &cid, "error"))
                            .await;
            ...
            (HttpMethod::Get, ["api", "v1", "gateway", "tokens"]) => {
                ...
                let snap = self
                    .db_call(move |conn| load_all_tokens(conn).unwrap_or_default())
                    .await;
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "tokens"]) => {
                ...
                self.db_call(move |conn| insert_token(conn, &token)).await?;
                Ok(ApiResponse {
                    status: 201,
                    body: resp,
                    headers: serde_json::json!({}),
            ...
            (HttpMethod::Delete, ["api", "v1", "gateway", "tokens", id]) => {
                ...
                let affected = self.db_call(move |conn| delete_token(conn, &tid)).await?;
                if affected == 0 {
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "tokens", id, "disable"]) => {
                let tid = (*id).to_string();
                let updated = self
                    .db_call(move |conn| -> Result<Option<ApiToken>, rusqlite::Error> {
                        let Some(mut t) = find_token(conn, &tid)? else {
                            return Ok(None);
                        };
                        t.enabled = false;
            ...
            (HttpMethod::Get, ["api", "v1", "gateway", "stats"]) => {
                ...
                let stats = self
                    .db_call(move |conn| {
                        ...
                        // SQL 直出聚合（COUNT/SUM/CASE）——不再全表载入内存逐条算
                        // （v0.1.49 快赢 E.3 / 审查 A2-1；等价性对拍测试
                        // stats_sql_aggregation_matches_in_memory）
                        let (log_count, log_total_tokens, log_success) =
                            aggregate_logs(conn).unwrap_or((0, 0, 0));
                        ...
                        let total_requests: u64 =
                            log_count.max(channels.iter().map(|c| c.request_count).sum::<u64>());
                        let success_rate = if log_count == 0 {
                            0.0
                        } else {
                            (log_success as f64 / log_count as f64) * 100.0
                        };
                        GatewayStats {
            ...
            (HttpMethod::Get, ["api", "v1", "gateway", "mappings"]) => {
                let mappings = self
                    .db_call(move |conn| load_all_mappings(conn).unwrap_or_default())
                    .await;
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "mappings"]) => {
                ...
                let resp = to_value(&mapping)?;
                {
                    ...
                    self.db_call(move |conn| upsert_mapping(conn, &mapping))
                        .await?;
                }
                Ok(ApiResponse {
                    status: 201,
            ...
            (HttpMethod::Delete, ["api", "v1", "gateway", "mappings", name]) => {
                ...
                let affected = self
                    .db_call(move |conn| delete_mapping(conn, &mname))
                    .await?;
                if affected == 0 {
            ...
            (HttpMethod::Get, ["api", "v1", "gateway", "model-ratios"]) => {
                let ratios = self
                    .db_call(move |conn| load_all_model_ratios(conn).unwrap_or_default())
                    .await;
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "model-ratios"]) => {
                ...
                {
                    let ratio_for_db = ratio.clone();
                    self.db_call(move |conn| upsert_model_ratio(conn, &ratio_for_db))
                        .await?;
                }
            ...
            (HttpMethod::Delete, ["api", "v1", "gateway", "model-ratios", model]) => {
                let mname = (*model).to_string();
                let affected = self
                    .db_call(move |conn| delete_model_ratio(conn, &mname))
                    .await?;
                if affected == 0 {
            ...
            (HttpMethod::Get, ["api", "v1", "gateway", "group-ratios"]) => {
                let ratios = self
                    .db_call(move |conn| load_all_group_ratios(conn).unwrap_or_default())
                    .await;
                Ok(ok_json(to_value(&ratios)?))
            }
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "group-ratios"]) => {
                ...
                {
                    let ratio_for_db = ratio.clone();
                    self.db_call(move |conn| upsert_group_ratio(conn, &ratio_for_db))
                        .await?;
                }
                Ok(ok_json(to_value(&ratio)?))
            }
            ...
            (HttpMethod::Get, ["api", "v1", "gateway", "redeem-codes"]) => {
                let codes = self
                    .db_call(move |conn| load_all_redeem_codes(conn).unwrap_or_default())
                    .await;
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "redeem-codes"]) => {
                ...
                let now = now_iso();
                let quota_amount = body.quota_amount;
                let created: Vec<RedeemCode> = self
                    .db_call(move |conn| -> Result<Vec<RedeemCode>, rusqlite::Error> {
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "redeem"]) => {
                ...
                // 2. 单锁：查令牌 + 查码 + 标记已用 + 扣减 quota_used
                //
                // **计费专项批 3（2026-09-25）落地**：查码-检未用-标记-加配额的
                // 多步原子整段包 `db_call`——闭包在 `spawn_blocking` 线程里单锁
                // 一次持锁跑完全部步骤（同一把锁、锁内无 await，原子性不破），
                // 锁等待与 SQLite 同步 IO 离开 tokio worker。并发不超扣加压：
                // `redeem_concurrent_50_same_code_exactly_one_wins`（50 路同码
                // 恰 1 成功）。
                let key_for_db = key;
                let outcome = self
                    .db_call(
                        move |conn| -> Result<
                            Result<(String, String, u64), (u16, String)>,
                            rusqlite::Error,
                        > {
                            let Some(token) = find_token_by_key(conn, &key_for_db)? else {
                                return Ok(Err((401, "无效的 API Key".into())));
                            };
                            if !token.enabled || token.status == "disabled" {
                                return Ok(Err((401, "令牌已禁用".into())));
                            }
                            let Some(mut code) = find_redeem_code(conn, &body.code)? else {
                                return Ok(Err((404, "兑换码不存在".into())));
                            };
                            if code.used_by.is_some() {
                                return Ok(Err((409, "兑换码已被使用".into())));
                            }
                            // 过期校验（expires_at 非空且已过期）
                            if let Some(exp) = &code.expires_at {
                                if !exp.is_empty() && is_expired(exp) {
                                    return Ok(Err((410, "兑换码已过期".into())));
                                }
                            }
                            // 标记已用
                            let now = now_iso();
                            ...
                            Ok(Ok((code.code, token.id, code.quota_amount)))
                        },
                    )
                    .await?;
                match outcome {
                    Ok((code, token_id, added_quota)) => Ok(ok_json(serde_json::json!({
                        "ok": true,
                        "code": code,
                        "token_id": token_id,
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "payments"]) => {
                ...
                {
                    let tid = body.token_id.clone();
                    let order_for_db = order.clone();
                    let token_exists = self
                        .db_call(move |conn| -> Result<bool, rusqlite::Error> {
                            // 目标令牌必须存在（404），且积分加给有效令牌
                            if find_token(conn, &tid)?.is_none() {
                                return Ok(false);
                            }
                            insert_payment_order(conn, &order_for_db)?;
                            Ok(true)
                        })
                        .await?;
                    if !token_exists {
            ...
            (HttpMethod::Get, ["api", "v1", "gateway", "payments"]) => {
                ...
                let orders = self
                    .db_call(move |conn| load_payment_orders(conn, status.as_deref()))
                    .await?;
            ...
            //
            //    **计费专项批 3（2026-09-25）落地**：查单与确认写段各自整段包
            //    `db_call`（闭包内单锁一次持锁，原子性不破）；链上核验 `.await`
            //    仍在两段锁**之间**（rusqlite 守卫非 Send，锁不跨异步点）。
            //    写段闭包内加**幂等复核**（重新查单仍 pending 才入账）——原先
            //    「锁外核验 await 期间另一 confirm 已入账」的双入账窗口就此关闭。
            (HttpMethod::Post, ["api", "v1", "gateway", "payments", id, "confirm"]) => {
                ...
                let found = self.db_call(move |conn| find_payment_order(conn, &pid)).await?;
                ...
                // （计费入账原子：查-检-写-加积分整段在 db_call 闭包内单锁一次
                // 完成；锁内幂等复核关闭核验 await 期间的双入账窗口）
                let order_for_db = order.clone();
                let txid_for_db = txid.clone();
                let credited = self
                    .db_call(
                        move |conn| -> Result<Option<PaymentOrder>, rusqlite::Error> {
                            // 幂等复核：锁外链上核验 await 期间订单可能已被另一
                            // confirm 迁离 pending——非 pending 不再二次入账
                            match find_payment_order(conn, &order_for_db.id)?.as_ref() {
                                Some(cur) if cur.status == "pending" => {}
                                _ => return Ok(None),
                            }
                            let mut o = order_for_db;
                            o.status = "confirmed".into();
                            o.confirmed_at = Some(now_iso());
                            if let Some(txid) = txid_for_db {
                                o.txid = Some(txid);
                            }
                            insert_payment_order(conn, &o)?;
                            add_token_credits(conn, &o.token_id, o.credits)?;
                            Ok(Some(o))
                        },
                    )
                    .await?;
                let Some(order) = credited else {
                    return Ok(error_response(
                        409,
                        "订单状态已变更（并发确认），仅 pending 可确认",
                    ));
                };
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "payments", id, "reject"]) => {
                ...
                let pid = (*id).to_string();
                // (Some(order), None)=已拒；(_, Some(status))=状态冲突 → 409；
                // (None, None)=订单不存在 → 404（独立写类：查-检-写单锁完成，
                // 与原先同一把锁同一批步骤，批 2 搬 spawn_blocking）
                let (rejected, conflict) = self
                    .db_call(
                        move |conn| -> Result<(Option<PaymentOrder>, Option<String>), rusqlite::Error> {
                            let Some(mut order) = find_payment_order(conn, &pid)? else {
                                return Ok((None, None));
                            };
                            if order.status != "pending" {
                                return Ok((None, Some(order.status)));
                            }
                    ...
                    .await?;
                if let Some(status) = conflict {
                    return Ok(error_response(
                        409,
                        &format!("订单状态为 {status}，仅 pending 可拒绝"),
                    ));
                }
                let Some(order) = rejected else {
                    return Ok(error_response(404, &format!("订单不存在: {id}")));
                };
...
/// 默认 DB 路径：env `NEXOS_GATEWAY_DB` 覆写（隔离实例验证用，同 film 侧
/// `NEXOS_FILM_DB` 先例）→ /tank/os-data/gateway.db → /var/lib/os/gateway.db
/// → ./gateway.db（保底）。
fn default_db_path() -> String {
    if let Ok(p) = std::env::var("NEXOS_GATEWAY_DB") {
...
fn open_db(path: &str) -> rusqlite::Result<Connection> {
    ...
    // busy_timeout=3000ms（审计 E#6：全仓统一补齐，DB 锁等待不直接报错）
...
/// logs 表 SQL 直出聚合三元组 `(total_requests, total_tokens, success_count)`
/// ——stats 端点聚合下推到 SQLite（COUNT/SUM/CASE 单行返回），不再全表载入
/// 内存逐条算（v0.1.49 快赢 E.3 / 审查 A2-1）。
///
/// 与旧内存算法（`load_all_logs` 后逐条 map/filter/sum）等价：对拍测试
/// `stats_sql_aggregation_matches_in_memory` 以同 fixture 双算断言一致。
/// 空表 → `(0, 0, 0)`（COALESCE 兜底 + query_row unwrap_or）。
fn aggregate_logs(conn: &Connection) -> rusqlite::Result<(u64, u64, u64)> {
    conn.query_row(
        "SELECT COUNT(*),
...
/// demo 调用日志时间戳（相对当前时刻倒推 `m` 分钟）。v0.1.49 保留裁剪上线后
/// 固定历史时间串（原 2026-08-08）会随日历推进被 30 天策略当过期行删掉，
/// 令 demo 断言随机月份腐化——相对时间一劳永逸。
#[cfg(test)]
fn demo_log_ts(minutes_ago: i64) -> String {
...
mod tests {
    ...
    // ---- SQL 聚合 + logs 保留策略（v0.1.49 快赢 E.3 / 审查 A2-1）----

    /// 测试时间戳助手：`d` 天前（与 now_iso 同格式，保证字典序=时间序）。
    fn ts_days_ago(d: i64) -> String {
    ...
    /// 测试时间戳助手：`m` 分钟前。
    fn ts_minutes_ago(m: i64) -> String {
        (chrono::Local::now() - chrono::Duration::minutes(m))
            .format("%Y-%m-%dT%H:%M:%S%:z")
            .to_string()
    }

    /// 造一条指定形态的日志（db 直插，同 seed_token 手法）。
    fn mk_log(id: &str, status: &str, total_tokens: u32, created_at: String) -> CallLog {
        CallLog {
            id: id.into(),
            token_id: "tok-t".into(),
            token_name: "t".into(),
            channel_id: "ch-t".into(),
            channel_name: "c".into(),
            model: "m".into(),
            prompt_tokens: 0,
    ...
    /// 聚合等价对拍：混合 fixture（成功/失败/0 token/多 token）上，stats 端点
    /// 的 SQL 直出聚合与**旧内存算法**（load_all_logs 全量载入逐条算——原
    /// 2304-2315 行实现）结果一致。
    #[tokio::test]
    async fn stats_sql_aggregation_matches_in_memory() {
        let h = ApiGatewayRouteHandler::with_empty();
        {
            let conn = h.db.lock().unwrap();
            insert_log(&conn, &mk_log("log-s1", "success", 200, ts_minutes_ago(50))).unwrap();
            insert_log(&conn, &mk_log("log-s2", "success", 80, ts_minutes_ago(40))).unwrap();
            insert_log(&conn, &mk_log("log-s3", "success", 0, ts_minutes_ago(30))).unwrap();
            insert_log(&conn, &mk_log("log-f1", "failed", 0, ts_minutes_ago(20))).unwrap();
            insert_log(&conn, &mk_log("log-f2", "failed", 0, ts_minutes_ago(10))).unwrap();
        }
        // 旧算法（内存逐条——被替换前的实现原样复刻）
        let (old_total_requests, old_total_tokens, old_success_count, old_len) = {
            let conn = h.db.lock().unwrap();
            let logs = load_all_logs(&conn).unwrap();
            let total_requests: u64 = logs.iter().map(|_| 1u64).sum();
            ...
            (total_requests, total_tokens, success_count, logs.len())
        };
        // 新算法直查（等价性的另一半：不依赖端点，隔离验证 aggregate_logs 本体）
        let sql = {
            let conn = h.db.lock().unwrap();
            aggregate_logs(&conn).unwrap()
        };
        assert_eq!(sql.0, old_total_requests, "COUNT(*) 与逐条计数应一致");
        assert_eq!(
            sql.1, old_total_tokens,
            "SUM(total_tokens) 与逐条求和应一致"
        );
        assert_eq!(
            sql.2, old_success_count as u64,
            "SUM(CASE success) 与过滤计数应一致"
        );
        // 端点整链路（含 SQL 聚合接线 + max 渠道计数语义；空渠道和=0 不干扰）
        let resp = h.handle(get_req("/api/v1/gateway/stats")).await.unwrap();
        assert_eq!(resp.status, 200);
        assert_eq!(
            resp.body["total_requests"].as_u64().unwrap(),
            old_total_requests
        );
        assert_eq!(
            resp.body["total_tokens"].as_u64().unwrap(),
            old_total_tokens
        );
        let expect_rate = if old_len == 0 {
            0.0
        } else {
            (old_success_count as f64 / old_len as f64) * 100.0
        };
        assert!(
            (resp.body["success_rate"].as_f64().unwrap() - expect_rate).abs() < 1e-9,
            "success_rate 应与旧算法一致: {}",
            resp.body["success_rate"]
        );
    }

    /// 保留裁剪：过期行（>缺省 30 天）删除，窗内行保留（强制触发——
    /// min_interval=ZERO 绕过节流；env 未设 → 缺省 30 天口径）。
    #[test]
    fn logs_retention_prunes_expired_keeps_recent() {
        let h = ApiGatewayRouteHandler::with_empty();
        let conn = h.db.lock().unwrap();
        insert_log(&conn, &mk_log("log-old", "success", 10, ts_days_ago(31))).unwrap();
        insert_log(&conn, &mk_log("log-edge", "success", 20, ts_days_ago(29))).unwrap();
        insert_log(&conn, &mk_log("log-new", "failed", 0, ts_days_ago(1))).unwrap();
        h.prune_logs_if_due(&conn, Duration::ZERO);
        let ids: Vec<String> = load_all_logs(&conn)
            .unwrap()
            .into_iter()
            .map(|l| l.id)
            .collect();
        assert!(
            !ids.contains(&"log-old".to_string()),
            "31 天前应被裁剪: {ids:?}"
        );
        assert!(
            ids.contains(&"log-edge".to_string()),
            "29 天前应保留: {ids:?}"
        );
        assert!(
            ids.contains(&"log-new".to_string()),
            "1 天前应保留: {ids:?}"
        );
    }

    /// 节流：同节流窗内不重复 DELETE（别每次全删）；窗归零后立即再裁。
    #[test]
    fn logs_retention_throttled_within_interval() {
        let h = ApiGatewayRouteHandler::with_empty();
        let conn = h.db.lock().unwrap();
        let ids_now = || -> Vec<String> {
            load_all_logs(&conn)
                .unwrap()
                .into_iter()
                .map(|l| l.id)
        ...
        insert_log(&conn, &mk_log("log-old1", "success", 1, ts_days_ago(40))).unwrap();
        h.prune_logs_if_due(&conn, LOG_PRUNE_MIN_INTERVAL);
        assert!(
            !ids_now().contains(&"log-old1".to_string()),
            "首轮（从未裁过）应立即执行"
        );
        insert_log(&conn, &mk_log("log-old2", "success", 1, ts_days_ago(40))).unwrap();
        h.prune_logs_if_due(&conn, LOG_PRUNE_MIN_INTERVAL);
        assert!(
            ids_now().contains(&"log-old2".to_string()),
            "节流窗内再次到期不应重复 DELETE"
        );
        h.prune_logs_if_due(&conn, Duration::ZERO);
        assert!(
            !ids_now().contains(&"log-old2".to_string()),
            "min_interval=ZERO 强制触发应再裁"
        );
    }

    /// 写入端接线：record_success 首调即触发一轮裁剪（惰性——不靠查询端）。
    #[tokio::test]
    async fn record_success_triggers_lazy_prune() {
        let h = ApiGatewayRouteHandler::with_empty();
        {
            let conn = h.db.lock().unwrap();
            insert_log(&conn, &mk_log("log-old", "success", 5, ts_days_ago(45))).unwrap();
        }
        let token = demo_tokens().remove(0);
        let channel = demo_channels().remove(0);
        h.record_success(&token, &channel, "m-test", 1, 2, 3, 10, None)
            .await;
        let logs = h.logs_snapshot();
        assert!(
            logs.iter().all(|l| l.id != "log-old"),
            "写入路径应顺带裁掉过期行: {:?}",
            logs.iter().map(|l| l.id.clone()).collect::<Vec<_>>()
        );
        assert!(
            logs.iter()
                .any(|l| l.status == "success" && l.total_tokens == 3),
            "本次调用日志应照常落库"
        );
    }

    /// env 解析纯函数（同 parse_upstream_timeout_secs 手法，不设进程 env——
    /// 并行测试下 set_var 有互踩风险，纯函数入参注入即可覆盖全分支）。
    #[test]
    fn parse_log_retain_days_variants() {
        assert_eq!(parse_log_retain_days(None), 30, "缺省 30 天");
        assert_eq!(parse_log_retain_days(Some("7")), 7);
        assert_eq!(parse_log_retain_days(Some(" 14 ")), 14, "空白容忍");
        assert_eq!(parse_log_retain_days(Some("0")), 0, "0=显式关闭");
        assert_eq!(parse_log_retain_days(Some("")), 30, "空串回缺省");
        assert_eq!(parse_log_retain_days(Some("abc")), 30, "非数字回缺省");
        assert_eq!(parse_log_retain_days(Some("-3")), 30, "负数非法回缺省");
    }
...
data: {\"usage\":{\"prompt_tokens\":9,\"completion_tokens\":8,\"total_tokens\":17}}\n\n";
    ...
    async fn logs_pagination_limit_enforced() {
        ...
        let h = ApiGatewayRouteHandler::with_empty();
        // created_at 递增（now-5..now-1 分钟），确保 ORDER BY 后顺序稳定（最新在前）。
        // 相对时间：固定历史串会被 30 天保留裁剪当过期行删掉（GET logs 已接线裁剪）
            ...
            for i in 0..5 {
                insert_log(
                    ...
                    &CallLog {
                        ...
                        status: "success".into(),
                        error: None,
                        created_at: (base - chrono::Duration::minutes(5 - i as i64))
                            .format("%Y-%m-%dT%H:%M:%S%:z")
                            .to_string(),
                    },
                )
    ...
    #[tokio::test]
    async fn redeem_concurrent_50_same_code_exactly_one_wins() {
        // 计费专项批 3 加压（2026-09-25）：50 路 tokio 并发兑**同一张码**——
        // 查-检-扣整段在 db_call 闭包内单锁一次持锁，应恰 1 路成功（200），
        // 其余 409（已被使用）；配额只入账一份（quota_used 恰减一份额度），
        // used_by 只指向一个受益人。这是 redeem 面防并发超扣/重复入账的
        // 数字证明（与 try_charge_image_concurrent_never_overcharges 同族）。
        let h = std::sync::Arc::new(ApiGatewayRouteHandler::with_empty());
        {
            let conn = h.db.lock().unwrap();
            insert_token(
                ...
                &ApiToken {
                    id: "tok-race50".into(),
                    name: "并发兑换".into(),
                    key: "sk-os-race50".into(),
                    status: "active".into(),
                    enabled: true,
                    // limit=0（无限）：只验证 quota_used 的扣减侧，不与余额闸门耦合
                    quota_limit: 0,
                    quota_used: 1000,
                    allowed_models: vec![],
                    allowed_channels: vec![],
        ...
        for _ in 0..50 {
            let h2 = h.clone();
            jobs.push(tokio::spawn(async move {
                h2.handle(post_req_with_auth(
                    "/api/v1/gateway/redeem",
                    serde_json::json!({"code": "REDEEM-RACE-0001"}),
                    "sk-os-race50",
                ))
                .await
                .expect("handle 不 panic")
            }));
        }
        let mut ok_200 = 0usize;
        let mut conflict_409 = 0usize;
        for j in jobs {
            let resp = j.await.unwrap();
            match resp.status {
                200 => ok_200 += 1,
                409 => conflict_409 += 1,
                _ => panic!("非预期状态 {}: {resp:?}", resp.status),
            }
        }
        assert_eq!(ok_200, 1, "50 路同码并发应恰 1 路成功（实际 {ok_200}）");
        assert_eq!(
            conflict_409, 49,
            "其余 49 路应全部 409 已被使用（实际 {conflict_409}）"
        );
        // 入账只发生一份：quota_used 1000 → 500（若重复入账会到 0）
        let t = h
            .tokens_snapshot()
            .into_iter()
            .find(|t| t.id == "tok-race50")
            .unwrap();
        assert_eq!(t.quota_used, 500, "配额只入账一份（不重复扣减）");
        // 码的 used_by 只指向唯一受益人
        let used_by = {
            let conn = h.db.lock().unwrap();
            find_redeem_code(&conn, "REDEEM-RACE-0001")
                .unwrap()
                .and_then(|c| c.used_by)
        };
        assert_eq!(used_by.as_deref(), Some("tok-race50"), "码恰好被用一次");
    }
