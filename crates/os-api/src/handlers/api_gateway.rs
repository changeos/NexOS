...
pub struct ApiGatewayRouteHandler {
    ...
    log_pruned_at: Mutex<Option<std::time::Instant>>,
...
impl ApiGatewayRouteHandler {
    ...
    fn prune_logs_if_due(&self, conn: &Connection, min_interval: Duration) {
        let mut last = self.log_pruned_at.lock().expect("log prune state poisoned");
    ...
    pub fn record_success(
        ...
        self.prune_logs_if_due(&conn, LOG_PRUNE_MIN_INTERVAL);
        // 渠道计数 + last_used
...
mod tests {
    ...
    #[test]
    fn record_success_triggers_lazy_prune() {
        let h = ApiGatewayRouteHandler::with_empty();
        {
            let conn = h.db.lock().unwrap();
