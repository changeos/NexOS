...
impl ApiGatewayRouteHandler {
    ...
    ///   media_gen / http.rs SSE 特挂共享本实例——它们与直锁路径（如
    ///   [`Self::try_charge_image`] 的查-检-扣计费原子）经**同一把锁**互斥，
    ///   搬运不改变互斥性；
    ...
    /// 查-检-扣分支——那是 [`Self::try_charge_image`] 的原子面，本批不动）。
    ...
    ///
    /// **批 2（2026-09-25）明确保留直锁原样**（跨多步原子面清单之一）：查-检-扣
    /// 计费原子是本组件与 media_gen / llm_external 共享实例的正确性核心，
    /// 本批不动计费面；后续方案——把整段（单锁一次、锁内无 await）整体包进
    /// `gw_db_call` 闭包可保持原子性（闭包内仍是同一把锁一次持锁），随计费
    /// 专项批次 + 并发不超扣测试一起做，不与读类搬运混批。
    ...
    async fn handle(&self, req: ApiRequest) -> Result<ApiResponse, ApiGatewayError> {
        ...
        match (req.method, segs.as_slice()) {
            ...
            (HttpMethod::Post, ["api", "v1", "gateway", "redeem"]) => {
                ...
                //
                // **批 2（2026-09-25）明确保留直锁原样**（跨多步原子面清单之一）：
                // 查码-检未用-标记-加配额是配额授予侧的查-检-扣原子（与
                // try_charge_image 同类的多步原子，不可拆）——本批不动计费面；
                // 后续随计费专项批次整体包 db_call（单锁一次，原子性不破）。
                let conn = self.db.lock().expect("db poisoned");
            ...
            //
            //    **批 2（2026-09-25）明确保留直锁原样**（跨多步原子面清单之一）：
            //    确认写段（订单状态迁移 + add_token_credits 加积分）是积分入账侧
            //    的计费多步原子，本批不动计费面；后续随计费专项批次整体包
            //    db_call（单锁一次，原子性不破）。
