...
static ZFS_AVAILABLE: Lazy<bool> = Lazy::new(|| {
    let avail = zfs_probe_from(std::env::var_os("PATH"), std::env::var(ENV_ZFS_PROBE).ok());
    if avail {
...
";
    ...
    fn is_zfs_binary_missing_classification() {
        ...
        // 业务错误（池不存在等）与二进制无关
        assert!(!is_zfs_binary_missing(&StorageError::PoolNotFound(
            "p".into()
        )));
    }
    ...
    async fn zfs_unavailable_scrub_quota_degrade_without_500() {
        ...
        let resp = h
            .handle(get("/api/v1/pools/tank/scrub-status"))
            .await
            .unwrap();
        assert_eq!(resp.status, 200);
    ...
    async fn zfs_binary_missing_backend_error_degrades_reactively() {
        ...
        let h =
            handler_with(FixtureRunner::new().on("zpool", "list", CommandOutput::fail(127, "")));
        let resp = h.handle(get("/api/v1/pools")).await.unwrap();
        assert_eq!(resp.status, 200);
        assert_eq!(resp.body["zfs_available"], false);
    }
