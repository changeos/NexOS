...
//! | GET | `/api/v1/p2p/node-meta` | 公开 | 节点元数据注册表快照（meta 组件：id/addrs/first_seen/last_seen/state/source + 自述 agent_version/hostname/platform/gpu——未自述对端字段缺席，按健康分降序、Inactive 殿后） |
...
//!   条目另含节点自述四字段 agent_version/hostname/platform/gpu，2026-09-14
//!   联邦节点信息协议——探测链 [`probe_node_self_desc`] 在启动装配注入）。
//!
//! # 节点自述探测链（2026-09-14，v0.1.45 批——联邦节点信息协议）
//!
//! node-meta 只有地址簿，集群侧无法确证节点版本/硬件（Spark 版本测不出 /
//! 新 3060 节点身份未知两次踩坑）——os-api 启动时探测一次自述
//! （[`probe_node_self_desc`]：版本 `CARGO_PKG_VERSION` / hostname 与 discover
//! 同源 / platform 取 os-release PRETTY_NAME 或内核首段 / gpu 经轻量
//! `nvidia-smi -L` 一次即缓存），main.rs 装配经 `Handle::set_self_desc` 注入
//! ——digest 自广播首条携带（60s 常驻 gossip 心跳搭车，零新流量），对端
//! merge 落库、node-meta 响应透传。os-p2p crate 保持纯不做硬件探测（setter
//! 注入面）；旧节点双向兼容（收不到 = null，旧节点忽略未知字段）。
//!
//! # 请求体解析
//!
...
// ----------------------------------------------------------------------------
// 节点自述探测链（联邦节点信息协议，2026-09-14 v0.1.45 批）
// ----------------------------------------------------------------------------

/// 探测本机节点自述（os-api 启动时一次，main.rs 装配注入
/// `Handle::set_self_desc`——digest 自广播首条携带，集群侧据此知道每个节点的
/// agent 版本/hostname/platform/GPU）。
///
/// 背景踩坑：node-meta 只有地址簿，集群侧无法确证节点版本/硬件——Spark 版本
/// 测不出 / 新 3060 节点 0x03f13c02 身份未知两次踩坑。自述随 60s 常驻 gossip
/// 心跳搭车（零新流量）；旧节点（无 desc 字段）双向兼容。
///
/// **探测链在本层（os-api 网关进程）**：os-p2p crate 保持纯不做硬件探测
/// （setter 注入面），版本取本 crate `CARGO_PKG_VERSION`、hostname 与
/// discover 同源、gpu 复用 capabilities 同款 nvidia-smi 调用（`-L` 轻量形态，
/// 一次探测即注入快照——进程内缓存）。
///
/// blocking 探测（nvidia-smi 子进程 + 两个 /proc|/etc 文件读）：启动装配路径
/// 单次调用可接受；失败降级 None（如 nvidia-smi 缺失 → gpu="none"，CPU-only
/// 节点照常自述）。
#[must_use]
pub fn probe_node_self_desc() -> os_p2p::NodeSelfDesc {
    os_p2p::NodeSelfDesc {
        agent_version: Some(env!("CARGO_PKG_VERSION").to_string()),
        hostname: Some(crate::handlers::discover::detect_hostname()).filter(|h| !h.is_empty()),
        platform: detect_platform(),
        gpu: Some(probe_gpu_summary()),
    }
}

/// platform 自述：`/etc/os-release` 的 `PRETTY_NAME`（如 "Ubuntu 24.04.1 LTS"）
/// → 缺失回退 `/proc/sys/kernel/osrelease` 首段（"linux 6.8.0-30-generic"——
/// 只取首段内核号，整串太长）→ 都没有 None。
fn detect_platform() -> Option<String> {
    if let Ok(content) = std::fs::read_to_string("/etc/os-release") {
        if let Some(pretty) = parse_os_release_pretty_name(&content) {
            return Some(pretty);
        }
    }
...
/// 解析 os-release 内容取 `PRETTY_NAME` 值（去引号；空值/缺行 None）。
...
/// GPU 摘要探测：`nvidia-smi -L`（轻量列表形态——与 capabilities 探测链同款
/// 命令的列表用法，启动一次即缓存）。无 nvidia-smi / 失败 / 无卡 → "none"
/// （诚实自述——CPU-only 节点照常参与联邦）。
fn probe_gpu_summary() -> String {
    let out = std::process::Command::new("nvidia-smi")
        ...
        .output();
    let names = match out {
        Ok(o) if o.status.success() => parse_nvidia_smi_l(&String::from_utf8_lossy(&o.stdout)),
        _ => Vec::new(),
    };
    gpu_summary_from_names(&names)
}

/// 解析 `nvidia-smi -L` 输出的卡名列表（每行 `GPU <idx>: <name> (UUID: …)`，
/// MIG 子设备行 `MIG …` 跳过——按物理卡计数）。坏行跳过。
fn parse_nvidia_smi_l(output: &str) -> Vec<String> {
    output
        .lines()
        .filter_map(|l| {
            let rest = l.trim().strip_prefix("GPU ")?;
            let after_idx = rest.split_once(": ")?;
            let mut name = after_idx.1.trim();
            // 截掉尾部 "(UUID: …)"（部分驱动形态无括号则整名保留）
...
/// 卡名列表 → 自述摘要：同型多卡合并带 ×N（"NVIDIA GeForce RTX 3060 ×2"），
/// 保序去重；空列表 → "none"。
fn gpu_summary_from_names(names: &[String]) -> String {
    if names.is_empty() {
...
mod tests {
    ...
    async fn node_meta_lists_registry_entries_with_shape() {
        ...
        // 自述四字段（2026-09-14 联邦节点信息协议）：未自述对端 = 字段缺席
        // （skip_serializing_if——旧前端/旧节点线格式零变化）；带自述的端到端
        // 见 self_desc_gossip_reaches_peer_node_meta
        for f in ["agent_version", "hostname", "platform", "gpu"] {
            assert!(
                entry.get(f).is_none(),
                "未自述对端不带 desc 字段 {f}（线格式零变化）"
            );
        }
    ...
    // 3. 节点自述联邦端到端（联邦节点信息协议，2026-09-14 v0.1.45 批）：
    //    隔离双实例 A 注入自述（Handle::set_self_desc——os-api 启动装配的
    //    落点，探测链见 probe_node_self_desc）→ 60s 常驻 gossip 自广播首条
    //    携带 → B 的 node-meta 注册表学到 A 的 agent_version/hostname/
    //    platform/gpu；REST node-meta 响应透传新字段；反向对称（B→A）。
    #[tokio::test]
    async fn self_desc_gossip_reaches_peer_node_meta() {
        let (a, b) = two_node_mesh().await;
        let desc = os_p2p::NodeSelfDesc {
        ...
        let stored = a.set_self_desc(desc.clone()).await;
        assert_eq!(stored, desc, "注入应答回存快照");
        // 等 B 经 gossip 学到 A 的自述（每 6 tick 一轮广播；测试节奏下秒级，
        // 宽限 10s——merge 对 Active 条目按报告新鲜度采纳自述）
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(10);
        let learned = loop {
            let metas = b.node_meta().await;
            let hit = metas
                .iter()
                .find(|e| e.id == *a.self_id())
                .filter(|e| e.agent_version.as_deref() == Some("0.1.45"))
                .cloned();
            if let Some(e) = hit {
                break e;
            }
            assert!(
                std::time::Instant::now() < deadline,
                "10s 内 B 应经 gossip 学到 A 的自述: {metas:?}"
            );
            tokio::time::sleep(std::time::Duration::from_millis(100)).await;
        };
        assert_eq!(learned.hostname.as_deref(), Some("nexos-a"));
        assert_eq!(learned.platform.as_deref(), Some("Ubuntu 24.04.1 LTS"));
        assert_eq!(
            learned.gpu.as_deref(),
            Some("NVIDIA GeForce RTX 3060 ×2"),
            "同型多卡 ×N 摘要原样到达"
        );
        // REST 透传：GET node-meta 响应里 A 的条目带自述字段
        let h = P2pRouteHandler::new(b.clone(), "node-b".into(), false);
        let resp = h.handle(get_req(PATH_NODE_META)).await.unwrap();
        assert_eq!(resp.status, 200);
        let entry = resp
            .body
            ...
            .unwrap()
            .iter()
            .find(|e| e["id"] == a.self_id().to_hex())
            .expect("B 的注册表含 A");
        assert_eq!(entry["agent_version"], serde_json::json!("0.1.45"));
        assert_eq!(entry["hostname"], serde_json::json!("nexos-a"));
        assert_eq!(entry["platform"], serde_json::json!("Ubuntu 24.04.1 LTS"));
        assert_eq!(
            entry["gpu"],
            serde_json::json!("NVIDIA GeForce RTX 3060 ×2")
        );
        // 反向对称：B 注入自述 → A 学到（gossip 双向）
        let desc_b = os_p2p::NodeSelfDesc {
            agent_version: Some("0.1.45".into()),
            hostname: Some("nexos-b".into()),
            platform: Some("linux 6.8.0-30-generic".into()),
            gpu: Some("none".into()),
        };
        b.set_self_desc(desc_b).await;
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(10);
        loop {
            ...
            let hit = metas
                ...
                .filter(|e| e.hostname.as_deref() == Some("nexos-b"))
                .cloned();
            if let Some(e) = hit {
                assert_eq!(e.gpu.as_deref(), Some("none"), "无卡节点的诚实自述到达");
                break;
            }
            assert!(
                std::time::Instant::now() < deadline,
                "10s 内 A 应经 gossip 学到 B 的自述: {metas:?}"
            );
            tokio::time::sleep(std::time::Duration::from_millis(100)).await;
        }
        a.shutdown().await;
    ...
    // —— 节点自述探测链（probe_node_self_desc 及纯解析函数）——

    // 3a. os-release PRETTY_NAME 解析：取值去引号；缺行/空值 → None
    #[test]
    fn parse_os_release_pretty_name_variants() {
        let ubuntu = "NAME=\"Ubuntu\"\nVERSION=\"24.04.1 LTS (Noble Numbat)\"\n\
                      PRETTY_NAME=\"Ubuntu 24.04.1 LTS\"\nVERSION_ID=\"24.04\"\n";
        assert_eq!(
            parse_os_release_pretty_name(ubuntu).as_deref(),
            Some("Ubuntu 24.04.1 LTS")
        );
        // 无引号形态（合法 os-release）
        assert_eq!(
            parse_os_release_pretty_name("PRETTY_NAME=NexOS\n").as_deref(),
            Some("NexOS")
        );
        // 缺行 / 空值 → None
        assert_eq!(parse_os_release_pretty_name("NAME=X\nID=x\n"), None);
        assert_eq!(parse_os_release_pretty_name("PRETTY_NAME=\"\"\n"), None);
        assert_eq!(parse_os_release_pretty_name(""), None);
    }

    // 3b. nvidia-smi -L 解析：GPU 行取卡名（截 UUID 括注）；MIG 子设备行跳过；
    //     坏行/空输出跳过
    #[test]
    fn parse_nvidia_smi_l_variants() {
        let dual = "GPU 0: NVIDIA GeForce RTX 3060 (UUID: GPU-aaaa-bbbb-cccc)\n\
                    GPU 1: NVIDIA GeForce RTX 3060 (UUID: GPU-dddd-eeee-ffff)\n";
        assert_eq!(
            parse_nvidia_smi_l(dual),
            vec![
        ...
        let mixed = "GPU 0: NVIDIA GB200 (UUID: GPU-1111)\n\
                     MIG 1g.12g  0: NVIDIA GB200 (UUID: MIG-2222)\n\
                     GPU 1: NVIDIA A100-SXM4-80GB\n";
        assert_eq!(
            parse_nvidia_smi_l(mixed),
            vec![
                "NVIDIA GB200".to_string(),
                "NVIDIA A100-SXM4-80GB".to_string()
            ],
            "MIG 子设备行跳过；无 UUID 括注的整名保留"
        );
        assert!(parse_nvidia_smi_l("").is_empty(), "无卡空输出");
        assert!(
            parse_nvidia_smi_l("garbage line\nnot a gpu listing\n").is_empty(),
            "坏行跳过"
        );
    }

    // 3c. GPU 摘要合成：同型多卡合并 ×N、异卡 \" + \" 连接、空 → \"none\"
    #[test]
    fn gpu_summary_dedupe_and_none() {
        assert_eq!(
            gpu_summary_from_names(&[
                "RTX 3060".to_string(),
                "RTX 3060".to_string(),
                "RTX 3060".to_string()
            ]),
        ...
        assert_eq!(
            gpu_summary_from_names(&["RTX 3060".to_string()]),
            "RTX 3060",
            "单卡不带 ×1"
        );
        assert_eq!(
            gpu_summary_from_names(&["A100".to_string(), "RTX 3060".to_string()]),
            "A100 + RTX 3060",
            "异卡保序 + 连接"
        );
        assert_eq!(gpu_summary_from_names(&[]), "none", "无卡诚实自述");
    }

    // 3d. 探测链整体形状：版本 = os-api CARGO_PKG_VERSION；hostname 非空（与
    //     discover 同源）；platform 在 Linux 上至少内核回退；gpu 恒有值
    //     （\"none\" = 无 nvidia-smi 的 CPU-only 机器诚实自述——不失败降级）
    #[test]
    fn probe_node_self_desc_shape() {
        let d = probe_node_self_desc();
        assert_eq!(
            d.agent_version.as_deref(),
            Some(env!("CARGO_PKG_VERSION")),
            "版本取 os-api crate 版本"
        );
        assert!(d.hostname.is_some_and(|h| !h.is_empty()), "hostname 非空");
        assert!(d.platform.is_some(), "platform 至少内核回退（Linux）");
        assert!(d.gpu.is_some(), "gpu 恒有值（none = CPU-only 诚实自述）");
    }
    ...
    async fn conn_watcher_reports_new_connections_once() {
        ...
        spawn_conn_watcher(a.clone(), std::time::Duration::from_millis(50), move |id| {
        ...
        assert!(rx.try_recv().is_err(), "已见 peer 的后续拍不应重复回调");
    ...
    fn market_login(h: &crate::handlers::api_market::ApiMarketRouteHandler) -> (String, String) {
        use k256::elliptic_curve::rand_core::OsRng;
        let sk = k256::ecdsa::SigningKey::random(&mut OsRng);
        let pubkey = format!(
            "0x{}",
    ...
    async fn market_fed_directed_replay_via_relay_topology() {
        ...
        // A↔B 无直连（前置断言——本测试的全部意义在此）。
        {
            let peers = a.peers().await;
            assert!(
                !peers.iter().any(|x| x.id == *b.self_id() && x.connected),
                "A 与 B 不应直连（中继拓扑前提）"
            );
        }
        ...
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(10);
        loop {
            let learned = a.node_meta().await.iter().any(|e| {
                e.id == *b.self_id() && matches!(e.state, os_p2p::MetaState::Active { .. })
            });
