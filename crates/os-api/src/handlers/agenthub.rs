...
impl WebState {
    ...
    fn start_blocking(
        ...
        loop {
            if self.launcher.port_open(desc.port) {
                eprintln!(
                    "[agenthub] Web 服务 {agent_id} 已就绪（pid {pid}，端口 {}）",
                    desc.port
                );
    ...
    fn stop_blocking(&self, agent_id: &str, port: u16) -> Result<bool, String> {
        ...
        // 其子进程——fuser 按端口杀掉它
        eprintln!("[agenthub] Web 服务 {agent_id} 杀 pid 后端口 {port} 仍开，fuser 按端口兜底");
        self.launcher.kill_port(port)?;
        for _ in 0..10 {
            if !self.launcher.port_open(port) {
                self.procs
        ...
        Err(format!(
            "进程已发终止信号且按端口兜底，端口 {port} 仍被占用（可能被其它进程占用）"
        ))
    }
    ...
    async fn handle_web_start(
        ...
        let Some(desc) = agent.web.clone() else {
            return Ok(error_response(
                400,
                &format!(
                    "{} 未标注 Web 界面描述符（仅实测确认有 Web UI 的 agent 提供）",
                    agent.name
                ),
            ));
        };
        ...
        let started =
            tokio::task::spawn_blocking(move || web.start_blocking(&agent_id, &desc_for_job))
                .await
...
mod tests {
    ...
    async fn routes_declares_sixteen_endpoints_all_agenthub() {
        ...
        assert!(
            routes
                .iter()
                .any(|r| r.path == "/api/v1/agenthub/web/:agentId/status" && !r.requires_auth),
            "缺 Web status 路由"
        );
    }
    ...
    impl WebLauncher for MockWebLauncher {
        fn spawn(
            ...
            self.spawned.lock().unwrap().push(argv.to_vec().join(" "));
    ...
    fn web_handler(launcher: Arc<MockWebLauncher>) -> (AgentHubRouteHandler, TempDirGuard) {
    ...
    fn web_base_host_raw_three_branches() {
        ...
        assert_eq!(
            web_base_host_raw(Some("192.168.1.5:8558"), None),
            "192.168.1.5"
        );
        assert_eq!(
            web_base_host_raw(Some("nas.lan"), Some("1.2.3.4")),
            "nas.lan"
        );
        // 空/空白 Host 视为缺失
        assert_eq!(
            web_base_host_raw(Some("  "), Some("10.0.0.9:9000")),
            "10.0.0.9"
        );
        // 分支 2：通告 host（去端口）
        assert_eq!(
    ...
    async fn web_start_ready_idempotent_stop_roundtrip() {
        ...
        assert_eq!(
            resp.body["port"].as_u64(),
            Some(u64::from(OPENCODE_WEB_PORT))
        );
        ...
        assert!(
            argv.contains("--hostname 0.0.0.0"),
            "跨机访问需绑 0.0.0.0: {argv}"
        );
    ...
    async fn web_stop_falls_back_to_kill_port_for_shim_processes() {
        ...
        assert_eq!(
            resp.status, 200,
            "shim 场景 stop 应经 fuser 兜底成功: {resp:?}"
        );
        assert_eq!(launcher_holder.killed.lock().unwrap().as_slice(), [4242u32]);
        assert_eq!(
    ...
    async fn web_start_failure_port_never_ready() {
        ...
        assert_eq!(
            launcher_holder.killed.lock().unwrap().len(),
            1,
            "失败应 kill 子进程"
        );
        // 状态回到未运行（表条目已清）
