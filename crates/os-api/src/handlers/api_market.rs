...
fn apply_access_info_mask(out: &mut serde_json::Value, info: &AccessInfo, reveal: bool) {
...
pub fn merge_server_config(body: Option<ServerConfig>, probed: ServerConfig) -> ServerConfig {
    ...
    let gpu_vram_mb = b
        .gpu_vram_mb
        .or_else(|| gpus.first().and_then(|g| g.vram_mb));
...
impl RelayStream {
    ...
    pub async fn next_chunk(&mut self) -> Option<Result<Vec<u8>, String>> {
        match tokio::time::timeout(self.idle, self.rx.recv()).await {
            ...
            Ok(Some(Ok(ApiRelayEvent::Head { .. }))) => Some(Err("协议错误：重复 Head 帧".into())),
        }
    }
}
...
impl ApiMarketRouteHandler {
    ...
    fn access_info_revealed(&self, req: &ApiRequest, publisher_pubkey: &str) -> bool {
        ...
        chain_auth::bearer_token(&req.headers)
            .is_some_and(|t| self.admin_token.as_deref().is_some_and(|admin| admin == t))
    }
    ...
    /// DB 短事务统一入口（审计 A1-1/Top1 批 1，2026-09-25，与 im.rs 同款）：
    /// 闭包在 `spawn_blocking` 线程里拿锁执行——锁等待与 SQLite 同步 IO 离开
    /// tokio worker，async 任务只 `.await` 结果。锁语义不变：同一把组件级
    /// 互斥锁、同一条连接（与联邦端点共享的 `Arc`）、短锁快放不跨 `.await`；
    /// panic 经 `resume_unwind` 原样上抛（等价原 `expect("db poisoned")`）。
    async fn db_call<T, F>(&self, f: F) -> T
    where
        T: Send + 'static,
        F: FnOnce(&Connection) -> T + Send + 'static,
    {
        market_db_call(Arc::clone(&self.db), f).await
    }
}

/// api-market DB 短事务统一入口（见 [`ApiMarketRouteHandler::db_call`] 文档；
/// 独立成自由函数供中继请求处理器等持 `Arc<Mutex<Connection>>` 的非方法
/// 上下文共用）。
async fn market_db_call<T, F>(db: Arc<Mutex<Connection>>, f: F) -> T
where
    T: Send + 'static,
    F: FnOnce(&Connection) -> T + Send + 'static,
{
...
impl ApiMarketFedEndpoint {
            ...
            .swap(true, std::sync::atomic::Ordering::SeqCst)
        {
            ...
            tokio::spawn(async move {
                loop {
                    ...
                    let limits = inner.limits.lock().map(|l| *l).unwrap_or_default();
    ...
    pub fn set_relay_limits_for_test(&self, limits: RelayLimits) {
        *self
            ...
            .limits
            .lock()
            .expect("api-market fed limits poisoned") = limits;
    }
    ...
    pub fn broadcast_entry(&self, entry: &ApiListing) {
        ...
        (t.broadcast)(build_api_market_fed_payload(
            &t.node_hex,
            &t.node_name,
            entry,
        ));
    ...
    pub async fn backfill_to(&self, peer: &os_p2p::NodeId) -> usize {
        ...
        for (i, entry) in entries.iter().enumerate() {
            ...
            (send_to)(
                peer,
                build_api_market_fed_payload(&node_hex, &node_name, entry),
            );
        }
    ...
    fn ingest_inner(
        ...
        let key = format!("api_market\u{0}{node}\u{0}{}\u{0}{snapshot}", entry.id);
        {
            let mut seen = self
                ...
                .seen
                .lock()
                .expect("api-market fed seen poisoned");
    ...
    pub async fn relay_roundtrip(
        ...
        let limits = *self.inner.limits.lock().map_err(|_| "内部状态锁中毒")?;
        ...
        loop {
            match tokio::time::timeout(budget(), rx.recv()).await {
                Err(_) => return Err(format!("中继超时（{}s 未收完响应）", timeout.as_secs())),
    ...
    fn handle_relay_req(&self, from: &os_p2p::NodeId, payload: &serde_json::Value) {
        let limits = *self
            ...
            .limits
            .lock()
            .expect("api-market fed limits poisoned");
        ...
        if ci == 0 {
            ...
            if cn == 1 {
                ...
                let parts: Vec<Option<Vec<u8>>> =
                    (0..cn).map(|i| (i == 0).then(|| chunk.clone())).collect();
                partials.insert(
                    req_id,
                    PartialRelayReq {
                        from: from.clone(),
        ...
        let ready = {
            ...
            if p.received == p.parts.len() {
                ...
                let mut done = partials
                    .remove(&req_id)
                    .expect("received==len 前刚 get_mut 命中");
    ...
    fn ingest_relay_resp(&self, from: &os_p2p::NodeId, payload: &serde_json::Value) {
        ...
        if p.target != *from {
            eprintln!("[api-market-relay] 丢弃伪造应答 req_id={req_id}（发送方 != 定向目标）");
            return;
        }
        p.last_seen = std::time::Instant::now();
        let tx = p.tx.clone();
...
async fn relay_execute_and_reply(
    ...
    let mk_frame = |seq: u64,
     ...
     -> serde_json::Value {
        ...
        if let Some(c) = chunk {
            f["chunk_base64"] =
                serde_json::Value::String(base64::engine::general_purpose::STANDARD.encode(c));
        }
        if let Some(e) = error {
    ...
    let allowed =
        market_db_call(Arc::clone(&inner.db), local_published_endpoints).await;
        ...
        match (req.method, segs.as_slice()) {
            ...
            (HttpMethod::Post, ["api", "v1", "api-market", "publish"]) => {
                ...
                let existing = {
                    ...
                    let pubkey = publisher_pubkey.clone();
                    self.db_call(move |conn| {
                        find_by_name_owner(conn, &api_name, &pubkey).map_err(db_err)
                    })
                    .await?
                };
                ...
                {
                    let listing_for_db = listing.clone();
                    self.db_call(move |conn| {
                        insert_listing(conn, &listing_for_db).map_err(db_err)
                    })
                    .await?;
                }
            ...
            (HttpMethod::Get, ["api", "v1", "api-market"]) => {
                ...
                let mut list = {
                    let q = q.clone();
                    self.db_call(move |conn| load_listings(conn, q.as_deref(), sort))
                        .await
                        .map_err(db_err)?
                };
                if scope != "all" {
            ...
            (HttpMethod::Get, ["api", "v1", "api-market", id]) => {
                let listing = {
                    ...
                    self.db_call(move |conn| find_by_id(conn, &lid))
                        .await
                        .map_err(db_err)?
                };
            ...
            (HttpMethod::Delete, ["api", "v1", "api-market", id]) => {
                ...
                {
                    let lid = (*id).to_string();
                    self.db_call(move |conn| delete_listing(conn, &lid))
                        .await
                        .map_err(db_err)?;
                }
            ...
            (HttpMethod::Post, ["api", "v1", "api-market", id, "heartbeat"]) => {
                ...
                {
                    // 心跳是 api-market 最高频写（A1-1 点名热点）——blocking 池执行
                    let lid = (*id).to_string();
                    let hb = heartbeat_at.clone();
                    self.db_call(move |conn| {
                        update_heartbeat(conn, &lid, &hb, &load).map_err(db_err)
                    })
                    .await?;
                }
            ...
            (HttpMethod::Post, ["api", "v1", "api-market", id, "federate"]) => {
                ...
                let saved = {
                    ...
                    self.db_call(move |conn| {
                        ...
                        Ok::<ApiListing, ApiGatewayError>(e2)
                    })
                    .await?
                };
...
mod tests {
    ...
    fn heartbeat_sweep_refreshes_stale_local_only() {
        ...
        {
            let conn = h.db.lock().expect("db poisoned");
            insert_listing(&conn, &mk("sweep-none", "local", None, None)).expect("种入无心跳条目");
            insert_listing(
                &conn,
                &mk("sweep-stale", "local", Some(stale_hb.clone()), Some(load)),
            )
            .expect("种入过期心跳条目");
            insert_listing(
                &conn,
                &mk("sweep-fresh", "local", Some(iso_offset_secs(0)), None),
            )
            .expect("种入新鲜心跳条目");
            insert_listing(
                &conn,
...
Socket(s):                               1\n";
    ...
    fn apply_unified_vram_fills_unified_entries_only() {
        ...
        let gb10 = &gpus[0];
        assert!(
            gb10.unified_vram_mb.unwrap_or(0) > 0,
            "池总量应填: {gb10:?}"
        );
    ...
    async fn access_info_publish_refresh_and_rotate() {
        ...
        body["access_info"] =
            serde_json::json!({ "api_key": " sk-os-rotated-9999 ", "notes": "  " });
        let resp = h
            .handle(authed(HttpMethod::Post, PATH_PUBLISH, &token, body))
            .await
            .unwrap();
    ...
    async fn access_info_masking_by_viewer_matrix() {
        ...
        let own_detail = h
            ...
            .unwrap();
        assert_eq!(
            own_detail.body["access_info"]["api_key"], "sk-os-abcdefgh1234",
            "本人明文"
        );
        let own_list = h
            .handle(authed(
        ...
        let a = own_list
            ...
            .unwrap();
        assert_eq!(
            a["access_info"]["api_key"], "sk-os-abcdefgh1234",
            "本人列表明文"
        );
        ...
        let b_own = h
            ...
            .unwrap();
        assert_eq!(
            b_own.body["access_info"]["api_key"], "short8ky",
            "本人短 key 明文"
        );
        ...
        let admin_view = h
            ...
            .unwrap();
        assert_eq!(
            admin_view.body["access_info"]["api_key"], "sk-os-abcdefgh1234",
            "admin 读面明文"
        );
    ...
    async fn access_info_admin_principal_reveal_matrix() {
        ...
        req.auth = crate::http::extract_principal(&req.headers, None, Some(&gw_admin)).await;
        assert!(
            is_admin(req.auth.as_ref()),
            "无头请求默认注入 admin Principal"
        );
        let resp = h.handle(req).await.unwrap();
        assert_eq!(
            resp.body["access_info"]["api_key"], "sk-os-reveal12345",
            "① 默认注入 admin → 明文"
        );
        ...
        assert!(
            is_admin(req.auth.as_ref()),
            "admin token 精确匹配注入 admin Principal"
        );
        let resp = h.handle(req).await.unwrap();
        assert_eq!(
            resp.body["access_info"]["api_key"], "sk-os-reveal12345",
        ...
        let resp = h.handle(req).await.unwrap();
        assert_eq!(
            resp.body["access_info"]["api_key"], "sk-o***2345",
            "③ 他人链上身份 → 脱敏"
        );
    ...
    fn curl_auth_header_line_plaintext_and_placeholder_branches() {
        ...
        assert_eq!(
            ...
            (format!("-H 'Authorization: Bearer {ph}'"), true)
        );
        assert_eq!(
    ...
    fn context_len_serde_and_merge_contract() {
        ...
        let old: ServerConfig = serde_json::from_value(
            serde_json::json!({ "model_name": "M", "max_model_len": 32768 }),
        )
        .unwrap();
    ...
    async fn federate_owner_only_and_404() {
        ...
        let resp = h
            ...
            .unwrap();
        assert_eq!(resp.status, 404);
        assert!(t.0.lock().unwrap().is_empty(), "全部失败路径零广播");
    ...
    async fn fed_ingest_idempotent_merge() {
        ...
        let payload = |node: &str, e: serde_json::Value| serde_json::json!({ "fed": FED_KIND_API_MARKET_LOBBY, "node": node, "entry": e });
        // ① 新条目 → Written：source_node=载荷来源节点、本地计数清零（对端
        //    计数是它的活跃度）、federated 随载荷、access_info 落库。
        assert_eq!(
            h.federation()
                .ingest(&payload("node-106", entry(10, "fed-api"))),
            ApiMarketFedIngest::Written
        );
        ...
        assert_eq!(
            saved.access_info.api_key.as_deref(),
            Some("sk-os-fed-abcdef99")
        );
        ...
        assert_eq!(
            h.federation()
                .ingest(&payload("node-106", entry(20, "fed-api"))),
            ApiMarketFedIngest::Refreshed
        );
        let saved = &h.listings_snapshot()[0];
        ...
        assert_eq!(
            h.federation()
                .ingest(&payload("node-b", entry(99, "fed-api"))),
            ApiMarketFedIngest::Skipped
        );
        assert_eq!(
        ...
        assert_eq!(
            h.federation()
                .ingest(&serde_json::json!({"fed": "nexhub_lobby"})),
            ApiMarketFedIngest::Invalid
        );
        assert_eq!(
            h.federation().ingest(&serde_json::json!({
    ...
    async fn bridge_dispatches_api_market_lobby_payload() {
        ...
        let fed_list = h
            .handle(get_req(&format!("{PATH_LIST}?scope=fed")))
            .await
            .unwrap();
        ...
        let local_list = h
            .handle(get_req(&format!("{PATH_LIST}?scope=local")))
            .await
            .unwrap();
    ...
    async fn relay_stream_chunks_in_order() {
        ...
        assert!(
            text.contains("\"content\":\"chunk-a\""),
            "第一块在前: {text}"
        );
        assert!(
            text.contains("\"content\":\"chunk-b\""),
            "第二块在后: {text}"
        );
        assert!(text.contains("data: [DONE]"), "收尾帧透传: {text}");
        assert!(
            text.find("\"content\":\"chunk-a\"").unwrap()
                < text.find("\"content\":\"chunk-b\"").unwrap(),
            "帧序即字节序"
        );
    }
    ...
    async fn fed_replay_round_duplicate_then_refresh_after_heartbeat() {
        let p = backfill_pair();
        p.b_online.store(true, std::sync::atomic::Ordering::SeqCst); // B 在线收广播
        let (_, token) = login(&p.a, &new_key());
    ...
    fn fed_direct_replay_targets_filter_matrix() {
        ...
        let meta_entry = |id: &os_p2p::NodeId, state: os_p2p::MetaState| os_p2p::NodeMetaEntry {
            id: id.clone(),
            addrs: vec![],
            first_seen: 1,
            last_seen: 2,
    ...
    async fn fed_replay_directed_to_known_active_without_conn() {
        ...
        assert_eq!(a_fed.replay_round().await, 1);
        assert_eq!(sent.lock().unwrap().len(), 2, "目标集空 → 定向零帧");
    ...
    async fn fed_ingest_accepts_anonymous_peer_node() {
        ...
        let payload = |node: Option<&str>, e: serde_json::Value| match node {
            Some(n) => {
                serde_json::json!({ "fed": FED_KIND_API_MARKET_LOBBY, "node": n, "entry": e })
            }
