...
//! ├── story/source-*.txt    # 方式一：导入原文（多份支持；上限 env NEXOS_FILM_SOURCE_MAX_MB 缺省 64MB）
//! ├── story/cleaned-*.txt   # v0.1.37 管线 ①：规则+可选 LLM 深清产物（同名 .report.json 清理报告）
//! ├── story/chapters/index.json  # v0.1.37 管线 ②：章节清单 [{no,title,start_line,end_line,words,auto}]
//! ├── story/chapters/ch-<NN>.md  # 每章一文件（front-matter no/title/words/source；hub 树可浏览）
//! ├── story/characters-profile.json  # v0.1.37 管线 ③：人物档案 [{name,aliases,…,first_chapter,arc}]
...
//!   b64 JSON 信封 `{filename, content_b64}`。**body 上限链路**（2026-09-06
//!   v0.1.37 排查）：主分发面无限制（usize::MAX），网关流式面 64MiB 仅辖
//!   gateway OpenAI 路由——导入上限只看 `NEXOS_FILM_SOURCE_MAX_MB`
//!   （缺省 64MB，0=无限），无需分片续传。
...
//!
//! # 路由（21+3 条，component=film；读公开/写 admin；未装应用全 404）
//!
//! 见 [`hub_routes`] 与 FILM_STUDIO.md 端点表（含 POST script 兼容别名；
//! v0.1.37 增 story/clean · story/chapterize · story/profile 三管线端点）。
...
/// story 导入原文上限缺省（MB；env `NEXOS_FILM_SOURCE_MAX_MB` 覆写，0=真正
/// 无限——不推荐）。v0.1.37 前为 2MB 硬上限，3MB 小说被拒（用户诉求）；
/// 64MB 是超宽护栏——纯防异常请求把内存打爆，正常小说（中文 10 万字≈
/// UTF-8 300KB）远够用。
/// **全链路 body 上限排查结论（2026-09-06 v0.1.37）**：axum 主分发面
/// （http.rs `request_to_api`）body 读取上限为 `usize::MAX`（无限制）；网关
/// 流式面 `GATEWAY_STREAM_MAX_BODY`=64MiB 仅辖 `/api/v1/gateway/v1/*`
/// OpenAI 兼容路由；deploy 无反代 body 限制——**本护栏即 film story 链路
/// 唯一上限**，无需放宽网关、无需分片续传端点（BGM 20MB b64 JSON 同路径
/// 先例可证）。
const STORY_SOURCE_MAX_MB_DEFAULT: usize = 64;

/// 纯函数：env 值 → 上限 MB（解析失败回缺省；供单测）。
#[must_use]
pub fn parse_mb_limit(raw: &str, default_mb: usize) -> usize {
...
/// story 导入原文上限（解码后字节数；env 缺省 64MB；0=无限 → None）。
fn story_import_max_bytes() -> Option<usize> {
    let mb = std::env::var("NEXOS_FILM_SOURCE_MAX_MB")
        .unwrap_or_default();
    let mb = parse_mb_limit(&mb, STORY_SOURCE_MAX_MB_DEFAULT);
    (mb > 0).then(|| mb * 1024 * 1024)
}

// —— story 文档处理管线（v0.1.37）：清理 / 分章 / 人物梳理 ——

/// 清理 LLM 深清分块字符上限（8K：规则清理已先行缩量，深清只处理广告与排版）。
const CLEAN_LLM_CHUNK_CHARS: usize = 8_000;

/// 分章/人物梳理分块字符上限（devdocs 翻译先例：≤6K 块）。
const PIPE_CHUNK_CHARS: usize = 6_000;

/// 分章块的上文重叠字符数（跨块衔接上下文；断点只认本块）。
const PIPE_CHUNK_OVERLAP_CHARS: usize = 200;

/// 无章节结构时的自动分段目标字数（规格 3-5K 取中）。
const AUTO_SEG_TARGET_CHARS: usize = 4_000;

/// 管线分块并发上限（本地 9B 或渠道，防打满）。
const PIPE_CHUNK_CONCURRENCY: usize = 2;
...
// ----------------------------------------------------------------------------
// story 文档处理管线：纯函数（v0.1.37——清理/分章/人物梳理，易单测）
// ----------------------------------------------------------------------------

/// 常见广告/运营噪音行子串（规则清理用；无 regex 依赖——子串判定 + 短行启发）。
const AD_LINE_MARKERS: [&str; 22] = [
    "www.",
    "http://",
    "https://",
    ".com",
...
/// 行是否广告/运营噪音（子串命中 + 行长 ≤40 字符启发——正文行通常更长）。
#[must_use]
pub fn is_ad_line(line: &str) -> bool {
    let t = line.trim();
...
/// 全半角标点归一：紧邻 CJK 文字的 ASCII `, ; : ! ? ( )` 折叠为全角（中文
/// 小说排版口径）；纯 ASCII 语境（英文句中逗号、冒号时间等）保持不动。
#[must_use]
pub fn normalize_punct_width(line: &str) -> String {
    const MAP: [(char, char); 7] = [
        (',', '，'),
        (';', '；'),
        (':', '：'),
        ('!', '！'),
...
pub struct CleanReport {
    ...
    pub chars_in: usize,
    pub chars_out: usize,
    pub ad_lines_removed: usize,
    pub blank_lines_removed: usize,
}

/// 规则清理（本地零成本）：去广告行 / 连续空行折叠为单空行 / 行首尾空白
/// 归一 / 全半角标点归一（CJK 邻接）/ 首尾空行去除。
#[must_use]
pub fn clean_text_rules(text: &str) -> (String, CleanReport) {
    ...
    for raw in text.lines() {
        ...
        blank_run = 0;
        out_lines.push(normalize_punct_width(trimmed));
    }
    while out_lines.first().is_some_and(String::is_empty) {
        out_lines.remove(0);
        report.blank_lines_removed += 1;
    }
    ...
    report.lines_out = out.lines().count();
    report.chars_out = out.chars().count();
    (out, report)
}

/// 文本分块（带绝对起始行号；段落感知：空行分段，超长段按行硬切，段落
/// 打包至 ≤max_chars）。分章/人物梳理/LLM 深清共用。
#[derive(Debug, Clone)]
pub struct ChunkSpan {
    /// 起始行号（1 起，原全文口径）。
    pub start_line: usize,
    /// 覆盖行数（[start_line, start_line + line_count)）。
    pub line_count: usize,
    pub text: String,
}

#[must_use]
pub fn split_chunks_by_lines(text: &str, max_chars: usize) -> Vec<ChunkSpan> {
    // 1) 段落切（连续非空行；记录段落起始行号）
    ...
    for (i, line) in text.lines().enumerate() {
        ...
        if line.trim().is_empty() {
            if !cur.is_empty() {
                paragraphs.push((cur_start, std::mem::take(&mut cur)));
            }
            cur_start = lineno + 1;
        } else {
            if cur.is_empty() {
    ...
    if !cur.is_empty() {
        paragraphs.push((cur_start, cur));
    }
    // 2) 超长段落按行硬切（行数上限 = max_chars 行——单行超长原样保留）
    let max_lines = max_chars.max(1);
    let mut pieces: Vec<(usize, Vec<String>)> = Vec::new();
    for (start, lines) in paragraphs {
        if lines.len() <= max_lines {
            pieces.push((start, lines));
            continue;
        }
        let mut off = 0usize;
        while off < lines.len() {
            let end = (off + max_lines).min(lines.len());
            pieces.push((start + off, lines[off..end].to_vec()));
            off = end;
        }
    }
    // 3) 段落打包成块（字符预算；单段超预算独立成块）
    let para_chars = |lines: &[String]| -> usize {
        lines.iter().map(|l| l.chars().count() + 1).sum::<usize>().max(1)
    };
    let mut chunks: Vec<ChunkSpan> = Vec::new();
    let mut acc: Vec<String> = Vec::new();
    let mut acc_start = 1usize;
    let mut acc_chars = 0usize;
    for (pstart, plines) in pieces {
        let pc = para_chars(&plines);
        if !acc.is_empty() && acc_chars + pc > max_chars {
...
/// 解析 LLM 分章输出：`{"chapters":[{"no":1,"title":"…","quote":"…"}]}`；
/// 容错（``` 围栏 / 首尾大括号候选）；title 空丢弃。chapters 键存在即认
/// （空数组 = 本块无章节，合法）。
pub fn parse_chapter_breaks(text: &str) -> Result<Vec<RawChapter>, String> {
    let stripped = super::film::strip_think_blocks(text);
    for cand in json_object_candidates(&stripped) {
        let Ok(v) = serde_json::from_str::<Value>(&cand) else {
            continue;
        };
        let Some(arr) = v.get("chapters").and_then(Value::as_array) else {
            continue;
        };
        let mut out = Vec::new();
        for it in arr {
            ...
            out.push(RawChapter {
                no: it.get("no").and_then(Value::as_u64).unwrap_or(0) as u32,
                title,
                quote: it
                    .get("quote")
                    .and_then(Value::as_str)
    ...
    Err("无法从 LLM 输出解析章节断点 JSON".to_string())
}

/// 定位章节标题行：按序在全文行里找标题/引文匹配行（0 基行号；找不到或
/// 已被前章覆盖 → 跳过该断点——合入前章）。
#[must_use]
pub fn locate_chapter_lines<'a>(
    lines: &[&str],
    raws: &'a [RawChapter],
) -> Vec<(usize, &'a RawChapter)> {
    let collapse = |s: &str| -> String {
        s.chars().filter(|c| !c.is_whitespace()).collect::<String>()
    };
    let mut out: Vec<(usize, &RawChapter)> = Vec::new();
    let mut cursor = 0usize;
    for raw in raws {
        let title_c = collapse(&raw.title);
        let quote_c = collapse(&raw.quote);
        let quote_probe = quote_c.chars().take(10).collect::<String>();
        let mut hit: Option<usize> = None;
        for (i, line) in lines.iter().enumerate().skip(cursor) {
            let line_c = collapse(line);
            if !title_c.is_empty() && line_c.contains(&title_c) {
                hit = Some(i);
                break;
            }
            if !quote_probe.is_empty()
                && quote_probe.chars().count() >= 5
                && line_c.contains(&quote_probe)
            {
                hit = Some(i);
                break;
            }
...
/// 无章节结构的自动分段（每 ~target 字在段落边界切；标注「自动分段」用）。
/// 返回 (start_line, end_line) 1 基闭区间列表。
#[must_use]
pub fn auto_segment(text: &str, target_chars: usize) -> Vec<(usize, usize)> {
    let lines: Vec<&str> = text.lines().collect();
    if lines.is_empty() {
        return vec![];
    }
    let mut out: Vec<(usize, usize)> = Vec::new();
    let mut start = 0usize; // 0 基
    let mut chars = 0usize;
    for i in 0..lines.len() {
        chars += lines[i].chars().count();
        let at_paragraph_end =
            i + 1 == lines.len() || lines[i + 1].trim().is_empty() || lines[i].trim().is_empty();
        if chars >= target_chars && at_paragraph_end && i + 1 > start {
            out.push((start + 1, i + 1));
            start = i + 1;
            chars = 0;
        }
    }
...
/// 人物档案条目（profile 块产物与合并载体；serde 宽容——LLM 字段类型不定）。
...
pub struct ProfileEntry {
    ...
    /// [{name, relation}]（宽容 Value——归一时抽字符串字段）。
...
/// 解析 LLM 人物梳理块输出：根数组或 {"characters":[…]}；name 空丢弃。
pub fn parse_profile_chunk(text: &str) -> Result<Vec<ProfileEntry>, String> {
    let stripped = super::film::strip_think_blocks(text);
    let mut candidates = json_object_candidates(&stripped);
    // 根数组形态补候选（首尾中括号）
    let trimmed = stripped.trim();
    if let (Some(a), Some(b)) = (trimmed.find('['), trimmed.rfind(']')) {
        if a < b {
            candidates.push(trimmed[a..=b].to_string());
        }
    }
    for cand in candidates {
        ...
        let arr = match &v {
            ...
            Value::Object(_) => v.get("characters").and_then(Value::as_array).cloned(),
            _ => None,
        };
        ...
        for it in &arr {
            ...
            let aliases = it
                ...
                .map(|a| a.iter().map(value_to_string).filter(|s| !s.is_empty()).collect())
                .unwrap_or_default();
            let relations = it
                .get("relations")
                .and_then(Value::as_array)
                .map(|a| {
                    a.iter()
                        .filter_map(|r| {
                            let rn = r.get("name").map(value_to_string);
                            let rr = r.get("relation").map(value_to_string);
                            match (rn, rr) {
                                (Some(n), Some(rel)) if !n.is_empty() && !rel.is_empty() => {
                                    Some(serde_json::json!({"name": n, "relation": rel}))
                                }
                                (Some(n), None) if !n.is_empty() => Some(
                                    serde_json::json!({"name": n, "relation": ""}),
                                ),
                                _ => None,
                            }
                        })
            ...
            out.push(ProfileEntry {
                name,
                aliases,
                gender: it.get("gender").map(value_to_string).unwrap_or_default(),
                age: it.get("age").map(value_to_string).unwrap_or_default(),
                appearance: it.get("appearance").map(value_to_string).unwrap_or_default(),
                personality: it.get("personality").map(value_to_string).unwrap_or_default(),
                relations,
                first_chapter: it
                    .get("first_chapter")
                    .and_then(Value::as_u64)
                    .map(|n| n as u32),
                arc: it.get("arc").map(value_to_string).unwrap_or_default(),
            });
        }
        return Ok(out);
    }
    Err("无法从 LLM 输出解析人物档案 JSON".to_string())
}

/// 名字键归一（trim + 去包裹引号「」“”『』《》 + 小写——英文别名不敏感）。
fn profile_name_key(s: &str) -> String {
    let t = s.trim();
    let t = t
        .trim_start_matches(['「', '“', '『', '《', '"', '\''])
        .trim_end_matches(['」', '”', '』', '》', '"', '\'']);
...
/// 跨块合并去重（别名归并）：名字/别名集合相交即同一人——别名并集、
/// 空字段补全、外貌/性格/成长线去重拼接、关系按 name+relation 去重、
/// first_chapter 取最小。输出按 first_chapter（缺省推后）+ 名字排序。
#[must_use]
pub fn merge_character_profiles(entries: Vec<ProfileEntry>) -> Vec<ProfileEntry> {
    /// 去重拼接（acc 已含该片段则跳过；分隔符"；"）。
    fn join_distinct(acc: &mut String, add: &str) {
        let add = add.trim();
        if add.is_empty() || acc.contains(add) {
            return;
        }
        if acc.is_empty() {
            *acc = add.to_string();
    ...
    for e in entries {
        let mut e_keys: Vec<String> = Vec::new();
        for k in std::iter::once(e.name.as_str()).chain(e.aliases.iter().map(String::as_str)) {
            let k = profile_name_key(&k);
            if !k.is_empty() && !e_keys.contains(&k) {
                e_keys.push(k);
            }
        }
        let hit_idx = keys
            .iter()
            .position(|ks| e_keys.iter().any(|k| ks.contains(k)));
        match hit_idx {
            Some(idx) => {
                let m = &mut merged[idx];
                for a in &e.aliases {
                    let ak = profile_name_key(a);
                    if ak.is_empty() || m.aliases.iter().any(|x| profile_name_key(x) == ak) {
                        continue;
                    }
                    m.aliases.push(a.clone());
                }
                if m.gender.is_empty() {
                    m.gender = e.gender.clone();
                }
                if m.age.is_empty() {
                    m.age = e.age.clone();
                }
                join_distinct(&mut m.appearance, &e.appearance);
                join_distinct(&mut m.personality, &e.personality);
                join_distinct(&mut m.arc, &e.arc);
                for r in e.relations {
                    let dup = m.relations.iter().any(|x| {
                        x.get("name").map(value_to_string) == r.get("name").map(value_to_string)
                            && x.get("relation").map(value_to_string)
                                == r.get("relation").map(value_to_string)
                    });
                    if !dup {
                        m.relations.push(r);
                    }
                }
                m.first_chapter = match (m.first_chapter, e.first_chapter) {
    ...
    merged.sort_by(|a, b| {
        let ka = a.first_chapter.unwrap_or(u32::MAX);
        let kb = b.first_chapter.unwrap_or(u32::MAX);
        ka.cmp(&kb).then_with(|| a.name.cmp(&b.name))
    });
    merged
}

/// LLM 输出去围栏（``` 围栏整块剥除；深清"原样输出"后处理）。
#[must_use]
...
/// 源文件名 → 清理产物基名（`source-小说.txt` / `cleaned-小说.txt` /
/// `小说.md` → `小说`；前端徽章派生同一口径的镜像）。
#[must_use]
pub fn source_base_slug(source_rel: &str) -> String {
    let name = source_rel.rsplit('/').next().unwrap_or(source_rel);
    let stem = name.rsplit_once('.').map(|(s, _)| s).unwrap_or(name);
    let stem = stem.strip_prefix("source-").unwrap_or(stem);
    let stem = stem.strip_prefix("cleaned-").unwrap_or(stem);
...
/// story/story.md 占位正文（生成真实剧情时被整文件覆盖）。
const STORY_PLACEHOLDER_BODY: &str = "（占位）在剧情页导入原文或让 AI 创作，生成后本文件即为剧情正稿。";

/// story/story.md 占位内容（front-matter words=0 是「未创作」标记——
/// [`read_story_body`] 见 0 视为缺失，占位不会被当成真剧情喂给下游）。
fn story_placeholder_md() -> String {
    let mut fm = BTreeMap::new();
    fm.insert("source".to_string(), "-".to_string());
    fm.insert("words".to_string(), "0".to_string());
    fm.insert("summary".to_string(), "待创作".to_string());
    render_doc(&fm, STORY_PLACEHOLDER_BODY)
}

/// 默认树占位文件清单（v0.1.36：树扫描只认文件——空目录在 files 树里不可见，
/// 每目录放一个自文档占位，既撑起完整层级又说明该目录放什么）。
/// 返回 (hub 相对路径, 内容)。
#[must_use]
pub fn hub_placeholder_files() -> Vec<(String, String)> {
    let mut out: Vec<(String, String)> = vec![
        ("story/story.md".to_string(), story_placeholder_md()),
        (
            "story/_sources.md".to_string(),
            "（目录说明）导入的剧情原文会以 `source-<名字>.txt` 落在本目录；剧情页的「AI 改编」可任选其中一份作为改编底稿（story/generate 的 source_file）。"
                .to_string(),
        ),
        (
            "storyboard/storyboard.json".to_string(),
            serde_json::to_string_pretty(&serde_json::json!({
                "version": 1, "shots": [], "meta": {"note": "分镜页生成后写入"},
            }))
            .unwrap_or_default(),
        ),
        (
            "casting/extraction.json".to_string(),
            serde_json::to_string_pretty(&serde_json::json!({
                "characters": [], "weapons": [], "pets": [],
                "formations": [], "actions": [], "scenes": [],
            }))
            .unwrap_or_default(),
        ),
    ];
    let mut about = |t: &str, text: &str| {
        out.push((format!("casting/{t}/_about.md"), text.to_string()));
    };
    about(
        "characters",
        "（目录说明）人物定妆：每个对象一个子目录 `<名字>/`，内含 card.md（name/voice/portrait 与外形描述）与 views/ 多视图定妆图（front/side/back/action-N）。在定妆页新建，或用 casting/extract 从剧情提取后逐个建。",
    );
    about(
        "props",
        "（目录说明）武器与关键道具定妆（extraction.json 报告里的 weapons 类落在这里）：每个对象一个子目录，建法同人物（card.md + views/）。",
    );
    about(
        "pets",
        "（目录说明）宠物与动物定妆：每个对象一个子目录，建法同人物（card.md + views/）。",
    );
    about(
        "formations",
        "（目录说明）多人同框的站位排列定妆（如「三人前二后一」）：每个排列一个子目录，建法同人物。",
    );
    about(
        "actions",
        "（目录说明）跨镜头重复出现的高频动作定妆（如「拔剑」「雨中奔跑」）：每个动作一个子目录，建法同人物。",
    );
    about(
        "scenes",
        "（目录说明）跨镜头重复出现的高频场景定妆（如「灯塔顶」）：每个场景一个子目录，建法同人物。",
    );
    out.push((
        "audio/bgm/_about.md".to_string(),
        "（目录说明）BGM 音轨库：每条音轨一个子目录 `<track>/`，内含 info.md（trigger=global 全局或 scene:<场景名> 场景触发 / mood / duration）与 track.mp3。在音频页建轨、导入或 AI 生成。"
            .to_string(),
    ));
    out.push((
        "cache/_about.md".to_string(),
        "（目录说明）半成品区：镜头试生成图、临时音频等先落这里，确认满意后经「转正」（POST cache/<file>/commit）移入项目正式产物；不进 dist。"
            .to_string(),
    ));
    out.push((
        "dist/_about.md".to_string(),
        "（目录说明）成品区：合成阶段输出版本化成片 final-vYYYYMMDD-HHMM.mp4 与随行报告 compose-report.json。"
            .to_string(),
    ));
    out
}
...
/// 补默认树占位文件（**仅缺失时写**——幂等）：
/// - 新建项目即得完整可浏览层级（v0.1.36 用户诉求 ①）；
/// - 旧项目 export/惰性初始化补骨架不重复造已有文件（story/storyboard 等
///   真值文件原样保留；生成动作直接覆盖同路径占位，_about.md 永不被触碰）。
async fn backfill_hub_placeholders(root: &str) -> Result<(), String> {
    for (rel, content) in hub_placeholder_files() {
        ...
        tokio::fs::write(&path, content)
            .await
            .map_err(|e| format!("写占位文件失败 {path}: {e}"))?;
    }
    Ok(())
}
...
/// story 管线子阶段（clean/chapterize/profile）进度更新：README 阶段**不低于**
/// 当前值——已在 storyboard/casting 等更晚阶段的项目只留任务日志与 activity，
/// 不被拉回 story（v0.1.37）。
async fn set_readme_progress_story(root: &str, progress: &str) {
    let path = format!("{root}/README.md");
    if let Ok(text) = tokio::fs::read_to_string(&path).await {
        let (fm, _) = split_front_matter(&text);
        let cur = fm.get("stage").map(String::as_str).unwrap_or("");
        if !cur.is_empty() && cur != "story" {
            return;
        }
    }
    set_readme_stage(root, "story", progress).await;
}
...
async fn export_inner(ctx: &FilmCtx, project: &FilmProject) -> Result<Vec<String>, String> {
    ...
    // 例外：树内 storyboard 的 shots 为空数组 = v0.1.36 默认树**占位**（无真值
    // 可保）——script.json 存在即平移（tmpfs 等文件系统 mtime 同拍平局会让
    // 严格大于判定抖动，占位判定不依赖 mtime）。
    ...
    let sb_is_placeholder = std::fs::read_to_string(&sb_path)
        .ok()
        .and_then(|raw| serde_json::from_str::<Value>(&raw).ok())
        .is_some_and(|v| {
            v.get("shots")
                .and_then(Value::as_array)
                .is_some_and(<Vec<Value>>::is_empty)
        });
    let sb_stale = match (script_mtime, sb_mtime) {
        (Some(_), Some(_)) if sb_is_placeholder => true,
        (Some(s), Some(b)) => s > b,
        (Some(_), None) => true,
        _ => false,
    };
...
/// casting 定妆对象）。已初始化则仅补缺失目录与占位骨架（幂等）。
pub(crate) async fn ensure_hub(ctx: &FilmCtx, project: &FilmProject) -> Result<String, String> {
    ...
    if Path::new(&format!("{root}/project.md")).is_file() {
        ...
        // 默认树骨架补齐（仅缺失时写——旧版本建的树升级后也能浏览全层级）
        backfill_hub_placeholders(&root).await?;
        return Ok(root);
    }
    ...
    // 默认树占位骨架**最后**补（仅缺失时写）：export 已落的真值文件
    // （如 script.json 平移出的 storyboard.json）不会被占位覆盖。
    backfill_hub_placeholders(&root).await?;
    Ok(root)
}

/// 新建项目即建 hub 树（POST /film/projects 调用；失败仅日志不拦建项）。
/// v0.1.36：连带默认树占位骨架——建项即得完整可浏览层级（GET files 树
/// 含 story/storyboard/casting 六类/audio/cache/dist 全部目录）。
pub(crate) async fn init_hub_for_new(project: &FilmProject) {
    ...
    if let Err(e) = backfill_hub_placeholders(&root).await {
        eprintln!("[filmhub] 新项目 hub 骨架占位失败（{}）: {e}", project.id);
    }
    // 元文件**仅缺失时写**（幂等——2026-09-06 film-101 事故回归：即便建项
    // id 分配防线失效落进既有目录，也不覆盖既有 hub 真值/流水）
    let write_if_missing = |rel: String, content: String| {
        let path = format!("{root}/{rel}");
        if !Path::new(&path).is_file() {
            let _ = std::fs::write(&path, content);
        }
    };
    write_if_missing("project.md".to_string(), project_md_content(project));
    write_if_missing(
        "README.md".to_string(),
        readme_content("story", "项目已创建，待剧情导入或 AI 撰写"),
    );
    let _guard = HUB_META_LOCK.lock();
    write_if_missing(
        "ownership.json".to_string(),
        serde_json::to_string_pretty(&serde_json::json!({
            "members": [], "sections": {}, "casting_objects": {}
        }))
        .unwrap_or_default(),
    );
    write_if_missing("activity.json".to_string(), "[]".to_string());
    write_if_missing("assets.json".to_string(), "[]".to_string());
    write_if_missing(
        "budget.json".to_string(),
        serde_json::to_string_pretty(&serde_json::json!({
...
/// story 清理（v0.1.37 管线 ①）：mode=rules 本地零成本（model_ref 可省）；
/// mode=llm 规则先行 + 8K 块深清（须 model_ref）。source_file 缺省走
/// serde default + 手动 400（缺字段给可读错误而非解析失败）。
#[derive(Debug, Deserialize)]
struct StoryCleanBody {
    #[serde(default)]
...
/// story 分章（管线 ②）：source_file 缺省取最近清理产物/唯一源；cleaned 优先。
#[derive(Debug, Deserialize)]
struct StoryChapterizeBody {
    model_ref: ModelRef,
    #[serde(default)]
    source_file: Option<String>,
    #[serde(default)]
...
/// story 人物梳理（管线 ③）：source_file 与 chapter_range 二选一
///（chapter_range 如 "1-5"/"3" 走已分章产物）。
#[derive(Debug, Deserialize)]
struct StoryProfileBody {
    model_ref: ModelRef,
    #[serde(default)]
    source_file: Option<String>,
    #[serde(default)]
...
/// 读剧情正稿正文（story.md；缺失 → None）。占位骨架（front-matter
/// words=0 的默认树占位）同视为缺失——占位只是自文档，不当真值喂下游。
async fn read_story_body(project: &FilmProject) -> Option<String> {
    let text = tokio::fs::read_to_string(format!("{}/story/story.md", hub_root(project)))
        ...
        .ok()?;
    let (fm, body) = split_front_matter(&text);
    if fm.get("words").is_some_and(|w| w.trim() == "0") {
        return None;
    }
...
// ----------------------------------------------------------------------------
// story 文档处理管线执行器（v0.1.37：清理 / 分章 / 人物梳理）
// ----------------------------------------------------------------------------

/// 剧情原文解析：`story/<want>` 直存名 → `story/source-<slug>.txt` →
/// （prefer_cleaned 时优先）`story/cleaned-<slug>.txt`。返回 (绝对路径, hub 相对路径)。
fn resolve_story_source(root: &str, want: &str, prefer_cleaned: bool) -> Option<(PathBuf, String)> {
    let want = want.trim();
    if want.is_empty() {
        return None;
    }
    let base = source_base_slug(want);
    let mut rels = vec![format!("story/{want}")];
    let slug_rel = format!("story/source-{base}.txt");
    let cleaned_rel = format!("story/cleaned-{base}.txt");
    if prefer_cleaned {
        rels.push(cleaned_rel);
        rels.push(slug_rel);
    } else {
        rels.push(slug_rel);
        rels.push(cleaned_rel);
...
/// 管线分块并发执行（并发 ≤ [`PIPE_CHUNK_CONCURRENCY`]，保序返回；每块失败
/// 重试 1 次——重试仍败记 Err 由调用方定夺）。`make(i)` 每次调用产出第 i 块
/// 的新 future（重试即重调，prompt 相同）。
async fn run_chunks_concurrent<T, F, Fut>(n: usize, make: F) -> Vec<Result<T, String>>
where
    F: Fn(usize) -> Fut,
    Fut: std::future::Future<Output = Result<T, String>>,
{
    ...
    stream::iter(0..n)
        .map(|i| async move {
            match make(i).await {
                Ok(v) => Ok(v),
                Err(first) => match make(i).await {
                    Ok(v) => Ok(v),
                    Err(second) => Err(format!("{first}；重试仍失败: {second}")),
                },
            }
        })
        .buffered(PIPE_CHUNK_CONCURRENCY)
        .collect()
        .await
}

/// usage 三元组累加。
fn add_usage(acc: &mut (u32, u32, u32), u: Option<(u32, u32, u32)>) {
    if let Some((p, c, t)) = u {
        acc.0 = acc.0.saturating_add(p);
        acc.1 = acc.1.saturating_add(c);
        acc.2 = acc.2.saturating_add(t);
    }
}

/// 管线 ① 清理：规则预处理（本地零成本）+ 可选 LLM 深清（8K 块）→
/// `story/cleaned-<源基名>.txt` + `story/cleaned-<源基名>.report.json`。
pub(crate) async fn run_story_clean_stage(
    ctx: &FilmCtx,
    task_id: &str,
    project: FilmProject,
    mr: Option<ModelRef>,
    ...
    let log = |line: String| task_log(&tasks, task_id, &line);
    let started = Instant::now();
    let cost = |bytes: u64, tokens: Option<(u32, u32, u32)>| CostSpec {
        stage: "story.clean",
        shot: None,
        model_ref: mr.as_ref(),
        started,
    ...
    let fail = |msg: String, bytes: u64, tokens: Option<(u32, u32, u32)>| {
        finish_stage(ctx, &tasks, task_id, &project, "error", &msg, None, cost(bytes, tokens))
    };
    log(format!("原文清理（{mode}）：源 {source_file}"));
    let root = match ensure_hub(ctx, &project).await {
        Ok(r) => r,
        Err(e) => return fail(e, 0, None),
    };
    let Some((path, rel)) = resolve_story_source(&root, &source_file, false) else {
        return fail(
            format!("导入原文不存在: {source_file}（先 POST story/import）"),
            0,
            None,
        );
    };
    let original = match tokio::fs::read_to_string(&path).await {
        Ok(t) => t,
        Err(e) => return fail(format!("读取原文失败 {}: {e}", path.display()), 0, None),
    };
    log(format!(
        "原文 {} 字符（{rel}）→ 规则清理：去广告行 / 空行折叠 / 标点归一",
        original.chars().count()
    ));
    let (mut cleaned, report) = clean_text_rules(&original);
    log(format!(
        "规则清理完成：广告行 -{}，空行 -{}，{} → {} 字符",
        report.ad_lines_removed,
        report.blank_lines_removed,
        report.chars_in,
        report.chars_out
    ));
    let mut usage_acc: (u32, u32, u32) = (0, 0, 0);
    let mut has_usage = false;
    if mode == "llm" {
        let Some(mr_ref) = mr.as_ref() else {
            return fail(
                "mode=llm 须带 model_ref".to_string(),
                cleaned.len() as u64,
                None,
            );
        };
        let chunks = split_chunks_by_lines(&cleaned, CLEAN_LLM_CHUNK_CHARS);
        let n = chunks.len();
        log(format!(
            "LLM 深清：{n} 块 × ≤{}K 字符（并发≤{PIPE_CHUNK_CONCURRENCY}，失败块重试 1 次）",
            CLEAN_LLM_CHUNK_CHARS / 1000
        ));
        let system = "你是小说文本清理引擎。去掉广告、乱码、网站水印与重复标题行，修复对话排版（对话独立成段），原样输出清理后的正文。禁止总结、禁止改写情节、禁止添加任何解释或标记。";
        let results: Vec<Result<(String, Option<(u32, u32, u32)>), String>> =
            run_chunks_concurrent(n, |i| {
                let user = format!(
                    "清理下面的文本块，原样输出清理后的正文：\n\n{}",
                    chunks[i].text
                );
                async move { ctx.chat_text_with_usage(mr_ref, system, &user).await }
            })
            .await;
        ...
        for (i, r) in results.into_iter().enumerate() {
            match r {
                Ok((text, usage)) => {
                    add_usage(&mut usage_acc, usage);
                    has_usage = has_usage || usage.is_some();
                    parts.push(strip_code_fences(text.trim()));
                    log(format!("块 {}/{} 完成", i + 1, n));
                }
                Err(e) => {
                    return fail(
                        format!("LLM 深清块 {}/{} 失败：{e}", i + 1, n),
                        cleaned.len() as u64,
                        has_usage.then_some(usage_acc),
                    );
                }
            }
        }
    ...
    final_report.chars_out = cleaned.chars().count();
    final_report.lines_out = cleaned.lines().count();
    let base = source_base_slug(&rel);
    let out_rel = format!("story/cleaned-{base}.txt");
    let out_path = format!("{root}/{out_rel}");
    if let Err(e) = tokio::fs::write(&out_path, &cleaned).await {
        return fail(
            format!("写清理产物失败 {out_path}: {e}"),
            cleaned.len() as u64,
            has_usage.then_some(usage_acc),
        );
    }
    let report_rel = format!("story/cleaned-{base}.report.json");
    let report_doc = serde_json::json!({
        "version": 1,
        "source": rel,
        "mode": mode,
        "model": mr.as_ref().map(|m| m.label()).unwrap_or_default(),
        "generated_at": now_iso(),
        "report": final_report,
    });
    let _ = tokio::fs::write(
        format!("{root}/{report_rel}"),
        serde_json::to_string_pretty(&report_doc).unwrap_or_default(),
    )
    .await;
    set_readme_progress_story(
        &root,
        &format!(
            "原文已清理（{mode}）：广告行 -{}，{} → {} 字符",
            final_report.ad_lines_removed, final_report.chars_in, final_report.chars_out
        ),
    )
    .await;
    append_activity(&root, &author, "story.clean", &out_rel).await;
    log(format!(
        "清理产物 {out_rel}（{} → {} 字符）+ 报告 {report_rel}",
        final_report.chars_in, final_report.chars_out
    ));
    finish_stage(
        ctx,
        &tasks,
        ...
        &project,
        "done",
        &format!("清理产物已存 {out_rel}（{} → {} 字符）", final_report.chars_in, final_report.chars_out),
        Some(out_rel),
        cost(cleaned.len() as u64, has_usage.then_some(usage_acc)),
    );
}

/// 管线 ② 分章：≤6K 块+重叠上下文逐块出章节断点 → 合并定位行号 →
/// `story/chapters/index.json` + `story/chapters/ch-<NN>.md`；无章节结构 →
/// 按字数自动分段（标注 auto）。
pub(crate) async fn run_story_chapterize_stage(
    ctx: &FilmCtx,
    task_id: &str,
    project: FilmProject,
    mr: ModelRef,
    ...
    log(format!("章节划分：模型 {}", mr.label()));
    let root = match ensure_hub(ctx, &project).await {
        Ok(r) => r,
        Err(e) => return fail(e, 0, None),
    };
    // 源解析：显式给出 → cleaned 优先；缺省 → 树内最近清理产物，否则唯一源
    let source_rel: Option<String> = match source_file.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        Some(want) => resolve_story_source(&root, want, true).map(|(_, rel)| rel),
        None => latest_source_preferring_cleaned(&root),
    };
    let Some(source_rel) = source_rel else {
        return fail("未指定 source_file 且树内无可清理/导入原文".to_string(), 0, None);
    };
    let text = match tokio::fs::read_to_string(format!("{root}/{source_rel}")).await {
        Ok(t) => t,
        Err(e) => return fail(format!("读取原文失败 {source_rel}: {e}"), 0, None),
    };
    let chars = text.chars().count();
    let chunks = split_chunks_by_lines(&text, PIPE_CHUNK_CHARS);
    let n = chunks.len();
    log(format!(
        "原文 {chars} 字符 → {n} 块 × ≤{}K（重叠上下文 {} 字符；并发≤{PIPE_CHUNK_CONCURRENCY}）",
        PIPE_CHUNK_CHARS / 1000,
        PIPE_CHUNK_OVERLAP_CHARS
    ));
    let system = "你是小说结构分析器。只输出 JSON 对象，不要输出任何解释文字。";
    let mr_ref = &mr;
    let mut usage_acc: (u32, u32, u32) = (0, 0, 0);
    let mut has_usage = false;
    let results: Vec<Result<(Vec<RawChapter>, Option<(u32, u32, u32)>), String>> =
        run_chunks_concurrent(n, |i| {
            let overlap: String = if i == 0 {
                String::new()
            } else {
                let prev = &chunks[i - 1].text;
                prev.chars()
                    .rev()
                    .take(PIPE_CHUNK_OVERLAP_CHARS)
                    .collect::<Vec<_>>()
                    .into_iter()
                    .rev()
                    .collect()
            ...
            let user = format!(
                "在下面的文本块中找出章节断点。只报告本块内开始的章节，输出 JSON：\n{{\"chapters\":[{{\"no\":1,\"title\":\"章节标题（用原文标题，如「第一章 风起」）\",\"quote\":\"该章开头前 10-20 字引文\"}}]}}\n规则：找不到章节结构输出 {{\"chapters\":[]}}；标题保持原文；quote 必须是本块正文的连续片段。\n\n【上文（衔接参考，勿在其中检测）】\n{overlap}\n\n【本块】\n{}",
                chunks[i].text
            );
            async move {
                let (out, usage) = ctx.chat_text_with_usage(mr_ref, system, &user).await?;
                let raws = parse_chapter_breaks(&out)?;
                Ok((raws, usage))
            }
        })
        .await;
    let mut all_raws: Vec<RawChapter> = Vec::new();
    for (i, r) in results.into_iter().enumerate() {
        match r {
            Ok((raws, usage)) => {
                add_usage(&mut usage_acc, usage);
                has_usage = has_usage || usage.is_some();
                log(format!("块 {}/{} 完成（{} 个章节断点）", i + 1, n, raws.len()));
                all_raws.extend(raws);
            }
            Err(e) => {
                return fail(
                    format!("分章块 {}/{} 失败：{e}", i + 1, n),
                    text.len() as u64,
                    has_usage.then_some(usage_acc),
                );
            }
    ...
    // 定位行号（全文行序匹配标题/引文）
    let lines: Vec<&str> = text.lines().collect();
    let located = locate_chapter_lines(&lines, &all_raws);
    let auto = located.len() < 2 && chars > AUTO_SEG_TARGET_CHARS * 2;
    let spans: Vec<(usize, usize)> = if auto {
        log("LLM 判不出章节结构 → 按字数自动分段（每章 ~3-5K 字）".to_string());
        auto_segment(&text, AUTO_SEG_TARGET_CHARS)
            .into_iter()
            .map(|(s, e)| (s - 1, e - 1)) // 转 0 基
            .collect()
    } else {
        located
            .iter()
    ...
    if spans.is_empty() {
        return fail(
            "章节划分为空（原文过短或断点全部无法定位）".to_string(),
            text.len() as u64,
            has_usage.then_some(usage_acc),
        );
    }
    // 写产物（chapters/ 重建——旧章文件清掉防残留）
    let chapters_dir = format!("{root}/story/chapters");
    let _ = tokio::fs::remove_dir_all(&chapters_dir).await;
    if let Err(e) = tokio::fs::create_dir_all(&chapters_dir).await {
        return fail(
            format!("建章节目录失败 {chapters_dir}: {e}"),
            text.len() as u64,
            has_usage.then_some(usage_acc),
        );
    }
    let mut index_chapters: Vec<Value> = Vec::new();
    let mut total_bytes = 0usize;
    for (no, (s, e)) in spans.iter().enumerate() {
        ...
        let title = if auto {
            ...
            located
                ...
                .unwrap_or_else(|| format!("第{}章", no + 1))
        };
        let mut fm = BTreeMap::new();
        fm.insert("no".to_string(), (no + 1).to_string());
        fm.insert("title".to_string(), title.clone());
        fm.insert("words".to_string(), words.to_string());
        fm.insert("source".to_string(), source_rel.clone());
        let doc = render_doc(&fm, &body);
        total_bytes += doc.len();
        let ch_rel = format!("story/chapters/ch-{:02}.md", no + 1);
        if let Err(er) = tokio::fs::write(format!("{root}/{ch_rel}"), &doc).await {
            return fail(
                format!("写章节文件失败 {ch_rel}: {er}"),
                total_bytes as u64,
                has_usage.then_some(usage_acc),
            );
        }
        index_chapters.push(serde_json::json!({
            "no": no + 1,
            "title": title,
            "start_line": s + 1,
            "end_line": e + 1,
    ...
    let index_rel = "story/chapters/index.json";
    ...
    if let Err(e) = tokio::fs::write(
        format!("{root}/{index_rel}"),
        serde_json::to_string_pretty(&index_doc).unwrap_or_default(),
    )
    .await
    {
        return fail(
            format!("写章节清单失败 {index_rel}: {e}"),
            total_bytes as u64,
            has_usage.then_some(usage_acc),
        );
    }
    set_readme_progress_story(
        ...
        &format!(
            "已分章：{} 章（{}，源 {}）",
            spans.len(),
            if auto { "自动分段" } else { "章节结构" },
            source_rel
        ),
    )
    .await;
    append_activity(&root, &author, "story.chapterize", index_rel).await;
    log(format!("分章 {} 章 → {index_rel}", spans.len()));
    finish_stage(
        ctx,
        &tasks,
        task_id,
        &project,
        "done",
        &format!("已分 {} 章 → {index_rel}", spans.len()),
        Some(index_rel.to_string()),
        cost(total_bytes as u64, has_usage.then_some(usage_acc)),
    );
}

/// 树内最近剧情源（cleaned-*.txt 优先，mtime 最新；否则 source-*.txt）。
fn latest_source_preferring_cleaned(root: &str) -> Option<String> {
    let read = |sub: &str| -> Vec<(String, std::time::SystemTime)> {
        let dir = format!("{root}/story");
        let mut out = Vec::new();
        if let Ok(rd) = std::fs::read_dir(&dir) {
            for e in rd.filter_map(Result::ok) {
                let name = e.file_name().to_string_lossy().into_owned();
                if !name.starts_with(sub) || !name.ends_with(".txt") {
                    continue;
                }
                if let Ok(meta) = e.metadata() {
                    if let Ok(m) = meta.modified() {
                        out.push((format!("story/{name}"), m));
                    }
                }
            }
        }
...
/// chapter_range 解析（"1-5"/"3" → (1,5)/(3,3)；非法 None）。
#[must_use]
pub fn parse_chapter_range(s: &str) -> Option<(u32, u32)> {
    let s = s.trim();
...
/// 管线 ③ 人物梳理：分块提取 → 别名归并 → `story/characters-profile.json`。
pub(crate) async fn run_story_profile_stage(
    ctx: &FilmCtx,
    task_id: &str,
    project: FilmProject,
    mr: ModelRef,
    ...
    log(format!("人物梳理：模型 {}", mr.label()));
    let root = match ensure_hub(ctx, &project).await {
        Ok(r) => r,
        Err(e) => return fail(e, 0, None),
    };
    // 语料：chapter_range（走已分章产物，每章一带标签 part）或 source_file
    //（cleaned 优先，单一无标签 part）
    let (parts, source_rel) = if let Some(range) = chapter_range
        .as_deref()
        .map(str::trim)
        .filter(|s| !s.is_empty())
    {
        let Some((lo, hi)) = parse_chapter_range(range) else {
            return fail(format!("chapter_range 非法（应如 1-5 或 3）: {range}"), 0, None);
        };
        let index_path = format!("{root}/story/chapters/index.json");
        let index: Value = match tokio::fs::read_to_string(&index_path).await {
            Ok(s) => match serde_json::from_str(&s) {
                Ok(v) => v,
                Err(e) => {
                    return fail(format!("story/chapters/index.json 非合法 JSON: {e}"), 0, None);
                }
            },
            Err(_) => {
                return fail(
                    "chapter_range 需要 story/chapters/index.json（先运行分章）".to_string(),
                    0,
                    None,
                )
            }
        };
        // 每章一 part 并携带章号——人物首现章**精确**到章（chunk 跨章边界
        // 不再有近似误差）；source 分支则整体一块（行号近似映射）。
        let mut parts: Vec<(Option<u32>, String)> = Vec::new();
        ...
        for ch in index
            ...
            .unwrap_or(&[])
        {
            let no = ch.get("no").and_then(Value::as_u64).unwrap_or(0);
            if no < u64::from(lo) || no > u64::from(hi) {
                continue;
            }
            let file = ch.get("file").and_then(Value::as_str).unwrap_or("");
            if src.is_empty() {
                src = index
                    .get("source")
                    .and_then(Value::as_str)
            ...
            if let Ok(body) = tokio::fs::read_to_string(format!("{root}/{file}")).await {
                let (_, b) = split_front_matter(&body);
                parts.push((Some(no as u32), b));
            }
        }
        if parts.is_empty() {
            return fail(format!("章节范围 {range} 无可读内容"), 0, None);
        }
        (parts, src)
    } else {
        let source_rel = match source_file
            .as_deref()
            .map(str::trim)
            .filter(|s| !s.is_empty())
        {
        ...
        let Some(source_rel) = source_rel else {
            return fail(
                "未指定 source_file/chapter_range 且树内无导入原文".to_string(),
                0,
                None,
            );
        };
        match tokio::fs::read_to_string(format!("{root}/{source_rel}")).await {
            Ok(t) => (vec![(None, t)], source_rel),
            Err(e) => return fail(format!("读取原文失败 {source_rel}: {e}"), 0, None),
        }
    };
    let chars: usize = parts.iter().map(|(_, t)| t.chars().count()).sum();
    // 分块：每 part 独立切块并继承其章号标签（Some=精确 / None=行号映射）
    let chunks: Vec<(Option<u32>, ChunkSpan)> = parts
        .iter()
        .flat_map(|(no, t)| {
            split_chunks_by_lines(t, PIPE_CHUNK_CHARS)
                .into_iter()
                .map(move |c| (*no, c))
        })
        .collect();
    let n = chunks.len();
    log(format!(
        "语料 {chars} 字符（{source_rel}）→ {n} 块 × ≤{}K（并发≤{PIPE_CHUNK_CONCURRENCY}）",
        PIPE_CHUNK_CHARS / 1000
    ));
    // 章节 index（first_chapter 行号映射用；缺失 → 不标）
    let chapter_lines: Option<Vec<(u32, usize, usize)>> = tokio::fs::read_to_string(format!(
        "{root}/story/chapters/index.json"
    ))
    .await
    .ok()
    .and_then(|s| serde_json::from_str::<Value>(&s).ok())
    .and_then(|v| {
        v.get("chapters")?.as_array().map(|arr| {
            arr.iter()
                .filter_map(|c| {
                    let no = c.get("no")?.as_u64()? as u32;
                    let s = c.get("start_line")?.as_u64()? as usize;
                    let e = c.get("end_line")?.as_u64()? as usize;
                    Some((no, s, e))
                })
                .collect()
        })
    });
    let system = "你是小说人物档案整理器。只输出 JSON，不要输出任何解释文字。";
    let mr_ref = &mr;
    let mut usage_acc: (u32, u32, u32) = (0, 0, 0);
    let mut has_usage = false;
    let results: Vec<Result<(Vec<ProfileEntry>, Option<(u32, u32, u32)>), String>> =
        run_chunks_concurrent(n, |i| {
            let user = format!(
                "提取下面文本块中的全部人物，输出 JSON 数组（或 {{\"characters\": [...]}}）：\n[{{\"name\":\"姓名\",\"aliases\":[\"别名/称呼\"],\"gender\":\"性别\",\"age\":\"年龄\",\"appearance\":\"外貌特征\",\"personality\":\"性格\",\"relations\":[{{\"name\":\"对方姓名\",\"relation\":\"关系\"}}],\"arc\":\"本块内该人物的剧情走向摘要\"}}]\n只收录有台词或情节作用的人物；无人物输出 []。\n\n【文本块】\n{}",
                chunks[i].1.text
            );
            async move {
                let (out, usage) = ctx.chat_text_with_usage(mr_ref, system, &user).await?;
                let entries = parse_profile_chunk(&out)?;
                Ok((entries, usage))
            }
        })
        .await;
    ...
    for (i, r) in results.into_iter().enumerate() {
        match r {
            Ok((mut es, usage)) => {
                add_usage(&mut usage_acc, usage);
                has_usage = has_usage || usage.is_some();
                // first_chapter：章号标签（chapter_range 语料，精确）；
                // 无标签 → 块起始行号近似映射所在章
                match chunks[i].0 {
                    Some(no) => {
                        for e in es.iter_mut() {
                            if e.first_chapter.is_none() {
                                e.first_chapter = Some(no);
                            }
                        }
                    }
                    None => {
                        if let Some(chs) = &chapter_lines {
                            let cl = chunks[i].1.start_line;
                            for e in es.iter_mut() {
                                if e.first_chapter.is_none() {
                                    e.first_chapter = chs
                                        .iter()
                                        .find(|(_, s, ee)| cl >= *s && cl <= *ee)
                                        .map(|(no, _, _)| *no);
                                }
                            }
                        }
                ...
                log(format!("块 {}/{} 完成（{} 个人物）", i + 1, n, es.len()));
                entries.append(&mut es);
            }
            Err(e) => {
                return fail(
                    format!("人物梳理块 {}/{} 失败：{e}", i + 1, n),
                    chars as u64,
                    has_usage.then_some(usage_acc),
                );
            }
    ...
    let merged = merge_character_profiles(entries);
    if merged.is_empty() {
        return fail(
            "人物梳理结果为空（LLM 未提取出任何人物）".to_string(),
            chars as u64,
            has_usage.then_some(usage_acc),
        );
    }
    let out_rel = "story/characters-profile.json";
    let out_path = format!("{root}/{out_rel}");
    let doc = serde_json::to_string_pretty(&merged).unwrap_or_default();
    if let Err(e) = tokio::fs::write(&out_path, &doc).await {
        return fail(
            format!("写人物档案失败 {out_path}: {e}"),
            doc.len() as u64,
            has_usage.then_some(usage_acc),
        );
    }
    set_readme_progress_story(
        &root,
        &format!("人物梳理完成（{} 位人物，源 {}）", merged.len(), source_rel),
    )
    .await;
    append_activity(&root, &author, "story.profile", out_rel).await;
    log(format!(
        "人物档案 {} 位（跨块别名归并后）→ {out_rel}",
        merged.len()
    ));
    finish_stage(
        ctx,
        ...
        &project,
        "done",
        &format!("人物档案已存 {out_rel}（{} 位）", merged.len()),
        Some(out_rel.to_string()),
        cost(doc.len() as u64, has_usage.then_some(usage_acc)),
    );
}
...
/// FilmHub 新增路由（component=film；读公开/写 admin；v0.1.37 增 3 条 story 管线）。
pub fn hub_routes() -> Vec<RouteSpec> {
    vec![
        ...
        spec_admin(HttpMethod::Post, "/api/v1/film/projects/:id/story/clean"),
        spec_admin(
            HttpMethod::Post,
            "/api/v1/film/projects/:id/story/chapterize",
        ),
        spec_admin(HttpMethod::Post, "/api/v1/film/projects/:id/story/profile"),
        spec_admin(
            HttpMethod::Post,
...
pub async fn try_handle(
    ...
    match (req.method, rest) {
        ...
        (HttpMethod::Post, ["story", "import"]) => {
            ...
            if let Some(max) = story_import_max_bytes() {
                if bytes.len() > max {
                    return bad(format!(
                        "导入原文超过上限 {}MB（当前 {:.1}MB；env NEXOS_FILM_SOURCE_MAX_MB 可调，0=无限）",
                        max / 1024 / 1024,
        ...
        // —— 2b. story 清理（v0.1.37 管线 ①：rules 本地零成本 / llm 深清）——
        (HttpMethod::Post, ["story", "clean"]) => {
            let body: StoryCleanBody =
                parse_body(&req.body, "story 清理").map_err(ApiGatewayError::Internal)?;
            let mode = body.mode.clone().unwrap_or_else(|| "rules".to_string());
            if !matches!(mode.as_str(), "rules" | "llm") {
                return bad(format!("mode 应为 rules|llm（当前 {mode}）"));
            }
            if body.source_file.trim().is_empty() {
                return bad("source_file 不可为空".to_string());
            }
            if let Some(mr) = body.model_ref.as_ref() {
                if let Err(msg) = validate_model_ref(mr, "chat") {
                    return bad(msg);
                }
            } else if mode == "llm" {
                return bad("mode=llm 须带 model_ref（chat 能力）".to_string());
            }
            let author = author_of(&body.author);
            let task_id = h.spawn_stage_task(&project, "story.clean", |tid| {
                let ctx = ctx.clone();
                let project = project.clone();
                let mr = body.model_ref;
                let (source, mode, author) = (body.source_file, mode, author.clone());
                async move {
                    run_story_clean_stage(&ctx, &tid, project, mr, source, mode, author).await;
                }
            });
            Ok(Some(task_accepted(&h.tasks, &task_id)?))
        }

        // —— 2c. story 分章（管线 ②：cleaned 优先；无章节自动分段）——
        (HttpMethod::Post, ["story", "chapterize"]) => {
            let body: StoryChapterizeBody =
                parse_body(&req.body, "story 分章").map_err(ApiGatewayError::Internal)?;
            if let Err(msg) = validate_model_ref(&body.model_ref, "chat") {
                return bad(msg);
            }
            let author = author_of(&body.author);
            let task_id = h.spawn_stage_task(&project, "story.chapterize", |tid| {
                let ctx = ctx.clone();
                let project = project.clone();
                let mr = body.model_ref;
                let (source, author) = (body.source_file, author.clone());
                async move {
                    run_story_chapterize_stage(&ctx, &tid, project, mr, source, author).await;
                }
            });
            Ok(Some(task_accepted(&h.tasks, &task_id)?))
        }

        // —— 2d. story 人物梳理（管线 ③：别名归并 → 转定妆底稿）——
        (HttpMethod::Post, ["story", "profile"]) => {
            let body: StoryProfileBody =
                parse_body(&req.body, "story 人物梳理").map_err(ApiGatewayError::Internal)?;
            if let Err(msg) = validate_model_ref(&body.model_ref, "chat") {
                return bad(msg);
            }
            if let Some(range) = body.chapter_range.as_deref().map(str::trim) {
                if !range.is_empty() && parse_chapter_range(range).is_none() {
                    return bad(format!("chapter_range 非法（应如 1-5 或 3）: {range}"));
                }
            }
            let author = author_of(&body.author);
            let task_id = h.spawn_stage_task(&project, "story.profile", |tid| {
                let ctx = ctx.clone();
                let project = project.clone();
                let mr = body.model_ref;
                let (source, range, author) =
                    (body.source_file, body.chapter_range, author.clone());
                async move {
                    run_story_profile_stage(&ctx, &tid, project, mr, source, range, author).await;
                }
            });
            Ok(Some(task_accepted(&h.tasks, &task_id)?))
        }
