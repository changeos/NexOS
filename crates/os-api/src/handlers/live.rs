...
pub fn fed_room_id(node_hex: &str, room_id: &str) -> String {
    let short: String = node_hex.trim_start_matches("0x").chars().take(8).collect();
...
#[must_use]
pub fn build_live_lobby_payload(
    node_hex: &str,
    node_name: &str,
    room: &LiveRoom,
) -> serde_json::Value {
...
/// 构造中继退订载荷（纯函数）。
#[must_use]
pub fn build_relay_unsub_payload(
    node_hex: &str,
    node_name: &str,
    fed_id: &str,
) -> serde_json::Value {
...
impl LiveHub {
    ...
    fn add_relay_bytes(&self, id: &str, bytes: u64) {
        if let Some(state) = self.rooms.lock().expect("live: rooms lock").get_mut(id) {
...
static SHARED_FED: Lazy<LiveFedEndpoint> = Lazy::new(|| LiveFedEndpoint::new(LiveHub::shared()));
...
impl LiveFedEndpoint {
    ...
    fn install(
        ...
        *self
            ...
            .transport
            .lock()
            .expect("live-fed: transport lock") = Some(LiveFedTransport {
            send_to,
            broadcast,
            node_hex,
            node_name,
    ...
    fn spawn_fed_tasks(
        inner: Arc<LiveFedInner>,
        mut tap_rx: tokio::sync::mpsc::UnboundedReceiver<LiveFrameTap>,
    ) {
        tokio::spawn(async move {
        ...
        tokio::spawn(async move {
            ...
            while let Some(ev) = rx.recv().await {
                let (node_name, send_to) = {
                    let t = inner.transport.lock().expect("live-fed: transport lock");
    ...
    pub fn ingest_relay_frame(&self, payload: &serde_json::Value) -> RelayFrameOutcome {
        ...
        let Some(frame) = self
            .inner
            .push_chunk(fed_id, seq, ci as usize, cn as usize, bytes)
        else {
...
impl RouteHandler for LiveRouteHandler {
    ...
    async fn handle(&self, req: ApiRequest) -> Result<ApiResponse, ApiGatewayError> {
        ...
        match (req.method, segs.as_slice()) {
            ...
            (HttpMethod::Get, ["api", "v1", "live", "rooms"]) => Ok(ok_json(serde_json::json!({
...
mod tests {
    ...
    async fn room_lifecycle_create_list_end() {
        ...
        let resp = h.handle(get_req("/api/v1/live/rooms")).await.unwrap();
        assert_eq!(
            resp.body["local"].as_array().unwrap().len(),
            1,
            "结束后出表"
        );
    ...
    fn fed_announcement_payload_shape() {
        ...
        let frame = build_relay_frame_payload("node-a", "0279be66:live-3", 9, 1, 2, b"abc", false);
    ...
    async fn fed_merge_idempotent_ttl_and_ended() {
        ...
        let bad_id =
            build_live_lobby_payload(NODE_A_HEX, "node-a", &room_view_fixture("bad id!", "x", 1));
    ...
    async fn relay_sub_chunk_reassembly_unsub_ended_loop() {
        ...
        let (gen, _kick) = hub_a
            .attach_publisher(&room_id, &token)
            .expect("A 主播接入");
        assert!(
        ...
        assert!(
            fed_b.federated_rooms().is_empty(),
            "ended 宣告 → 联邦条目出表"
        );
    }
    ...
    async fn live_ws_e2e_federated_shadow_room_view() {
        ...
        let announce = build_live_lobby_payload(
            NODE_A_HEX,
            "node-a",
            &room_view_fixture("live-777", "远端直播", 2),
        );
