...
impl TranslateRegistry {
    ...
    fn register(&self, lang: &str, path: &str, chunks_total: usize) -> String {
        let id = format!("ddt-{}", self.seq.fetch_add(1, Ordering::SeqCst) + 1);
...
impl DevDocsRouteHandler {
    ...
    /// 唯一真实构造体。
    fn build(root: Option<PathBuf>, fallback: Option<String>, i18n: Option<I18nConfig>) -> Self {
        Self {
...
fn strip_outer_fence(s: &str) -> String {
    ...
    let wrapper_info_ok =
        |info: &str| matches!(info.trim(), "" | "markdown" | "md" | "text" | "plain");
    let first_ok = ["```", "~~~"]
        .iter()
...
mod tests {
    ...
    fn spawn_mock_gateway(
        ...
        std::thread::spawn(move || {
            for stream in listener.incoming() {
                ...
                let call = recorded.lock().map(|rec| rec.len()).unwrap_or(0);
    ...
    fn long_doc() -> String {
        ...
        for i in 0..300 {
            overlong.push_str(&format!(
                "段落 {i}：这是一段用于撑满分块上限的中文说明文字。\n\n"
            ));
        }
        format!(
    ...
    fn translate_chunker_sections_cap_fence_and_frontmatter() {
        ...
        // 末块以末节标题开头。
        assert!(
            chunks.last().unwrap().starts_with("## 末节"),
            "末节独立成块"
        );
        // 内容完整性（重组校验）。
    ...
    async fn translate_lang_zh_is_zero_overhead_original() {
        ...
        let cache =
            std::env::temp_dir().join(format!("nexos-devdocs-i18n-zh-{}", std::process::id()));
        ...
        for path in [
            "/api/v1/devdocs/doc/a.md",
            "/api/v1/devdocs/doc/a.md?lang=zh",
        ] {
    ...
    async fn translate_full_pipeline_task_then_cached() {
        ...
        let cache =
            std::env::temp_dir().join(format!("nexos-devdocs-i18n-full-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&cache);
        let recorded: Arc<Mutex<Vec<RecordedGwReq>>> = Arc::new(Mutex::new(Vec::new()));
        ...
        for r in &reqs {
            ...
            assert!(
                r.path.contains("/api/v1/gateway/v1/chat/completions"),
                "{}",
                r.path
            );
            ...
            assert!(
                sys.contains("NodeID") && sys.contains("ZFS") && sys.contains("vLLM"),
                "术语表在位"
            );
            ...
            let expect_tokens = (sys.chars().count() + user.chars().count()) / 2 + 2048;
            assert_eq!(
        ...
        assert!(
            std::fs::read_to_string(cache.join("zh-TW/dev/guide.md")).is_ok(),
            "zh-TW 独立缓存文件"
        );
        let _ = std::fs::remove_dir_all(&cache);
    }
    ...
    async fn translate_running_task_reused_no_duplicate() {
        ...
        write(&root.join("a.md"), "# A\n\n正文。\n\n## B\n\n更多。\n");
        let cache =
            std::env::temp_dir().join(format!("nexos-devdocs-i18n-dedup-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&cache);
        let recorded: Arc<Mutex<Vec<RecordedGwReq>>> = Arc::new(Mutex::new(Vec::new()));
        ...
        let r1 = h
            .handle(get_req("/api/v1/devdocs/doc/a.md?lang=en"))
            .await
            .unwrap();
    ...
    async fn translate_stale_source_mtime_retranslates() {
        ...
        write(&root.join("a.md"), "# A\n\n旧内容。\n");
        let cache =
            std::env::temp_dir().join(format!("nexos-devdocs-i18n-stale-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&cache);
        let recorded: Arc<Mutex<Vec<RecordedGwReq>>> = Arc::new(Mutex::new(Vec::new()));
        let base = spawn_mock_gateway(gw_echo_translation, Arc::clone(&recorded));
        let h = translate_handler(&root, &cache, base, Some("tk"), "m");
        ...
        assert!(
            recorded.lock().unwrap().len() > calls_after_first,
            "重译产生新网关调用"
        );
        ...
        assert!(
            r3.body["markdown"].as_str().unwrap().contains("新内容"),
            "新译内容生效"
        );
        let _ = std::fs::remove_dir_all(&cache);
    }
    ...
    async fn translate_no_channel_503_honest_degrade_and_retry() {
        ...
        write(&root.join("a.md"), "# A\n\n正文。\n");
        let cache =
            std::env::temp_dir().join(format!("nexos-devdocs-i18n-nomodel-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&cache);
        let recorded: Arc<Mutex<Vec<RecordedGwReq>>> = Arc::new(Mutex::new(Vec::new()));
    ...
    async fn translate_task_404_and_missing_token_503() {
        ...
        write(&root.join("a.md"), "# A\n");
        let cache =
            std::env::temp_dir().join(format!("nexos-devdocs-i18n-misc-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&cache);
        let recorded: Arc<Mutex<Vec<RecordedGwReq>>> = Arc::new(Mutex::new(Vec::new()));
        let base = spawn_mock_gateway(gw_echo_translation, Arc::clone(&recorded));
        ...
        assert!(
            msg.contains("本节点无可用本地模型"),
            "统一降级文案开头：{msg}"
        );
        assert!(
            msg.contains("NEXOS_DEVDOCS_GATEWAY_TOKEN"),
            "给出 env 配置指引"
        );
    ...
    async fn translate_thinking_occupied_retries_with_soft_switch() {
        ...
        write(&root.join("a.md"), "# A\n\n正文一段。\n"); // 单块文档
        let cache =
            std::env::temp_dir().join(format!("nexos-devdocs-i18n-think-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&cache);
        let recorded: Arc<Mutex<Vec<RecordedGwReq>>> = Arc::new(Mutex::new(Vec::new()));
        ...
        assert!(
            retry_user.ends_with("/no_think"),
            "软开关追加：{retry_user:?}"
        );
        assert_eq!(
            reqs[1].body["chat_template_kwargs"]["enable_thinking"],
            false
        );
        // 任务日志含区分文案（前端可见重试动作）。
        let log = settled["log"]
    ...
    async fn translate_thinking_occupied_retry_exhausted_error() {
        ...
        write(&root.join("a.md"), "# A\n\n正文。\n");
        let cache =
            std::env::temp_dir().join(format!("nexos-devdocs-i18n-think2-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&cache);
        let recorded: Arc<Mutex<Vec<RecordedGwReq>>> = Arc::new(Mutex::new(Vec::new()));
