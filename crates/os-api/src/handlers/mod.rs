...
//!   （写操作需 admin）。
//! - （已剥离）`streaming` / `surveillance` / `film` / `film_hub` 三引擎
//!   （v0.1.51 删除出 os-api——独立产品线 FilmStudio / StreamingStudio 于各自
//!   NexHub 仓承接；探测函数下沉 [`probe_utils`]，应用中心安装拦截见
//!   apps_handler）。
...
/// 静态托管（SQLite apps 表持久化）；qrtransfer 等引擎门控共享 `AppRegistry`
/// 实例（未装应用 → 引擎业务端点 404），已剥离引擎（film/streaming）安装
/// 拦截 400，docs/APPS.md。
...
/// 外部工具探测工具集（ffmpeg/blender 三态解析链 + is_executable）——
/// v0.1.51 三引擎剥离批自 film.rs / film_hub/blender.rs 下沉（函数体零改
/// 只搬家），monitor.rs / capabilities.rs 等系统内消费者共用。
pub mod probe_utils;
pub mod qr_transfer;
pub mod share;
pub mod storage;
pub mod system;
pub mod tasks_common;
pub mod terminal;
pub mod transfer;
pub mod update;
pub mod user;
...
pub use tasks_common::{UnifiedStatus, UnifiedTask, UnifiedTaskStore, UnifiedTasksRouteHandler};
pub use terminal::{
