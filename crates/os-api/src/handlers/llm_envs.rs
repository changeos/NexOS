...
fn parse_channel(raw: Option<&str>) -> Result<String, String> {
    ...
    if valid_channel(c) {
        ...
        Err(format!(
            "channel 非法（{CHANNEL_STABLE} 或 {CHANNEL_NIGHTLY}）: {c}"
        ))
    }
}
...
pub fn create_env_schema(conn: &Connection) -> rusqlite::Result<()> {
    ...
    let _ = conn.execute("ALTER TABLE llm_environments ADD COLUMN channel TEXT", []);
    Ok(())
}
...
pub struct LlmEnvState {
    ...
    /// 统一任务登记表（v0.1.54 双写旁路；None=未注入零旁路）。
    unified: Option<Arc<crate::handlers::tasks_common::UnifiedTaskStore>>,
}
...
impl LlmEnvState {
    ...
    /// 注入统一任务登记表（v0.1.54 统一任务框架：环境创建/更新任务双写
    /// `unified_tasks`，kind=llm_env；内存 HashMap 保留不动；轻旁路）。
    pub fn set_unified_tasks(&mut self, store: Arc<crate::handlers::tasks_common::UnifiedTaskStore>) {
...
/// 任务收尾（状态 + finished_at + 收尾日志行 + 统一登记终态）。
fn task_finish(ctx: &TaskCtx, kind: &str, status: &str, line: &str) {
    ...
    // 统一任务框架旁路（done→done / error→error；轻旁路失败仅日志）
    if let Some(store) = &ctx.unified {
        let label = if kind == "create" {
            format!("创建推理环境 {}", ctx.env_name)
        } else {
            format!("更新推理环境 {}", ctx.env_name)
        };
        store.finish(
            "llm_env",
            &ctx.task_id,
            crate::handlers::tasks_common::UnifiedStatus::from_source("llm_env", status),
            &label,
            Some(line),
            (status == "error").then_some(line),
        );
    }
    eprintln!(
        "[llm-env] 任务{}：{}（{kind} 环境 {}）",
...
fn locate_uv_in(
    ...
    task_log(
        ctx,
        &format!("全链未命中 uv，自动安装：curl -LsSf {url} | sh"),
    );
...
pub fn handle(
    ...
    match (method, segs) {
        ...
        (HttpMethod::Post, [name, "update"]) => {
            ...
            let channel =
                match parse_channel(body.channel.as_deref().or(Some(existing_channel.as_str()))) {
                    Ok(c) => c,
                    Err(msg) => return Ok(error_response(400, &msg)),
                };
...
impl LlmEnvState {
    ...
    fn register_task(&self, kind: &str, env_name: &str) -> String {
        ...
        // 统一任务框架旁路登记（running；内存 HashMap 保留不动）
        if let Some(store) = &self.unified {
            store.register(
                &crate::handlers::tasks_common::UnifiedTask {
                    id: task_id.clone(),
                    kind: "llm_env".into(),
                    status: crate::handlers::tasks_common::UnifiedStatus::Running,
                    label: if kind == "create" {
                        format!("创建推理环境 {env_name}")
                    } else {
                        format!("更新推理环境 {env_name}")
                    },
                    stage: Some(kind.to_string()),
                    log_tail: String::new(),
                    output: None,
...
mod tests {
    ...
    fn locate_ctx(
        executor: Arc<dyn EnvExecutor>,
    ) -> (TaskCtx, Arc<Mutex<HashMap<String, EnvTask>>>) {
    ...
    fn parse_channel_defaults_and_rejects() {
        ...
        assert_eq!(
            parse_channel(Some(" nightly ")).unwrap(),
            "nightly",
            "首尾空白应裁剪"
        );
    ...
    fn pip_install_argv_nightly_matches_preset_example() {
        ...
        assert!(
            kv.is_empty(),
            "nightly 不走 UV_PIP_INDEX_URL env（避免镜像顶成主源）"
        );
        assert!(
            !argv.iter().any(|a| a.starts_with("vllm==")),
            "nightly 不钉版本"
        );
    ...
    async fn nightly_create_runs_preset_command_and_marks_channel() {
        ...
        assert!(
            joined.contains(
                "-U vllm --torch-backend=auto --extra-index-url https://wheels.vllm.ai/nightly"
            ),
    ...
    async fn update_switches_channel_nightly_and_back() {
        ...
        assert!(
            !joined.contains("wheels.vllm.ai"),
            "stable 命令不应带 nightly 源: {joined}"
        );
        let row = env_row(&state, "main");
    ...
    /// 统一任务框架双写（v0.1.54）：创建任务 running 登记 → 终态 done/error
    /// 落表；未注入时零旁路。
    #[tokio::test]
    async fn unified_dual_write_create_task_lifecycle() {
        use crate::handlers::tasks_common::{UnifiedStatus, UnifiedTaskStore};
        let dir = std::env::temp_dir().join(format!(
            "nexos-llm-envs-unified-{}",
            os_core::Uuid::new_v4()
        ));
        ...
        let mut state = state_with(Arc::new(Echo));
        state.set_unified_tasks(Arc::clone(&unified));

        let t1 = create_env(&state, "main");
        let body = wait_task_done(&state, &t1);
        assert_eq!(body["status"], "done", "Echo 执行器应成功");
        // 终态落 unified_tasks（kind=llm_env、done、label 含环境名）
        let rows = unified.list(Some("llm_env"), None, 50);
        assert_eq!(rows.len(), 1, "{rows:?}");
        assert_eq!(rows[0].id, t1);
        assert_eq!(rows[0].status, UnifiedStatus::Done);
        assert!(rows[0].label.contains("main"), "label 含环境名");
        assert!(rows[0].finished_at.is_some());
        // 内存任务面不受影响（保留不动）
        let resp = get(&state, "/tasks");
        assert_eq!(resp.body["tasks"].as_array().unwrap().len(), 1);

        // 失败路径（Failing 执行器）→ error 落表
        struct Failing;
        impl EnvExecutor for Failing {
            fn run(
                &self,
                _argv: &[&str],
                _env_kv: &[(String, String)],
        ...
        let unified2 = Arc::new(UnifiedTaskStore::with_path(
            dir.join("tasks2.db").to_str().unwrap(),
        ));
        let mut state2 = state_with(Arc::new(Failing));
        state2.set_unified_tasks(Arc::clone(&unified2));
        let t2 = create_env(&state2, "spare");
        let body = wait_task_done(&state2, &t2);
        assert_eq!(body["status"], "error");
        let rows = unified2.list(Some("llm_env"), Some("error"), 50);
        assert_eq!(rows.len(), 1, "失败落 error: {rows:?}");
        assert!(rows[0].error.is_some());
        // 未注入零旁路
        let state3 = state_with(Arc::new(Echo));
        let t3 = create_env(&state3, "solo");
        wait_task_done(&state3, &t3);
        assert!(
            unified2.list(Some("llm_env"), None, 50).len() == 1,
            "未注入不双写"
        );
