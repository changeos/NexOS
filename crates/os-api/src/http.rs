...
async fn request_to_api(
    ...
    // 请求体上限：NEXOS_BODY_MAX_MB（缺省 128MB），审计 A3-1① 快赢#2。
    request_to_api_limited(req, state, body_limit_bytes()).await
}

/// 请求体上限缺省值（MB）。
///
/// 128MB 覆盖现有最大合法载荷：film 源文件 b64 JSON 信封
/// （NEXOS_FILM_SOURCE_MAX_MB=64 → body ~85MB）在内；ISO 级大文件走
/// provisioning 已验证的流式直挂路由（256KiB unfold + Range 206），不经
/// 此整包通道。
const DEFAULT_BODY_MAX_MB: usize = 128;

/// 解析 `NEXOS_BODY_MAX_MB`（缺省/无效回退 128；下限钳到 1MB 防误配零上限）。
fn parse_body_max_mb(raw: Option<&str>) -> usize {
    raw.and_then(|s| s.trim().parse::<usize>().ok())
        .map(|mb| mb.max(1))
        .unwrap_or(DEFAULT_BODY_MAX_MB)
}

/// 当前请求体字节上限（每请求读 env——无缓存，改配置即生效）。
fn body_limit_bytes() -> usize {
    parse_body_max_mb(std::env::var("NEXOS_BODY_MAX_MB").ok().as_deref()) * 1024 * 1024
}

/// [`request_to_api`] 的可注入上限版（测试边界用；生产走 env 上限）。
async fn request_to_api_limited(
    req: Request,
    state: &GatewayState,
    limit: usize,
) -> Result<ApiRequest, (axum::http::StatusCode, String)> {
    ...
    // 上限（审计 A3-1① 快赢#2）：读 limit+1 字节以区分「超限」（413 如实
    // 拒）与「读失败」（400 照旧）；远超上限时 to_bytes 以 length limit
    // exceeded 错误提前中止，同样归 413。
    let bytes = match axum::body::to_bytes(body_axum, limit.saturating_add(1)).await {
        Ok(b) => {
            if b.len() > limit {
                return Err((
                    axum::http::StatusCode::PAYLOAD_TOO_LARGE,
                    format!(
                        "请求体 {} 字节超过上限 {} 字节（{}MB，env NEXOS_BODY_MAX_MB 可调）",
                        b.len(),
                        limit,
                        limit / 1024 / 1024
                    ),
        ...
        Err(e) => {
            // http-body-util 超限错误文案「length limit exceeded」→ 413；
            // 其余（连接中断等读失败）维持 400。
            if e.to_string().contains("length limit exceeded") {
                return Err((
                    axum::http::StatusCode::PAYLOAD_TOO_LARGE,
                    format!(
                        "请求体超过上限 {} 字节（{}MB，env NEXOS_BODY_MAX_MB 可调）",
                        limit,
                        limit / 1024 / 1024
                    ),
                ));
    ...
    match parse_cgi_output(&output.stdout) {
        Some(cgi) => {
            ...
            if git_path.ends_with("/git-receive-pack") && cgi.status == 200 {
                ...
                // NexHub Webhooks（v0.1.50 top1）：push 事件回调投递（照 CI push_hook
                // 同一成功路径挂钩；fire-and-forget tokio::spawn，服务未装配时 no-op，
                // 绝不影响 push 响应）。remote_user = token 推送者（匿名回落 anonymous）。
                os_nexhub::webhooks::fire_push(&repo, &remote_user);
            }
...
impl StreamAccounting {
    ...
    ///
    /// 批 2（2026-09-25）起 async：网关 `record_success` 已搬 `spawn_blocking`
    /// （api_gateway 批 2），这里跟着 `.await`——调用点都在流收尾的 async 段。
    async fn record_success(&mut self) {
        ...
        self.recorded = true;
        let usage =
            crate::handlers::api_gateway::parse_stream_usage(&String::from_utf8_lossy(&self.tail));
    ...
    async fn record_failure(&mut self, err: &str) {
        if self.recorded {
            return;
        }
        self.recorded = true;
        self.gw
            .record_failure(&self.token, &self.channel, &self.model, err)
            .await;
    }
}
...
pub async fn gateway_openai_handler(State(state): State<GatewayState>, req: Request) -> Response {
    ...
    let plan = match gw.resolve_forward_plan(&headers, &body_json).await {
        Ok(p) => p,
    ...
    S: futures::Stream<Item = Result<Bytes, String>> + Send + 'static,
{
    ...
    struct FwdState {
        ...
        first: Option<Bytes>,
        inner: std::pin::Pin<Box<dyn futures::Stream<Item = Result<Bytes, String>> + Send>>,
    ...
    futures::stream::unfold(st, |mut st| async move {
        ...
        match st.inner.next().await {
            ...
            Some(Err(e)) => {
                st.acc.record_failure(&format!("上游流中途断开: {e}")).await;
                st.errored = true;
                let comment = Bytes::from(format!(": gateway: upstream stream aborted ({e})\n\n"));
                Some((Ok(comment), st))
            }
            None => {
                st.acc.record_success().await;
                None
            }
        }
    })
...
// ----------------------------------------------------------------------------
// NexHub Release 附件大载荷流式通道（v0.1.53）：特挂直收 / 直挂
// ----------------------------------------------------------------------------

/// 流式通道错误响应（与 b64 信封路径同形 `{"error": ...}` JSON）。
fn asset_stream_error(status: u16, msg: &str) -> Response {
    (
...
/// query string 单键取值（`+` → 空格 + `%XX` 解码，与 issues.rs `query_params`
/// 同语义；无该键返回 None）。
fn asset_query_param(query: Option<&str>, key: &str) -> Option<String> {
    let q = query?;
    for pair in q.split('&') {
        let mut it = pair.splitn(2, '=');
        if it.next() == Some(key) {
            return Some(percent_decode_path(&it.next().unwrap_or("").replace('+', " ")));
        }
    }
    None
}

/// POST `/api/v1/coderepo/repos/{name}/releases/{tag}/assets` 特挂 handler
/// （v0.1.53 大载荷流式通道）：
///
/// - `?stream=1`：请求体 `application/octet-stream` **直收**——逐块写盘
///   （tokio::fs）+ sha256 累计，全程常量级内存（≈分块大小，不再整包进内存）；
///   鉴权/校验/落库在 `os_nexhub::issues::IssuesCore`（begin → 流式写盘 →
///   commit/abort 三段式）。附件名取 `?name=`（必填）。
/// - 其余请求（无 `stream=1`）：原样回落 [`dispatch_handler`]——b64-JSON 信封
///   兼容端点行为零变化（照 2c `chat_stream_handler` 先例）。
///
/// 护栏：Content-Length 预检（begin 内 413/400）+ 运行中逐块累计上限
/// （Content-Length 缺失/谎报时兜底 413，半成品文件即弃）。
pub async fn release_asset_stream_upload_handler(
    State(state): State<GatewayState>,
    ...
    let wants_stream = asset_query_param(query, "stream").as_deref() == Some("1");
    if !wants_stream {
        // b64-JSON 信封兼容路径：原请求（头/体未动）交回通用分发。
        return dispatch_handler(State(state), Request::from_parts(parts, body)).await;
    }
    let Some(core) = os_nexhub::issues::shared_asset_stream() else {
        return asset_stream_error(503, "流式通道未装配（code_repo 组件未初始化；请用 b64 信封端点 POST {name, content_base64}）");
    };
    let Some(name) = asset_query_param(query, "name").filter(|s| !s.trim().is_empty()) else {
        return asset_stream_error(400, "流式上传需 ?name=<附件名> 查询参数");
    };
    ...
    // 鉴权头 → Json（小写键，同 request_to_api 形态——IssuesCore::caller 消费）
    let mut headers_map = serde_json::Map::new();
    for (hname, value) in &parts.headers {
        let key = hname.as_str().to_lowercase();
        if let Ok(s) = value.to_str() {
            headers_map.insert(key, serde_json::Value::String(s.to_string()));
        }
    }
    ...
    // 1) begin：鉴权 → release/名字校验 → Content-Length 预检 → 同名 409 预检
    let ticket = match core.begin_asset_stream_upload(
        &repo,
        &tag,
        &headers,
        &name,
        ...
        Ok(t) => t,
        Err((status, msg)) => return asset_stream_error(status, &msg),
    };

    // 2) 流式写盘：逐块 write + sha256 累计 + 运行中大小护栏
    let mut file = match tokio::fs::File::create(&ticket.path).await {
        Ok(f) => f,
        Err(e) => {
            core.abort_asset_stream_upload(&ticket);
            return asset_stream_error(500, &format!("创建附件临时文件失败: {e}"));
        }
    };
    use tokio::io::AsyncWriteExt;
    ...
    while let Some(chunk) = stream.next().await {
        let chunk = match chunk {
            Ok(c) => c,
            Err(e) => {
                io_err = Some(format!("读取上传流失败: {e}"));
                break;
            }
        };
        total += chunk.len() as u64;
        if total as usize > os_nexhub::issues::MAX_ASSET_BYTES {
            core.abort_asset_stream_upload(&ticket);
            return asset_stream_error(
                413,
                &format!(
                    "附件过大（≤{} 字节，运行中累计 {} 字节中止）",
                    os_nexhub::issues::MAX_ASSET_BYTES,
                    total
                ),
            );
        }
    ...
    if let Some(drop_reason) = io_err.or_else(|| {
        (total == 0).then(|| "附件内容不可为空（流为空）".to_string())
    }) {
        core.abort_asset_stream_upload(&ticket);
        return asset_stream_error(if total == 0 { 400 } else { 500 }, &drop_reason);
    }
    if let Err(e) = file.flush().await {
        core.abort_asset_stream_upload(&ticket);
        return asset_stream_error(500, &format!("落盘 flush 失败: {e}"));
    }
    drop(file);

    // 3) commit：大小复核 + 落库（并发同名后到者 409，半成品清理在核心内）
    let sha = format!("{:x}", hasher.finalize());
    match core.commit_asset_stream_upload(&ticket, total, &sha) {
        Ok(asset) => (
            axum::http::StatusCode::CREATED,
            axum::Json(serde_json::json!({ "ok": true, "asset": asset })),
        )
            .into_response(),
        Err((status, msg)) => asset_stream_error(status, &msg),
    }
}

/// GET `/api/v1/coderepo/repos/{name}/releases/{tag}/assets/{aid}` 特挂 handler
/// （v0.1.53）：附件内容 **octet-stream 流式直挂**——照 ISO 直传先例
/// （`netboot_stream_response`：tokio::fs + 256KiB unfold + Range 206/416），
/// 不经网关 JSON 信封（b64 信封会把整文件读进内存再解码）。附件行（名字/
/// sha256）经 `IssuesCore` 解析；`<a download>` 直链语义不变（同路径、
/// 同 content-disposition）。
pub async fn release_asset_stream_download_handler(
    Path((repo, tag, aid)): Path<(String, String, String)>,
    headers: axum::http::HeaderMap,
) -> Response {
    ...
    let Some(core) = os_nexhub::issues::shared_asset_stream() else {
        return asset_stream_error(503, "流式通道未装配（code_repo 组件未初始化）");
    };
    let (asset, path) = match core.resolve_asset_stream_download(&repo, &tag, &aid) {
        Ok(v) => v,
        Err((status, msg)) => return asset_stream_error(status, &msg),
    };
    let file = match tokio::fs::File::open(&path).await {
        Ok(f) => f,
        Err(_) => {
            return asset_stream_error(404, &format!("附件文件缺失: {aid}"));
        }
    };
    let size = match file.metadata().await {
        Ok(m) if m.is_file() => m.len(),
        _ => return asset_stream_error(404, "附件文件不可读"),
    };
    let range_header = headers
        .get(axum::http::header::RANGE)
        .and_then(|v| v.to_str().ok());
    let range = match crate::handlers::provisioning::parse_range_header(range_header, size) {
        Ok(r) => r,
        Err(()) => {
            // 416 + Content-Range: bytes */<size>（RFC 7233，同 ISO 先例）
            return (
                StatusCode::RANGE_NOT_SATISFIABLE,
                [("content-range", format!("bytes */{size}"))],
                axum::Json(serde_json::json!({
                    "error": format!("Range 不可满足（size={size}）")
                })),
            )
                .into_response();
        }
    ...
    let (start, len, status) = match range {
        crate::handlers::provisioning::HttpRange::Full => (0u64, size, StatusCode::OK),
        crate::handlers::provisioning::HttpRange::Partial { start, len } => {
            (start, len, StatusCode::PARTIAL_CONTENT)
        }
    };
    let mut file = file;
    if let Err(e) = file.seek(std::io::SeekFrom::Start(start)).await {
        return asset_stream_error(500, &format!("seek 失败: {e}"));
    }

    // 256KiB 分块流（netboot 同款 unfold；NETBOOT_STREAM_CHUNK 复用同一常量）
    let chunk = crate::handlers::provisioning::NETBOOT_STREAM_CHUNK;
    let stream = futures::stream::unfold((file, len), move |(mut f, remaining)| async move {
        if remaining == 0 {
            return None;
        }
    ...
    let mut builder = axum::response::Response::builder()
        ...
        .header("x-nexos-sha256", asset.sha256.clone());
    if let crate::handlers::provisioning::HttpRange::Partial { start, len } = range {
    ...
    builder
        ...
        .unwrap_or_else(|_| asset_stream_error(500, "body build failed"))
}
...
pub fn build_router(state: GatewayState, ws_path: Option<&str>) -> Router {
    ...
    // 2d) 网络装机 P0 流式直传（`direct_passthrough_bytes` 的流式扩展——3GB 级
    //     ISO 不能进 base64 网关 body）：GET /api/v1/provisioning/isos/{file}
    //     （完整 ISO，casper `url=` 拉取，带 Range 断点续传）与
    //     /api/v1/provisioning/boot/{id}/{file}（ISO 内提取的 casper
    //     vmlinuz/initrd）。tokio::fs 分块流 + 206/416 + 仓白名单防穿越
    //     （见 handlers/provisioning.rs::netboot_stream_response）。同 /git/*
    //     先例直挂 axum（不进 RouteSpec/dispatch——JSON 信封装不下字节流）；
    //     两条路径与 3) 循环注册的静态段 `/isos`（GET 清单）不冲突。
    router = router.route(
        "/api/v1/provisioning/isos/{file}",
        axum::routing::get(crate::handlers::provisioning::netboot_iso_stream_handler),
    );
    router = router.route(
        "/api/v1/provisioning/boot/{id}/{file}",
        axum::routing::get(crate::handlers::provisioning::netboot_boot_stream_handler),
    );

    // 2e) NexHub Release 附件大载荷流式通道（v0.1.53）：POST
    //     /api/v1/coderepo/repos/{name}/releases/{tag}/assets 的 `?stream=1`
    //     请求 octet-stream 直收（逐块落盘 + sha256，非流式回落 dispatch——b64
    //     信封端点保留兼容）与 GET .../assets/{aid} octet-stream 流式直挂
    //     （256KiB unfold + Range 206/416，netboot 同款；鉴权/落库经
    //     os_nexhub::issues 共享内核——与 b64 路径同源判定）。同 /git/* 先例
    //     直挂 axum（不进 RouteSpec/dispatch——JSON 信封装不下字节流）；spec
    //     循环里跳过这两条避免 axum 路由重复注册 panic。
    router = router.route(
        "/api/v1/coderepo/repos/{name}/releases/{tag}/assets",
        axum::routing::post(release_asset_stream_upload_handler),
    );
    router = router.route(
        "/api/v1/coderepo/repos/{name}/releases/{tag}/assets/{aid}",
        axum::routing::get(release_asset_stream_download_handler),
    );
    ...
    for spec in routes {
        ...
        // 2e 的特挂路由已占用这两条（附件流式上传 ?stream=1 + 非流式回落
        // dispatch / 附件流式直挂下载），跳过避免 axum 路由重复注册 panic。
        if spec.method == crate::gateway::HttpMethod::Post
            && spec.path == "/api/v1/coderepo/repos/:name/releases/:tag/assets"
        {
            continue;
        }
        if spec.method == crate::gateway::HttpMethod::Get
            && spec.path == "/api/v1/coderepo/repos/:name/releases/:tag/assets/:aid"
        {
            continue;
        }
...
async fn static_handler(req: Request) -> Response {
    ...
    if path == "/" {
        return match crate::webui::get_asset("index.html") {
            Some((data, mime)) => serve_bytes(data, mime, cache_control_for_static("index.html")),
            None => not_found(),
        };
    }
    ...
    if !rel.is_empty() {
        if let Some((data, mime)) = crate::webui::get_asset(rel) {
            return serve_bytes(data, mime, cache_control_for_static(rel));
        }
        // 尝试去掉 static/ 前缀（旧版兼容）
        if let Some(key) = rel.strip_prefix("static/") {
            if !key.is_empty() {
                if let Some((data, mime)) = crate::webui::get_asset(key) {
                    return serve_bytes(data, mime, cache_control_for_static(key));
                }
            }
        }
    }
...
/// 静态资源 Cache-Control 策略（审计 A3-2 / 快赢#1，2026-09-24 批 1/3）。
///
/// - `assets/` 前缀（Vite 构建产物目录，文件名自带内容哈希——如
///   `AdminConsole-Dr_56sik.js`，内容变则名变）→ 一年 `immutable`：哈希名
///   即资源身份，长缓存零陈旧风险，省掉每次访问 ~700KB gzip 重下；
/// - 其余一切（`index.html` / SPA fallback / favicon / wallpapers / 旧版
///   `static/` 兜底文件——入口与非哈希名文件）→ `no-cache`：可存储但每次
///   回源取最新，保发版（rust-embed 重嵌）即时生效。
///
/// `/apps-assets/:id/*` 应用静态托管复用同一策略（审计 §G 风险项：独立
/// 路由同批覆盖，见 apps_handler）。
pub(crate) fn cache_control_for_static(rel: &str) -> &'static str {
...
/// 把字节流 + MIME 包成 axum `Response`（200 + Content-Type + Cache-Control）。
fn serve_bytes(data: Vec<u8>, mime: &'static str, cache_control: &'static str) -> Response {
    (
...
pub(crate) mod tests {
        ...
        let resp = router
            ...
            .oneshot(
                axum::http::Request::builder()
                    ...
                    .header("content-type", "application/json")
                    .header("authorization", format!("Bearer {GW_TEST_ADMIN_TOKEN}"))
                    .body(axum::body::Body::from(
    ...
    const SSE_CHUNK1: &str =
        "data: {\"id\":\"x\",\"choices\":[{\"delta\":{\"content\":\"Hel\"}}]}\n\n";
    const SSE_CHUNK2: &str =
    ...
    async fn gateway_stream_forwards_chunks_incrementally_with_usage_billed() {
        ...
        let rest = axum::body::to_bytes(axum::body::Body::from_stream(data_stream), usize::MAX)
            .await
            .unwrap();
    ...
    async fn gateway_stream_failover_before_first_byte_on_http_error() {
        ...
        assert!(
            logs.iter()
                .any(|l| l.status == "failed" && l.error.as_deref().unwrap_or("").contains("500")),
            "失败渠道应记 failed: {logs:?}"
        );
        assert!(
    ...
    async fn gateway_stream_no_failover_after_first_byte() {
        ...
        assert!(
            logs.iter().any(|l| l.status == "failed"
                && l.error.as_deref().unwrap_or("").contains("上游流中途断开")),
            "应记中途断开: {logs:?}"
        );
        assert!(
            !logs.iter().any(|l| l.status == "success"),
    ...
    fn gw_sse_relay_pair(
        base: &str,
    ) -> (crate::handlers::api_market::ApiMarketFedEndpoint, String) {
    ...
    // —— 静态资源 Cache-Control（审计 A3-2 / 快赢#1，2026-09-24 批 1/3） ——

    #[test]
    fn cache_control_policy_split() {
        // assets/（Vite 内容哈希产物目录）→ 一年 immutable；
        // 入口与非哈希名文件（index/favicon/wallpapers/旧版 static）→ no-cache
        assert_eq!(
            cache_control_for_static("assets/AdminConsole-Dr_56sik.js"),
            "public, max-age=31536000, immutable"
        );
        assert_eq!(cache_control_for_static("index.html"), "no-cache");
        assert_eq!(cache_control_for_static("favicon.svg"), "no-cache");
        assert_eq!(
            cache_control_for_static("wallpapers/nexos-ember.jpg"),
            "no-cache"
        );
        // 旧版兜底池（static/ 剥前缀后）非哈希名 → no-cache
        assert_eq!(cache_control_for_static("css/style.css"), "no-cache");
    }

    #[tokio::test]
    async fn static_hashed_assets_get_immutable_cache_control() {
        // 从内嵌 index.html 解析真实哈希产物路径（前端重构建哈希变，测试不失效）
        let (data, _mime) = crate::webui::get_asset("index.html").expect("index.html 必在");
        let html = String::from_utf8_lossy(&data);
        let needle = "/assets/";
        let pos = html
            .find(needle)
            .expect("index.html 应引用 /assets/ 哈希产物");
        let asset_file = html[pos + needle.len()..]
            .split(['"', '\''])
            .next()
            .unwrap_or_default();
        assert!(
            asset_file.ends_with(".js") || asset_file.ends_with(".css"),
            "应截到产物文件名：{asset_file}"
        );

        // 两种 URL 形态（/assets/* 与 /static/assets/* 旧前缀）都命中 immutable
        for uri in [
            format!("/assets/{asset_file}"),
            format!("/static/assets/{asset_file}"),
        ] {
            let router = empty_router();
            let resp = router
                .oneshot(
                ...
                .await
                .unwrap();
            assert_eq!(resp.status(), 200, "{uri} 应 200");
            assert_eq!(
                resp.headers()
                    .get(axum::http::header::CACHE_CONTROL)
                    .unwrap(),
                "public, max-age=31536000, immutable",
                "{uri} 哈希产物应一年 immutable"
            );
        }
    }

    #[tokio::test]
    async fn static_entries_get_no_cache_control() {
        // 入口与 SPA fallback 与非哈希静态文件 → no-cache（保发版即时生效）
        for uri in ["/", "/favicon.svg", "/storage"] {
            let router = empty_router();
            let resp = router
                .oneshot(
                    axum::http::Request::builder()
    ...
    #[test]
    fn body_max_mb_env_parsing() {
        assert_eq!(parse_body_max_mb(None), 128, "缺省 128MB");
        assert_eq!(parse_body_max_mb(Some("64")), 64);
        assert_eq!(parse_body_max_mb(Some(" 8 ")), 8, "容忍空白");
        assert_eq!(parse_body_max_mb(Some("abc")), 128, "无效回退缺省");
        assert_eq!(parse_body_max_mb(Some("0")), 1, "下限 1MB 防零上限");
        if std::env::var("NEXOS_BODY_MAX_MB").is_err() {
            assert_eq!(body_limit_bytes(), 128 * 1024 * 1024);
        }
    }

    /// 构造一个 POST Request（body 为 n 字节 'x'）。
    fn post_body_request(n: usize) -> Request {
        axum::http::Request::builder()
            .method("POST")
            .uri("/api/v1/pools")
            .body(axum::body::Body::from(vec![b'x'; n]))
            .unwrap()
    }

    #[tokio::test]
    async fn body_limit_boundary_exactly_limit_passes() {
        // 恰好等于上限 → 放行（上限是「> 才拒」，不是「>= 拒」）
        let gw = InProcessGateway::new();
        let state = gw.make_state(None, None);
        let api = request_to_api_limited(post_body_request(16), &state, 16).await;
        assert!(api.is_ok(), "恰好等于上限的 body 应放行");
    }

    #[tokio::test]
    async fn body_limit_limit_plus_one_rejected_413() {
        // 上限 +1 字节 → 413，文案如实（实际字节数 + 调参入口）
        let gw = InProcessGateway::new();
        let state = gw.make_state(None, None);
        let err = request_to_api_limited(post_body_request(17), &state, 16)
            .await
            .expect_err("超限 1 字节应 413");
        assert_eq!(err.0, axum::http::StatusCode::PAYLOAD_TOO_LARGE);
        assert!(err.1.contains("17"), "应给出实际字节数：{}", err.1);
        assert!(
            err.1.contains("NEXOS_BODY_MAX_MB"),
            "应指明调参入口：{}",
            err.1
        );
    }
    ...
    #[tokio::test]
    async fn body_limit_far_over_limit_rejected_413() {
        // 远超上限 → to_bytes 以 length limit exceeded 提前中止 → 同样 413
        //（不是 400——超限与读失败必须区分）
        let gw = InProcessGateway::new();
        let state = gw.make_state(None, None);
        let err = request_to_api_limited(post_body_request(64), &state, 16)
            .await
            .expect_err("远超上限应 413");
        assert_eq!(err.0, axum::http::StatusCode::PAYLOAD_TOO_LARGE);
        assert!(
            err.1.contains("NEXOS_BODY_MAX_MB"),
            "应指明调参入口：{}",
    ...
    // ============ 网络装机 P0：ISO/casper 流式直传路由（Range/白名单） ============

    #[tokio::test]
    async fn netboot_stream_routes_range_206_and_whitelist() {
        use crate::handlers::provisioning::NETBOOT_ENV_LOCK;

        // 仓 fixture：一件合法 ISO（含 sidecar）+ 引导介质 + boot 提取件
        let repo = std::env::temp_dir().join(format!("os-api-http-netboot-{}", std::process::id()));
        let isos = repo.join("isos");
        let boot = repo.join("boot").join("26.04.1-amd64");
        std::fs::create_dir_all(&isos).unwrap();
        std::fs::create_dir_all(&boot).unwrap();
        // 300KiB 内容（> 分块 256KiB，验证跨块流）
        let content: Vec<u8> = (0..300 * 1024).map(|i| (i % 251) as u8).collect();
        let iso_name = "ubuntu-26.04.1-live-server-amd64.iso";
        std::fs::write(isos.join(iso_name), &content).unwrap();
        std::fs::write(
            isos.join(format!("{iso_name}.sha256")),
            format!("{}  {iso_name}\n", "a".repeat(64)),
        )
        .unwrap();
        std::fs::write(isos.join("ipxe.iso"), b"IPXE-BIN").unwrap();
        std::fs::write(boot.join("vmlinuz"), &content[..1024]).unwrap();
        ...
        // 1) 整文件 GET → 200 + content-length + accept-ranges + sha256 头
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("GET")
                    .uri(format!("/api/v1/provisioning/isos/{iso_name}"))
                    .body(axum::body::Body::empty())
                    .unwrap(),
            )
            .await
        ...
        assert_eq!(
            resp.headers().get("x-nexos-sha256").unwrap(),
            &"a".repeat(64)
        );
        let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
            .unwrap();
        assert_eq!(&bytes[..], &content[..], "整文件字节一致");

        // 2) Range: bytes=0-1023 → 206 + content-range + 精确切片
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
        ...
        assert_eq!(
            ...
            &format!("bytes 0-1023/{}", content.len())
        );
        assert_eq!(resp.headers().get("content-length").unwrap(), "1024");
        let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
            .unwrap();
        assert_eq!(&bytes[..], &content[..1024], "Range 切片字节一致");

        // 3) Range 中段（跨分块边界）+ 后缀区间
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("GET")
                    .uri(format!("/api/v1/provisioning/isos/{iso_name}"))
                    .header("range", "bytes=262144-262159")
                    .body(axum::body::Body::empty())
                    .unwrap(),
            )
            .await
        ...
        let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
            .unwrap();
        assert_eq!(&bytes[..], &content[262144..262160]);
        ...
        // 4) 起点越界 → 416 + content-range: bytes */size
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
        ...
        // 5) 引导介质白名单 + casper 提取件流
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("GET")
                    .uri("/api/v1/provisioning/isos/ipxe.iso")
                    .body(axum::body::Body::empty())
                    .unwrap(),
            )
            .await
        ...
        let resp = router
            ...
            .oneshot(
                axum::http::Request::builder()
                    .method("GET")
                    .uri("/api/v1/provisioning/boot/26.04.1-amd64/vmlinuz")
        ...
        let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
            .unwrap();
        assert_eq!(&bytes[..], &content[..16]);

        // 6) 白名单拒绝：穿越/异形名 → 404（不存在路径拼接）
        for bad in [
            "../etc/passwd",
            "..%2F..%2Fetc%2Fpasswd",
            "ubuntu-26.04.1-live-server-amd64.iso%2F..%2F..",
            "SHA256SUMS",
            "bootstrap.ipxe",
        ] {
            let resp = router
                ...
                .oneshot(
                    axum::http::Request::builder()
                        .method("GET")
                        .uri(format!("/api/v1/provisioning/isos/{bad}"))
                        .body(axum::body::Body::empty())
                        .unwrap(),
                )
                .await
                .unwrap();
            assert_eq!(resp.status(), 404, "白名单应拒绝: {bad}");
        }
        for bad_uri in [
            "/api/v1/provisioning/boot/26.04.1-amd64/vmlinuz.exe",
            "/api/v1/provisioning/boot/26.04.1-i386/vmlinuz",
            "/api/v1/provisioning/boot/..%2F..%2Fetc/vmlinuz",
            "/api/v1/provisioning/boot/26.04.1-amd64/initrd",
        ] {
            let resp = router
                .clone()
                .oneshot(
                ...
                .await
                .unwrap();
            assert_eq!(resp.status(), 404, "boot 白名单应拒绝/缺文件: {bad_uri}");
        }
        // initrd 未提取 → 404（白名单过但文件缺，附 scan 指引）
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("GET")
                    .uri("/api/v1/provisioning/boot/26.04.1-amd64/initrd")
                    .body(axum::body::Body::empty())
                    .unwrap(),
            )
            .await
    ...
    // ============ NexHub Release 附件大载荷流式通道（v0.1.53） ============

    /// 流式通道 e2e 测试的 admin token（注入核心，绕 env 竞态）。
    const ASSET_STREAM_ADMIN: &str = "os-api-asset-stream-test-admin";

    /// 造流式通道测试路由：临时目录 + lobby release 行 + 裸仓目录（附件面只查
    /// 目录存在）+ code_repo 组件（IssuesService 注入——构造即注册进程共享槽，
    /// 特挂路由与 dispatch 两路同源）。
    async fn asset_stream_router(dir: &str) -> axum::Router {
        // lobby schema（hub_releases）+ release 行（存在性权威）
        let _lobby = os_nexhub::nexhub_lobby::NexHubLobbyRouteHandler::with_db_path(
            &format!("{dir}/hub_lobby.db"),
            dir,
        );
        {
            let conn = rusqlite::Connection::open(format!("{dir}/hub_lobby.db")).unwrap();
            conn.execute(
                "INSERT OR REPLACE INTO hub_releases \
                 (id,repo_name,tag,title,notes,created_by,created_at) \
                 VALUES ('rel-demo-v1.0.0','demo','v1.0.0','t','','admin',datetime('now'))",
                [],
            )
            .unwrap();
        }
        std::fs::create_dir_all(format!("{dir}/demo.git")).unwrap();
        let issues = os_nexhub::IssuesService::with_paths(
            &format!("{dir}/repo_issues.db"),
            &format!("{dir}/hub_lobby.db"),
            dir,
        )
        .with_admin_token(ASSET_STREAM_ADMIN);
        let gw = InProcessGateway::new();
        gw.register_component(
            "code_repo",
            Box::new(os_nexhub::CodeRepoRouteHandler::with_issues(issues)),
        )
        .await
        .unwrap();
        build_router(gw.make_state(None, None), None)
    }

    /// 分块请求体（流式上传载荷——80 × 64KiB = 5MiB，验证多帧流式收包）。
    fn chunked_body(content: &[u8], chunk: usize) -> axum::body::Body {
        let chunks: Vec<Result<axum::body::Bytes, std::io::Error>> = content
            .chunks(chunk)
    ...
    #[tokio::test]
    async fn release_asset_stream_upload_download_and_b64_compat_matrix() {
        use sha2::Digest;

        let dir = std::env::temp_dir().join(format!(
            "os-api-http-asset-stream-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        let router = asset_stream_router(dir.to_str().unwrap()).await;

        // 5MiB 内容（> 单帧；80 × 64KiB 分帧）+ 期望摘要（sha2 参考实现）
        let content: Vec<u8> = (0..5 * 1024 * 1024).map(|i| (i % 251) as u8).collect();
        let mut h = sha2::Sha256::new();
        h.update(&content);
        let expect_sha = format!("{:x}", h.finalize());

        // —— 1) 流式上传（?stream=1，octet-stream 直收）→ 201 + size/sha256
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("POST")
                    .uri("/api/v1/coderepo/repos/demo/releases/v1.0.0/assets?stream=1&name=app-stream.bin")
                    .header("content-type", "application/octet-stream")
                    .header("content-length", content.len().to_string())
                    .header("authorization", format!("Bearer {ASSET_STREAM_ADMIN}"))
                    .body(chunked_body(&content, 64 * 1024))
                    .unwrap(),
            )
            .await
            .unwrap();
        assert_eq!(resp.status(), 201, "流式上传应 201");
        let body: serde_json::Value =
            serde_json::from_slice(&axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap())
                .unwrap();
        let asset = &body["asset"];
        assert_eq!(asset["name"], "app-stream.bin");
        assert_eq!(asset["size"], (content.len() as u64));
        assert_eq!(asset["sha256"], expect_sha.as_str());
        let aid = asset["id"].as_str().unwrap().to_string();

        // —— 2) b64 信封兼容端点（无 stream 参数 → 回落 dispatch）同内容对拍
        //    （b64 用 base64 crate 编码——与 issues.rs 纯 std 编码器互为独立实现，
        //    两路摘要一致即跨实现验证）
        use base64::Engine;
        ...
        let resp = router
            ...
            .oneshot(
                axum::http::Request::builder()
                    .method("POST")
                    .uri("/api/v1/coderepo/repos/demo/releases/v1.0.0/assets")
                    .header("content-type", "application/json")
                    .header("authorization", format!("Bearer {ASSET_STREAM_ADMIN}"))
                    .body(axum::body::Body::from(
                        serde_json::json!({ "name": "app-b64.bin", "content_base64": b64 })
                            .to_string(),
                    ))
                    .unwrap(),
            )
            .await
            .unwrap();
        assert_eq!(resp.status(), 201, "b64 兼容端点应 201");
        let body2: serde_json::Value =
            serde_json::from_slice(&axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap())
                .unwrap();
        assert_eq!(
            body2["asset"]["sha256"], asset["sha256"],
            "流式与 b64 两路产物 sha256 一致"
        );

        // —— 3) 清单（dispatch 注册路径仍活）：两条附件
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
        ...
        // —— 4) 流式直挂下载：整文件 200 + 头部契约 + 字节一致
        let dl = format!("/api/v1/coderepo/repos/demo/releases/v1.0.0/assets/{aid}");
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), 200, "流式下载应 200");
        ...
        assert_eq!(resp.headers().get("accept-ranges").unwrap(), "bytes");
        assert_eq!(resp.headers().get("x-nexos-sha256").unwrap(), expect_sha.as_str());
        assert!(resp.headers().get("content-disposition").unwrap().to_str().unwrap().contains("app-stream.bin"));
        let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        assert_eq!(&bytes[..], &content[..], "整文件字节一致（跨 256KiB 分块流）");

        // —— 5) Range 206：首段 / 跨 256KiB 边界中段 / 后缀；416 越界
        for (range, expect) in [
            ("bytes=0-1023", content[..1024].to_vec()),
            ("bytes=262144-262159", content[262144..262160].to_vec()),
            (
                "bytes=-16",
                content[content.len() - 16..].to_vec(),
            ),
        ] {
            let resp = router
                .clone()
                ...
                .await
                .unwrap();
            assert_eq!(resp.status(), 206, "Range {range} 应 206");
            let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap();
            assert_eq!(&bytes[..], &expect[..], "Range {range} 切片一致");
        }
        let resp = router
            .clone()
            .oneshot(
        ...
        // —— 6) 404 面：未知 aid / 越权仓库 / 未知 release
        for uri in [
            format!("/api/v1/coderepo/repos/demo/releases/v1.0.0/assets/ast-nope"),
            ...
            let resp = router
                ...
                .await
                .unwrap();
            assert_eq!(resp.status(), 404, "{uri} 应 404");
        }

        // —— 7) 上传鉴权：无凭据 401 / 未知 token 401
        for auth in [None, Some("Bearer wrong-token")] {
            let mut b = axum::http::Request::builder()
                .method("POST")
                .uri("/api/v1/coderepo/repos/demo/releases/v1.0.0/assets?stream=1&name=x.bin")
                .header("content-type", "application/octet-stream")
            ...
            let resp = router
                .clone()
                .oneshot(b.body(axum::body::Body::from("abcd")).unwrap())
                .await
                .unwrap();
            assert_eq!(resp.status(), 401, "流式上传缺/错凭据应 401");
        }

        // —— 8) 参数校验：缺 name → 400；release 不存在 → 404；同名 → 409
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("POST")
                    .uri("/api/v1/coderepo/repos/demo/releases/v1.0.0/assets?stream=1")
                    .header("authorization", format!("Bearer {ASSET_STREAM_ADMIN}"))
        ...
        let resp = router
            ...
            .oneshot(
                axum::http::Request::builder()
                    .method("POST")
                    .uri("/api/v1/coderepo/repos/demo/releases/v8.8.8/assets?stream=1&name=y.bin")
                    .header("authorization", format!("Bearer {ASSET_STREAM_ADMIN}"))
                    .body(axum::body::Body::from("abcd"))
                    .unwrap(),
            )
            .await
            .unwrap();
        assert_eq!(resp.status(), 404, "release 不存在应 404");
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
        ...
        // —— 9) 超限 413：Content-Length 预检（超限头 + 微载体即拒）
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("POST")
                    .uri("/api/v1/coderepo/repos/demo/releases/v1.0.0/assets?stream=1&name=over.bin")
                    .header("authorization", format!("Bearer {ASSET_STREAM_ADMIN}"))
                    .header("content-length", (os_nexhub::issues::MAX_ASSET_BYTES + 1).to_string())
                    .body(axum::body::Body::from("tiny"))
                    .unwrap(),
            )
            .await
            .unwrap();
        assert_eq!(resp.status(), 413, "Content-Length 超限应 413（预检）");

        // —— 10) 超限 413：运行中累计护栏（无 Content-Length，逐块累计越限中止；
        //     1MiB 零块 ×101 次即过 100MB 上限，半成品即弃不入清单）
        use futures::StreamExt as _;
        let over: Vec<u8> = vec![0u8; 1024 * 1024];
        let oversize_stream = futures::stream::repeat_with(move || {
            Ok::<_, std::io::Error>(axum::body::Bytes::from(over.clone()))
        })
        .take(101);
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("POST")
                    .uri("/api/v1/coderepo/repos/demo/releases/v1.0.0/assets?stream=1&name=over-run.bin")
                    .header("authorization", format!("Bearer {ASSET_STREAM_ADMIN}"))
                    .body(axum::body::Body::from_stream(oversize_stream))
                    .unwrap(),
            )
            .await
            .unwrap();
        assert_eq!(resp.status(), 413, "运行中累计超限应 413");
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
        ...
        let list: serde_json::Value =
            serde_json::from_slice(&axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap())
                .unwrap();
        assert_eq!(
            list["assets"].as_array().unwrap().len(),
            2,
            "超限中止的半成品不应入清单"
        );

        // —— 11) percent-encoded 附件名（encodeURIComponent 客户端形态）
        let resp = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("POST")
                    .uri("/api/v1/coderepo/repos/demo/releases/v1.0.0/assets?stream=1&name=app%20stream%20cn.bin")
                    .header("authorization", format!("Bearer {ASSET_STREAM_ADMIN}"))
                    .header("content-length", "4")
                    .body(axum::body::Body::from("abcd"))
                    .unwrap(),
        ...
        let body: serde_json::Value =
            serde_json::from_slice(&axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap())
                .unwrap();
        assert_eq!(body["asset"]["name"], "app stream cn.bin", "%XX 名应解码");
