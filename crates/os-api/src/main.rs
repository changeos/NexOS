...
use os_api::handlers::{
    ...
    ExitConfig, ExitService, FederationBridge, FilesRouteHandler,
    FirewallManager, ForwardingRouteHandler, IdentityRouteHandler, ImAuth, ImFederation,
    ImRouteHandler, LlmRouteHandler, MediaGenRouteHandler, MediaRouteHandler, ModelHubRouteHandler,
    MonitorRouteHandler, NetworkExitRouteHandler, NetworkRouteHandler, NexhubCiRouteHandler,
    NexhubCliRouteHandler, NodeViewRouteHandler, NotesRouteHandler, P2pLobbyTransport,
    P2pRouteHandler, PowerRouteHandler, ProvisioningRouteHandler, QrTransferRouteHandler,
    RealSources, ShareRouteHandler, StorageRouteHandler,
    SystemRouteHandler, TerminalRouteHandler, TransferRouteHandler,
    UpdateRouteHandler, UserRouteHandler,
};
...
async fn build_gateway(
    ...
    // 统一任务登记表（v0.1.54 统一任务框架 #3）：四套任务面（apps 装/卸 +
    //   provisioning iso/deploy + nexhub_ci runs + llm_envs）创建/终态双写
    //   unified_tasks（tasks.db，轻旁路——登记失败仅日志）；聚合端点
    //   GET /api/v1/tasks?kind=&status=&limit=50 一处看全部；重启悬挂行归谬
    //   error「进程重启」+ 终态行 30 天惰性保留（NEXOS_TASKS_RETAIN_DAYS）。
    //   构造即完成悬挂归谬（详见 handlers/tasks_common.rs 模块头）。
    let unified_tasks = std::sync::Arc::new(os_api::handlers::UnifiedTaskStore::new());
    // v0.1.54：注入统一任务登记表——iso/deploy 任务双写 unified_tasks。
    let provisioning_shared = Arc::new(
        ProvisioningRouteHandler::new()
            .with_update_registry(update_shared.clone())
            .with_unified_tasks(std::sync::Arc::clone(&unified_tasks)),
    );
    gw.register_component(
        "provisioning",
    ...
    // apps 注册表（共享单实例，Arc）：apps 组件 REST 面（下方注册）+ 内置
    //   引擎门控（qr_transfer）每请求直查同一 SQLite apps 表——在门控组件
    //   注册前构造。
    //   （v0.1.51 三引擎剥离：streaming/surveillance/film 引擎已删除出 os-api，
    //   独立产品线 StreamingStudio/FilmStudio 于各自 NexHub 仓承接；应用中心
    //   对已剥离引擎的应用安装拦截 400，见 apps_handler.rs。）
    //   （v0.1.54：unified_tasks 单例已在上方 update 段构造——此处仅注入。）
    let apps_registry = std::sync::Arc::new(
        AppRegistry::new().with_unified_tasks(std::sync::Arc::clone(&unified_tasks)),
    );
    ...
    //   真实 zpool list，vraminfo 显存采集（探测式可选依赖，缺失降级不报错），写操作需 admin）：
    //   GET /api/v1/monitor/metrics / services / alerts / history / zpools / stats
    //   / GET /api/v1/monitor/vram（admin，显存厂商/类型/结温）
    ...
    // Mutex<Connection>。
    // （v0.1.54：构造后、Arc 化前注入统一任务登记表——推理环境创建/更新
    //   任务双写 unified_tasks，kind=llm_env。）
    let mut llm_handler = LlmRouteHandler::new();
    llm_handler.set_env_unified_tasks(std::sync::Arc::clone(&unified_tasks));
    let llm_shared = std::sync::Arc::new(llm_handler);
    ...
    // apps → AppsRouteHandler（应用包运行时，2026-09-04，docs/APPS.md——应用
    //   包（manifest.json + web/ 静态资源的 git 仓库）运行时，经应用中心从
    //   NexHub nexos-app-* 仓库一键安装）：
    ...
    //   扩展名 mime）。读公开 / 写 admin。注册表实例与引擎门控（qr_transfer）
    //   共享（上方构造的单例，with_app_registry 注入——未装应用时对应引擎
    //   端点全 404；已剥离引擎 film/streaming 的应用安装即 400 拦截）。
    gw.register_component(
        "apps",
    ...
    gw.register_component(
        "qr_transfer",
        Box::new(QrTransferRouteHandler::new().with_app_registry(apps_registry.clone())),
    )
    .await
    ...
    gw.register_component(
        "nexhub_ci",
        Box::new(NexhubCiRouteHandler::new().with_unified_tasks(std::sync::Arc::clone(
            &unified_tasks,
        ))),
    )
    .await
    .expect("注册 nexhub_ci handler");
    // tasks → UnifiedTasksRouteHandler（v0.1.54 统一任务框架 #3 聚合面）：
    //   GET /api/v1/tasks?kind=&status=&limit=50（公开读）——四套任务面
    //   （apps/iso/deploy/ci/llm_env）unified_tasks 统一列表，按登记序倒排。
    //   与四套生产者共享同一 store 实例（上方 unified_tasks 单例）。
    gw.register_component(
        "tasks",
        Box::new(os_api::handlers::UnifiedTasksRouteHandler::new(std::sync::Arc::clone(
            &unified_tasks,
        ))),
    )
    .await
    .expect("注册 tasks handler");
    ...
    let (
        ...
        net_exit_handler,
        p2p_handle,
    ) = spawn_p2p_if_enabled(im_federation, nexhub_fed, live_fed, api_market_fed).await;
...
/// 再连带装配 P2P 传输组件；2026-09-14 v0.1.45 批连带注入节点自述）。
///
...
/// - **节点自述注入**（联邦节点信息协议，2026-09-14）：spawn 成功后探测一次
///   （`handlers::p2p::probe_node_self_desc`——agent 版本/hostname/platform/gpu，
///   gpu 经轻量 `nvidia-smi -L` 一次即缓存）经 `Handle::set_self_desc` 注入
///   ——digest 自广播首条携带（60s 常驻 gossip 心跳搭车，零新流量），集群侧
///   据此知道每个节点的版本/机器名/系统/GPU（Spark 版本测不出 / 新 3060 节点
///   身份未知两次踩坑）。os-p2p crate 保持纯（不做硬件探测），探测链在本进程；
...
async fn spawn_p2p_if_enabled(
    ...
    match os_p2p::P2pNode::spawn(cfg) {
        Ok(handle) => {
            ...
            // 节点自述注入（联邦节点信息协议，2026-09-14 v0.1.45 批）：探测一次
            // （agent 版本/hostname/platform/gpu——gpu 经轻量 nvidia-smi -L 一次
            // 即缓存）整包注入，digest 自广播首条携带（60s 常驻 gossip 心跳搭车，
            // 零新流量）。注入先行——先于网关对外服务，首条 gossip 即带自述。
            let desc = os_api::handlers::p2p::probe_node_self_desc();
            let stored = handle.set_self_desc(desc.clone()).await;
            if stored == desc {
                eprintln!(
                    "[os-api] P2P: 节点自述已注入 v{} / {} / {} / gpu={}（下一轮 gossip 自广播携带）",
                    desc.agent_version.as_deref().unwrap_or("-"),
                    desc.hostname.as_deref().unwrap_or("-"),
                    desc.platform.as_deref().unwrap_or("-"),
                    desc.gpu.as_deref().unwrap_or("-"),
                );
            } else {
                eprintln!(
                    "[os-api] P2P: 节点自述注入应答异常（停机边缘？）——下一轮自广播可能不带自述"
                );
            }
            ...
            tokio::spawn(async move {
                loop {
                    match rx.recv().await {
                        Ok(m) => {
                            // payload 截断至 128 字节 + 总长标注（审计 A4-1 快赢#5：
                            // gossip digest 64KB/联邦广播大帧原文刷 journald）
                            eprintln!(
                                "[os-api][p2p] recv from={} hops={} ttl={} payload={}",
                                short_node_id(&m.from.to_hex()),
                                m.hops,
                                m.ttl,
                                os_api::p2p_payload_for_log(&m.payload)
                            );
                            bridge.dispatch(&m);
                        }
...
async fn run_check(gw: &InProcessGateway, addr: &str) -> bool {
    ...
    //    / cloudsync / notes / backup / monitor / llm / api_gateway
    ...
    //    （v0.1.51 三引擎剥离：surveillance / streaming / film 三组件已删）
    let extra_suffix = if gw.component_count() > 34 {
        " + extra"
    } else {
        ""
    };
    println!(
        "[check] 已注册组件（{} 个）：storage + compute + system + share + user + discover + im + network + provisioning + media + media-gen + files + downloads + containers + cloudsync + notes + backup + monitor + llm + api_gateway + blockchain + model_hub + app_store + agenthub + qr_transfer + ble_hub + code_repo + nexhub-lobby + api-market + forwarding + p2p + identity + update + devdocs + power + terminal{extra_suffix}",
        gw.component_count()
    );
