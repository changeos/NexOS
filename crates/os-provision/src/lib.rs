...
//! - [`netboot`]：网络装机 P0——iPXE 引导链脚本（bootstrap/动态菜单/boot 条目）+
//!   autoinstall nocloud 种子模板（24.04/26.04 参数化）+ ISO 仓条目模型（滚动
//!   版本策略：自动装机只列每架构最新稳定版）。纯逻辑，见 docs/NET_BOOT.md。
//! - [`iso9660`]：ISO9660 只读文件提取器（从 live-server ISO 提取 casper
//!   vmlinuz/initrd，纯 Rust 免外部工具）。
