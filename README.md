...
**NexOS 是独立个体的操作系统（连接 OS）**——不是 NAS 系统，存储是附带能力。核心方向是 P2P 组网：每个 NexOS 是对等的独立节点，经全分布式 Kademlia overlay 互联，打破设备/数据/AI/身份/代码五类信息孤岛（理念见 [PHILOSOPHY.md](PHILOSOPHY.md)）。

系统核心以 **28 个 Rust crate** 从零构建，覆盖：ZFS 存储、KVM 虚拟机与容器、网络与防火墙、文件/块协议栈、P2P 组网与联邦（大厅/IM/直播/传输/出口共享）、链上身份与多链验签、去中心化共识（openraft）、A/B 无缝更新、网络装机（iPXE），以及四语言与跨端 SDK；网关层内嵌 **Vue3 Web 桌面（28 个内置应用 + 应用中心运行时可装应用包）**，并提供 NexHub 代码枢纽（git 双通道/CI/webhooks/Issues-PR）、IM 链上身份、AI 网关计费、链上支付验真等上层能力。

媒体应用（影片制作/流媒体中心/监控转码）已剥离为**独立产品线**（FilmStudio / StreamingStudio，各自 NexHub 仓迭代），经应用中心安装——os-api 只保留系统职责（P2P 联邦直播除外，常开）。
...
- **媒体**：FFmpeg 真实转码（HLS 单/多档位 ABR）+ **CLIP 语义搜索**（candle 纯 Rust 推理，CUDA GPU 加速）+ **媒体生成**（本地 sd-turbo 文生图，显存互斥保护；视频生成任务框架）+ **vraminfo 显存监控**（GDDR6X/7 结温 MMIO 直读，nvidia-smi 在 Linux 不提供；sudo 自适应提权）。
- **A/B 无缝更新**：双槽位 bootloader 管理（GRUB/systemd-boot `bootctl`/`grub2-reboot`）+ 签名包验证（ed25519）+ 回滚 + 版本感知升级（NexHub tag 更新源，install.sh 开箱即有源）。
- **去中心化共识**：真实 Raft（openraft）+ SQLite 元数据存储，支持 HA 故障转移。
- **P2P 组网与联邦**：全分布式 Kademlia（Swarm 同款拓扑）+ ECDH/AESGCM 加密 overlay + NAT 中继/TCP 打洞 + mDNS LAN 种子；联邦大厅（NexHub/模型/API 三厅）/ IM 联邦与 DM / P2P 联邦直播 / 网状分发（transfer）/ WAN 出口共享（SOCKS5 over overlay）/ 跨网 API 中继。
- **链上身份与验签**：BIP-322（legacy + simple）、BTC（rust-bitcoin）、EVM（alloy）多链钱包与签名；**IM/大厅链上身份**（secp256k1 公钥即身份，挑战-签名三步认证，os-common chain_auth 共享内核）；**链上支付验真**（EVM JSON-RPC 核验交易 receipt，BSC 等多链公共 RPC fallback 链）。
- **NexHub（代码枢纽）**：自托管 git（SSH + HTTP Smart Git 双通道）、项目发布/搜索/一键克隆、付费门禁与悬赏 bounty、Issues/PR、**内置 CI**（push 自动触发，Cargo/npm 流水线）、**Webhooks**（push/issues/pr/release 四事件，HMAC-SHA256 签名）、CLI（`curl | sh` 直装）。
- **AI 网关与变现**：LLM 渠道聚合 + token 四计费模式（free/per_token/per_image/credits）+ USDT/BTC/EVM 充值订单（docs/GATEWAY_MONETIZATION.md）+ vLLM 推理环境/实例管理（docs/LLM_ENVIRONMENTS.md、docs/LLM_MONITORING.md）+ 联邦渠道中继（via_node）。
- **应用生态**：应用包运行时（引擎随 os-api 编译、应用按装启用）+ `@nexos/app-sdk` 能力面 SDK + standalone 独立运行 + `publish-app` 一键发布（docs/APPS.md）；FilmStudio / StreamingStudio 独立产品线经此分发。
- **网络装机**：U 盘/网络 iPXE + autoinstall（装完自动入集群闭环）+ ISO 仓 sha256 对账 + 纯 Rust ISO9660 提取器 + ISO 流式直传（docs/NET_BOOT.md）。
- **远程转发**：SSH 隧道（-L/-R/-D）spawn + 纯 Rust RDP TCP 代理 + .rdp 文件生成（docs/FORWARDING.md）。
- **28 个内置桌面应用**：存储/文件/备份/监控/下载（aria2）/容器/笔记/云同步（rclone）/区块链/模型管理（vLLM 实例+推理环境+配方库）/聊天/联邦节点/网络（P2P 拓扑/WAN 出口/防火墙）/远程转发/终端（Web Terminal）/开发者中心/AgentHub 等（docs/FEATURE_SURVEY.md、docs/APPS_REFERENCE.md）；另有应用中心运行时可装应用包（影片制作/流媒体/QR 传输独立版等）。
- **多语言**：简中 / 繁中 / 英 / 日（前端四语；后端 os-i18n 三语），自实现 TOML 翻译器 + 运行时切换。
...
28 个 crate（26 业务 + `os-integration` 跨 crate 集成测 + `nettest` 网络冒烟）按职责分层（自底向上），每层只依赖其下层：

```
┌──────────────────────────────────────────────────────────────┐
│ 客户端层    │ os-cli · os-mobile · os-desktop · os-mcp     │  ← 运维 CLI / 移动端 / 桌面端 SDK / MCP Server
├──────────────────────────────────────────────────────────────┤
│ 部署更新层  │ os-provision · os-iso · os-update             │  ← iPXE 网络装机 / ISO 构建 / A/B 更新
│             │ os-im · os-api · os-nexhub                     │  ← IM agent / HTTP-WS 网关+Web UI / NexHub 枢纽
├──────────────────────────────────────────────────────────────┤
│ 服务编排层  │ osd · os-services · os-meta · os-discover    │  ← 守护进程 / 监控+媒体 / Raft 共识 / 设备发现
│             │ os-guest                                        │  ← 访客网络认证
├──────────────────────────────────────────────────────────────┤
│ 组网身份层  │ os-p2p · os-identity                           │  ← Kademlia overlay+NAT 中继 / 指纹账本
├──────────────────────────────────────────────────────────────┤
│ 协议计算层  │ os-protocols · os-compute · os-wallet         │  ← 文件/块协议 / VM+容器 / 链上钱包
├──────────────────────────────────────────────────────────────┤
│ 存储网络层  │ os-storage · os-network · os-security         │  ← ZFS / 网络防火墙 / 鉴权+证书
├──────────────────────────────────────────────────────────────┤
│ 核心层      │ os-core · os-common · os-i18n                 │  ← 领域类型/事件总线 / 公共 DTO+chain_auth+网关契约 / 多语言
└──────────────────────────────────────────────────────────────┘
```
...
- **代码规模**：28 crate workspace（26 业务 crate + `os-integration` 跨 crate 集成测 + `nettest` 网络冒烟）；网关层 46 个常驻 RouteHandler 组件（+1 条件 extra；含 os-nexhub 桥接 2 个）、377 条静态 API 路由（grep `spec(` 剔除测试模块口径，另有 /git、/dist、/ws 等网关特挂路由）；28 个内置桌面应用 + 应用中心运行时可装应用包。2026-09-25 口径。
- **测试**：os-api `cargo test` 基线 **1,715**；全 workspace 测试函数约 **5,000+**（grep 口径；ignored = 需 root / 真实硬件的真实环境测，默认不跑）。
...
- **真实环境验证**：26 项本机实跑验证（ZFS 池全链 / nftables 事务 / rtnetlink / libvirt VM 生命周期 / runc 容器 / SMB-NFS 落盘 / iSCSI+NVMe-oF configfs / FFmpeg 转码 / CLIP CUDA 推理 / cgroup v2 / systemd / chrony / bootloader / ISO 构建 / sd-turbo 生图 / aria2 / docker / rclone...），过程中发现并修复 10+ 个真实 bug；多节点集群（跨 LAN/NAT/公网，x86+aarch64）长期在跑。
- **生产运行**：os-api 以 systemd 服务常驻（生产节点经 `NEXOS_HTTP_PORT` 配 8558，缺省 8080；Web UI + API + git smart HTTP + /metrics 单端口四流量），鉴权/计费/收款等 env 见 docs/DEPLOYMENT.md §9。
